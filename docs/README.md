...
- [**Protocol Gateway**](protocols/protocol.md) - The `GovernanceEnvelope`, transaction flow, L1Doctrine/L2Consensus/L3Notary gates, receipts, and session rules.
- [**Governance Hierarchy**](protocols/protocol.md#3-layer-governance-bedrock) - L1Doctrine/L2Consensus/L3Notary validation model.
- [**Security Principles**](protocols/protocol.md#host-sovereignty--audit) - Host sovereignty, local-first audit, and fail-closed execution.

## 2. Gateway Components
The reference Go implementation compiles from a single codebase into two role-specific binaries:

- [**Governance Gateway (g8eg)**](architecture/gateway.md) (`g8e.gateway`) - The central PDP / BFT-governed Policy Decision Point running in Gateway mode (--doctrine, --consensus, or --notary).
- [**Governed Operator (g8eo)**](architecture/operator.md) (`g8e.operator`) - The host-side PEP / sovereign execution agent and MCP Server.

## 3. g8e-Compatible Applications
Optional producers and consumers demonstrating the protocol in action.

- [**g8e-Compatible Applications**](guides/g8e-compatible-apps.md) - How to build conforming producers and consumers of the protocol.
...
- [**CLI Reference**](guides/cli.md) - Help for the `./g8e` management tool.
- [**Codemap**](codemap.md) - Unified codebase architecture overview.
