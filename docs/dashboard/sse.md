...
The standard dashboard deployment is a static SPA host. The browser authenticates directly to the Gateway, while `dashboard/server.js` serves assets and `/g8e-config.js`; it does not proxy Gateway requests or mount the retained dashboard SSE routes. The first-party dashboard SSE connection is therefore not operational in the standard separate-origin deployment. The current manager uses the dashboard origin, the Gateway polling path instead of the live stream path, and a legacy flat event envelope that does not match Gateway stream frames.
