...
g8ed is the first-party browser interface for g8e. A minimal Node.js 22 / Express 5 process serves a framework-free JavaScript single-page application, while the browser authenticates and calls the g8e Gateway directly over HTTPS.
...
1. The container entrypoint waits for the gateway's plain-HTTP health endpoint.
2. `server.js` requires `G8E_GATEWAY_URL` and resolves the dashboard's `g8ed` workload identity through `AppEnrollmentService`.
...
The Express host does not currently mount `dashboard/routes/` or instantiate the prepared g8eg HTTP and pub/sub clients. Code that targets `ServiceName.g8ed` API paths therefore represents retained migration inventory, not an active dashboard backend.
