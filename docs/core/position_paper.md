...
2. **The platform-compatible agentic ensemble** forms machine intent. It translates a request into a concrete typed payload and reaches consensus that the payload is a faithful, safe realization of the request. Its output is a consistency proof — a set of Ed25519 signatures over the transaction hash from independent reasoners, produced through a Tribunal deliberation process. The Tribunal is an enrolled agentic application whose members evaluate the transaction and cast L2 votes; the gateway itself holds no signing key and cannot self-sign.

3. **The Governance Gateway (`g8eg`)** admits. It assigns cryptographic identity (SPIFFE URI SANs over mTLS), enforces replay defense, distributes state roots, and relays envelopes to the right host. Critically, it holds **no privileged bypass** and **no L2 signing key**. It delegates L2 deliberation to the enrolled Tribunal and verifies the returned votes; it is a stateless coordinator, not a trusted authority.
