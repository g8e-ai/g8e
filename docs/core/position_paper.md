...
**Zero standing privileges.** The Operator holds no permanent administrative credentials. Permissions are minted just-in-time, derived from the verified intent inside the envelope, scoped to a single action, and dissolved on completion. The Gateway enforces this through JIT user provisioning and JWT validation: user accounts are created dynamically on first successful JWT validation rather than pre-provisioned, JWT tokens are short-lived and signed by an external Identity Provider carrying only the claims needed for the current session, and the Gateway validates the token, extracts minimal identity and role information, and injects it into the envelope as `tenant_id` and `binding_persona`. The Operator receives only this pre-validated metadata and never stores persistent user credentials or long-lived sessions. A compromise of any layer — a hijacked session, a poisoned reasoning state, or even the Gateway's user database — cannot exfiltrate reusable credentials, because none exist. Every authentication requires a fresh, cryptographically signed token from the IdP, and every permission is scoped to a single transaction. You cannot steal what was never standing.
