...
1.  **Protobuf Reflection (`forbidden_patterns`)**: 
    Enforced in `services/g8eo/internal/services/governance/transaction_verifier.go` via `validateL1Governance`. It scans incoming messages for the `forbidden_patterns` custom option defined in `protocol/proto/operator.proto`. For example, the `command` field in `CommandRequested` prohibits `sudo`, `su`, and `rm -rf /`.
...
4.  **L2 Signature**: 
    ...
    `g8eo` verifies this signature in `services/g8eo/internal/services/governance/transaction_verifier.go`. It uses an Operator-owned `SignerStore` (database-backed) for O(1) public key lookup. The verifier rejects every missing-key or unknown-key transaction until signer material is provisioned.
...
*   **Auto-Approval (Skip-Approval)**: 
    Defined in `@/home/bob/g8e/services/g8ee/config/auto_approved.json`. Benign diagnostic base commands (e.g., `uptime`, `df`, `free`) can bypass the human prompt if `enable_auto_approve` is active.
...
**Canonicalization rules** (implemented in `services/g8eo/pkg/uap/types.go`):
