...
**The standard deployment.** g8eo acts as a remote Operator that dials into the platform (g8es). This enables execution on machines behind strict firewalls without requiring inbound firewall rules.

### 2. Listen Mode (`--listen`)
**The platform core (g8es).** In this mode, g8eo switches from an execution agent to a persistent data bus and pub/sub broker. It serves as the "brain" that coordinates all other Operators and Engine components.
...
Components (g8ed, g8ee, and g8eo in `--listen` mode) use a shared `internal_auth_token` for internal traffic.

- **Authoritative Source**: `/g8es/ssl/internal_auth_token` on the shared volume.
- **Generation**: Generated by g8es (g8eo in `--listen` mode) on first start.
