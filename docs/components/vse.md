...
VSE is the AI engine for g8e. It provides an agentic, LLM-powered interface for infrastructure operations and troubleshooting, with full human-in-the-loop safety controls, data sovereignty, and multi-provider LLM abstraction.
...
VSE is a Python/FastAPI service. The `ChatPipelineService` is the central coordinator — it assembles context, drives the AI agent, and handles post-response persistence.
...
graph LR
    ...
    VSOD -- "HTTP (Internal)" --> VSE
    VSE -- "HTTP / WebSocket" --> VSODB
    VSOD -- "WebSocket (mTLS)" --> VSA
    VSA -- "Pub/Sub" --> VSE
```

- **VSOD** -- Web gateway; relays browser requests to VSE and SSE events back to the browser.
...
VSE enforces a strict hierarchy for data access: **Clients** handle raw transport/protocol, **Handler Services** wrap clients with domain logic, and **Orchestrators** (like `ChatPipelineService`) consume the services.

VSE maintains 5 core data clients, each with exactly one handler service:
...
VSE uses a unified error model derived from `VSOError`. Custom error signatures have been updated to support component attribution and detailed context.

| Error Class | Purpose | Key Parameters |
|-------------|---------|----------------|
| `ResourceNotFoundError` | Resource not found in DB | `message`, `resource_type`, `resource_id`, `component="vse"` |
| `AuthorizationError` | Permission denied | `message="Insufficient permissions"`, `component="vse"` |
| `ValidationError` | Request/model validation failure | `message`, `field`, `constraint`, `component="vse"` |
...
VSE uses `CacheAsideService` to manage synchronization between the authoritative `DBService` and the `KVService` (VSODB).
...
`InvestigationService` (`components/vse/app/services/investigation/investigation_context.py`) is the single entry point for building the context object the agent receives on every turn. It orchestrates `InvestigationDataService`, `OperatorDataService`, and `MemoryDataService` to assemble a complete picture of the current state.
...
Before invoking the primary model, VSE classifies each incoming message as `simple` or `complex` using the `triage_message` utility. This avoids the full model for messages that can be handled cheaply by the `assistant` model. 
...
Automatic Function Calling (AFC) is always disabled. VSE uses a custom sequential function-calling loop to preserve thought signatures across multi-step operations.
...
`OperatorApprovalService` (`app/services/operator/approval_service.py`) is a first-class service on `app.state.approval_service`, independently constructed in `main.py` and injected into `OperatorCommandService.build()`. The VSE router for `/api/internal/operator/approval/respond` depends on `OperatorApprovalService` directly — approval responses do not pass through `OperatorCommandService`.
...
VSE implements an **MCP Client Adapter** that translates outbound tool calls into the Model Context Protocol (MCP) JSON-RPC 2.0 format. This is part of g8e' provider-agnostic event system design—MCP is one protocol translator among potentially many, all mapping to internal event types.
...
- **Structured Reconstruction**: VSE uses a two-tier reconstruction logic for MCP results:
...
VSE tracks all operator-related actions and results to maintain a continuous picture of the environment.
...
While VSODB stores a summary of recent activity, the **Operator remains the system of record** via LFAA. VSE dispatches audit events to the operator's local vault for long-term retention and cryptographic verification.
...
When local storage is enabled on an Operator (the default), all tool call outputs — command stdout/stderr, file contents, directory listings — are stored in the Operator's working directory. VSE receives only metadata (hashes, sizes) with a `stored_locally=true` flag, not the raw data itself.
...
- **VSE Python scrubber** — scrubs sensitive data from user messages before they reach the AI (27 patterns: service tokens, cloud credentials, PII, connection strings, private keys).
...
VSE processes multi-modal file attachments for LLM consumption.

### Flow

1. **VSOD** stores full `AttachmentData` JSON (including base64 data, filename, content type) in VSODB Blob Store and forwards a `store_key` reference to VSE.
2. **VSE** retrieves the full attachment from Blob Store on demand, classifies the attachment type (PDF, image, text), and formats it as a `Part` object for the LLM provider.
...
VSE uses three distinct clients for data operations.
...
| `InternalHttpClient ` | HTTP | VSE → VSOD internal API — SSE push, operator queries, heartbeat forwarding, intent management |
...
VSE publishes commands to `cmd:{operator_id}:{operator_session_id}` and subscribes to the corresponding `results:*` and `heartbeat:*` channels. The canonical channel listing and wire format are in [components/vsodb.md — Channel Naming Convention](vsodb.md#channel-naming-convention).

#### Subscribe-and-Wait Contract

All channel subscriptions in VSE follow a **subscribe-and-wait** flow: establish the connection, send the subscribe action, wait for the broker's `{"type":"subscribed","channel":"..."}` ack, then return. No publish or command dispatch may occur on a channel until the subscription ack has been received.
...
VSE communicates with other components via direct HTTP using `X-Internal-Auth` for authentication and standard `VSOHttpContext` headers.
...
| `/api/internal/operators/register-operator-session` | POST | Subscribes VSE to heartbeat/result channels for a new session |

#### Authentication Discovery
VSE discovers the authoritative `internal_auth_token` by reading `/vsodb/ssl/internal_auth_token` at startup (or via `INTERNAL_AUTH_TOKEN` env var). This is the absolute source of truth for service-to-service authentication.

#### Context Propagation
The canonical header list and ownership rules are in [components/vsod.md — Internal HTTP Communication](vsod.md#internal-http-communication-vsod--vse).

Key fields consumed by VSE:
...
When a user sends their first message in a new conversation, no `case_id` or `investigation_id` exists yet. VSOD signals this by setting `X-VSO-New-Case: true` and sending `UNKNOWN_ID` sentinels for both `X-VSO-Case-ID` and `X-VSO-Investigation-ID`. VSE reads `vso_context.new_case` and branches into inline case and investigation creation.
...
**VSE side** (`app/dependencies.py` → `get_vso_http_context`):
...
**VSE router** (`app/routers/internal_router.py` → `internal_chat`):
...
`X-VSO-Bound-Operators` is the **exclusive source of truth** for which operators are available to the AI on any given request. VSE performs no independent lookup against the operator document store to determine binding state.

To prevent header bloat, this header carries only minimal identity and status (`operator_id`, `operator_session_id`, `status`). Full metadata such as `operator_type` and `system_info` is fetched by VSE from the shared KV cache when needed.
...
**Rule:** Never add a fallback that queries the operator document store by `web_session_id` to resolve binding state in VSE. If `vso_context.bound_operators` is empty, the session has no bound operators — that is the correct answer.

#### VSOHttpContext Application Barrier Contract

`VSOHttpContext` is the **single authoritative context object** within the VSE application boundary. It must be passed intact to all internal service methods — never dismantled into individual loose parameters (`web_session_id`, `case_id`, `investigation_id`, `user_id`) and then re-assembled downstream.
...
`InternalHttpClient ` exposes typed methods for all VSE → VSOD communication:
...
All prompt file paths, agent modes, and section labels are defined in `shared/constants/prompts.json` — the authoritative source. VSE loads this file and populates three enums in `app/constants/prompts.py`: `AgentMode`, `PromptSection`, and `PromptFile`.
...
VSE supports two distinct search modes with different data flows:

### Mode 1: `search_web` Function Tool (Vertex AI Search)

The AI explicitly calls `search_web` as a function tool. VSE executes a Vertex AI Search (Discovery Engine) query via the `searchLite` SDK method against a pre-configured website data store and returns structured results to the LLM as a `ToolResponse` part. No grounding metadata is produced — results are plain text the model reasons over.
...
`WebSearchProvider` is constructed once at application startup in `lifespan()` (`app/main.py`) and stored on `app.state.web_search_provider`. It is never instantiated inside another service. `get_vseweb_search_provider()` in `app/dependencies.py` exposes it as a FastAPI dependency.
...
The following keys are read from the `settings` map inside the `platform_settings` VSODB document. VSOD owns writing this document; VSE reads it. All values are strings; booleans use `"true"`/`"false"`, numbers use their string representation.
...
Temperatures are fixed per Tribunal member and are not configurable. Values are sourced from shared/constants/agents.json (single source of truth across VSE, VSOD, and VSA): Axiom → 0.0, Concord → 0.4, Variance → 0.8.
...
| Key | Default | Purpose |
|-----|---------|---------|
| `internal_auth_token` | — | Component-to-component auth (`X-Internal-Auth` header); required before VSE accepts requests |
...
See [testing.md — VSE](../testing.md#vse--python) for test infrastructure, pytest fixtures, mock locations, markers, AI integration test details, and how to run tests.
