...
- Single external entry point — all inbound traffic (browser HTTPS, Operator auth, Operator pub/sub) enters through VSOD
...
```
          Browser / Operator
               ...
               │  WSS /ws/pubsub (proxied to VSODB)
               ▼
┌─────────────────────────────────────────────────────────┐
│                       VSOD                              │
│                                                         │
...
VSOD reads **zero** environment variables via `process.env` at runtime. Bootstrap transport URLs are defined as constants in `constants/http_client.js`. Bootstrap secrets (auth token, CA cert, session encryption key) are read from the **Shared SSL Volume** by `BootstrapService`. All other configuration flows through `SettingsService` from the VSODB `platform_settings` collection.

The following `G8E_*` variables are consumed by `docker-compose.yml` to configure the container environment. They are **not** read by VSOD application code:

| Variable | Default | Description |
|----------|---------|-------------|
| `G8E_INTERNAL_HTTP_URL` | `https://vsodb` | VSODB HTTP URL — read once at startup before VSODB is reachable |
| `G8E_INTERNAL_PUBSUB_URL` | `wss://vsodb` | VSODB pub/sub WebSocket URL — read once at startup before VSODB is reachable |
...
All configuration — LLM provider, API keys, passkey config, cert paths, session tuning, app URL, CORS — flows through `SettingsService.getConfig()`. The resolved config object (`VSODConfig`) is passed to every service that needs it at construction time.

#### Precedence Chain

VSOD enforces a strict precedence chain when resolving configuration values. This allows for sensible defaults, deployment-time overrides via environment variables, and runtime persistence in VSODB.
...
2.  **Platform Settings (DB)**: Values stored in VSODB `platform_settings` collection, `platform_settings` document.
3.  **Defaults**: Defined in the schema (`components/vsod/services/platform/settings_service.js`).

For **Platform settings** (`PLATFORM_SETTINGS`):
1.  **Platform Settings (DB)**: Values stored in VSODB `platform_settings` collection, `platform_settings` document.
2.  **Defaults**: Defined in the schema.

**Exception: Bootstrap Secrets**
Critical bootstrap secrets (`internal_auth_token`, `session_encryption_key`) use the **Shared SSL Volume** as the absolute source of truth. They are read directly from `/vsodb/ssl/` at startup and are **never stored in the database**. This decouples platform identity from the database lifecycle.

#### Seeding and Protection

On the first boot of a new deployment, VSOD automatically **seeds** the `platform_settings` collection in VSODB with configuration values from the environment (e.g. LLM keys, URLs).

**Core Secrets Protection:**
Unlike other settings, `internal_auth_token` and `session_encryption_key` are no longer generated or submitted by the Setup Wizard, and they are not present in the Settings UI. They are managed exclusively by VSODB on the SSL volume. This ensures the integrity of the platform's authoritative credentials even across full database wipes or resets.

---

## Data Connectivity

VSOD connects to VSODB using two separate transports — one per concern.
...
| Class | File | Purpose |
|-------|------|---------|
| `VSODBDocumentClient` | `services/clients/vsodb_document_client.js` | HTTP document store (collections API). All requests authenticated via `X-Internal-Auth`. |
| `KVClient` | `services/clients/vsodb_kv_cache_client.js` | HTTP KV store. All requests authenticated via `X-Internal-Auth`. |
| `VSODBPubSubClient` | `services/clients/vsodb_pubsub_client.js` | WebSocket pub/sub. Connection authenticated via `X-Internal-Auth` (header or query param). |
| `VSODBBlobClient` | `services/platform/vsodb_blob_client.js` | HTTP blob store for large binary data (e.g. attachments). |
...
VSOD uses the cache-aside pattern for consistency between VSODB KV and the document store. Document store is authoritative; KV is the read cache. Invalidate-on-update — never update-in-place.
...
When a user sends their first message in a new conversation, `chat_routes.js` receives a request body with no `case_id`. `InternalHttpClient.buildVSOContextHeaders` detects the missing `case_id` as the new-case signal:

```
request body has no case_id
    → vsoContext.case_id = null
    → buildVSOContextHeaders detects !context.case_id
...
VSOD exposes a single Streamable HTTP MCP endpoint at `POST /mcp` for external MCP clients such as Claude Code. The endpoint accepts JSON-RPC 2.0 requests and dispatches by `method`:
...
4. Fetch the operator document from VSODB via `OperatorDataService.getOperator(operator_id)` — provides current `status`, `system_info`, `operator_type`.
...
Sessions are stored in both the VSODB document store (`web_sessions` / `operator_sessions` collections) and the VSODB KV store (fast path). The `api_key` field is encrypted with AES-256-GCM using `SESSION_ENCRYPTION_KEY`. `operator_id` is only present on `OperatorSessionDocument` and is also encrypted.
...
**Backing stores:**
- **VSODB KV** (fast path) — two bidirectional keys per binding, read on every routed request
- **VSODB Document Store via `CacheAside`** — `bound_sessions` collection, one document per web session (`id = webSessionId`), for durability and audit
...
GET https://<host>
    ...
    → user trusts the VSODB-generated platform CA on their workstation
...
5-step browser wizard:
  ...
  Step 5 — Finish
    ...
    Summary card shown
    →  POST /api/setup/config                     →  settings saved to VSODB
...
- Settings saved via the wizard are stored in VSODB (`platform_settings` collection). `INTERNAL_AUTH_TOKEN` and `SESSION_ENCRYPTION_KEY` are generated and stored in VSODB on first start — they are not injected via environment variables and do not need to be set anywhere
...
VSOD serves a public HTTP onboarding page specifically for workstation CA trust bootstrap. This page is not the application itself — it exists so the browser can reach g8e without certificate warnings before the workstation trusts the platform CA.
...
- VSODB generates and stores the CA and server certificates
- VSOD reads those certificates from the VSODB SSL volume (`/vsodb/`)
- VSOD serves the browser trust portal, OS-specific installer scripts, and the operator g8e script
...
VSOD serves a POSIX shell script at `GET /g8e` (HTTP port 80) that automates operator deployment on any remote Linux system. The script is generated per-request with the platform's host and port baked in.
...
`SSEService` receives its dependencies (`OperatorDataService`, `vsodConfig`, `internalHttpClient`, `boundSessionsService`) at construction time via `initialization.js` — not as call-time arguments.
...
Operator binaries are stored in the VSODB blob store (namespace `operator-binary`). VSOD has no local binary storage — every download request fetches directly from VSODB's blob store via `OperatorDownloadService` (`GET /blob/operator-binary/{os}-{arch}`).
...
|---|---|---|
| `dlk_` Device Link token | VSODB KV (ephemeral) | No |
| `g8e_key` | User document (`users` collection) — looked up via `UserService.getUserByApiKey()` | No |
| Operator API key | VSODB document store via `ApiKeyService` | No |
...
2. VSOD generates a `dlk_<32-char>` token and returns an operator command
3. User runs the command on the target system
4. Binary collects system fingerprint and registers with VSOD (operator does not need to know its `operator_id` beforehand)
5. VSOD resolves or provisions a slot, then provides the `operator_id` to the binary during the bootstrap response
6. Operator activates immediately — no browser approval needed
7. Operator appears in the panel as active

**Token format:** `dlk_[A-Za-z0-9_-]{32}` — 24 cryptographically random bytes, validated by regex before any VSODB operations.
...
**Security controls:**
- Token format validated before any VSODB operations (injection prevention)
...
- Atomic claim via VSODB KV prevents race conditions
...
VSOD uses structured response models instead of generic `data` containers to ensure type safety and self-documenting APIs. All response models extend `VSOBaseModel` and declare explicit fields.

### VSOBaseModel Constructor Pattern

The `VSOBaseModel` constructor uses `fields` as the parameter name (not `data`) to prevent the anti-pattern of generic data containers:
...
Attachments uploaded with a chat message are stored in the VSODB Blob Store. Metadata is stored in VSODB KV at `g8e:investigation:{inv_id}:attachment:{att_id}` (1h TTL). An ordered index of attachment IDs for the investigation is maintained at `g8e:investigation:{inv_id}:attachment.index`.

### Attachment Lifecycle

| Phase | What happens |
|---|---|
| **Upload** | `AttachmentService.storeAttachments()` validates, sanitizes filename, writes binary to VSODB Blob Store, then writes metadata to VSODB KV |
...
| `/health/store` | GET | Readiness probe (VSODB connectivity) |
...
serviceClient.get(ServiceName.VSOD, ApiPaths.operator.list());
...
VSOD separates HTML templates from JS component logic. Templates live in `public/js/components/templates/` as `.html` files and are loaded via `templateLoader` (`public/js/utils/template-loader.js`).
...
VSOD uses Content Security Policy with dynamic nonces. All styles must be in CSS files — no inline `style=` attributes, no `style.display` manipulation, no dynamically created `<style>` elements.
...
- VSODB KV keys scoped to `attachment:{investigation_id}:{attachment_id}`
...
| `GET /health/store` | Internal | Readiness probe (VSODB connectivity) |
...
- VSODB KV key count, pub/sub connections, document store size
