...
| `test/unit/frontend/components/console-page.unit.test.js` | Console page — **Static analysis**: `console.ejs` uses `EventSource` not `EventType` for SSE, references correct wire protocol event type strings (`g8e.v1.platform.console.log.connected.confirmed`, `g8e.v1.platform.console.log.entry.received`), no short-form type checks (`"connected"`, `"log"`), no polling via `setInterval`. **`apiFetch` response restructuring**: flat backend response (`{ success, message, users, ... }`) restructured to `{ success, message, data: { users, ... } }` for overview/KV scan/DB query/DB collections/component health/login audit/AI stats/cache clear/POST requests, `success`/`message`/`error` excluded from `data`. **SSE wire protocol contract**: `EventType` constant values match wire strings, `console.ejs` type checks match constant values. **`connectLogs` behavior**: `EventSource` created with correct URL/`withCredentials`, badge lifecycle (Connecting/Live/Disconnected), log entry appending on `PLATFORM_CONSOLE_LOG_ENTRY_RECEIVED`, unknown type ignored, error closes connection and updates badge, reconnect closes previous `EventSource` and clears container, level selection reflected in URL, multiple sequential entries, malformed JSON handled gracefully. **Response shape contract**: `loadOverview`/`loadComponentHealth`/`kvScan`/`dbQuery`/`populateCollections` all consume via `json.data` property |
...
#### Console route unit test files

| File | Coverage |
|------|----------|
| `test/unit/routes/platform/console_routes.unit.test.js` | `createConsoleRouter` — **Overview**: fetch and return platform overview using typed `PlatformOverviewResponse`, service error handling with 500 status. **Metrics routes**: user stats, operator stats, session stats, AI usage stats, login audit stats, real-time metrics, component health — all with success/error paths. **Cache**: clear cache POST endpoint. **KV Store**: scan with default parameters, key fetch with success/400 on missing key. **DB**: collections list, query with valid collection/reject invalid collection. **SSE Log Stream** (`LOGS_STREAM`): correct SSE headers (`text/event-stream`, `no-cache`, `keep-alive`, `X-Accel-Buffering: no`), connected event with correct `EventType.PLATFORM_CONSOLE_LOG_CONNECTED_CONFIRMED` wire value (`g8e.v1.platform.console.log.connected.confirmed`), buffered log entries from ring buffer before connected event with `EventType.PLATFORM_CONSOLE_LOG_ENTRY_RECEIVED` wire value, level filtering (warn level excludes debug/info, passes warn/error), ring buffer limit enforcement (last N entries), limit cap at 500, listener registration and removal on connection close, live entry streaming through registered listener with correct type, level-filtered live entries dropped, null ring buffer handling, default level info |
| `test/unit/services/platform/console_metrics_service.unit.test.js` | Console metrics service — platform overview aggregation, user/operator/session/AI stats, login audit, real-time metrics, cache management, component health checks, KV scan/key operations, DB collection queries |
