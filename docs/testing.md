...
|----------|---------|---------|
| `ci.yml` | push/PR to `main` | Run all tests (matrix: g8ee, vsod, vsa), build images |
...
- A separate `build-images` job (requires `test` to pass) builds operator binaries for all architectures and all Docker images (`docker compose build g8ee vsod vsodb`) as a post-test smoke check; images are not pushed.
...
**`g8e-pod-image.yml`** triggers on changes to: `components/g8e-pod/Dockerfile`, `components/g8e-pod/scripts/**`, `components/g8ee/requirements.txt`, `components/vsod/package*.json`, `components/vsa/go.mod`/`go.sum`/`vendor/**`. Also supports `workflow_dispatch`. Pushes to `ghcr.io/g8e-ai/g8e-g8e-pod` with `latest` and `sha` tags.
...
**Shared SSE Fixtures** (`shared/test-fixtures/sse-events.json`) — Canonical event structures shared across g8ee and VSOD to prevent interface drift. Used by contract tests to ensure both components emit/consume compatible event formats.
...
- **`@vsod/services/clients/internal_http_client.js`** — the HTTP boundary to g8ee; mocking this is correct for VSOD-only integration tests
...
| `tests/unit/utils/test_shared_bound_operator_context.py` | Contract test: g8ee BoundOperator model fields must exactly match `shared/models/wire/bound_operator_context.json` — prevents desynchronization between VSOD's BoundOperatorContext.forWire() output and g8ee's BoundOperator parsing logic |
...
- [developer.md](developer.md) — Universal code quality rules, constants/model/serialization patterns, service and error handling rules for VSA, g8ee, and VSOD
