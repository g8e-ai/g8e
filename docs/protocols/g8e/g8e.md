...
Protocol errors follow standardized JSON-RPC codes (defined in `internal/responder/responder.go`):
...
Each operator session owns an isolated, encrypted git repository (`.g8e/data/ledger/sessions/<id>/`). Every file mutation triggers a native Go `go-git` commit tracking the `LedgerHashBefore` and `LedgerHashAfter`.
