...
| gateway_signed | [bool](#bool) |  | Flag indicating if the transaction was signed by the local gateway without full Tribunal consensus. This is used for single-agent MCP clients that bypass the L2 consensus layer. |
| l2_valid | [bool](#bool) |  | Flag indicating if L2 (Consensus/Tribunal) signature was valid. Set to false if signature was missing or invalid but posture allowed execution. |
| l3_valid | [bool](#bool) |  | Flag indicating if L3 (Notary/Human) proof was valid. Set to false if proof was missing or invalid but posture allowed execution. |
...
| state_root_at_commit | [string](#string) |  | StateRootAtCommit is the Gateway&#39;s state root at the moment the Auditor signed this attestation. It is checked against the envelope.StateMerkleRoot the Tribunal/L2 signed over. |
