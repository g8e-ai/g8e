...
- `g8e compliance report generate --scope <scope.json> [--demo-run <run-id>] [--eval-run <run-id>] [--source <operational-source-dir>] [standalone source flags] --report-id <report-id> --profile <public|restricted> --signing-metadata <path> --signing-private-key <path> [--evidence-trust <path>]` imports only sources admitted by the protected `AssessmentScope`, copies exact source bytes into admission-local protected paths, grades the evidence, constructs canonical analysis and framework profiles, renders every supported format, signs the complete bundle, persists it under the runtime report tree, and prints the descriptor path. The scope supplies the assessment window and assessment-as-of time; source trust remains separate and signed standalone sources require the external assessed evidence trust input during generation.
- `g8e compliance report verify <bundle-manifest.json> --trust-policy <assessed-report-trust.json> [--evidence-trust <assessed-evidence-trust.json>]` verifies protected scope and source bodies, directory inventory, signed roots, external assessed trust, independent source replay, protected analysis equality, and deterministic renderer reproduction offline and emits a canonical `ComplianceVerificationReport`. Evidence trust is required whenever commitment or customer or assessor attestation evidence is represented.
