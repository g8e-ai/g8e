...
To refresh the snapshot after a current-tree eval or demo campaign:

1. Select eval runs whose manifests bind source revision, tree hash, suite version, dataset hash, grader hash, prompt hash, and model mapping. Retain failed attempts and invalid evidence in the analytical population; do not cherry-pick only successful rows.
2. Run `g8e-evals verify-receipts` with every producing actuator public key. Capture a machine-readable receipt verification result. Require a nonzero receipt total, zero failed signatures, zero failed persistence attestations, zero missing keys, and verified equal to total.
3. Run `g8e compliance demo-run verify <run-id>` for every selected demo and retain the canonical `ComplianceVerificationReport` JSON. Require `"valid": true` and zero failures.
4. Export only synthetic or hash-safe projections: eval manifests, attempts, metrics, a canonical receipt projection, stage records needed for displayed governance claims, the receipt verification result, and the demo reports. Keep restricted prompts, model outputs, and raw canaries encrypted and out of the public snapshot.
5. Compute SHA-256 for every artifact, populate `docs/evidence/readme/current/index.json`, and scan the directory for private keys, credentials, raw canary values, absolute paths, and undeclared files.
6. Run `make readme` to regenerate `README.md`, then run `make readme-check` to confirm it is byte-identical and `make readme-test` to confirm the generator still passes.
7. Review every displayed claim against its linked source record. The README may state only the bounded receipt verification result and verified metric outcomes until a complete eval-bundle verifier exists.
