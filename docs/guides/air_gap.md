...
- **Protocol Library (Go)**: The protocol is part of the root Go module `github.com/g8e-ai/g8e/v2`. Since all dependencies are vendored, `go get github.com/g8e-ai/g8e/v2@v2.1.7` works offline once the vendor directory is populated. No additional downloads are required.
- **Protocol Library (Python)**: The `g8e` Python package is published to PyPI. For air-gapped environments, download the wheel on a connected host and transfer it:
  ...
  ```bash
  # On connected host:
  pip download g8e==2.1.7 -d /tmp/g8e-python-pkg

  # Transfer /tmp/g8e-python-pkg/ to the air-gapped host, then:
  pip install --no-index --find-links /tmp/g8e-python-pkg g8e==2.1.7
  ```
