...
**What to expect:** A JSON file at `~/live-demo/tribunal-bootstrap.json`. The gateway will read this at startup, derive a key pair from the seed, register the public key as a trusted tribunal signer, and save the private key to disk so the in-process `LocalDeliberator` can sign L2 votes.

> **Background:** Notary posture enforces both L2 signatures and L3 proofs. The gateway bootstraps the Tribunal under `consensus || notary` (fixed in this release). The `bootstrapTribunalPolicy` function saves the seed-derived private key to disk for each member app ID, so `LocalDeliberator` can sign L2 votes.
...
> **Notes:**
> - No `--doctrine-dir` flag — L1 doctrine rules are compiled into the binary (~60 regex detectors + protobuf forbidden patterns). There is no runtime doctrine loading mechanism.
...
**2. What happens under the hood:** Claude calls the `run_shell_command` tool with `{"command":"python3","args":["/home/bob/g8e/demos/live-swarm/drone_cmd.py","takeoff","20"]}`. The gateway receives the call, runs it through L1 doctrine (passes), L2 Tribunal (in-process deliberator signs), then L3 — the warden rejects with `ErrL3ProofMissing` and the gateway **suspends the transaction**.
...
| Poll interval | **10 seconds** | Up to a ~10s beat between tapping your passkey and Claude proceeding — use this gap to show the signed receipt in the browser |
...
| Max poll attempts | **30** (5 minutes total) | If you don't approve within 2 minutes the TTL expires; if you approve but the poll window closes, Claude won't get the result |
...
**Technical notes:**
- `goto_location` takes **absolute (AMSL)** altitude. The `set_altitude` command converts relative → AMSL using telemetry data.
