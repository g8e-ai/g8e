...
5. The trail is scanned for `g8e.v1.ai.governance.L5Actuator.receipt.signed`. If present, its `transaction_hash` becomes the Gateway `transaction_id` and `ReceiptCollector` polls `/api/audit/receipts?tx_id=...` for the signed `ActionReceipt`.
...
|---|---|---|
| Answer-only turn (no Consensus→L5Actuator mutation) | `None` | `UNBOUND` |
| L5Actuator signed an ActionReceipt during the turn | receipt `transaction_hash` | `RECEIPT_BOUND` |
...
`g8e_evals.receipts.verify.verify_receipt_signature(receipt, L5Actuator_pub)` performs offline Ed25519 verification:

- Loads the L5Actuator public key from `${G8E_PKI_DIR:-.g8e/pki}/L5Actuator_pub.pem` (exported by Listen Mode startup in `cmd/g8eo/main.go`).
- Verifies the signature over `transaction_hash + "|true"` with the receipt's declared `key_id` matching the L5Actuator key id.
...
- `--assistant-provider/--assistant-model`, `--lite-provider/--lite-model` - per-role overrides for Consensus/Auditor and lightweight calls.
