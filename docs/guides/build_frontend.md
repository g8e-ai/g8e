...
- `enroll()` — write path. Loads any persisted unexpired pending attempt; if none exists or the persisted attempt has expired, generates an ECDSA P-256 key and CSR, fetches the CA bundle from the gateway's plain-HTTP discovery surface, submits a platform enrollment request with the CSR and system fingerprint, persists pending state (private key, requester token, request ID, CSR fingerprints, and expiry) to `pki/pending-enrollment/dashboard.json` with `0600` permissions, polls status with bounded backoff, signs the canonical completion transcript, validates the returned identity against the expected SAN, and writes the credentials. On restart with an unexpired pending attempt, it resumes the same request and restores its instance ID without generating new keys.

The enrollment is resumable and idempotent: on restart with a valid, non-near-expiry cert, the reuse path short-circuits. On restart while an unexpired platform enrollment request is pending, the service loads the persisted pending state and resumes polling the same request. An expired attempt is replaced with a new request so a persistent dashboard volume cannot trap the container in a restart loop. On enrollment failure, the dashboard container exits non-zero (fail-closed) so Docker's healthcheck and restart policy surface the failure. The enrolled credentials persist across container restarts in the `g8e-dashboard-data` named volume. See [auth.md](../architecture/auth.md) §1.5 for the owner-approved platform enrollment protocol.
