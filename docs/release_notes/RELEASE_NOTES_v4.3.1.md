...
**Root cause:** SSE event pushes from VSE to VSOD were missing the `user_id` field. The `SSEPushRequest` model did not require `user_id`, and VSE services were not including it when publishing events. This caused operator panel updates to fail silently when the heartbeat service attempted to push events without the required user context.
...
- Updated all VSE services to include `user_id` when publishing events via `vsod_event_service`:
  - `AgentSSEService`
  - `AgentToolLoop`
  - `ChatPipeline`
  - `ChatTaskManager`
...
- **VSE event publishing** — All VSE services now include `user_id` when publishing events via `vsod_event_service`
...
| Component | Changes |
|-----------|---------|
| **VSE** | Added `user_id` to all event publishing calls across 7 services |
