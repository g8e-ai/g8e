...
- **CodeQL & Static Analysis Compliance** — platform-wide security review completed, fixing potential path traversal, URL validation, and command injection risks.
- **No-DNS / Direct-IP Operations** — print MCP configurations using direct IP addresses or plain HTTP without requiring hosts file or DNS resolution.
- **Standardized cross-platform setups** — official, automated setup scripts added for Linux, macOS, and Windows.
