...
> **Note (v1.7.1):** The two-phase model introduced here was later collapsed back
> into a single-phase constructor. `RuntimeDependencies` was merged into
> `Dependencies`, `SetRuntimeDeps`/`runtimeReady`/`getRuntimeDeps` were removed,
> and lazy forwarding adapters (`pubsub.GatewayEnvProcAdapter`,
> `pubsub.GatewaySessionValidatorAdapter`) were added to break the circular
> dependency that originally motivated the two-phase split. See the v1.7.1
