...
* **Canonical wire format** - Formalized canonical JSON (`protojson`) as the required client-facing wire format for all Governance Envelopes instead of g8e Node protobuf bytes, ensuring universal BYO client compatibility.
...
* **mTLS bootstrap** - First-time mTLS setup sequence fixed; PKI test initialization refactored for
  isolation.
* **g8e Node build process** - "build once, then copy" pattern enforces a single compilation artifact
  per g8e Node, eliminating race conditions during `gw start`.
