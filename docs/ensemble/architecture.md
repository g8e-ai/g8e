...
| g8ee to target Operator | Dispatches typed host-operation requests and receives correlated results through Gateway HTTP. | `GatewayOperatorClient.dispatch()` posts a registered request `event_type` and serialized protobuf payload to `POST /api/v1/operators/commands` over the enrolled app mTLS connection. The Gateway validates the event, derives `action_type` from the registry, constructs the governed envelope, and forwards it to the matching Operator session. |
...
The enrolled app certificate is g8ee's normal Gateway identity for the DB, KV, blob, event, and HTTP clients. The configured Gateway URL identifies the Gateway-facing HTTPS surfaces from the deployment's network perspective. An Operator session is separate delegated authority: g8ee discovers session data through its application workflow and validates caller-supplied sessions with the Gateway. The app certificate is not an Operator certificate and does not grant unrestricted host access.

After identity resolution, the lifespan creates the mTLS transport configuration and connects the DB, KV, and blob clients. It loads platform settings through the cache-aside service, overlays them on local bootstrap settings, constructs `GovernanceClient`, `GatewayOperatorClient`, and all domain, Operator, chat, and provider services, then starts certificate initialization and the HTTP client service. g8ee does not connect to Gateway pub/sub. Operator heartbeat persistence and freshness evaluation are Gateway-owned; g8ee does not subscribe to heartbeat channels or run stale-heartbeat monitors. FastAPI yields readiness only after these startup hooks complete. Shutdown waits up to five seconds for tracked chat tasks before stopping background services and transports.
...
For the governed dispatch path, the Gateway constructs the envelope from the caller's registered request `event_type` and the target Operator verifies it. For protected application-record writes, g8ee supplies the envelope and the Gateway verifies it. In either case the active posture determines the required gates:
