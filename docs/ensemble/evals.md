...
The workflow uses these commands in order:

1. `qualification hash-source` reads an owner-approved `SourceManifestAuthority`, walks only its explicit manifest entries, applies its path-component exclusions, rejects symlinks and missing entries, and emits a typed result carrying both the authority's logical content hash and exact rendered-file hash plus the computed source-tree hash. Run it separately for the full-source and execution-source authorities.
2. `qualification candidate` reads both source-manifest results, hashes the exact host binary, validates the canonical Gateway/Operator, Ensemble, and Dashboard image mapping, and emits one content-addressed candidate identity.
3. `qualification run-gate` executes an argument vector directly without a shell and emits a candidate-bound gate result containing timestamps, exit status, stdout and stderr hashes, tool versions, skipped-test declarations, and the exact command. Failed commands still produce failed evidence and cannot satisfy qualification.
4. `./g8e test public-loop --candidate <candidate.json> --output <public-loop.json>` runs the isolated provider-free durable publication harness. It proves authenticated ingest, ordered outage retry, anonymous bootstrap/history/snapshot/SSE, proof download, mirror restart, replacement-key registration, old-key revocation, continued publication, and exactly zero inference invocations.
5. `qualification collect-runtime` reads the candidate, exact host/Gateway/Operator `version --json` records, public PKI certificates, and the explicitly observed owner, session, health, and enrollment state. It computes certificate DER fingerprints and verifies exact SPIFFE SANs, version, and execution-source stamps. It never reads a private key.
6. `qualification build` resolves the typed build request's candidate, runtime, gate, public-loop, authority, prior-candidate, and invalidation files; recomputes each content and file hash; rejects missing, duplicate, failed, stale, mismatched, symlinked, or escaping evidence; and writes a deterministic `owner_review_required` draft. `--check` compares a draft byte for byte without rewriting it.

Content hashes and file hashes are distinct. A content hash covers compact canonical JSON with the `content_hash` field omitted. A file hash covers the exact rendered bytes, including whitespace and the stored `content_hash`. Every artifact binding names both values so formatting changes cannot be mistaken for semantic changes and semantic changes cannot be hidden by a filename.

Historical hand-assembled r1-r5 qualification records, their approvals, leases, and invalidations remain immutable. `verify_historical_qualification` reproduces their logical and file hashes in compare-only mode. New drafts use the typed workflow and supersede prior qualifications by content hash; no legacy construction shim exists.
