...
North Star model campaigns score real models through the same `POST /api/v1/chat` path used in production. The campaign CLI does not call Ollama or model APIs directly. Instead, `g8e eval campaign execute` drives assignments that:
