...
This document defines the deterministic execution constraints for code generated for the g8e platform, the build/dependency lifecycle, and the documentation-authoring conventions. The platform is an open-source, self-hosted AI governance layer designed for offline operation. The substrate is the **g8e Protocol** (shared protobuf schemas + the canonical-JSON wire contract). `g8eo` is the reference Operator implementation in Go; `g8ee` is a reference application-layer adapter in Python. Both are replaceable by any conforming Operator or BYO client.

For contribution workflow, code review process, and CLA, see the root `CONTRIBUTING.md`. For security disclosure, see `SECURITY.md`. For community standards, see `CODE_OF_CONDUCT.md`.
...
See also: [Protocol](protocol.md), [Operator](operator.md), [g8ee Service](g8ee.md).
