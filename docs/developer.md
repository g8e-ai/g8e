...
1. Setup:
   ...
   - Node.js 22+ (only when developing the optional g8ed adapter)
...
2. Commands:
   ...
   - `./g8e apps start [g8ed|g8ee|all]`: Starts optional application-layer adapters explicitly.
...
3. UAP JSON First: The canonical mutation envelope is UAP JSON (`components/g8eo/pkg/uap/types.go`). All mutation commands use UAPEnvelope carrying structured intent_data. Protobuf in `shared/proto/*.proto` defines typed request/result payloads (e.g., CommandRequested, HeartbeatResult) but not the mutation envelope itself.
...
2. g8ed (Node.js/Express, optional application-layer adapter)
   ...
   - Adapter Boundary: Must not add new substrate responsibilities or private Operator access paths.

3. g8ee (Python/FastAPI, optional application-layer adapter)
...
3. Contract Tests: Enforce alignment between the Operator, optional adapters, and `shared/` constants/models.