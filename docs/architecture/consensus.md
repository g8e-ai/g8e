...
- **Gateway-internal deliberation**: Under `consensus` and `notary` postures, the MCP gateway's `processGatewayTransaction` automatically sends the envelope to the `LocalDeliberator` for L2 deliberation before dispatch. Demo scenarios use `MCPToolsCall` (Path A) — the harness calls the MCP `tools/call` endpoint and the gateway builds the `GovernanceEnvelope` internally, runs L2 deliberation, and suspends for L3 notary approval if required. The harness then approves via `ApproveWithWebAuthn` using a `SoftAuthenticator` (software WebAuthn passkey). `SubmitMaximal`/`Ensemble` remain as conformance testing infrastructure only.
- **Software passkey for L3**: The `SoftAuthenticator` (`internal/tools/agent_harness/client/authenticator.go`) generates genuine ECDSA P-256 WebAuthn assertions. The gateway verifies them with full real verification via `PasskeyService.VerifyL3Proof` — it cannot distinguish them from browser-based authenticators. The `G8E_L3_MOCK` environment variable has been removed; the gateway always requires real WebAuthn proof.
