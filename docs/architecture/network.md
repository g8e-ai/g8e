...
CLI-initiated passkey enrollment uses dedicated endpoints separate from the console bootstrap and JIT flows:

| Route | Port | Auth | Purpose |
|------|------|------|---------|
| `/api/v1/auth/passkeys/enrollment/register/challenge` | HTTPS only | Enrollment token | Validate the one-time enrollment token and return a WebAuthn registration challenge |
| `/api/v1/auth/passkeys/enrollment/register/verify` | HTTPS only | Enrollment token | Consume the token (one-time) and verify the attestation; sets a web session cookie |

The enrollment token is generated by the CLI via the mTLS-protected `/api/v1/auth/enrollment-token/generate` endpoint and passed to the browser via the `#enroll=1&token=...` URL fragment. The gateway derives `user_id` and `cli_session_id` from the token; neither is sent in the request body. The challenge step validates the token; the verify step consumes it.

### Operator Command Dispatch Routes

| Route | Port | Auth | Purpose |
|------|------|------|---------|
| `/api/v1/operators/commands` | HTTPS only | mTLS only | Dispatch a command to an operator via the governance pipeline |
| `/api/v1/operators/session/{id}` | HTTPS only | mTLS only | Look up an operator by session ID |

The dispatch endpoint accepts a typed `OperatorCommandRequest`, routes it through the governance pipeline, and returns a `DispatchResponse` with the dispatch ID and result.
