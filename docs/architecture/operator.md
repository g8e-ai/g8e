...
- **Protocol-First**: Every mutation action is governed by UAP JSON envelopes carrying structured intent_data. Result payloads use typed Protobuf messages from `operator.proto`.
...
6. **Steady State**: Subscribes to `cmd:{operator_id}` for UAP JSON mutation commands.
