...
**Interrogation Protocol**: Per GDD §14.1, clarifying questions are generated by reasoning agents (**Dash** for simple requests, **Sage** for complex requests) — not by Triage. When an agent emits questions, tool execution is deferred until answers are received. Question-answer pairs are persisted to the conversation ledger as structured context.
