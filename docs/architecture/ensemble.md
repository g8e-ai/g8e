...
The dashboard (`g8ed`) follows the same enrollment contract with its own Node.js implementation: `dashboard/services/infra/app-enrollment-service.js` mirrors the ensemble's `AppEnrollmentService` (two explicit operations, `loadIdentity()` read path and `enroll()` write path, no hidden fallback), enrolls as app name `g8ed` to obtain the SPIFFE identity `spiffe://g8e.local/app/g8ed`, and persists credentials to its own `g8e-dashboard-data` volume under the same runtime tree layout (`pki/issued/apps/g8ed.crt`, `g8ed.key`, `pki/trust/hub-bundle.pem`). The dashboard's `G8E_GATEWAY_HTTP_URL` and `G8E_RUNTIME_DIR` env vars point at the gateway's plain-HTTP bootstrap surface and the dashboard's own runtime directory, mirroring the ensemble's `G8E_GATEWAY_HTTP_URL`/`G8E_RUNTIME_DIR`. See [Dashboard (g8ed)](./dashboard.md) for the dashboard architecture and the dashboard app enrollment plan for the parallel implementation details.
