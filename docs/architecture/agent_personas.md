...
- **Prompt Source**: Persona voice in `shared/constants/agents.json` (`dash`) + modular doctrine / mode / context sections in `components/g8ee/app/prompts_data/`. Dash routes through the **same** `build_modular_system_prompt` path as Sage and loads the full safety / loyalty / dissent / mode / tools / response-constraints stack. Per the Tribunal GDD (`.local.dev/dev/tribunal-game.md` §14.1) the GDD's "Dash" interrogator role lives in the **triage** agent, not in the code's `dash` agent; the code's `dash` plays the same game as Sage on the assistant model tier.
- **Injection Point**: `build_modular_system_prompt(agent_name=AgentName.DASH, ...)` uses the persona's system prompt in place of `core/identity.txt`. Only the persona block differs from Sage; everything else in the prompt stack is identical.
