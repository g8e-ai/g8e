...
L1 provides hardcoded, non-negotiable safety invariants that are enforced before and during any generation.
...
*   **Auto-Approval (Skip-Approval)**: To minimize click fatigue, benign diagnostic commands (e.g., `uptime`, `df`, `free`) can be marked as L3 approved after L1 and L2 have already passed.
*   **Safety Invariant**: Auto-approval **NEVER** bypasses L1 Technical Bedrock or L2 Consensus. It only represents L3 authorization state for commands already verified as technically safe and low-risk.
