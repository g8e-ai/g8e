...
The canonical transaction container is the **GovernanceEnvelope**, a signed protobuf message that binds identity, intent, state, replay-protection material, and governance proofs into a single transaction. See [Authentication & Authorization](./auth.md) for the identity and session fields, and [Protocol](./protocol.md) for the wire format.
...
Consensus enforces everything from doctrine plus L2 consensus signature verification. The envelope must include L2 metadata with at least one vote, the signer and consensus stores must be configured, the consensus policy must exist and be enabled, signatures must verify against trusted public keys, and the affirmative vote count from valid distinct members must meet quorum. L3 notary proofs remain audited only.
...
Notary is the default posture for outbound (operator) mode, so operators running outbound require full L1, L2, and L3 enforcement by default. Because the L3 notary is nil in outbound mode unless explicitly configured, mutations fail closed until a notary is provided. See [Authentication & Authorization](./auth.md) for which action types are classified as mutations and how notary proofs are produced.

### Choosing a Posture

The doctrine and consensus postures allow mutations to execute without human authorization or, for doctrine, without multi-party consensus. Selecting such a posture is itself an act of human intent; the `--posture` flag is the authorization and the gateway logs a warning at startup. An unrecognized posture name causes startup to fail rather than silently running under a weaker posture.

| Mode | Default Posture | Configured Via |
|---|---|---|
| Gateway mode | Doctrine | `--posture` flag; defaults to doctrine when omitted |
| Outbound (operator) mode | Notary | Defaults to notary when omitted |
...
The **Producer** wraps the intent in a signed GovernanceEnvelope carrying the typed payload, principal identity, nonce, and state root. Under `consensus` and `notary` postures, the gateway obtains L2 consensus votes before dispatch by sending the envelope to the Consensus for deliberation. Under `doctrine` posture, L2 votes are not required.
