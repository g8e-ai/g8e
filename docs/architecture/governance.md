...
1.  **Protobuf Reflection (`forbidden_patterns`)**: 
    Enforced in `components/g8eo/internal/services/governance/transaction_verifier.go` via `validateL1Governance`. It scans incoming messages for the `forbidden_patterns` custom option defined in `protocol/proto/operator.proto`. For example, the `command` field in `CommandRequested` prohibits `sudo`, `su`, and `rm -rf /`.
    
2.  **Sentinel Guard (Threat Analysis)**:
    Enforced in `components/g8eo/internal/services/sentinel/sentinel.go`. Sentinel performs pre-execution regex analysis against 90+ threat patterns mapped to the MITRE ATT&CK framework (e.g., reverse shells, privilege escalation, data exfiltration).
...
4.  **L2 Signature**: 
    ...
    `g8eo` verifies this signature in `components/g8eo/internal/services/governance/transaction_verifier.go`. It uses an Operator-owned `SignerStore` (database-backed) for O(1) public key lookup. The verifier rejects every missing-key or unknown-key transaction until signer material is provisioned.
