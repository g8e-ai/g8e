...
1.  **Protobuf Reflection (`forbidden_patterns`)**: 
    Enforced in `components/g8eo/internal/services/governance/transaction_verifier.go` via `validateL1Governance`. It scans incoming messages for the `forbidden_patterns` custom option defined in `shared/proto/operator.proto`. For example, the `command` field in `CommandRequested` prohibits `sudo`, `su`, and `rm -rf /`.
