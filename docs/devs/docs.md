...
| --- | --- | --- |
| Root `README.md` | Handwritten product overview sourced from verified current behavior and linked evidence | Re-read changed sections, validate links, and run every command whose behavior the prose documents. |
...
Never hand-edit generated outputs without changing their owner.
...
- [Root README](../../README.md): Product overview, evidence boundaries, quick start, architecture summary, and documentation routes.
...
- Manual edits to generated protobuf reference or OpenAPI output without updating and running its source owner.
...
| Root README | Re-read changed sections, validate links, run documented commands, and run website checks when rendering changes |
...
| Ensemble or native evaluation documentation | `make ensemble-test`, `make ensemble-lint`, `./g8e test unit`, and the real `./g8e eval` command as applicable |
