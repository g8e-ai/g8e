...
The root `README.md` is generated by `scripts/generate_readme.py` from `docs/templates/README.md.tmpl` and the public proof snapshot in `docs/evidence/readme/current/`. The generator validates the selection manifest (`index.json`), verifies declared artifact SHA-256 checksums, rejects path traversal and undeclared files, aggregates eligible metric rows from `metrics.jsonl`, projects receipt verification and canonical demo compliance reports, and renders the template. It is offline, deterministic, and credential-free. Run `make readme` to regenerate, `make readme-check` to detect drift without writing files, and `make readme-test` to run the generator unit tests. The CI workflow runs `make readme-test` and `make readme-check` to prevent drift. The public snapshot contains only reviewed synthetic or hash-safe artifacts; restricted evidence remains encrypted and out of the snapshot. The evidence refresh procedure is documented separately in the evidence/gauntlet guide.
