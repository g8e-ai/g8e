...
│   │   ├── consensus.ConsensusService (direct *consensus.ConsensusService injected at construction; nil only under doctrine posture where the consensus route is unregistered, so the 503-on-nil path is unreachable — posture-gated route registration returns 404 instead)
...
│   ├── consensus.L2ConsensusDeliberator (consensus.LocalDeliberator in gateway mode; construction-phase, wired from the bootstrapped consensus service — nil only in doctrine posture where the deliberator is not called)
...
- The synchronous envelope-processing endpoint at `/api/v1/governance/envelopes` and all governance logic live on `GovernanceController` in `governance_controller.go`. `GovernanceController` receives a direct `*consensus.ConsensusService` (nil only under doctrine posture, where the consensus route is unregistered) and a non-nil `governance.EnvelopeProcessor` (always wired in gateway mode via concrete `*pubsub.OperatorPubSubService` injection) as constructor arguments. The consensus deliberate route is posture-gated: registered only in consensus/notary postures, so doctrine posture returns 404 (route unregistered) rather than 503. There are no 503-on-nil guards — mode and posture are enforced at the type and route-registration level, not at the call site.
