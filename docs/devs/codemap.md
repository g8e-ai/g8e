...
- **Posture sub-typing (B1)**: within gateway mode, posture (doctrine / consensus / ratify / notary) determines whether `Consensus` is present. `GatewayModeDeps.Consensus` is a typed optional (`*consensus.ConsensusService`, nil when the posture does not require L2); the `GovernanceController` consensus route is registered only in consensus/notary postures, so the previous 503-on-nil guard is unreachable (the route is unregistered and returns 404 in doctrine and ratify postures).
...
- **`ksi.go`**: Typed KSI model with `KSI`, `KSIMethod`, `KSIResult`, `KSIResultSet`, `KSICatalog`, `Evidence`, `AutomatedMethod` structs. Typed enums for `KSICategory`, `KSIStatus`, `CertificationClass`, `ValidationCycle`, `EvidenceType`. No raw maps. Catalog loaded from `docs/reference/ksi-catalog.json` (31 KSIs across 10 categories, CR26-derived legacy evaluator catalog). See [v2.1.3 proof-backed compliance reporting](../plans/in-progress/v2.1.3_proof_backed_compliance_reporting.md) for replacement and expansion work.
...
- **`oscal.go`**: `OSCALExporter` struct with `GenerateComponentDefinition` and `GenerateAssessmentResults` methods. Typed OSCAL structs implement the current component-definition and assessment-results renderer. See [v2.1.3 proof-backed compliance reporting](../plans/in-progress/v2.1.3_proof_backed_compliance_reporting.md) for proof-backed bundle generation work.
