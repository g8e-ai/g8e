...
**Governance testing** uses cryptographic actors (per-member Ed25519 signers for L2 consensus, `SoftAuthenticator` software passkey for L3 approval) to exercise governance flows via `MCPToolsCall` (Path A). The gateway runs `L2ConsensusDeliberator` internally and suspends transactions requiring L3 notary approval. The `SoftAuthenticator` (`client/authenticator.go`) generates genuine WebAuthn assertions (ECDSA P-256, ES256) that the gateway verifies with full real verification — it cannot distinguish them from browser-based authenticators. `ApproveWithWebAuthn` POSTs the assertion to the approval verify endpoint. Each consensus member signs with its own distinct key derived from `member_seeds` in the bootstrap config, making `RequireDistinct` and quorum cryptographically meaningful. `SubmitMaximal`/`Ensemble` remain as conformance testing infrastructure only. Unit tests for `SoftAuthenticator` (12 tests in `authenticator_test.go`), `ApproveWithWebAuthn` (`client_test.go`), and `shellCommandMap` (`shell_command_test.go`) provide coverage. A fail-closed regression test (`TestGatewayModeService_GetGovernanceDeps_AlwaysUsesRealNotary`) verifies the gateway ignores `G8E_L3_MOCK` and always requires real WebAuthn proof.
