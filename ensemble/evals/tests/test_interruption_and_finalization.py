...
class TestInterruptionVsTerminal:
    ...
    def test_resume_after_interruption_writes_new_index_generation(self, tmp_path: Path):
        """Resume after interruption writes a new index generation.

        Runs a campaign, simulates a process kill by removing analysis.json
        and corrupting the campaign status, then resumes. The resumed run
        should write a new index generation (RESUME or SUPERSESSION).
        """
        spec = _make_spec()
        runner = CampaignRunner(
            spec=spec,
        ...
        gen_count_before = len(lines1)

        # Simulate process kill: remove analysis.json (interruption evidence)
        (result1.report_dir / ANALYSIS_JSON).unlink()

        # Resume — the runner should continue and write a new generation
        ...
        assert len(lines2) > gen_count_before, "resume should write a new index generation"

    def test_interrupted_partial_directory_preserved_as_evidence(self, tmp_path: Path):
        """An interrupted partial directory is preserved as immutable evidence.

        After a process kill, the partial report directory (with its
        attempts and metrics) must remain on disk. Resume must not delete
        ...
        attempts_path = result1.report_dir / ATTEMPTS_JSONL
        metrics_path = result1.report_dir / METRICS_JSONL
        ...
        # The original attempts and metrics must still be present
        # (evidence preserved, not deleted)
        assert attempts_path.exists(), "attempts.jsonl must be preserved after resume"
        assert metrics_path.exists(), "metrics.jsonl must be preserved after resume"
        # The attempts content must be preserved (not truncated or emptied)
        assert attempts_path.read_text() == original_attempts, (
            "attempts.jsonl content must be unchanged after resume"
        )
        # The metrics content must be preserved (not truncated or emptied)
        assert metrics_path.read_text() == original_metrics, (
            "metrics.jsonl content must be unchanged after resume"
        )
