# Copyright (c) 2026 Lateralus Labs, LLC.
# Use of this source code is governed by the Business Source License
# included in the LICENSE file.
#
# As of the Change Date listed in the LICENSE file, this software is
# released under the Apache License, Version 2.0.

"""Tier 2 integration tests for Piper Phase 2 verifier remediation items 5-9.

Covers the corrected verifier deepening that Ion's Phase 2 work did not
exercise:

5. ExpectedRecordPolicy binding to CampaignBinding.required_record_policy_hash
   and the report suite; DERIVED cardinality through a closed typed
   derivation vocabulary.
6. VERIFIED evidence resolution fails when the evidence index is absent or
   empty; run/attempt ownership verification for evidence index entries.
7. Bijection between provider inference stages and resource observations
   using exact (attempt_id, inference_id, stage_id) identities, not count
   equality alone. Rejects duplicates, cross-attempt joins, orphan stages.
8. Cross-binding of event records to child/report identity and environment
   scope comparison against the run binding's hardware and environment
   authority.
9. Typed metric recomputation: unit mismatch, unknown metric ID, and value
   inconsistency with terminal status fail closed.
"""

from __future__ import annotations

import asyncio
import hashlib
import json
from dataclasses import dataclass
from pathlib import Path

import pytest

pytestmark = pytest.mark.integration

from g8e_evals.analysis.canonical import (
    ClaimPolicy,
    ContinuousTestPolicy,
    PreregistrationConfig,
)
from g8e_evals.arms import Arm
from g8e_evals.campaign import (
    InitialStateAssignmentManifest,
    ModelCohort,
    RetryPolicy,
    RoleModelBinding,
    SamplingSettings,
    TaskAssignmentManifest,
    compute_initial_state_hash,
    compute_model_cohort_hash,
    compute_task_assignment_hash,
)
from g8e_evals.campaign_verify import verify_campaign
from g8e_evals.constants import (
    ATTEMPTS_JSONL,
    CAMPAIGN_MANIFEST_JSON,
    CORRELATED_ERRORS_JSONL,
    EVIDENCE_INDEX_JSONL,
    ESCALATION_RECORDS_JSONL,
    EXPECTED_RECORD_POLICY_JSON,
    METRICS_JSONL,
    REPORT_CHECKSUM_JSON,
    RESOURCE_OBSERVATIONS_JSONL,
    SECURITY_EVENTS_JSONL,
    STAGES_JSONL,
    TOOL_CALL_SCORECARDS_JSONL,
)
from g8e_evals.expected_record_policy import (
    CardinalityRule,
    ExpectedRecordEntry,
    ExpectedRecordPolicy,
    RecordApplicability,
    compute_expected_record_policy_hash,
)
from g8e_evals.harness import Response, Score, Task
from g8e_evals.index import (
    MeasurementAvailability,
    MeasurementScope,
    ResourceObservation,
    UnavailableMeasurement,
)
from g8e_evals.models import ScoreDetails, TaskMetadata
from g8e_evals.runner import CampaignRunner, CampaignSpec
from g8e_evals.schema import (
    EscalationOutcome,
    EscalationRecord,
    EvidenceIndex,
    EvidenceMediaType,
    StageKind,
    StageObservation,
)

_CAMPAIGN_ID = "v2.1.8-ifeval-pipeline-integrity"
_RELEASE_VERSION = "v2.1.8"
_TASK_IDS = ["task-1001", "task-1019"]
_COHORT_ID = "cohort-qwen3-8b"
_DATASET_HASH = "5eee4bb145007b67e3fe38899fc18a49a8b29b1d6ad844c76a160795bc9b6d37"
_NO_STATE_HASH = "0" * 64
_VALID_HASH = "a" * 64

_MEASUREMENT_FIELDS = (
    "model_load_time_seconds",
    "peak_resident_memory_bytes",
    "peak_accelerator_memory_bytes",
    "artifact_bytes",
    "measured_energy_joules",
    "end_to_end_latency_seconds",
    "provider_call_latency_seconds",
    "output_throughput_tokens_per_second",
    "hidden_reasoning_throughput_tokens_per_second",
    "time_to_first_token_seconds",
    "generation_duration_seconds",
    "accelerator_memory_before_bytes",
    "gpu_utilization_percent",
    "gpu_temperature_celsius",
    "gpu_power_draw_watts",
    "gpu_clock_mhz",
)


@dataclass
class _FakeSUT:
    model_id: str
    answer: str = "This is a test answer with no commas."

    async def get_answer(self, task: Task) -> Response:
        return Response(answer=self.answer, model=self.model_id, arm=Arm.DIRECT)


@dataclass
class _FakeGrader:
    grader_id: str = "ifeval_subset_verifier"
    grader_version: str = "1.0.0"

    def grade(self, task: Task, response: Response) -> Score:
        return Score(task_id=task.id, passed=True, details=ScoreDetails())


def _make_spec() -> CampaignSpec:
    cohort = ModelCohort(
        cohort_id=_COHORT_ID,
        role_bindings=[RoleModelBinding(
            role="primary",
            model_id="qwen3:8b",
            provider="ollama",
            endpoint="http://192.168.1.2:11434",
            sampling_settings=SamplingSettings(temperature=0.0, top_p=1.0, max_tokens=4096, seed=42),
            timeout_seconds=120.0,
            seed_capable=True,
        )],
        content_hash=compute_model_cohort_hash(_COHORT_ID, [RoleModelBinding(
            role="primary",
            model_id="qwen3:8b",
            provider="ollama",
            endpoint="http://192.168.1.2:11434",
            sampling_settings=SamplingSettings(temperature=0.0, top_p=1.0, max_tokens=4096, seed=42),
            timeout_seconds=120.0,
            seed_capable=True,
        )]),
    )
    task_assignment = TaskAssignmentManifest(
        task_assignment_id="task-assignment-v1",
        suite_id="ifeval_subset",
        dataset_hash=_DATASET_HASH,
        task_ids=_TASK_IDS,
        content_hash=compute_task_assignment_hash("task-assignment-v1", "ifeval_subset", _DATASET_HASH, _TASK_IDS),
    )
    initial_state = InitialStateAssignmentManifest(
        initial_state_assignment_id="no-initial-state-v1",
        state_type="no_initial_state",
        snapshot_hash=_NO_STATE_HASH,
        content_hash=compute_initial_state_hash("no-initial-state-v1", "no_initial_state", _NO_STATE_HASH),
    )
    prereg = PreregistrationConfig(
        config_id="test-config-1",
        config_version="1.0.0",
        baseline_arm_id="direct",
        comparison_arm_ids=["ensemble_ungoverned"],
        model_cohort_ids=[_COHORT_ID],
        task_assignment_id="task-assignment-v1",
        initial_state_assignment_id="no-initial-state-v1",
        required_replicate_ids=["replicate-1"],
        required_replicate_count=1,
        primary_metric_ids=["ifeval_subset_verifier"],
        continuous_test_policy=ContinuousTestPolicy.PAIRED_T,
        bootstrap_count=10000,
        bootstrap_confidence=0.95,
        bootstrap_seed=0,
        significance_level=0.05,
        claim_policy=ClaimPolicy.DESCRIPTIVE_ONLY,
    )
    prompt_bundle = "\n".join(f"Prompt for {tid}" for tid in _TASK_IDS).encode()
    return CampaignSpec(
        campaign_id=_CAMPAIGN_ID,
        release_version=_RELEASE_VERSION,
        suite="ifeval_subset",
        suite_id="ifeval_subset",
        suite_version="1.0.0",
        dataset_hash=_DATASET_HASH,
        prompt_bundle_hash=hashlib.sha256(prompt_bundle).hexdigest(),
        grader_bundle_hash="g" * 64,
        preregistration=prereg,
        cohorts=[cohort],
        task_assignment=task_assignment,
        initial_state=initial_state,
        retry_policy=RetryPolicy(max_retries=1, retryable_terminal_statuses=["infrastructure_failed"]),
        randomization_seed=42,
    )


def _make_tasks() -> list[Task]:
    tasks = []
    for tid in _TASK_IDS:
        tasks.append(Task(
            id=tid,
            prompt=f"Prompt for {tid}",
            metadata=TaskMetadata(
                benchmark="ifeval_subset",
                instruction_id_list=["punctuation:no_comma"],
                kwargs=[{"no_comma": True}],
            ),
        ))
    return tasks


def _fake_sut_factory(cohort: ModelCohort, arm: Arm):
    return _FakeSUT(model_id=cohort.role_bindings[0].model_id)


def _run_campaign(tmp_path: Path) -> Path:
    spec = _make_spec()
    runner = CampaignRunner(
        spec=spec,
        sut_factory=_fake_sut_factory,
        tasks=_make_tasks(),
        grader=_FakeGrader(),
        output_dir=tmp_path,
    )
    result = asyncio.run(runner.run())
    return result.report_dir


def _all_attempt_identities(report_dir: Path) -> list[dict]:
    cm = json.loads((report_dir / CAMPAIGN_MANIFEST_JSON).read_text())
    attempts_path = report_dir / ATTEMPTS_JSONL
    lines = attempts_path.read_text().strip().splitlines()
    identities = []
    for line in lines:
        a = json.loads(line)
        identities.append({
            "campaign_id": cm["campaign_id"],
            "child_id": cm["campaign_id"],
            "run_id": a["run_id"],
            "task_id": a["task_id"],
            "assignment_id": a.get("assignment_id", ""),
            "attempt_id": a["attempt_id"],
        })
    return identities


def _all_unavailable() -> list[UnavailableMeasurement]:
    return [
        UnavailableMeasurement(
            field_name=f,
            availability=MeasurementAvailability.UNAVAILABLE,
            scope=MeasurementScope.PROVIDER_REMOTE,
            reason=f"{f} not available",
        )
        for f in _MEASUREMENT_FIELDS
    ]


def _make_observation_dict(**kwargs) -> dict:
    ident = kwargs.pop("ident", None)
    defaults = {
        "campaign_id": "campaign-1",
        "child_id": "campaign-1",
        "run_id": "run-1",
        "assignment_id": "assignment-1",
        "attempt_id": "att-1",
        "inference_id": "inf-1",
        "stage_id": "att-1:model_inference:1",
        "role": "primary",
        "model_variant_id": "qwen3:8b",
        "task_id": "task-1",
        "orchestrator_scope": "linux/amd64/cpu",
        "provider_scope": "unavailable",
        "observation_boundary": "provider_call",
        "clock_domain": "monotonic",
        "collection_tool": "psutil-5.9",
        "source_evidence_refs": ["evidence/test.json"],
        "source_evidence_sha256": _VALID_HASH,
        "verification_status": "verified",
        "unavailable_measurements": [u.model_dump() for u in _all_unavailable()],
    }
    for f in _MEASUREMENT_FIELDS:
        defaults[f] = None
    if ident:
        defaults.update(ident)
    defaults.update(kwargs)
    obs = ResourceObservation(**defaults)
    return json.loads(obs.model_dump_json())


def _write_observations_for_all_attempts(report_dir: Path) -> None:
    idents = _all_attempt_identities(report_dir)
    lines = []
    for i, ident in enumerate(idents):
        obs = _make_observation_dict(ident={**ident, "inference_id": f"inf-{i}", "stage_id": f"{ident['attempt_id']}:model_inference:1"})
        lines.append(json.dumps(obs))
    (report_dir / RESOURCE_OBSERVATIONS_JSONL).write_text("\n".join(lines) + "\n")


def _make_policy(
    *,
    entries: list[dict] | None = None,
    policy_id: str = "test-policy-1",
    suite_id: str = "ifeval_subset",
) -> ExpectedRecordPolicy:
    if entries is None:
        entries = [
            {"file_name": RESOURCE_OBSERVATIONS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
            {"file_name": TOOL_CALL_SCORECARDS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
            {"file_name": ESCALATION_RECORDS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_attempt"},
            {"file_name": SECURITY_EVENTS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_attempt"},
            {"file_name": CORRELATED_ERRORS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
        ]
    normalized = [
        {
            "file_name": e["file_name"],
            "applicability": e["applicability"],
            "cardinality_rule": e["cardinality_rule"],
            "expected_count": e.get("expected_count"),
            "derivation_rule": e.get("derivation_rule", ""),
        }
        for e in entries
    ]
    content_hash = compute_expected_record_policy_hash(
        schema_version="1.0.0",
        policy_id=policy_id,
        policy_version="1.0.0",
        suite_id=suite_id,
        entries=normalized,
    )
    return ExpectedRecordPolicy(
        schema_version="1.0.0",
        policy_id=policy_id,
        policy_version="1.0.0",
        suite_id=suite_id,
        entries=[
            ExpectedRecordEntry(
                file_name=e["file_name"],
                applicability=RecordApplicability(e["applicability"]),
                cardinality_rule=CardinalityRule(e["cardinality_rule"]),
                expected_count=e.get("expected_count"),
                derivation_rule=e.get("derivation_rule", ""),
            )
            for e in entries
        ],
        content_hash=content_hash,
    )


def _write_policy(report_dir: Path, policy: ExpectedRecordPolicy) -> None:
    (report_dir / EXPECTED_RECORD_POLICY_JSON).write_text(
        policy.model_dump_json()
    )


def _make_stage_dict(
    *,
    attempt_id: str,
    run_id: str,
    stage_id: str | None = None,
    kind: str = "model_inference",
    agent_role: str = "primary",
    provider: str = "ollama",
    model: str = "qwen3:8b",
) -> dict:
    if stage_id is None:
        stage_id = f"{attempt_id}:{kind}:1"
    stage = StageObservation(
        stage_id=stage_id,
        attempt_id=attempt_id,
        run_id=run_id,
        kind=StageKind(kind),
        agent_role=agent_role,
        provider=provider,
        model=model,
    )
    return json.loads(stage.model_dump_json())


def _write_stages_for_all_attempts(report_dir: Path) -> None:
    idents = _all_attempt_identities(report_dir)
    lines = []
    for ident in idents:
        stage = _make_stage_dict(
            attempt_id=ident["attempt_id"],
            run_id=ident["run_id"],
            stage_id=f"{ident['attempt_id']}:model_inference:1",
        )
        lines.append(json.dumps(stage))
    (report_dir / STAGES_JSONL).write_text("\n".join(lines) + "\n")


def _make_evidence_index_entry(
    *,
    artifact_id: str,
    run_id: str,
    attempt_id: str | None = None,
    sha256: str | None = None,
) -> dict:
    entry = EvidenceIndex(
        artifact_id=artifact_id,
        run_id=run_id,
        attempt_id=attempt_id,
        media_type=EvidenceMediaType.APPLICATION_JSON,
        sha256=sha256 or _VALID_HASH,
    )
    return json.loads(entry.model_dump_json())


def _write_evidence_index(report_dir: Path, entries: list[dict]) -> None:
    lines = [json.dumps(e) for e in entries]
    (report_dir / EVIDENCE_INDEX_JSONL).write_text("\n".join(lines) + "\n")


def _write_evidence_index_for_observations(report_dir: Path) -> None:
    """Write a single evidence index entry that all VERIFIED observations
    reference, with the correct run_id for ownership checks."""
    idents = _all_attempt_identities(report_dir)
    if not idents:
        return
    evidence_entries = [
        _make_evidence_index_entry(
            artifact_id="evidence/test.json",
            run_id=idents[0]["run_id"],
            attempt_id=None,
        )
    ]
    _write_evidence_index(report_dir, evidence_entries)


def _set_campaign_binding(report_dir: Path, *, policy_hash: str | None = None) -> None:
    """Inject or update the campaign_binding in manifest.json so the
    verifier's binding checks fire. The test CampaignSpec does not provide
    a campaign_profile or model_registry, so the runner leaves
    campaign_binding as None. This helper constructs a valid binding with
    the given policy hash (or a default) and writes it back."""
    manifest_path = report_dir / "manifest.json"
    manifest = json.loads(manifest_path.read_text())
    if policy_hash is None:
        policy_hash = _VALID_HASH
    manifest["campaign_binding"] = {
        "campaign_id": manifest.get("campaign_id", _CAMPAIGN_ID),
        "campaign_revision": _RELEASE_VERSION,
        "report_role": "single",
        "child_campaign_id": None,
        "child_campaign_revision": None,
        "campaign_profile_hash": "a" * 64,
        "model_registry_hash": "a" * 64,
        "required_record_policy_hash": policy_hash,
        "orchestrator_hardware_identity": "linux/amd64/cpu",
        "orchestrator_environment_stratum": "dev",
        "provider_hardware_identity": "unavailable",
        "provider_environment_stratum": "unavailable",
    }
    manifest_path.write_text(json.dumps(manifest))


def _make_escalation_dict(**kwargs) -> dict:
    ident = kwargs.pop("ident", None)
    defaults = {
        "record_id": "er-1",
        "campaign_id": "campaign-1",
        "child_id": "campaign-1",
        "assignment_id": "assignment-1",
        "attempt_id": "att-1",
        "inference_id": None,
        "run_id": "run-1",
        "task_id": "task-1",
        "agent_persona": "triage",
        "model_variant_id": "qwen3:8b",
        "expected_role": "lite",
        "ground_truth_complexity": "light",
        "routed_to_role": "lite",
        "outcome": EscalationOutcome.CORRECT_AUTONOMOUS,
        "task_succeeded": True,
    }
    if ident:
        defaults.update(ident)
    defaults.update(kwargs)
    er = EscalationRecord(**defaults)
    return json.loads(er.model_dump_json())


def _recompute_report_checksum(report_dir: Path) -> None:
    """Recompute the report checksum after tampering so the standalone
    validator does not fail on checksum mismatch."""
    from g8e_evals.report.validate import _compute_report_checksum
    new_checksum = _compute_report_checksum(report_dir)
    checksum_path = report_dir / REPORT_CHECKSUM_JSON
    if checksum_path.exists():
        checksum_data = json.loads(checksum_path.read_text())
        checksum_data["checksum"] = new_checksum
        checksum_path.write_text(json.dumps(checksum_data))


# ---------------------------------------------------------------------------
# Item 5: ExpectedRecordPolicy binding to CampaignBinding hash and suite
# ---------------------------------------------------------------------------


class TestPolicyHashAndSuiteBinding:
    """Layer 13: the loaded ExpectedRecordPolicy is bound to
    CampaignBinding.required_record_policy_hash and the report suite."""

    def test_policy_hash_mismatch_with_binding_fails(self, tmp_path: Path):
        """A policy whose content_hash does not match the
        CampaignBinding.required_record_policy_hash is rejected."""
        report_dir = _run_campaign(tmp_path)
        _write_observations_for_all_attempts(report_dir)
        _write_evidence_index_for_observations(report_dir)
        # Write a valid policy
        policy = _make_policy(entries=[
            {"file_name": RESOURCE_OBSERVATIONS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
            {"file_name": TOOL_CALL_SCORECARDS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
            {"file_name": ESCALATION_RECORDS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_attempt"},
            {"file_name": SECURITY_EVENTS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_attempt"},
            {"file_name": CORRELATED_ERRORS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
        ])
        _write_policy(report_dir, policy)
        # Set the binding's required_record_policy_hash to a different value
        _set_campaign_binding(report_dir, policy_hash="b" * 64)
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("policy" in f.lower() and "hash" in f.lower() for f in result.failures), (
            f"expected policy hash mismatch failure, got: {result.failures}"
        )

    def test_policy_suite_mismatch_fails(self, tmp_path: Path):
        """A policy whose suite_id does not match the campaign suite is
        rejected."""
        report_dir = _run_campaign(tmp_path)
        _write_observations_for_all_attempts(report_dir)
        _write_evidence_index_for_observations(report_dir)
        # Write a policy with a wrong suite_id
        policy = _make_policy(
            suite_id="wrong_suite",
            entries=[
                {"file_name": RESOURCE_OBSERVATIONS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
                {"file_name": TOOL_CALL_SCORECARDS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
                {"file_name": ESCALATION_RECORDS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_attempt"},
                {"file_name": SECURITY_EVENTS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_attempt"},
                {"file_name": CORRELATED_ERRORS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
            ],
        )
        _write_policy(report_dir, policy)
        # Set the binding hash to match the wrong policy so the hash
        # check passes and the suite check is the one that fires
        _set_campaign_binding(report_dir, policy_hash=policy.content_hash)
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("suite" in f.lower() for f in result.failures), (
            f"expected suite mismatch failure, got: {result.failures}"
        )

    def test_matching_policy_hash_and_suite_passes(self, tmp_path: Path):
        """A policy whose content_hash matches the binding and whose suite
        matches the campaign passes."""
        report_dir = _run_campaign(tmp_path)
        _write_stages_for_all_attempts(report_dir)
        _write_observations_for_all_attempts(report_dir)
        _write_evidence_index_for_observations(report_dir)
        policy = _make_policy(entries=[
            {"file_name": RESOURCE_OBSERVATIONS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
            {"file_name": TOOL_CALL_SCORECARDS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
            {"file_name": ESCALATION_RECORDS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_attempt"},
            {"file_name": SECURITY_EVENTS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_attempt"},
            {"file_name": CORRELATED_ERRORS_JSONL, "applicability": "optional", "cardinality_rule": "one_per_inference"},
        ])
        _write_policy(report_dir, policy)
        # Set the binding hash to match the policy
        _set_campaign_binding(report_dir, policy_hash=policy.content_hash)
        result = verify_campaign(report_dir)
        assert result.ok, f"matching policy hash and suite should pass: {result.failures}"


# ---------------------------------------------------------------------------
# Item 6: VERIFIED evidence resolution fails when evidence index is absent
# ---------------------------------------------------------------------------


class TestVerifiedEvidenceResolutionStrictness:
    """VERIFIED records must fail when the evidence index is absent or empty,
    and evidence index entries must match the record's run/attempt ownership."""

    def test_verified_observation_with_absent_evidence_index_fails(self, tmp_path: Path):
        """A VERIFIED resource observation with no evidence-index.jsonl file
        is rejected. The evidence index is required for VERIFIED records."""
        report_dir = _run_campaign(tmp_path)
        _write_observations_for_all_attempts(report_dir)
        # Ensure no evidence index exists
        evidence_path = report_dir / EVIDENCE_INDEX_JSONL
        if evidence_path.exists():
            evidence_path.unlink()
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("evidence" in f.lower() for f in result.failures), (
            f"expected evidence index absent failure, got: {result.failures}"
        )

    def test_verified_observation_with_empty_evidence_index_fails(self, tmp_path: Path):
        """A VERIFIED resource observation with an empty evidence-index.jsonl
        file is rejected. An empty index does not satisfy VERIFIED evidence
        requirements."""
        report_dir = _run_campaign(tmp_path)
        _write_observations_for_all_attempts(report_dir)
        # Write an empty evidence index
        (report_dir / EVIDENCE_INDEX_JSONL).write_text("")
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("evidence" in f.lower() for f in result.failures), (
            f"expected empty evidence index failure, got: {result.failures}"
        )

    def test_evidence_index_run_ownership_mismatch_fails(self, tmp_path: Path):
        """A VERIFIED observation whose source_evidence_ref resolves to an
        evidence index entry with a different run_id is rejected."""
        report_dir = _run_campaign(tmp_path)
        _write_observations_for_all_attempts(report_dir)
        # Write evidence index with a wrong run_id
        evidence_entries = [
            _make_evidence_index_entry(
                artifact_id="evidence/test.json",
                run_id="wrong-run-id",
                attempt_id=None,
            )
        ]
        _write_evidence_index(report_dir, evidence_entries)
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("evidence" in f.lower() and ("run" in f.lower() or "ownership" in f.lower()) for f in result.failures), (
            f"expected evidence run ownership failure, got: {result.failures}"
        )


# ---------------------------------------------------------------------------
# Item 7: Bijection between inference stages and resource observations
# ---------------------------------------------------------------------------


class TestStageObservationBijection:
    """Layer 19: a bijection between provider inference stages and resource
    observations using exact (attempt_id, inference_id, stage_id) identities,
    not count equality alone."""

    def test_duplicate_observation_identity_fails(self, tmp_path: Path):
        """Two resource observations with the same (attempt_id, inference_id,
        stage_id) triple are rejected as duplicates."""
        report_dir = _run_campaign(tmp_path)
        _write_stages_for_all_attempts(report_dir)
        _write_observations_for_all_attempts(report_dir)
        # Duplicate the first observation
        idents = _all_attempt_identities(report_dir)
        ident = idents[0]
        dup = _make_observation_dict(ident={
            **ident,
            "inference_id": "inf-0",
            "stage_id": f"{ident['attempt_id']}:model_inference:1",
        })
        existing = (report_dir / RESOURCE_OBSERVATIONS_JSONL).read_text()
        (report_dir / RESOURCE_OBSERVATIONS_JSONL).write_text(existing + json.dumps(dup) + "\n")
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("duplicate" in f.lower() for f in result.failures), (
            f"expected duplicate observation failure, got: {result.failures}"
        )

    def test_orphan_stage_fails(self, tmp_path: Path):
        """A model_inference stage with no matching resource observation is
        rejected as an orphan stage."""
        report_dir = _run_campaign(tmp_path)
        _write_stages_for_all_attempts(report_dir)
        _write_observations_for_all_attempts(report_dir)
        # Add an extra stage with no matching observation
        idents = _all_attempt_identities(report_dir)
        extra_stage = _make_stage_dict(
            attempt_id=idents[0]["attempt_id"],
            run_id=idents[0]["run_id"],
            stage_id=f"{idents[0]['attempt_id']}:model_inference:orphan",
        )
        existing = (report_dir / STAGES_JSONL).read_text()
        (report_dir / STAGES_JSONL).write_text(existing + json.dumps(extra_stage) + "\n")
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("orphan" in f.lower() or "stage" in f.lower() for f in result.failures), (
            f"expected orphan stage failure, got: {result.failures}"
        )


# ---------------------------------------------------------------------------
# Item 8: Event record cross-binding to child/report and environment scopes
# ---------------------------------------------------------------------------


class TestEventChildIdAndScopeBinding:
    """Event records are cross-bound to child/report identity and resource
    observations' environment scopes are compared against the run binding."""

    def test_event_child_id_mismatch_fails(self, tmp_path: Path):
        """An event record whose child_id does not match the campaign
        manifest's campaign_id is rejected."""
        report_dir = _run_campaign(tmp_path)
        idents = _all_attempt_identities(report_dir)
        # Write an escalation record with a wrong child_id
        er = _make_escalation_dict(ident={**idents[0], "child_id": "wrong-child-id"})
        (report_dir / ESCALATION_RECORDS_JSONL).write_text(json.dumps(er) + "\n")
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("child_id" in f.lower() for f in result.failures), (
            f"expected child_id mismatch failure, got: {result.failures}"
        )

    def test_observation_orchestrator_scope_mismatch_fails(self, tmp_path: Path):
        """A resource observation whose orchestrator_scope does not match the
        run binding's orchestrator_hardware_identity is rejected."""
        report_dir = _run_campaign(tmp_path)
        _write_observations_for_all_attempts(report_dir)
        _write_evidence_index_for_observations(report_dir)
        _set_campaign_binding(report_dir)
        # Tamper one observation's orchestrator_scope
        obs_path = report_dir / RESOURCE_OBSERVATIONS_JSONL
        lines = obs_path.read_text().strip().splitlines()
        first = json.loads(lines[0])
        first["orchestrator_scope"] = "wrong/scope/identity"
        lines[0] = json.dumps(first)
        obs_path.write_text("\n".join(lines) + "\n")
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("orchestrator" in f.lower() and "scope" in f.lower() for f in result.failures), (
            f"expected orchestrator scope mismatch failure, got: {result.failures}"
        )


# ---------------------------------------------------------------------------
# Item 9: Typed metric recomputation
# ---------------------------------------------------------------------------


class TestTypedMetricRecomputation:
    """Metrics are recomputed from authoritative records. Unit mismatch,
    unknown metric ID, and value inconsistency with terminal status fail
    closed."""

    def test_metric_unit_mismatch_fails(self, tmp_path: Path):
        """A metric observation whose unit does not match the registered
        metric definition's unit is rejected."""
        report_dir = _run_campaign(tmp_path)
        # Tamper a metric's unit
        metrics_path = report_dir / METRICS_JSONL
        lines = metrics_path.read_text().strip().splitlines()
        first = json.loads(lines[0])
        first["unit"] = "proportion"  # Should be "boolean"
        lines[0] = json.dumps(first)
        metrics_path.write_text("\n".join(lines) + "\n")
        _recompute_report_checksum(report_dir)
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("unit" in f.lower() for f in result.failures), (
            f"expected unit mismatch failure, got: {result.failures}"
        )

    def test_unknown_metric_id_fails(self, tmp_path: Path):
        """A metric observation with an unknown metric_id is rejected."""
        report_dir = _run_campaign(tmp_path)
        # Tamper a metric's ID to an unknown value
        metrics_path = report_dir / METRICS_JSONL
        lines = metrics_path.read_text().strip().splitlines()
        first = json.loads(lines[0])
        first["metric_id"] = "unknown_metric_id"
        lines[0] = json.dumps(first)
        metrics_path.write_text("\n".join(lines) + "\n")
        _recompute_report_checksum(report_dir)
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("unknown" in f.lower() and "metric" in f.lower() for f in result.failures), (
            f"expected unknown metric failure, got: {result.failures}"
        )

    def test_metric_value_inconsistent_with_terminal_status_fails(self, tmp_path: Path):
        """A completed attempt's metric value must be 0.0 or 1.0 for a
        boolean pass/fail metric. A value outside this range is rejected."""
        report_dir = _run_campaign(tmp_path)
        # Tamper a metric's value to an invalid range
        metrics_path = report_dir / METRICS_JSONL
        lines = metrics_path.read_text().strip().splitlines()
        first = json.loads(lines[0])
        first["value"] = 5.0  # Should be 0.0 or 1.0 for boolean
        lines[0] = json.dumps(first)
        metrics_path.write_text("\n".join(lines) + "\n")
        _recompute_report_checksum(report_dir)
        result = verify_campaign(report_dir)
        assert not result.ok
        assert any("value" in f.lower() for f in result.failures), (
            f"expected metric value inconsistency failure, got: {result.failures}"
        )
