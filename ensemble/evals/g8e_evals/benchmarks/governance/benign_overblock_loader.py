...
"""Suite loader for the synthetic benign-overblock eval suite.

Each task in the suite defines a benign action that the governance path
must allow (not over-block).  The loader reads the JSONL dataset,
validates it against the provenance manifest, and yields ``Task`` objects
with typed ``TaskMetadata`` carrying ``PolicyAttackAssertion`` records
with ``expected_outcome`` set to ``allow``.

The suite complements the policy-violating attack suite: together they
form the allow/block confusion matrix.  Every assertion in this suite
declares ``expected_outcome = allow`` and no ``expected_rejection_layer``,
so the ``PolicyAttackGrader`` checks that the governance path allows the
action and does not produce a false-positive block.
"""
...
class BenignOverblockLoader:
    """Loads tasks for the synthetic benign-overblock suite.

    Each JSONL row contains a ``key``, a ``description``, the typed
    ``policy_attack_assertions`` list (all with ``expected_outcome = allow``),
    and scenario parameters that the CLI uses to set up the
    ``LocalGovernanceSimulator``.
    """
    ...
    def load(self) -> Iterable[Task]:
        if not self.gold_set_path.exists():
            raise FileNotFoundError(
                f"benign overblock gold set not found at {self.gold_set_path}"
            )
        ...
        for data in rows:
            yield Task(
                ...
                metadata=TaskMetadata(
                    ...
                    expected_action_class=data.get("expected_action_class", "BENIGN_OVERBLOCK_PROBE"),
                    policy_attack_assertions=[
                        PolicyAttackAssertion(
                            ...
                            severity=AttackSeverity(a.get("severity", "low")),
