...
"""Suite loader for the synthetic final-state eval suite.

Each task in the suite declares an expected final-state predicate
(``state_root_changed`` or ``state_root_unchanged``) for a declared
action type, with typed ``FinalStateAssertion`` records.  The loader
reads the JSONL dataset, validates it against the provenance manifest,
and yields ``Task`` objects with typed ``TaskMetadata``.

The scenario parameters (per-assertion ``state_root_before`` and
``state_root_after`` values) are carried in the ``benchmark_specific``
scenario params so the CLI observer setup can configure the
``LocalFinalStateSimulator`` without embedding utility-critical known
shapes in free-form metadata.  The state-root values are simulator
...
class FinalStateLoader:
    """Loads tasks for the synthetic final-state suite.

    Each JSONL row contains a ``key``, a ``description``, the typed
    ``expected_final_state_assertions`` list, and scenario parameters
    that the CLI uses to set up the ``LocalFinalStateSimulator``.
    """
    ...
    def load(self) -> Iterable[Task]:
        if not self.gold_set_path.exists():
            raise FileNotFoundError(
                f"final-state gold set not found at {self.gold_set_path}"
            )
        ...
        for data in rows:
            yield Task(
                ...
                metadata=TaskMetadata(
                    ...
                    expected_action_class=data.get("expected_action_class", "FINAL_STATE"),
                    expected_final_state_assertions=[
                        FinalStateAssertion(
                            assertion_id=a["assertion_id"],
                            predicate=StateAssertionPredicate(a["predicate"]),
                            action_type=a["action_type"],
                        )
                        for a in data.get("expected_final_state_assertions", [])
                    ],
