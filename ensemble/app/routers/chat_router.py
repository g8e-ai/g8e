    ...
    if investigation:
        if investigation.user_id != authenticated_user_id:
            raise ResourceNotFoundError(
                ...
                resource_id=web_session_id,
                component="g8ee",
            )

        session_status = (
            ChatSessionStatus.ACTIVE
            if _is_chat_session_active(investigation.status)
            else ChatSessionStatus.INACTIVE
        )
...
async def get_latest_chat_session_for_case(
    ...
    if raw_case is None:
        raise ResourceNotFoundError(
            "Case not found", resource_type="case", resource_id=case_id, component="g8ee"
        )
    ...
    raw_investigations = (
        await investigation_service.investigation_data_service.get_case_investigations(
            case_id=case_id,
    ...
    for inv in raw_investigations:
        if not inv.conversation_history:
            continue
        if latest_investigation is None or (inv.updated_at or inv.created_at) > (
            latest_investigation.updated_at or latest_investigation.created_at
        ):
    ...
    if latest_investigation:
        logger.info(
            ...
            extra={
                ...
                "message_count": len(latest_investigation.conversation_history),
                "user_id": authenticated_user_id,
            },
        )
