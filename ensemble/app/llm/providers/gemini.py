...
class GeminiProvider(LLMProvider):
    ...
    def _build_genai_config(
        ...
        if isinstance(settings, PrimaryLLMSettings):
            ...
            logged_thinking_level = (
                thinking_translation.thinking_level
                if thinking_translation and thinking_translation.enabled
                else None
            )
            logged_include_thoughts = bool(
                thinking_translation and thinking_translation.include_thoughts
            )
            log_parts = [
        ...
        config_kwargs = {
            ...
            "response_json_schema": _response_format_to_genai(settings.response_format)
    ...
    def _sdk_chunk_to_stream_from_model_chunks(chunk) -> list[StreamChunkFromModel]:
        ...
        if not chunk.candidates:
            if chunk.usage_metadata:
                result.append(
                    StreamChunkFromModel(usage_metadata=_usage_from_sdk(chunk.usage_metadata))
                )
            return result
        ...
        if candidate.content and candidate.content.parts:
            for sdk_part in candidate.content.parts:
                ...
                if thought_flag and text:
                    result.append(
                        StreamChunkFromModel(text=text, thought=True, thought_signature=sig)
                    )
                elif fc:
                    result.append(
                        StreamChunkFromModel(
    ...
    async def generate_content_stream_primary(
        ...
        logger.info(
            ...
            if primary_llm_settings.system_instructions is not None
            else 0,
