...
class Part:
    ...
    def from_tool_response(
        cls, name: str, response: dict[str, Any], call_id: str | None = None
    ) -> Part:
...
class ResponseFormat:
    ...
    @classmethod
    def from_pydantic_schema(
        cls, json_schema: dict[str, Any], name: str = "response"
    ) -> ResponseFormat:
    ...
    def flatten_for_openai(self) -> dict[str, Any]:
        return {
            "type": "json_schema",
            "json_schema": self.json_schema.flatten_for_openai(),
            "version": 1,
        }
