...
def _format_operator_doc(op_doc, index: int) -> str:
    ...
    details = [
        ...
        f"arch={snapshot.system_identity.architecture}"
        if snapshot and snapshot.system_identity.architecture
        else None,
...
def build_investigation_context_section(investigation: EnrichedInvestigationContext | None) -> str:
    ...
    context_parts = [
        f"{label.value}: {getattr(investigation, field)}"
...
def build_triage_context_section(triage_result: TriageResult | None) -> str:
    ...
    posture = (
        triage_result.request_posture.value
        if hasattr(triage_result.request_posture, "value")
        else str(triage_result.request_posture)
    )
...
def build_command_constraints_message(
    ...
    if whitelisting_enabled:
        if whitelisted_commands:
            ...
            parts.append(
                f"Whitelisting is ENABLED. Only these commands are allowed: {', '.join(command_names)}"
            )
            ...
            parts.append(
                "Whitelisting is ENABLED, but no commands are whitelisted. ALL commands will be rejected."
            )
    ...
    if blacklisting_enabled:
        if blacklisted_commands:
            ...
            parts.append(
                f"Blacklisting is ENABLED. These commands are FORBIDDEN: {', '.join(blacklisted_names)}"
            )
        else:
...
def build_tribunal_auditor_context(
    ...
    if mode == "unanimous":
        ...
        parts.append(
            "Validate it for syntactic correctness, safety, and alignment with the request."
        )
        parts.append("\nResponse format:")
        ...
        parts.append(
            "YOU MUST DISAMBIGUATE. Pick the best cluster from the tied set or provide a revision."
        )
        parts.append("\nResponse format:")
...
def build_modular_system_prompt(
    ...
    if system_context:
        ...
        contexts_to_render = (
            system_context if isinstance(system_context, list) else [system_context]
        )
        logger.info(
            ...
            contexts_to_render[0].username
            if contexts_to_render and contexts_to_render[0]
            else None,
            contexts_to_render[0].uid if contexts_to_render and contexts_to_render[0] else None,
            contexts_to_render[0].hostname
            if contexts_to_render and contexts_to_render[0]
            else None,
