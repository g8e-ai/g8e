...
class CommandBlacklistValidator:
    ...
    def _load(self, path: Path) -> None:
        ...
        if not enabled:
            logger.warning(
                "Command blacklist is disabled via 'enabled: false'; "
        ...
        self._config = {key: data.get(key, []) for key in required_sections}
        self._compiled_patterns = []
        for entry in self._config["forbidden_patterns"]:
            ...
            try:
                ...
                raise ConfigurationError(
                    f"Invalid blacklist regex '{pattern}': {exc}", cause=exc
                ) from exc
    ...
    def validate_command(self, command_string: str) -> CommandBlacklistResult:
        ...
        forbidden_args = {
            entry.get("value")
            for entry in self._config["forbidden_arguments"]
            if entry.get("value")
        }
        if forbidden_args and any(arg in forbidden_args for arg in tokens[1:]):
            ...
            entry = next(
                item for item in self._config["forbidden_arguments"] if item.get("value") == hit
            )
            return CommandBlacklistResult(
                is_allowed=False,
                reason=entry.get("reason") or "",
    ...
    def get_forbidden_commands(self) -> list[dict[str, str]]:
        ...
        return [
            {"command": e.get("value", ""), "reason": e.get("reason", "")}
            for e in self._config["forbidden_commands"]
            if e.get("value")
        ]
    ...
    def get_forbidden_substrings(self) -> list[dict[str, str]]:
        ...
        return [
            {"substring": e.get("value", ""), "reason": e.get("reason", "")}
            for e in self._config["forbidden_substrings"]
            if e.get("value")
        ]
    ...
    def get_forbidden_patterns(self) -> list[dict[str, str]]:
        ...
        return [
            {"pattern": e.get("value", ""), "reason": e.get("reason", "")}
            for e in self._config["forbidden_patterns"]
            if e.get("value")
        ]
