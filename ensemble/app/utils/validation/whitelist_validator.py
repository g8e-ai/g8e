...
class CommandWhitelistValidator:
    ...
    def __init__(self, whitelist_path: str):
        ...
        if whitelist_path:
            ...
            if resolved_path.exists():
                ...
                logger.error(
                    "Command whitelist not found - AI command execution disabled for security"
                )
                raise ConfigurationError(
                    f"Required whitelist configuration not found at {resolved_path}"
                )
    ...
    def load_whitelist(self, whitelist_path: str) -> None:
        ...
        self.whitelist_data = load_json_config(
            Path(whitelist_path), config_name="whitelist configuration"
        )

        enabled = self.whitelist_data.get("enabled", True)
        if not enabled:
            logger.warning(
                "Command whitelist is disabled via 'enabled: false'; "
        ...
        logger.info("Loaded command whitelist with %d commands", len(self.all_commands))
    ...
    def _parse_whitelist_data(self) -> None:
        ...
        logger.info(
            "Parsed whitelist: %d categories, %d forbidden patterns",
            len(self.commands_by_category),
    ...
    def validate_command(
        ...
        if not command_string or not command_string.strip():
            ...
            return CommandValidationResult(is_valid=False, command="", reason="Empty command")
    ...
    def _validate_command_arguments(
        self, command: str, args: list[str], config: dict[str, Any], platform: Platform
    ) -> CommandValidationResult:
    ...
    def get_command_metadata(
        self, command: str, platform: Platform = Platform.LINUX
    ) -> WhitelistedCommand | None:
    ...
    def get_available_commands_with_metadata(
        self, platform: Platform = Platform.LINUX
    ) -> list[WhitelistedCommand]:
