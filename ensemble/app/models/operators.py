...
class AttachmentRecord(G8eIdentifiableModel):
    ...
    type: AttachmentType = Field(
        default=AttachmentType.OTHER, description="Classified attachment type"
    )
...
class CommandResultRecord(G8eBaseModel):
    ...
    status: ExecutionStatus = Field(
        default=ExecutionStatus.COMPLETED, description="Execution status"
    )
    ...
    operator_session_id: str | None = Field(
        default=None, description="Associated operator session ID"
    )
...
class HeartbeatNetworkInfo(G8eBaseModel):
    ...
    interfaces: list[str] | None = Field(
        default=None, description="List of network interface names"
    )
    connectivity_status: list[HeartbeatNetworkInterface] | None = Field(
        default=None, description="Active network interfaces with IP and MTU"
    )
...
class HeartbeatSnapshot(G8eBaseModel):
    ...
    system_identity: HeartbeatSystemIdentity = Field(
        default_factory=HeartbeatSystemIdentity, description="System identity information"
    )
    ...
    os_details: HeartbeatOSDetails = Field(
        default_factory=HeartbeatOSDetails, description="Operating system details"
    )

    # User details
    user_details: HeartbeatUserDetails = Field(
        default_factory=HeartbeatUserDetails, description="Current user details"
    )

    # Environment
    environment: HeartbeatEnvironment = Field(
        default_factory=HeartbeatEnvironment, description="Environment and container context"
    )

    # Operator version
    version_info: HeartbeatVersionInfo = Field(
        default_factory=HeartbeatVersionInfo, description="Operator version metadata"
    )

    # Detailed disk and memory sub-reports
    disk_details: HeartbeatDiskDetails = Field(
        default_factory=HeartbeatDiskDetails, description="Detailed root filesystem disk usage"
    )

    memory_details: HeartbeatMemoryDetails = Field(
        default_factory=HeartbeatMemoryDetails, description="Detailed memory usage"
    )

    # Network identity fields (top-level in g8eo payload, not nested under network_info)
    system_fingerprint: str | None = Field(
        default=None, description="SHA256 fingerprint of stable host attributes"
    )
    fingerprint_details: HeartbeatFingerprintDetails | None = Field(
    ...
    local_storage_enabled: bool = Field(
        default=False, description="True when local (Project Chronos) storage is active"
    )
    git_available: bool = Field(
        default=False, description="True when git is available on the operator host"
    )
...
class PendingApproval(G8eBaseModel):
    ...
    operation: FileOperation | None = Field(
        default=None, description="File operation (for FILE_EDIT approvals)"
    )
...
class ApprovalResult(G8eBaseModel):
    ...
    feedback: bool = Field(
        default=False, description="Whether user provided feedback instead of approve/deny"
    )
    operator_session_id: str | None = Field(
        default=None, description="Updated operator session ID from approval response"
    )
    operator_id: str | None = Field(
        default=None, description="Updated operator ID from approval response"
    )
    intent_name: str | None = Field(
        default=None, description="Normalized intent name (INTENT approvals only)"
    )
...
class ApprovalRequestBase(G8eBaseModel):
    ...
    g8e_context: G8eHttpContext = Field(
        description="HTTP context with session/case/investigation identity"
    )
...
class IntentApprovalRequest(ApprovalRequestBase):
    ...
    all_intents: list[str | CloudIntent] = Field(
        description="All intents being requested (including dependencies)"
    )
...
class AgentContinueApprovalRequest(ApprovalRequestBase):
    ...
    turns_completed: int = Field(
        description="Number of tool-use turns executed when the budget was reached"
    )
    task_id: str | None = Field(default=None, description="AI task identifier for SSE routing")
    operator_session_id: str = Field(
        default="", description="Operator session identifier (unused for agent-scoped approvals)"
    )
    operator_id: str = Field(
        default="", description="Operator identifier (unused for agent-scoped approvals)"
    )
...
class BatchOperatorExecutionResult(G8eBaseModel):
    ...
    result: CommandInternalResult | None = Field(
        default=None, description="Execution result payload"
    )
...
class ApprovalContext(G8eBaseModel):
    ...
    execution_id: str | None = Field(
        default=None, description="Operator execution ID at time of request"
    )
    requested_at: UTCDatetime = Field(
        default_factory=now, description="When the approval was requested"
    )
    timeout_seconds: int = Field(description="Approval timeout in seconds")
    justification: str = Field(description="AI justification for the operation")
    user_id: str | None = Field(default=None)
    task_id: str | None = Field(
        default=None, description="AI task identifier for routing the approval response"
    )
    batch_id: str | None = Field(
        default=None,
...
class OperatorStatusUpdatedPayload(G8eBaseModel):
    ...
    hostname: str | None = Field(
        default=None, description="Hostname from latest_heartbeat_snapshot.system_identity"
    )
    system_fingerprint: str | None = Field(
        default=None, description="SHA256 fingerprint of stable host attributes"
    )
    timestamp: UTCDatetime | None = Field(default=None, description="Transition timestamp")
    metrics: HeartbeatSnapshot | None = Field(
        default=None, description="Full HeartbeatSnapshot snapshot from latest_heartbeat_snapshot"
    )
...
class CommandExecutingBroadcastEvent(G8eBaseModel):
    ...
    port: int | None = Field(
        default=None, description="Port number for network port check operations"
    )
    timestamp: UTCDatetime = Field(default_factory=now)
...
class BindingValidationResult(G8eBaseModel):
    ...
    operator_bound_web_session_id: str | None = Field(
        default=None, description="Operator's bound web session ID"
    )
    expected_operator_session_id: str | None = Field(
        default=None, description="Expected operator session ID"
    )
...
class HealthCheckResultModel(G8eBaseModel):
    ...
    heartbeat_timestamp: UTCDatetime | None = Field(
        default=None, description="Timestamp from latest_heartbeat_snapshot"
    )
    session_expires_at: UTCDatetime | None = Field(
        default=None, description="Session expiration timestamp"
    )
    seconds_since_heartbeat: float | None = Field(
        default=None, description="Seconds since last heartbeat"
    )
