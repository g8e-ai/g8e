...
BoundOperator = _G8eBoundOperator


class RequestContext(_G8eRequestContext):
    """g8ee-specific RequestContext extending the protocol base.

    Adds operator_id and operator_session_id for governance envelope routing.
    source_component is a plain str (matching the protocol base) defaulting to
    G8EE_COMPONENT ("g8ee") for the ensemble's own outbound requests.
    """

    operator_id: str | None = Field(
        default=None, description="Operator ID for governance envelope routing"
    )
    ...
    def validate_session_identity(self):
        """Override g8e base validator — g8ee validates at G8eHttpContext level."""
        return self
...
class G8eHttpContext(G8eBaseModel):
    ...
    cli_session_id: str | None = Field(
        default=None, description="CLI session ID - used for routing SSE events to CLI clients"
    )
    ...
    organization_id: str | None = Field(
        default=None, description="Organization identifier - for multi-tenant data isolation"
    )
    case_id: str | None = Field(default=None, description="Current case ID being worked on")
    investigation_id: str | None = Field(
        default=None, description="Current investigation ID (AI chat session)"
    )
    ...
    timestamp: UTCDatetime = Field(default_factory=now, description="Timestamp of context creation")
    source_component: str = Field(
        default=G8EE_COMPONENT, description="Component that created this context"
    )
    system_fingerprint: str | None = Field(
        default=None, description="System fingerprint of the caller"
    )
    operator_id: str | None = Field(
        default=None, description="Operator ID for governance envelope routing"
    )
    ...
    def parse_bound_operators(cls, v):
        ...
        if isinstance(v, str):
            try:
                ...
                raise ValueError(f"bound_operators header contains malformed JSON: {exc}") from exc
        return v
    ...
    def validate_session_or_operator_auth(self):
        ...
        if self.source_component == ComponentName.CLIENT:
            ...
            if not self.web_session_id and not self.cli_session_id:
                raise ValueError(
                    "Context must have either web_session_id or cli_session_id for CLIENT source"
                )
        ...
        if self.bound_operators:
            for op in self.bound_operators:
                if op.status == OperatorStatus.BOUND and not op.operator_session_id:
                    raise ValueError(
                        f"Operator {op.operator_id} is BOUND but missing operator_session_id"
                    )
    ...
    def from_request_context(
        cls, request_context: RequestContext, is_exempt_path: bool = False
    ) -> G8eHttpContext:
