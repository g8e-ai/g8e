...
class AgentActivityMetadata(G8eIdentifiableModel):
    ...
    finish_reason: str | None = Field(
        default=None, description="Why the agent stopped (stop, length, error, etc.)"
    )
    ...
    triage_intent_summary: str | None = Field(
        default=None, description="Triage intent classification"
    )
    triage_intent_confidence: TriageConfidence | None = Field(
        default=None, description="Confidence in intent classification"
    )
    triage_request_posture: str | None = Field(
        default=None, description="Triage posture classification"
    )
    ...
    duration_seconds: float | None = Field(
        default=None, description="Total request duration in seconds"
    )
    ...
    attachment_total_bytes: int | None = Field(
        default=None, description="Total byte size of all attachments"
    )
    tool_response_sizes: list[int] | None = Field(
        default=None, description="Character sizes of individual tool responses"
    )
