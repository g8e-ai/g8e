...
class FileWriteArgs(TargetedOperatorBase):
    ...
    create_if_missing: bool = Field(
        default=False, description="Create file if it doesn't exist. Default: false."
    )
    create_backup: bool = Field(
        default=True, description="Create backup before overwriting. Default: true."
    )
...
class FileReadArgs(TargetedOperatorBase):
    ...
    start_line: int | None = Field(
        default=None,
        description="Starting line number, 1-indexed. First line to include in output.",
    )
    end_line: int | None = Field(
        default=None, description="Ending line number, 1-indexed inclusive. Last line to include."
    )
    max_lines: int | None = Field(
        default=None,
        description="Maximum number of lines to read. Use to limit output for large files.",
    )
...
class FileUpdateArgs(TargetedOperatorBase):
    ...
    create_backup: bool = Field(
        default=True, description="Create backup before modifying. Default: true."
    )
...
class CheckPortArgs(TargetedOperatorBase):
    ...
    host: str = Field(
        default="localhost",
        description="Host to check (IP address or hostname). Defaults to 'localhost' if not specified.",
    )
    protocol: str = Field(
        default="tcp", description="Protocol to use: 'tcp' or 'udp'. Defaults to 'tcp'."
    )
...
class RecursiveGrepArgs(TargetedOperatorBase):
    ...
    max_matches: int | None = Field(
        default=100, description="Maximum number of total matches to return. Default 100, max 500."
    )
...
class FetchExecutionOutputArgs(TargetedOperatorBase):
    ...
    execution_id: str = Field(
        ..., description="The unique execution ID of the command whose output you want to retrieve."
    )
...
class FetchSessionHistoryArgs(TargetedOperatorBase):
    ...
    operator_session_id: str = Field(
        ..., description="The operator session ID to fetch history for."
    )
    limit: int | None = Field(
        default=None, description="Maximum number of events to return (default: 50)."
    )
    offset: int | None = Field(
        default=None, description="Number of events to skip for pagination (default: 0)."
    )
...
class FetchFileHistoryArgs(TargetedOperatorBase):
    ...
    file_path: str = Field(
        ...,
        description="Absolute path to the file to get history for (e.g., /etc/nginx/nginx.conf).",
    )
    limit: int | None = Field(
        default=None, description="Maximum number of history entries to return (default: 50)."
    )
...
class RestoreFileArgs(TargetedOperatorBase):
    ...
    file_path: str = Field(
        ..., description="Absolute path to the file to restore (e.g., /etc/nginx/nginx.conf)."
    )
    commit_hash: str = Field(
        ..., description="Git commit hash to restore from (get this from fetch_file_history)."
    )
...
class FetchFileDiffArgs(TargetedOperatorBase):
    ...
    diff_id: str | None = Field(
        default=None, description="Specific diff ID to retrieve full diff content for."
    )
    operator_session_id: str | None = Field(
        default=None, description="Operator session ID to list all file diffs for."
    )
    file_path: str | None = Field(
        default=None,
        description="Filter results by file path (only used with operator_session_id).",
    )
    limit: int | None = Field(
        default=None,
        description="Maximum number of diffs to return (default: 50, only used with operator_session_id).",
    )
