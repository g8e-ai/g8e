...
class ExecutionResultsPayload(G8eBaseModel):
    ...
    payload_type: Literal["execution_result"] = Field(
        default="execution_result", description="Payload type discriminator"
    )
    execution_id: str = Field(..., description="Unique execution identifier")
    status: ExecutionStatus = Field(..., description="Final execution status")
    duration_seconds: float = Field(
        default=0.0, description="Wall-clock execution duration in seconds"
    )
    ...
    stored_locally: bool = Field(
        default=False, description="True when full output is in operator local vault"
    )
    ...
    error_type: str | None = Field(
        default=None, description="Machine-readable error classification"
    )
    completed_at: UTCDatetime | None = Field(
        default=None, description="When the command completed (UTC)"
    )
...
class ExecutionStatusPayload(G8eBaseModel):
    ...
    payload_type: Literal["execution_status"] = Field(
        default="execution_status", description="Payload type discriminator"
    )
    execution_id: str = Field(..., description="Unique execution identifier")
    status: ExecutionStatus = Field(..., description="Current execution status")
    process_alive: bool = Field(
        default=True, description="True if the underlying process is still running"
    )
    elapsed_seconds: float = Field(default=0.0, description="Elapsed time since execution started")
    new_output: str | None = Field(
        default=None, description="New stdout lines since last status update"
    )
    new_stderr: str | None = Field(
        default=None, description="New stderr lines since last status update"
    )
...
class CancellationResultPayload(G8eBaseModel):
    ...
    payload_type: Literal["cancellation_result"] = Field(
        default="cancellation_result", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Unique execution identifier of the cancelled command"
    )
...
class FileEditResultPayload(G8eBaseModel):
    ...
    payload_type: Literal["file_edit_result"] = Field(
        default="file_edit_result", description="Payload type discriminator"
    )
    execution_id: str = Field(..., description="Unique execution identifier")
    ...
    return_code: int | None = Field(default=None, description="Process exit code")
    completed_at: UTCDatetime | None = Field(
        default=None, description="When the file operation completed (UTC)"
    )
...
class FsListResultPayload(G8eBaseModel):
    ...
    payload_type: Literal["fs_list_result"] = Field(
        default="fs_list_result", description="Payload type discriminator"
    )
    execution_id: str = Field(..., description="Unique execution identifier")
    path: str | None = Field(default=None, description="Resolved absolute path that was listed")
    status: ExecutionStatus = Field(..., description="Operation status")
    total_count: int = Field(
        default=0, description="Total number of entries found before truncation"
    )
...
class FsGrepResultPayload(G8eBaseModel):
    ...
    payload_type: Literal["fs_grep_result"] = Field(
        default="fs_grep_result", description="Payload type discriminator"
    )
    execution_id: str = Field(..., description="Unique execution identifier")
    path: str | None = Field(default=None, description="Resolved absolute path that was searched")
    status: ExecutionStatus = Field(..., description="Operation status")
    total_matches: int = Field(
        default=0, description="Total number of matches found before truncation"
    )
...
class FsReadResultPayload(G8eBaseModel):
    ...
    payload_type: Literal["fs_read_result"] = Field(
        default="fs_read_result", description="Payload type discriminator"
    )
    execution_id: str = Field(..., description="Unique execution identifier")
...
class FetchLogsResultPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_logs_result"] = Field(
        default="fetch_logs_result", description="Payload type discriminator"
    )
    ...
    duration_ms: int | None = Field(
        default=None, description="Original execution duration in milliseconds"
    )
    ...
    timestamp: UTCDatetime | None = Field(
        default=None, description="When the original command was executed (UTC)"
    )
...
class FetchLogsErrorPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_logs_error"] = Field(
        default="fetch_logs_error", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
...
class FetchHistorySuccessPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_history_success"] = Field(
        default="fetch_history_success", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
    operator_session_id: str = Field(default="", description="Operator session ID that was queried")
    web_session: AuditSessionMetadata | None = Field(default=None, description="Session metadata")
    events: list[AuditEvent] = Field(
        default_factory=list, description="Audit events for the session"
    )
    total: int = Field(0)
    limit: int = Field(50)
    offset: int = Field(0)
...
class FetchHistoryErrorPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_history_error"] = Field(
        default="fetch_history_error", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
...
class FetchFileHistorySuccessPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_file_history_success"] = Field(
        default="fetch_file_history_success", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
    file_path: str = Field(..., description="Absolute path of the file queried")
    history: list[FileHistoryEntry] = Field(
        default_factory=list, description="Commit history entries"
    )
...
class FetchFileHistoryErrorPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_file_history_error"] = Field(
        default="fetch_file_history_error", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
...
class RestoreFileSuccessPayload(G8eBaseModel):
    ...
    payload_type: Literal["restore_file_success"] = Field(
        default="restore_file_success", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
...
class RestoreFileErrorPayload(G8eBaseModel):
    ...
    payload_type: Literal["restore_file_error"] = Field(
        default="restore_file_error", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
...
class FetchFileDiffByIdSuccessPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_file_diff_by_id_success"] = Field(
        default="fetch_file_diff_by_id_success", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
...
class FetchFileDiffBySessionSuccessPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_file_diff_by_session_success"] = Field(
        default="fetch_file_diff_by_session_success", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
    diffs: list[FileDiffEntry] = Field(
        default_factory=list, description="Multiple file diff entries"
    )
    total: int = Field(0)
...
class FetchFileDiffErrorPayload(G8eBaseModel):
    ...
    payload_type: Literal["fetch_file_diff_error"] = Field(
        default="fetch_file_diff_error", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
...
class PortCheckResultPayload(G8eBaseModel):
    ...
    payload_type: Literal["port_check_result"] = Field(
        default="port_check_result", description="Payload type discriminator"
    )
    execution_id: str = Field(
        ..., description="Execution identifier for request-response correlation"
    )
...
class G8eoHeartbeatPayload(G8eBaseModel):
    ...
    system_identity: G8eoHeartbeatSystemIdentity = Field(
        default_factory=G8eoHeartbeatSystemIdentity
    )
    ...
    performance_metrics: G8eoHeartbeatPerformanceMetrics = Field(
        default_factory=G8eoHeartbeatPerformanceMetrics
    )
    ...
    capability_flags: G8eoHeartbeatCapabilityFlags = Field(
        default_factory=G8eoHeartbeatCapabilityFlags
    )
...
class G8eoResultEnvelope(G8eBaseModel):
    ...
    id: str = Field(
        ...
        description="Unique per-message UUID v4 from g8eo. NOT a correlation key - do NOT use to match results to outbound commands. For correlation, use payload.execution_id.",
    )
    ...
    case_id: str | None = Field(
        default=None, description="Case ID propagated from the original command"
    )
    investigation_id: str | None = Field(
        default=None, description="Investigation ID propagated from the original command"
    )
    task_id: str | None = Field(
        default=None, description="Task ID propagated from the original command"
    )
...
class G8eMessage(G8eBaseModel):
    ...
    investigation_id: str | None = Field(
        default=None, description="Investigation ID associated with this message"
    )
    web_session_id: str | None = Field(
        default=None, description="Web session ID for targeted delivery to specific browser tabs"
    )
    cli_session_id: str | None = Field(
        default=None, description="CLI session ID for targeted delivery to specific CLI clients"
    )
    user_id: str | None = Field(
        default=None, description="User identifier associated with this message"
    )
    operator_session_id: str | None = Field(
        default=None, description="Operator session ID for g8eo Operator identification"
    )
    operator_id: str | None = Field(
        default=None, description="Operator ID for g8eo Operator identification"
    )
    api_key: str | None = Field(
        default=None,
        description="Operator API key carried on pub/sub messages for identity continuity",
    )
    ...
    def validate_session_ids(self) -> G8eMessage:
        ...
        if self.source_component == ComponentName.CLIENT:
            ...
            if not self.web_session_id and not self.cli_session_id:
                raise ValueError(
                    "Message must have either web_session_id or cli_session_id for CLIENT source"
                )
