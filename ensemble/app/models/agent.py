...
class SageOperatorRequest(TargetedOperatorBase):
    ...
    expected_output_lines: int = Field(
        default=10, description="Approximate number of stdout lines expected (used for UI sizing)."
    )
...
class ExecutorCommandArgs(TargetedOperatorBase):
    ...
    request: str | None = Field(
        default=None,
        description="Caller's natural-language request passed to the Tribunal (shown to the user as justification).",
    )
    guidelines: str | None = Field(
        default=None,
    ...
    correlation_id: str | None = Field(
        default=None,
        description="Tribunal correlation ID linking this command to the originating Tribunal session",
    )
    timeout_seconds: int = Field(
        default=300, description="Maximum seconds to wait for command completion before timing out."
    )
...
class AgentInputs(G8eBaseModel):
    ...
    def message_sender(self) -> MessageSender:
        ...
        return (
            ...
            if self.active_agent == ReasoningAgent.DASH
            else MessageSender.AI_PRIMARY
        )
