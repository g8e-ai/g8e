...
class TribunalConsensusFailedError(TribunalError):
    ...
    def __init__(self, request: str, vote_breakdown: VoteBreakdown) -> None:
        ...
        candidates = [
            f"- {member}: {cmd}" for member, cmd in vote_breakdown.candidates_by_member.items()
        ]
        summary = "\n".join(candidates)
...
class TribunalAuditorFailedError(TribunalError):
    ...
    def __init__(
        ...
        super().__init__(
            request=request,
            user_message=(
                f"Tribunal auditor failed ({reason.value}): {error or 'no error details'}"
            ),
        )
...
class ConsensusMemberResult(G8eBaseModel):
    ...
    confidence: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Confidence in the proposal."
    )
...
class CandidateCommand(G8eBaseModel):
    ...
    pass_index: int = Field(
        ge=0, description="Zero-based index of the generation pass that produced this candidate"
    )
    member: ConsensusMember = Field(description="Tribunal member that produced this candidate")
...
class TribunalAuditorCompletedPayload(G8eBaseModel):
    ...
    swap_to_member: str | None = Field(
        default=None,
        description="ConsensusMember id resolved from swap_to_cluster (internal mapping, safe to surface downstream)",
    )
    correlation_id: str | None = Field(
        default=None, description="Correlation ID for the Tribunal session"
    )
...
class TribunalSessionStartedPayload(G8eBaseModel):
    ...
    correlation_id: str = Field(
        description="Correlation ID to link Tribunal session with subsequent approval requests"
    )
