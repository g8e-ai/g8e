...
class HistoryEntry(G8eBaseModel):
    ...
    source_component: str = Field(..., description="Component that initiated the event")
    summary: str = Field(..., description="Human-readable description of the event")
    actor_id: str | None = Field(
        default=None, description="ID of the entity that performed the action"
    )
...
class CaseModel(G8eIdentifiableModel):
    ...
    user_email: str | None = Field(
        default=None, description="Email of the user who created the case"
    )
    status: CaseStatus = Field(default=CaseStatus.NEW)
    priority: Priority = Field(default=Priority.MEDIUM)
    severity: Severity = Field(default=Severity.MEDIUM)
    web_session_id: str | None = Field(
        default=None, description="Session ID associated with the case (from session cookie)"
    )
    ...
    history_trail: list[HistoryEntry] = Field(
        default_factory=list, description="Chronological list of key events"
    )
...
class CaseCreateRequest(G8eBaseModel):
    ...
    attachments: list[AttachmentMetadata] | None = Field(
        default_factory=list, description="List of attachment metadata objects for upload"
    )
    ...
    user_id: str = Field(
        ...,
        min_length=1,
        description="User ID - MUST be injected by client from authenticated session",
    )
    user_email: str | None = Field(
        default=None, description="User email - injected by client from session"
    )
    web_session_id: str | None = Field(
        default=None,
        description="Session ID - injected by client from validated session cookie (None for device token flows)",
    )
    ...
    severity: Severity = Field(
        default=Severity.MEDIUM, description="Server default - not client controllable"
    )
