...
class ConversationMessageMetadata(G8eBaseModel):
    ...
    event_type: EventType | None = Field(
        default=None, description="Event type for frontend filtering"
    )
    ...
    direct_execution: bool | None = Field(
        default=None, description="Whether this was a direct terminal execution"
    )
    ...
    feedback_reason: str | None = Field(
        default=None, description="User feedback reason when bypassing approval"
    )
    is_batch_execution: bool | None = Field(
        default=None, description="Whether this targets multiple operators"
    )
    batch_id: str | None = Field(
        default=None, description="Batch correlation ID when the approval covers multiple operators"
    )
    ...
    intent_name: str | None = Field(
        default=None, description="AWS intent name for intent approvals"
    )
    intent_question: str | None = Field(
        default=None, description="User-facing question for intent approval"
    )
    ...
    preview_command: str | None = Field(
        default=None, description="Preview command for stream approval"
    )
    requested_at: UTCDatetime | None = Field(
        default=None, description="When the approval was requested"
    )
    responded_at: UTCDatetime | None = Field(
        default=None, description="When the approval response was received"
    )
    completed_at: UTCDatetime | None = Field(
        default=None, description="When the operation completed"
    )
    backup_path: str | None = Field(
        default=None, description="Backup file path for file edit operations"
    )
    ...
    execution_time_seconds: float | None = Field(
        default=None, description="Command execution duration in seconds"
    )
    ...
    grounding_metadata: GroundingMetadata | None = Field(
        default=None, description="Grounding metadata from AI response"
    )
    token_usage: TokenUsage | None = Field(
        default=None, description="Token usage stats from AI response"
    )
    sentinel_mode: bool | None = Field(
        default=None, description="Sentinel mode active when message was created"
    )
    attachment_filenames: list[str] = Field(
        default_factory=list, description="Filenames of attachments sent with this message"
    )
    clarifying_questions: list[str] | None = Field(
        default=None, description="Triage clarifying questions"
    )
    triage_complexity: str | None = Field(
    ...
    question_index: int | None = Field(
        default=None, description="Index of the clarifying question being answered"
    )
    answer: bool | None = Field(
        default=None, description="User's yes/no answer to a clarifying question"
    )
...
class UserChatMetadata(ConversationMessageMetadata):
    ...
    sentinel_mode: bool | None = Field(
        default=None, description="Sentinel mode active when message was sent"
    )
    attachment_filenames: list[str] = Field(
        default_factory=list, description="Filenames of attachments sent with this message"
    )
...
class OperatorCommandMetadata(ConversationMessageMetadata):
    ...
    hostname: str | None = Field(default=None, description="Operator hostname")
    direct_execution: bool | None = Field(
        default=False, description="Always False - AI-initiated, not user-entered"
    )
    approval_id: str | None = Field(
        default=None, description="Approval ID if approval was required"
    )
    ...
    error_type: str | None = Field(default=None, description="Structured error type classification")
    requested_at: UTCDatetime | None = Field(
        default=None, description="When the command was requested"
    )
...
class ApprovalMetadata(ConversationMessageMetadata):
    ...
    justification: str | None = Field(
        default=None, description="Justification provided for approval"
    )
    approved: bool | None = Field(default=None, description="Whether the approval was granted")
...
class FileEditMetadata(ConversationMessageMetadata):
    ...
    error_type: str | None = Field(default=None, description="Structured error type classification")
    requested_at: UTCDatetime | None = Field(
        default=None, description="When the file edit was requested"
    )
    completed_at: UTCDatetime | None = Field(
        default=None, description="When the file edit completed"
    )
    timeout_seconds: int | None = Field(default=None, description="Timeout value for the operation")
    approval_id: str | None = Field(
        default=None, description="Approval ID if approval was required"
...
class ConversationHistoryMessage(G8eIdentifiableModel):
    ...
    metadata: ConversationMessageMetadata = Field(
        default_factory=ConversationMessageMetadata, description="Message metadata"
    )
    prev_hash: str = Field(
...
class InvestigationExecutionConstraints(G8eBaseModel):
    ...
    max_execution_time_seconds: int = Field(
        default=1800, ge=60, le=7200, description="Maximum execution time"
    )
    ...
    restricted_namespaces: list[str] | None = Field(
        default=None, description="Restricted Kubernetes namespaces"
    )
    auto_execute_risk_threshold: RiskThreshold = Field(
        default=RiskThreshold.LOW, description="Auto-execution risk threshold"
    )
    require_approval_for: list[str] = Field(
        default_factory=list, description="Actions requiring approval"
    )
    enable_runtime_execution: bool = Field(
        default=True, description="Whether runtime execution is enabled"
    )
...
class InvestigationCustomerContext(G8eBaseModel):
    ...
    compliance_requirements: list[str] | None = Field(
        default=None, description="Compliance requirements"
    )
...
class InvestigationCurrentState(G8eBaseModel):
    ...
    escalation_risk: EscalationRisk = Field(
        default=EscalationRisk.LOW, description="Escalation risk level"
    )
...
class InvestigationHistoryEntry(G8eBaseModel):
    ...
    actor: HistoryActor = Field(..., description="Who performed this action")
    summary: str = Field(..., description="Brief summary of what happened")
    investigation_attempt: G8eBaseModel | None = Field(
        default=None, description="Investigation attempt data"
    )
    details: ConversationMessageMetadata = Field(
        default_factory=ConversationMessageMetadata, description="Detailed event information"
    )
    prev_hash: str = Field(
        ..., description="Hash of previous entry in the chain (hex SHA256, 64 chars)"
    )
...
class InvestigationModel(G8eIdentifiableModel):
    ...
    task_id: str | None = Field(
        default=None,
        description="Associated task ID that triggered this investigation (optional for modular workflow)",
    )
    web_session_id: str | None = Field(
        default=None,
        description="Associated session ID (optional for chat-initiated investigations)",
    )
    user_email: str | None = Field(
        default=None, description="Email of the user who initiated the investigation"
    )
    user_id: str = Field(..., description="ID of the user who initiated the investigation")
    organization_id: str | None = Field(
        default=None, description="Organization ID for multi-tenant data isolation"
    )
    status: InvestigationStatus = Field(
        default=InvestigationStatus.OPEN, description="Current investigation status"
    )
    ...
    technical_context: InvestigationTechnicalContext | None = Field(
        default=None, description="Technical context"
    )
    created_with_case: bool = Field(
        default=False, description="Whether this investigation was created together with its case"
    )
    case_source: str | None = Field(
        default=None, description="Source of the case that triggered this investigation"
    )
    attachments: list[Attachment] = Field(
        default_factory=list, description="List of file attachment metadata"
    )
    conversation_history: list[ConversationHistoryMessage] = Field(
        default_factory=list,
        description="Sequential chat messages managed by g8ee - includes all user and AI messages including proposed solutions",
    )
    history_trail: list[InvestigationHistoryEntry] = Field(
        default_factory=list,
        description="Task creation/completion events managed by g8ee - does NOT include conversation data",
    )
    current_state: InvestigationCurrentState | None = Field(
        default=None, description="Current state tracking"
    )
    ...
    def update_status(
        self, new_status: InvestigationStatus, actor: HistoryActor, summary: str
    ) -> None:
...
class InvestigationCreateRequest(G8eBaseModel):
    ...
    task_id: str | None = Field(
        default=None, description="Associated task ID that triggered this investigation"
    )
    web_session_id: str | None = Field(
        default=None, description="Associated session ID (optional for modular workflow)"
    )
    priority: Priority = Field(default=Priority.MEDIUM, description="Investigation priority")
...
class InvestigationUpdateRequest(G8eBaseModel):
    ...
    case_title: str | None = Field(
        default=None, description="Updated case title (synced from Case document)"
    )
    customer_context: InvestigationCustomerContext | None = Field(
        default=None, description="Updated customer context"
    )
    technical_context: InvestigationTechnicalContext | None = Field(
...
class EnrichedInvestigationContext(InvestigationModel):
    ...
    operator_documents: list[OperatorDocument] = Field(
        default_factory=list, description="Bound OperatorDocument instances"
    )
    memory: InvestigationMemory | None = Field(
