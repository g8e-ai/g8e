...
class ErrorAnalysisContext(G8eBaseModel):
    ...
    working_directory: str | None = Field(
        default=None, description="Working directory when the error occurred"
    )
...
class FileOperationRiskContext(G8eBaseModel):
    ...
    backup_available: bool = Field(
        default=False, description="Whether a backup exists for the target file"
    )
...
class ErrorAnalysisResult(G8eBaseModel):
    ...
    suggested_command: str | None = Field(
        default=None, description="Exact command to fix the issue"
    )
...
class FileOperationRiskAnalysis(G8eBaseModel):
    ...
    safe_to_proceed: bool = Field(
        default=True, description="Whether the operation is safe to proceed"
    )
    blocking_issues: list[str] = Field(
        default_factory=list, description="Issues that block the operation"
    )
