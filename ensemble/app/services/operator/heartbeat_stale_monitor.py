...
def resolve_heartbeat_transition(
    current_status: OperatorStatus, is_stale: bool
) -> OperatorStatus | None:
...
class HeartbeatStaleMonitorService:
    ...
    async def tick(self) -> None:
        ...
        try:
            ...
            operators = await self._operator_data_service.query_operators(
                field_filters=[], bypass_cache=True
            )
            ...
            for op in operators:
                ...
                last_hb = (
                    op.latest_heartbeat_snapshot.timestamp if op.latest_heartbeat_snapshot else None
                )
                if not last_hb:
            ...
            if transitions > 0:
                logger.info(
                    "[HEARTBEAT-MONITOR] Reconciliation complete",
    ...
    async def _apply_transition(
        self, operator: OperatorDocument, target_status: OperatorStatus, age_seconds: float
    ) -> bool:
    ...
    async def _publish_transition(
        self, operator: OperatorDocument, target_status: OperatorStatus
    ) -> None:
        ...
        try:
            payload = OperatorStatusUpdatedPayload(
                ...
                system_fingerprint=operator.latest_heartbeat_snapshot.system_fingerprint
