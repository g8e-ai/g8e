...
class OperatorAuthService:
    ...
    async def authenticate_operator(
        ...
        return await self._authenticate_via_api_key(
            authorization_header, body, request_context, system_fingerprint
        )
    ...
    async def register_device_link_operator(
        ...
        if not operator:
            ...
            if system_fingerprint:
                operator = next(
                    (
                        op
                        for op in all_user_operators
                        if op.latest_heartbeat_snapshot
                        and op.latest_heartbeat_snapshot.system_fingerprint == system_fingerprint
                    ),
                    None,
                )
                if operator:
            ...
            if not operator:
                available_slots = [
                    op for op in all_user_operators if op.status == OperatorStatus.OFFLINE
                ]
                if available_slots:
        ...
        if not api_key:
            logger.error(
                "[OPERATOR-AUTH] Operator %s has no api_key on slot - slot creation bug",
                operator_id,
            )
            return {
                "success": False,
                "error": "Operator slot missing api_key - configuration error",
            }
    ...
    async def _authenticate_via_api_key(
        ...
        # Validate api_key
        success, key_doc, error = await self._api_key_service.validate_key(
            api_key, system_fingerprint
        )
