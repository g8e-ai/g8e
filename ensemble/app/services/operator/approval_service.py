...
class OperatorApprovalService:
    ...
    async def _audit(
        ...
        *,
        ...
        try:
            ...
            logger.info(
                "[%s] Investigation document not found (may be deleted during test cleanup)",
                log_tag,
            )
        except Exception as e:
    ...
    async def request_agent_continue_approval(
        ...
        try:
            ...
            logger.info(
                "[AGENT_CONTINUE_APPROVAL] Stored pending (key=%s); awaiting user response",
                approval_id,
            )
            ...
            logger.info(
                "[AGENT_CONTINUE_APPROVAL] Response received: approved=%s", pending.approved
            )
    ...
    async def _request_command_approval(
        ...
        try:
            ...
            if approval_event.is_batch_execution:
                logger.info(
                    "[APPROVAL] Batch execution: %d systems", len(approval_event.target_systems)
                )
    ...
    async def _request_file_edit_approval(
        ...
        try:
            ...
            logger.info(
                "[FILE_EDIT_APPROVAL] approval_id=%s execution_id=%s", approval_id, execution_id
            )
            if risk_analysis:
            ...
            if pending.feedback:
                logger.info(
                    "[FILE_EDIT_APPROVAL] User provided feedback: %s",
    ...
    async def _grant_intent_permission(
        ...
        try:
            ...
            error_msg = (
                f"Invalid intent '{intent_name}'. Valid intents: {', '.join(sorted(CloudIntent))}"
            )
        ...
        try:
            ...
            logger.info(
                "[INTENT_APPROVAL] approval_id=%s execution_id=%s operator_id=%s",
                approval_id,
                execution_id,
                operator_id,
            )

            intent_question: str = CLOUD_INTENT_QUESTIONS.get(
                intent, f"Should I have {intent} permission?"
            )
            ...
            return ApprovalResult(
                ...
                reason=pending.reason
                or (
                    "User approved intent grant" if pending.approved else "User denied intent grant"
                ),
                approval_id=approval_id,
