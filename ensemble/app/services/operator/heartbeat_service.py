...
class HeartbeatSnapshotService:
    ...
    async def start(self) -> None:
        ...
        if not self._pubsub_client:
            raise ConfigurationError(
                "pubsub_client must be set before calling start()", component="g8ee"
            )
        ...
        self._ready = True
        logger.info("[HEARTBEAT] Pub/sub client ready, pattern subscribed to %s", pattern)
    ...
    async def on_heartbeat_message(
        self, channel: str, data: str | bytes | dict[str, object]
    ) -> None:
        await self._on_heartbeat_message(channel, data)

    async def on_pattern_heartbeat_message(
    ...
    def validate_heartbeat_timestamp(
        self, payload: G8eoHeartbeatPayload
    ...
    async def _on_ws_disconnect(self) -> None:
        self._ready = False
        logger.warning(
            "[HEARTBEAT] WebSocket disconnected - ready state reset, preserving active sessions for re-subscription"
        )

    async def register_operator_session(self, operator_id: str, operator_session_id: str) -> None:
        """Track an operator session as active. Heartbeat subscription is handled
        by the pattern subscription in start(); this is pure in-memory bookkeeping."""
    ...
    async def deregister_operator_session(self, operator_id: str, operator_session_id: str) -> None:
        """Stop tracking an operator session. No per-channel pubsub calls;
        the pattern subscription in start() covers all operators."""
    ...
    async def _on_heartbeat_message(
        self, channel: str, data: str | bytes | dict[str, object]
    ) -> None:
        try:
            try:
            ...
            if not operator_id or not operator_session_id:
                logger.warning(
                    "[HEARTBEAT] Missing operator_id or operator_session_id in channel: %s", channel
                )
                return
            ...
            logger.info(
                ...
                extra={
                    ...
                    if payload.system_identity
                    else None,
                },
            )
            success = await self.process_heartbeat_message(
                operator_id, operator_session_id, payload
            )
            if not success:
                logger.warning(
                    "[HEARTBEAT] Heartbeat processing failed for operator %s", operator_id
                )
        except Exception:
    ...
    def _validate_heartbeat_timestamp(
        self, payload: G8eoHeartbeatPayload
    ) -> RequestValidationResult:
    ...
    async def _get_and_validate_operator(
        ...
        operator: OperatorDocument | None = await self.operator_data_service.get_operator(
            operator_id
        )
    ...
    async def _push_heartbeat_sse(
        ...
        try:
            ...
            logger.info("[HEARTBEAT] SSE event published successfully for operator %s", operator.id)
