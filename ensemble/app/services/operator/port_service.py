...
class OperatorPortService:
    ...
    async def execute_port_check(
        ...
        if port < 1 or port > 65535:
            ...
            logger.error("[PORT_CHECK] %s", error_msg)
            return PortCheckToolResult(
                success=False, error=error_msg, error_type=CommandErrorType.VALIDATION_ERROR
            )
        ...
        _hn = (
            resolved_operator.current_hostname
            or (
                resolved_operator.latest_heartbeat_snapshot.system_identity.hostname
                if resolved_operator.latest_heartbeat_snapshot
                else None
            )
            or "unknown"
        )
