...
class OperatorPubSubService:
    ...
    def set_pubsub_client(self, client: PubSubClient) -> None:
        if not client:
            raise ValidationError(
                "client is required for g8eo command communication", component="g8ee"
            )
    ...
    async def start(self) -> None:
        ...
        if self.pubsub_client is None:
            raise ValidationError(
                "pubsub_client not initialized - call set_pubsub_client() first", component="g8ee"
            )
    ...
    async def _unsubscribe_results_channel(
    ...
    async def _dispatch_results_message(
    ...
    async def _handle_pubsub_result_message(
        ...
        try:
            ...
            execution_id = (
                envelope.payload.execution_id if hasattr(envelope.payload, "execution_id") else None
            )
            if execution_id:
