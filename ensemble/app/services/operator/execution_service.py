...
class OperatorExecutionService(ExecutionServiceProtocol):
    ...
    async def send_command_to_operator(
        ...
        if subscribers == 0:
            channel = f"cmd:{operator_id}:{operator_session_id}"
            logger.error(
                "[NO-SUBSCRIBERS] No Operator listening on command channel '%s'",
                channel,
                extra={
                    "operator_id": operator_id,
                    "operator_session_id": operator_session_id,
    ...
    async def _wait_and_broadcast_direct_command_result(
        ...
        try:
            ...
            logger.warning(
                "[EXECUTION] Direct command timed out waiting for result for %s", execution_id
            )
        except Exception as e:
            logger.error(
                "[EXECUTION] Failed to broadcast direct command result for %s: %s",
                execution_id,
                e,
                exc_info=True,
            )
