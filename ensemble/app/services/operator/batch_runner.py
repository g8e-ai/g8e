...
class BatchRunResult[T: PerOperatorResultBase](BatchExecutionMeta):
    ...
    @classmethod
    def create(
        cls, per_operator_results: list[T], batch_id: str | None = None
    ) -> BatchRunResult[T]:
...
class LifecycleEmitter:
    ...
    async def emit_started(
        self, operator_id: str, hostname: str, execution_id: str, batch_id: str
    ) -> None:
    ...
    async def emit_completed[T: PerOperatorResultBase](
        ...
        raise NotImplementedError(
            "LifecycleEmitter.emit_completed must be implemented by subclasses"
        )

    async def emit_failed(
        self, operator_id: str, hostname: str, execution_id: str, batch_id: str, error: str
    ) -> None:
        ...
        *,
        ...
        for idx, result in enumerate(completed_results):
            ...
            if isinstance(result, asyncio.CancelledError):
                self._logger.debug(
                    "[BatchRunner] Task cancelled for %s (fail_fast)", operator.operator_id
                )
            elif isinstance(result, Exception):
                self._logger.error(
                    "[BatchRunner] Task failed for %s: %s", operator.operator_id, result
                )
                if error_factory:
