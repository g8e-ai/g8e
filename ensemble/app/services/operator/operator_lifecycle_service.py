...
class OperatorLifecycleService:
    ...
    async def claim_operator_slot(
        ...
        if not operator:
            logger.warning(
                "[OPERATOR-LIFECYCLE] Cannot claim non-existent operator %s", operator_id
            )
            return False
        ...
        try:
            ...
            logger.warning(
                "[OPERATOR-LIFECYCLE] Claim failed validation for operator %s: %s", operator_id, e
            )
            return False
        except Exception as e:
            logger.error(
                "[OPERATOR-LIFECYCLE] Atomic claim failed for operator %s: %s", operator_id, e
            )
            return False

        logger.info(
            "[OPERATOR-LIFECYCLE] Operator slot claimed %s",
            operator_id,
            extra={
                "operator_id": operator_id,
                "operator_session_id": operator_session_id,
    ...
    async def terminate_operator(
        ...
        try:
            ...
            logger.error(
                "[OPERATOR-LIFECYCLE] Atomic termination failed for operator %s: %s", operator_id, e
            )
            raise ValidationError(f"Failed to terminate operator {operator_id}: {e}") from e

        logger.info(
            "[OPERATOR-LIFECYCLE] Operator terminated %s",
            operator_id,
            extra={
                "operator_id": operator_id,
                "actor": actor.value,
