...
class OperatorIntentService:
        ...
        *,
        ...
        if not approval_result.approved:
            ...
            return IntentPermissionResult(
                ...
                error_type=CommandErrorType.USER_FEEDBACK
                if approval_result.feedback
                else CommandErrorType.USER_DENIED,
                intent_name=all_intents[0],
                all_intents=all_intents,
            )
        ...
        for intent in all_intents:
            ...
            if iam_result and iam_result.status != ExecutionStatus.COMPLETED:
                failed_intents.append(
                    FailedIntentResult(
                        intent=intent,
                        error=(iam_result.error if iam_result else None) or "Unknown error",
                    )
                )
            else:
        ...
        return IntentPermissionResult(
            ...
            intent_name=all_intents[0],
            all_intents=all_intents,
            message=f"Permission granted for {', '.join(all_intents)}",
            iam_results=iam_results,
            timestamp=now(),
        )
    ...
    async def execute_intent_revocation(
        ...
        *,
        ...
        return IntentPermissionResult(
            success=True,
            revoked_intents=requested_intents,
            iam_results=iam_results,
            timestamp=now(),
        )
