...
class CaseDataService:
    ...
    async def create_case(
        self, case_data: CaseCreateRequest, generated_title: str | None = None
    ) -> CaseModel:
        """
        Create a new case.
        ...
        async def _create():
            ...
            logger.info(
                "Case created: %s via governance envelope", case_id, extra={"case_id": case_id}
            )
            return case
    ...
    async def get_case(self, case_id: str) -> CaseModel:
        ...
        return await self._with_error_handling(
            _get(), action_msg="retrieve case", case_id=case_id, error_code=ErrorCode.DB_QUERY_ERROR
        )
    ...
    async def update_case(self, case_id: str, updates: CaseUpdateRequest) -> CaseModel:
        ...
        logger.info(
            "Updating case: %s",
            case_id,
            extra={"case_id": case_id, "fields": list(update_fields.keys())},
        )
        ...
        async def _update():
            ...
            if context:
                ...
                logger.info(
                    "Case updated: %s via governance envelope", case_id, extra={"case_id": case_id}
                )
            else:
    ...
    async def delete_case(self, case_id: str, context: RequestContext) -> None:
        ...
        async def _delete():
            ...
            logger.info(
                "Case deleted: %s via governance envelope", case_id, extra={"case_id": case_id}
            )
    ...
    async def publish_case_update_sse(
        ...
        if not web_session_id and not cli_session_id:
            logger.warning(
                "No session ID provided for case SSE update, skipping", extra={"case_id": case_id}
            )
            return
    ...
    async def get_case_history(self, query: CaseHistoryQuery) -> list[HistoryEntry]:
        filters = [FieldFilter(field="case_id", op="==", value=query.case_id)]
    ...
    async def get_case_tasks(
        self, case_id: str, task_status: TaskStatus
    ) -> list[dict[str, object]]:
        if not case_id:
            raise ValidationError("Case ID is required")

        filters = [FieldFilter(field="case_id", op="==", value=case_id)]
