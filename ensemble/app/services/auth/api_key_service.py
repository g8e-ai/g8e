...
class APIKeyService:
    ...
    async def validate_key(
        self, raw_key: str, system_fingerprint: str | None = None
    ) -> tuple[bool, APIKeyDocument | None, str | None]:
        ...
        if (
            doc.system_fingerprint
            and system_fingerprint
            and doc.system_fingerprint != system_fingerprint
        ):
            logger.error(
        ...
        try:
            ...
            if not doc.system_fingerprint and system_fingerprint:
                ...
                logger.info(
                    ...
                    extra={"doc_id": doc_id[:8] + "...", "system_fingerprint": system_fingerprint},
                )

            await self.cache.update_document(
                collection=self.collection, document_id=doc_id, data=updates, merge=True
            )
        except Exception as e:
