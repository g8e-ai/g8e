...
class CertificateService:
    ...
    async def initialize(self) -> None:
        ...
        if self.data_service:
            try:
                ...
                logger.info(
                    "[CERT-SERVICE] Loaded %s revocations from persistence",
                    len(self._revoked_serials),
                )
    ...
    async def generate_operator_certificate(
        ...
        logger.info(
            "[CERT-SERVICE] Requesting certificate for operator %s via operator", operator_id
        )
    ...
    async def revoke_certificate(
        self, serial: str, reason: str, operator_id: str | None = None
    ) -> bool:
        """Revoke a certificate by serial number."""
        serial_upper = serial.upper()
        logger.info(
            "[CERT-SERVICE] Revoking certificate %s (reason: %s, operator_id: %s)",
            serial_upper,
            reason,
            operator_id,
        )
