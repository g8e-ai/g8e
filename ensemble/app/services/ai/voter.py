...
def weighted_vote(
    candidates: list[CandidateCommand], total_members: int
    ...
    logger.info(
        "[TRIBUNAL-VOTING] Voting breakdown: %d candidates, %d unique commands, max_votes=%d",
        len(candidates),
        len(candidates_by_command),
        max_votes,
    )
    for cmd, members in candidates_by_command.items():
        logger.info(
            "[TRIBUNAL-VOTING]   Command: %r | Votes: %d | Members: %s", cmd, len(members), members
        )

    if len(top_candidates) == 1:
        winner = top_candidates[0]
        dissenters = {
            cmd: members for cmd, members in candidates_by_command.items() if cmd != winner
        }
        return (
            winner,
            max_votes / total_members,
            VoteBreakdown(
                candidates_by_member=candidates_by_member,
                candidates_by_command=candidates_by_command,
                winner=winner,
    ...
    if len({len(c) for c in top_candidates}) > 1:
        ...
        tied_candidates = [
            CandidateCommand(command=cmd, pass_index=0, member=ConsensusMember.AXIOM)
            for cmd in top_candidates
        ]
        return (
            winner,
            max_votes / total_members,
    ...
    # 2. Non-Nemesis cluster wins over Nemesis-including cluster
    nemesis_members = [ConsensusMember.NEMESIS.value]
