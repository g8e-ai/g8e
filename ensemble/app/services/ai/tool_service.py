...
class AIToolService:
    ...
    def get_tools(
        ...
        resolved_workflow = agent_mode or AgentMode.G8E_NOT_BOUND
    ...
    async def execute_tool_call(
        ...
        try:
            ...
            if not handler:
                ...
                logger.error(
                    "[TOOL_CALL] Available functions: %s", list(self._tool_handlers.keys())
                )
                return CommandExecutionResult(
                    success=False,
                    error=error_msg,
            ...
            return await handler(
                self, tool_args, investigation, g8e_context, request_settings, execution_id
            )
