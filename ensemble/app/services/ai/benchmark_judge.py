...
class PayloadMatcher(BaseModel):
    ...
    description: str = Field(
        default="", description="Human-readable description of what this pattern checks"
    )
...
class BenchmarkGrade(BaseModel):
    ...
    failures: list[str] = Field(
        default_factory=list, description="List of matcher failure descriptions"
    )

    tribunal_final_command: str | None = Field(
        default=None, description="Final command produced by the Tribunal"
    )
...
class TribunalCapture(BaseModel):
    ...
    outcome: str = Field(
        description="Tribunal pipeline outcome (VERIFIED, CONSENSUS, VERIFICATION_FAILED)"
    )
...
class BenchmarkJudge:
    ...
    def grade_tool_call(
        ...
        """
        forbidden_called = [
            tc.tool_name for tc in tool_calls if tc.tool_name in scenario.forbidden_tools
        ]
        if forbidden_called:
