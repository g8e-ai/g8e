...
class TurnState(BaseModel):
    ...
    def flush_thinking_block(self) -> None:
        ...
        if combined:
            self.model_response_parts.append(
                types.Part(
                    text=combined,
                    thought=True,
...
def handle_finish_reason_chunk(chunk: types.StreamChunkFromModel, state: TurnState) -> None:
    if chunk.finish_reason:
        ...
        if normalized:
            logger.info(
                "[TURN] Setting finish_reason: raw=%s normalized=%s previous=%s",
                chunk.finish_reason,
                normalized,
                state.finish_reason,
            )
...
def handle_thought_chunk(
    chunk: types.StreamChunkFromModel, state: TurnState
...
def handle_tool_call_chunk(
    ...
    if chunk.tool_calls:
        ...
        logger.info(
            "[TURN] Function call(s) received: count=%d names=%s",
            len(chunk.tool_calls),
            [fc.name for fc in chunk.tool_calls],
        )
        for i, fc in enumerate(chunk.tool_calls):
            state.model_response_parts.append(
                types.Part(
                    tool_call=fc,
...
async def process_provider_turn(
    ...
    async for chunk in stream_response:
        ...
        if chunk.tool_calls:
            if state.thinking_active:
                state.flush_thinking_block()
                yield StreamChunkFromModel(
                    type=StreamChunkFromModelType.THINKING_END, data=StreamChunkData()
                )
                state.thinking_active = False
        ...
        if chunk.thought_signature and not chunk.text and not chunk.tool_calls:
            if state.thinking_active:
                ...
                state.model_response_parts.append(
                    types.Part(thought_signature=chunk.thought_signature)
                )
            continue
    ...
    logger.info(
        ...
        state.output_tokens,
        state.total_tokens,
        len(state.model_response_parts),
...
def consolidate_model_parts(
    ...
    def _is_plain_text(p: types.Part) -> bool:
        return (
            p.text is not None
            and not p.thought
            and p.tool_call is None
            and p.thought_signature is None
        )
