...
class TribunalInvoker:
            ...
            if whitelisting_enabled:
                ...
                if csv_commands:
                    ...
                    whitelisted_commands = (
                        tool_executor.whitelist_validator.get_available_commands_with_metadata(
                            platform
                        )
                    )
    ...
    async def run(
        ...
        whitelisting_enabled, blacklisting_enabled, whitelisted_commands, blacklisted_commands = (
            TribunalInvoker._fetch_command_constraints(
                request_settings, investigation, tool_executor
            )
        )
        ...
        logger.info(
            ...
            whitelisting_enabled,
            blacklisting_enabled,
            len(whitelisted_commands),
            len(blacklisted_commands),
        )

        investigation_context_parts = [
            p for p in [investigation.case_title, investigation.case_description] if p
        ]
...
async def orchestrate_tool_execution(
    ...
    try:
        if tool_name == OperatorToolName.RUN_COMMANDS:
            ...
            if request:
                logger.info(
                    ...
                    len(request),
                    len(sage_request.guidelines or ""),
                    sage_request.target_operators,
                )
                logger.info(
        ...
        logger.info("[ORCHESTRATE-TOOL] Tool execution cancelled for %s", tool_name)
        raise
