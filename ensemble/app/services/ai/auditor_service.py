...
def parse_auditor_response(
    raw_text: str, mode: ConsensusAuditMode, cluster_ids: list[str]
) -> tuple[ConsensusAuditStatus, str | None, str | None]:
    ...
    try:
        ...
        status = (
            ConsensusAuditStatus(str(status_raw).lower())
            if status_raw is not None
            else ConsensusAuditStatus.OK
        )
...
async def call_auditor_llm(
    ...
    if model_config.supports_structured_output:
        response_format = ResponseFormat.from_pydantic_schema(
            TribunalAuditorResponse.model_json_schema(), name="TribunalAuditorResponse"
        )
...
async def run_auditor(
    ...
    for attempt in range(max_attempts):
        try:
            ...
            last_error = exc
            logger.warning(
                "[TRIBUNAL-AUDITOR] Attempt %d failed: %s (raw_text=%r)",
                attempt + 1,
                exc,
            ...
            if attempt == max_attempts - 1:
                ...
                total_duration_ms = (time.time() - auditor_start_time) * 1000
                logger.info(
                    "[TRIBUNAL-AUDITOR] Failed after %d attempts total_duration_ms=%.2f",
                    max_attempts,
                    total_duration_ms,
                )
                if isinstance(exc, OllamaEmptyResponseError):
                    await fail_auditor(
                        emitter, request, AuditorReason.EMPTY_RESPONSE, str(exc), target_cmd
                    )
                else:
                    await fail_auditor(
                        emitter,
                        request,
                        AuditorReason.NO_VALID_REVISION,
                        f"Failed to parse auditor response after {max_attempts} attempts: {exc!s}",
                        target_cmd,
                    )
            continue
