...
class AIRequestBuilder:
            ...
            for msg in conversation_history:
                ...
                if sender == MessageSender.USER_CHAT:
                    ...
                    filenames = (
                        ...
                        and msg.metadata.attachment_filenames
                        else []
                    )
                    if filenames:
                        text_for_ai = (
                            ATTACHMENT_FILENAMES_PREFIX_TEMPLATE.format(
        ...
        if attachments and contents:
            for i in range(len(contents) - 1, -1, -1):
                if contents[i].role == types.Role.USER:
                    ...
                    logger.info(
                        " [BUILD_CONTENTS] Appended %s attachment(s) to last user message",
                        len(attachments),
                    )
                    break
    ...
    def format_attachment_parts(
        self, attachments: list[ProcessedAttachment] | None
    ) -> list[types.Part]:
