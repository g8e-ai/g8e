...
class WebSearchProvider:
    ...
    def add_inline_citations(self, text: str, grounding_metadata: GroundingMetadata) -> str:
    ...
    async def search(self, query: str, num: int = 5) -> SearchWebResult:
        ...
        while attempt <= WEB_SEARCH_CLIENT_MAX_RETRIES:
            try:
                ...
                logger.info(
                    "[WEB_SEARCH] Returned %d results (total_size=%s)", len(results), total_size
                )
                ...
                logger.warning(
                    ...
                    attempt + 1,
                    WEB_SEARCH_CLIENT_MAX_RETRIES + 1,
                )
                ...
                return SearchWebResult(
                    success=False,
                    query=query,
                    error=f"Web search failed: {e}. Retry or use alternative information source.",
                )
