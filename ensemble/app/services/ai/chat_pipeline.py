...
class ChatPipelineService:
    ...
    async def _prepare_chat_context(
        ...
        3. Determine workflow type (G8E_BOUND vs NOT_BOUND)
        ...
        operator_bound = any(
            op.status == OperatorStatus.BOUND for op in g8e_context.bound_operators
        )
        agent_mode = AgentMode.G8E_BOUND if operator_bound else AgentMode.G8E_NOT_BOUND

        prior_history = (
            await self.investigation_service.investigation_data_service.get_chat_messages(
                investigation_id=investigation_id
            )
        )
    ...
    async def _persist_ai_response(
        ...
        if persisted:
            ...
            await self._detect_and_publish_interrogation(
                text=state.response_text, g8e_context=g8e_context, inputs=inputs
            )
        else:
    ...
    async def _detect_and_publish_interrogation(
        ...
        await self.event_service.publish_investigation_event(
            ...
            payload=TriageClarificationQuestionsPayload(
                ...
                complexity_confidence=str(inputs.triage_result.complexity_confidence)
                if inputs.triage_result
                else None,
                intent=inputs.triage_result.intent_summary if inputs.triage_result else None,
                intent_confidence=str(inputs.triage_result.intent_confidence)
                if inputs.triage_result
                else None,
                ...
                posture_confidence=str(inputs.triage_result.posture_confidence)
                if inputs.triage_result
                else None,
            ),
            web_session_id=g8e_context.web_session_id,
    ...
    async def _record_agent_activity_metadata(
        ...
        try:
            ...
            attachment_total_bytes = (
                sum(att.file_size or 0 for att in attachments) if attachments else 0
            )
            ...
            metadata = AgentActivityMetadata(
                ...
                triage_complexity_confidence=inputs.triage_result.complexity_confidence
                if inputs.triage_result
                else None,
                triage_intent_summary=inputs.triage_result.intent_summary
                if inputs.triage_result
                else None,
                triage_intent_confidence=inputs.triage_result.intent_confidence
                if inputs.triage_result
                else None,
                triage_request_posture=inputs.triage_result.request_posture
                if inputs.triage_result
                else None,
                triage_posture_confidence=inputs.triage_result.posture_confidence
                if inputs.triage_result
                else None,
                ...
                citation_count=state.grounding_metadata.citations_count
                if state.grounding_metadata
                else 0,
                ...
                tool_response_sizes=state.tool_response_sizes
                if state.tool_response_sizes
                else None,
                error=error,
            )
    ...
    async def run_chat(
        ...
        try:
            logger.info(
                ...
                if llm_primary_provider in [p.value for p in LLMProvider]
                else "REDACTED",
                llm_assistant_provider
                if llm_assistant_provider in [p.value for p in LLMProvider]
                else "REDACTED",
                llm_lite_provider
                if llm_lite_provider in [p.value for p in LLMProvider]
                else "REDACTED",
                llm_primary_model,
                llm_assistant_model,
                llm_lite_model,
            ...
            try:
                ...
                logger.error("[SSE-CHAT] Failed to send error event to frontend: %s", notify_err)
        finally:
    ...
    async def _run_chat_impl(
            ...
            resolved_settings = resolved_settings.model_copy(
                update={
                    "llm": resolved_settings.llm.model_copy(
                        update={"primary_provider": LLMProvider(primary_provider)}
                    )
                }
            )
        if primary_api_key:
        ...
        if primary_model:
            resolved_settings = resolved_settings.model_copy(
                update={
                    "llm": resolved_settings.llm.model_copy(update={"primary_model": primary_model})
                }
            )
        ...
        if assistant_model:
            resolved_settings = resolved_settings.model_copy(
                update={
                    "llm": resolved_settings.llm.model_copy(
                        update={"assistant_model": assistant_model}
                    )
                }
            )
        ...
        if lite_model:
            resolved_settings = resolved_settings.model_copy(
                update={"llm": resolved_settings.llm.model_copy(update={"lite_model": lite_model})}
            )
        ...
        if inputs.model_to_use and inputs.generation_config:
            ...
            async def _persist_iteration_text(text: str) -> None:
                ...
                await self._detect_and_publish_interrogation(
                    text=text, g8e_context=g8e_context, inputs=inputs
                )
        ...
        if state.token_usage:
            logger.info("[TOKEN_USAGE] SSE-CHAT final: %s", state.token_usage)
