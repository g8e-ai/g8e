...
"""``list_files_and_directories_with_detailed_metadata`` tool."""
...
from app.models.command_request_payloads import FsListRequestPayload
...
async def handle(
    ...
    result = await svc.operator_command_service.execute_fs_list(
        args=args,
        investigation=investigation,
        g8e_context=g8e_context,
    )
    logger.info("[LIST_DIRECTORY] entries=%d truncated=%s", result.total_count, result.truncated)
    return result
