...
class CacheAsideService(DocumentServiceProtocol):
    ...
    async def create_document(
        ...
        result = await self.db.create_document(
            collection=collection, data=data, document_id=document_id
        )
    ...
    async def get_document_with_cache(
        ...
        db_response = await self.db.get_document(collection=collection, document_id=document_id)
    ...
    async def invalidate_document(self, collection: str, document_id: str) -> bool:
        key = self.make_key(collection, document_id)
    ...
    async def batch_write(self, operations: list[BatchWriteOperation]) -> CacheOperationResult:
        ...
        return CacheOperationResult(success=True, cache_invalidated=True)
