...
class InvestigationService:
        ...
        try:
                        ...
                        if attempt > 0:
                            logger.info(
                                ...
                                extra={
                                    ...
                                    "total_wait_ms": sum(
                                        INVESTIGATION_LOOKUP_RETRY_DELAYS_MS[:attempt]
                                    ),
                                },
                            )
            ...
            if case_id:
                ...
                investigations = await self.investigation_data_service.get_case_investigations(
                    case_id=case_id, user_id=user_id, context=context
                )
                if investigations:
                    # Query already sorts by created_at desc, so [0] is the latest
    ...
    async def _attach_memory_context(
        self, context: EnrichedInvestigationContext
    ) -> EnrichedInvestigationContext:
        ...
        if not memory:
            logger.info(
                "No memory found for investigation", extra={"investigation_id": investigation_id}
            )
            return context
...
def extract_all_operators_context(
    ...
    if contexts:
        logger.info(
            ...
            len(contexts),
            [c.operator_id for c in contexts],
        )
    else:
