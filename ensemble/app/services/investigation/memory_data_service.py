...
class MemoryDataService(MemoryDataServiceProtocol):
    ...
    async def save_memory(
        self, memory: InvestigationMemory, is_new: bool, context: RequestContext
    ) -> None:
    ...
    async def get_user_memories(self, user_id: str) -> list[InvestigationMemory]:
        try:
            ...
            raise DatabaseError(
                "Failed to query memories for user", cause=exc, component="g8ee"
            ) from exc
    ...
    async def get_case_memories(self, case_id: str, user_id: str) -> list[InvestigationMemory]:
        try:
            ...
            raise DatabaseError(
                "Failed to query memories for case", cause=exc, component="g8ee"
            ) from exc
        memories = [InvestigationMemory.model_validate(d) for d in docs]
        logger.info(
            ...
            extra={
                "case_id": case_id,
                "memory_count": len(memories),
                "operation": "get_case_memories_complete",
            },
        )
        return memories
