...
as PubSubService for consistency across the g8ee service layer.
"""
...
class HTTPService:
    ...
    def set_http_client(self, client: HTTPClient, service_name: str) -> None:
        ...
        if not client:
            raise ValidationError(
                f"HTTP client is required for service '{service_name}'", component="g8ee"
            )
        if not service_name:
            raise ValidationError(
                "service_name is required for HTTP client registration", component="g8ee"
            )
    ...
    async def start(self) -> None:
        ...
        if not self._active_clients:
            logger.warning(
                "[HTTP] No HTTP clients registered - service ready but no clients available"
            )

        self._http_ready = True
        logger.info(
            "[HTTP] HTTP service ready with %s registered clients", len(self._active_clients)
        )
