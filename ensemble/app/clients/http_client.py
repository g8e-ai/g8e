        ...
        async with self.lock:
            if self.state == CircuitBreakerState.HALF_OPEN:
                ...
                if self.successes >= self.config.half_open_success_threshold:
                    ...
                    self.successes = 0
                    logger.info(
                        "Circuit breaker for %s is now CLOSED after successful recovery",
                        self.endpoint,
                    )
    ...
    async def record_failure(self) -> None:
        ...
        async with self.lock:
            ...
            if self.state == CircuitBreakerState.CLOSED:
                ...
                self.successes = 0
                logger.error(
                    "Circuit breaker for %s returned to OPEN state after failure in HALF_OPEN",
                    self.endpoint,
                )
        ...
        async with self.lock:
            ...
            if self.state == CircuitBreakerState.OPEN:
                if time.time() - self.last_failure_time > self.config.recovery_time:
                    ...
                    self.successes = 0
                    logger.info(
                        "Circuit breaker for %s is now HALF_OPEN, testing service recovery",
                        self.endpoint,
                    )
                    return True
                return False
...
class HTTPClient:
    ...
    def _get_circuit_breaker(self, url: str) -> CircuitBreaker:
        ...
        if endpoint not in self.circuit_breakers:
            self.circuit_breakers[endpoint] = CircuitBreaker(self.circuit_breaker_config, endpoint)
    ...
    async def get(
        ...
        return await self.request("GET", url, headers=headers, context=context, params=params)
...
def get_service_client(
    ...
    Args:
        ...
        source_service: The component making the request (e.g., G8EE_COMPONENT)
