...
TRIBUNAL_CORRELATION_ID_PREFIX = "tribunal"
...
# Matches gateway inference.RequestDeadline (ProviderRequestTimeout + governance margin).
INFERENCE_DISPATCH_HTTP_TIMEOUT_SECONDS = 330.0
...
AGENT_RETRYABLE_STATUS_CODES: frozenset[int] = frozenset(
...
OPERATOR_AVAILABLE_MESSAGE_TEMPLATE = (
    "{count} operator(s) bound and available for command execution"
)
...
FORBIDDEN_COMMAND_PATTERNS: tuple[str, ...] = tuple(
    _SECURITY_CONSTRAINTS.forbidden_command_patterns.get(
        "patterns", SecurityConstraintsConstants.get_default_forbidden_patterns()
    )
)
...
# G8EE_COMPONENT is the ensemble's own component identity string. The g8e
# protocol package (g8e.constants.ComponentName) defines CLIENT, G8EO, and
# G8EO_GATEWAY only — there is no G8EE value, and the Go SSOT
# (internal/constants/status.go) cannot be extended without violating the
# frozen-Go constraint. The ensemble uses this local string constant for its
# own outbound source_component attribution and internal bookkeeping. Inbound
# source_component validation against CLIENT/G8EO/G8EO_GATEWAY still uses the
# protocol ComponentName enum.
G8EE_COMPONENT = "g8ee"
...
CORS_ALLOWED_ORIGIN_G8EE = f"https://{_DYNAMIC_HOST}"
CORS_ALLOWED_ORIGIN_LOCALHOST = f"https://{_DYNAMIC_HOST}"
CORS_ALLOWED_ORIGIN_CLIENT_HTTP = "http://localhost:3000"
CORS_ALLOWED_ORIGIN_CLIENT_HTTPS = "https://localhost:3000"
