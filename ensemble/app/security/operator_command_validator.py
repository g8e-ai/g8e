...
class OperatorCommandValidator:
    ...
    def __init__(self, operator_cache, session_service=None):
        ...
        self.allowed_statuses = {OperatorStatus.ACTIVE, OperatorStatus.BOUND, OperatorStatus.STALE}
    ...
    async def validate_command_execution(
        ...
        if request_timestamp:
            ...
            if not ts_result.is_valid:
                logger.error(
                    "Command rejected - timestamp validation failed",
                    extra={
        ...
        if operator.status not in self.allowed_statuses:
            logger.error(
                "Operator status prohibits command execution",
                extra={
                    "operator_id": operator_id,
        ...
        if session_token and self.session_service:
            ...
            logger.error(
                "WebSession token required but not provided", extra={"operator_id": operator_id}
            )
            raise AuthenticationError(
                "WebSession token required for command execution", component="g8ee"
            )

        if operator.latest_heartbeat_snapshot:
            time_since_heartbeat = (
                now() - operator.latest_heartbeat_snapshot.timestamp
            ).total_seconds()
            if time_since_heartbeat > HEARTBEAT_STALE_THRESHOLD_SECONDS:
                logger.warning(
                    "Operator heartbeat is stale",
                    extra={
                        "operator_id": operator_id,
    ...
    async def _validate_session(
        self, operator_id: str, session_token: str, operator, system_info: dict[str, object] | None
    ):
        if operator.session_expires_at:
            if now() > operator.session_expires_at:
                ...
                raise AuthenticationError(
                    "WebSession expired - Operator must re-bootstrap", component="g8ee"
                )

        if operator.session_token and operator.session_token != session_token:
            logger.error(
                "WebSession token mismatch",
                extra={
                    "operator_id": operator_id,
        ...
        if self.session_service:
            try:
                await self.session_service.validate_operator_session(
                    session_token=session_token, operator_id=operator_id, system_info=system_info
                )
            except Exception as e:
                logger.error(
                    "WebSession service validation failed",
                    extra={"operator_id": operator_id, "error": str(e)},
                )
                raise AuthenticationError(
                    f"WebSession validation failed: {e!s}", component="g8ee"
                ) from e

        logger.info(
            "WebSession validated successfully",
            extra={"operator_id": operator_id, "session_token_prefix": session_token[:25] + "..."},
        )

    async def validate_operator_ownership(self, operator_id: str, user_id: str) -> bool:
        """
    ...
    async def validate_operator_binding(
        ...
        logger.info(
            "[BINDING-VALIDATED] Operator binding validated successfully",
            extra={
                "operator_id": operator_id,
                "operator_session_id": operator_session_id,
                "web_session_id": web_session_id,
    ...
    async def check_operator_health(self, operator_id: str) -> HealthCheckResultModel:
        ...
        health_status = HealthCheckResultModel(
            ...
            operator_id=operator_id,
            status=operator.status,
            heartbeat_timestamp=operator.latest_heartbeat_snapshot.timestamp
            if operator.latest_heartbeat_snapshot
            else None,
        ...
        if operator.latest_heartbeat_snapshot:
            ...
            health_status.heartbeat_stale = (
                seconds_since_heartbeat > HEARTBEAT_STALE_THRESHOLD_SECONDS
            )
