...
SUSPICIOUS_PATTERNS = [
    (
        r"(?i)\b(ignore|disregard|forget)\s+(all\s+)?(previous|prior|above)\s+(instructions?|rules?|constraints?)",
        SuspiciousPatternType.INSTRUCTION_OVERRIDE,
    ),
    (
        r"(?i)\b(new|updated|revised)\s+(instructions?|rules?|directives?)\s*:",
        SuspiciousPatternType.FAKE_INSTRUCTIONS,
    ),
    ...
    (
        r"(?i)\b(print|show|display|reveal|output)\s+(your\s+)?(system\s+)?(prompt|instructions?|rules?)",
        SuspiciousPatternType.PROMPT_EXTRACTION,
    ),
    (
        r"(?i)\bwhat\s+are\s+your\s+(instructions?|rules?|constraints?)",
        SuspiciousPatternType.PROMPT_EXTRACTION,
    ),
    (
        r"(?i)\b(send|transmit|post|upload)\s+.*(to|at)\s+https?://",
        SuspiciousPatternType.EXFILTRATION_TRIGGER,
    ),
    ...
    (
        r"(?i)\b(dan|jailbreak)\s*(mode|enabled|activated|prompt)",
        SuspiciousPatternType.JAILBREAK_MODE,
    ),
    ...
    (
        r"(?i)\b(no\s+restrictions?|without\s+restrictions?|remove\s+restrictions?)\b",
        SuspiciousPatternType.JAILBREAK_MODE,
    ),
    (
        r"(?i)\b(bypass|circumvent|disable|override)\s+(all\s+)?(safety|content|filter|guardrail|restriction)",
        SuspiciousPatternType.SAFETY_BYPASS,
    ),
    (r"(?i)\bsystem\s+override\b", SuspiciousPatternType.SAFETY_BYPASS),
    (
        r"(?i)\b(ignore|disable|turn\s+off)\s+(your\s+)?(safety|ethical|content)\s+(rules?|filter|guardrails?|training)",
        SuspiciousPatternType.SAFETY_BYPASS,
    ),
    (r"(?i)^\s*assistant\s*:", SuspiciousPatternType.FAKE_COMPLETION),
    (r"(?i)^\s*<assistant>", SuspiciousPatternType.FAKE_COMPLETION),
    (
        r"(?i)\b(repeat|continue|complete)\s+(after|from|the\s+following)\s*:",
        SuspiciousPatternType.FAKE_COMPLETION,
    ),
    (
        r"(?i)\b(print|show|display|reveal|output)\s+(your\s+)?(conversation|chat|message)\s+(history|log)",
        SuspiciousPatternType.HISTORY_EXTRACTION,
    ),
    (r"(?i)\bwhat\s+did\s+the\s+(user|human)\s+say\b", SuspiciousPatternType.HISTORY_EXTRACTION),
    (
        r"(?i)\brepeat\s+(everything|all)\s+(above|before|prior|from\s+the\s+start)",
        SuspiciousPatternType.HISTORY_EXTRACTION,
    ),
    (
        r"(?i)\b(print|show|display|reveal|output|leak)\s+(your\s+)?(api[\s_-]?key|secret|token|password|credential|env(ironment)?\s+var)",
        SuspiciousPatternType.CREDENTIAL_EXTRACTION,
    ),
    ...
    (
        r"(?i)^\s*(Thought|Observation|Action Input|Final Answer)\s*:",
        SuspiciousPatternType.AGENT_THOUGHT_INJECTION,
    ),
    ...
    (
        r"(?i)\b(base64|hex|rot13|url.?encoded?)\s*(decode|encode|of|:)",
        SuspiciousPatternType.ENCODED_INJECTION,
    ),
    (r"(?i)<img\s[^>]*src\s*=\s*['\"]?https?://", SuspiciousPatternType.MARKUP_EXFILTRATION),
    (
        r"(?i)<a\s[^>]*href\s*=\s*['\"]?https?://[^'\">\s]*[?&][^'\">\s]*=",
        SuspiciousPatternType.MARKUP_EXFILTRATION,
    ),
    (r"!\[.*?\]\(https?://[^)]*[?&][^)]*\)", SuspiciousPatternType.MARKUP_EXFILTRATION),
    (
        r"(?i)\b(output|respond|reply|answer|format)\s+(in|as|using)\s+(base64|hex|rot13|json|xml|yaml|markdown|csv)",
        SuspiciousPatternType.OUTPUT_FORMAT_EVASION,
    ),
    (
        r"(?i)\bencode\s+(your\s+)?(response|output|answer|reply)\s+(in|as|using)",
        SuspiciousPatternType.OUTPUT_FORMAT_EVASION,
    ),
]
...
def sanitize_g8eo_output(
    output: str, context: str, max_length: int = MAX_OUTPUT_LENGTH
) -> SanitizationResult:
    if not output:
    ...
    if suspicious_patterns:
        logger.warning(
            ...
            suspicious_patterns,
            extra={"context": context, "output_preview": sanitized[:200]},
        )
...
def sanitize_file_content(
    content: str, file_path: str, max_length: int = MAX_OUTPUT_LENGTH
) -> SanitizationResult:
    if not content:
        return SanitizationResult(
            sanitized_output="",
