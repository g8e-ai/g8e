...
def _cleanup_expired_nonces() -> None:
    ...
    expired_nonces = [nonce for nonce, ts in _nonce_cache.items() if ts < expired_threshold]
...
def validate_timestamp(
    timestamp_str: str | datetime | None, allow_missing: bool = False
...
async def check_nonce_kv(nonce: str, cache_aside_service: CacheAsideService) -> NonceCheckResult:
    """
    ...
    try:
        ...
        return NonceCheckResult(success=False, error=str(e), error_code=NonceErrorCode.CHECK_FAILED)
...
async def validate_request_timestamp(
    ...
    if not timestamp_result.is_valid:
        ...
        return RequestValidationResult(
            is_valid=False, error=timestamp_result.error, error_code=timestamp_result.error_code
        )
    ...
    if nonce:
        ...
        logger.warning(
            ...
            extra={"security_event": "missing_nonce", **ctx},
        )
        return RequestValidationResult(
            is_valid=False,
    ...
    logger.info(
        ...
        extra={"skew_seconds": timestamp_result.skew_seconds, "has_nonce": bool(nonce), **ctx},
    )
...
class RequestTimestampValidator:
    ...
    async def validate_async(
        self, timestamp_str: str | None, nonce: str, context: dict[str, object]
    ) -> RequestValidationResult:
    ...
    def validate_sync(self, timestamp_str: str | None, nonce: str) -> RequestValidationResult:
