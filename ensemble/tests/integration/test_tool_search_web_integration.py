...
from tests.fakes.agent_helpers import make_agent_run_args, make_event_service
...
class TestSearchWebResponseShape:
    ...
    async def test_result_items_are_web_search_result_item_type(self):
        ...
        svc, _ = _make_tool_service_with_search(search_result=_make_search_result(items=items))
...
class TestSearchWebFailureHandling:
    ...
    async def test_provider_failure_produces_success_false(self):
        provider = FakeWebSearchProvider(
            search_result=SearchWebResult(
                success=False, query="bad query", error="upstream unavailable"
            )
        )
...
class TestSearchWebGrounding:
    ...
    async def test_grounding_used_true_when_results_present(self):
        items = [
            WebSearchResultItem(
                title="Grafana", link="https://grafana.com/", snippet="Observability"
            )
        ]
        provider = FakeWebSearchProvider(
    ...
    async def test_grounding_chunks_populated_from_results(self):
        ...
        provider = FakeWebSearchProvider(search_result=_make_search_result(items=items))
...
class TestSearchWebSSEEvents:
    ...
    async def test_search_web_tool_call_fires_web_search_requested(self):
        chunks = [
        ...
        tool_call_events = [
            e for e in events if e.event_type == EventType.AI_LLM_TOOL_G8E_WEB_SEARCH_REQUESTED
        ]
    ...
    async def test_citations_chunk_fires_citations_ready_event(self):
        ...
        events = await _collect_sse_events(chunks)
        citation_events = [
            e for e in events if e.event_type == EventType.AI_LLM_CHAT_ITERATION_CITATIONS_RECEIVED
        ]
