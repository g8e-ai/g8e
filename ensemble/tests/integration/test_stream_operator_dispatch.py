...
def sample_g8e_context(
    unique_web_session_id, unique_user_id, unique_case_id, unique_investigation_id
):
...
async def test_stream_operator_dispatch_integration(
    tool_service, sample_g8e_context, sample_investigation, request_settings
):
...
@pytest.mark.asyncio
async def test_stream_executor_internal_flow(tool_service, sample_g8e_context):
    ...
    executor._internal_http_client.generate_operator_link = AsyncMock(
        return_value=OperatorLinkResponse(success=True, link_token="dlk_test_token")
    )
    executor._approval_service.request_stream_approval = AsyncMock(
        return_value=ApprovalResult(approved=True, approval_id="app-123")
    )
    ...
    with patch("asyncio.create_subprocess_exec") as mock_exec:
        ...
        result = await executor.execute_stream(
            args=args, g8e_context=sample_g8e_context, execution_id="exec-123"
        )
