...
class TestMemoryGenerationServiceIntegration:
    ...
    async def test_memory_generation_from_conversation(
        ...
        assert len(tech_bg_stripped.split()) >= 5, (
            "Technical background should be substantive with multiple words"
        )
        ...
        # Verify the content is substantive (not just a few words)
        assert len(approach_stripped.split()) >= 3, (
            "Problem-solving approach should be substantive with multiple words"
        )
        ...
        # Verify the content is substantive (not just a few words)
        assert len(summary_stripped.split()) >= 5, (
            "Investigation summary should be substantive with multiple words"
        )
    ...
    async def test_memory_generation_with_existing_memory(
        ...
        all_text = " ".join(part.text for c in contents for part in c.parts if part.text)
        ...
        assert len(keywords_found) >= 3, (
            f"Original technical keywords missing from: {tech_bg}. Found only: {keywords_found}"
        )
        ...
        new_found = any(
            any(concept in tech_bg for concept in concept_group) for concept_group in new_concepts
        )
        ...
        assert original_summary_found, (
            f"Original investigation concepts not found in summary: {summary}"
        )
