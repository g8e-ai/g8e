...
def mock_event_service():
    """Mock event service for deterministic testing."""
    service = AsyncMock()
...
@pytest.fixture
def tool_service(
    mock_web_search_provider, mock_operator_command_service, mock_investigation_service
):
...
class TestCommandExecutionTools:
    ...
    async def test_run_commands_tool_payload_error_handling(
        ...
        mock_operator_command_service.execute_command.side_effect = ValidationError(
            "Invalid command"
        )
...
class TestNetworkSearchTools:
    ...
    async def test_search_web_tool_unavailable_handling(
        ...
        with pytest.raises(
            ...
            match="g8e_web_search called but WebSearchProvider is not configured",
        ):
...
class TestToolIntegration:
    ...
    async def test_tool_declaration_consistency(self, tool_service):
