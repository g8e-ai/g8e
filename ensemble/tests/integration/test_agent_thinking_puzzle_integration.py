...
async def test_agent_thinking_puzzle(llm_provider, cache_aside_service, all_services):
    ...
    thinking_events = [
        e for e in events if e.event_type == EventType.AI_LLM_CHAT_ITERATION_THINKING_STARTED
    ]
    ...
    if thinking_update_events:
        for event in thinking_update_events:
            assert event.payload.thinking is not None, (
                "Thinking UPDATE events should contain thinking content"
            )
            assert len(event.payload.thinking.strip()) > 0, "Thinking content should not be empty"

    # Check for text events
    text_events = [
        e for e in events if e.event_type == EventType.AI_LLM_CHAT_ITERATION_TEXT_CHUNK_RECEIVED
    ]
    ...
    assert len(found_keywords) >= 2, (
        f"Expected answer to contain puzzle solution concepts, got: {full_text}"
    )
