...
class TestInvestigationContextResolution:
    ...
    async def test_resolve_by_investigation_id_success(
        ...
        result = await service.get_investigation_context(
            context=build_request_context(
                investigation_id=created_investigation.id, user_id=created_investigation.user_id
            ),
    ...
    async def test_resolve_by_case_id_returns_latest(
        ...
        inv1 = await investigation_data_service.create_investigation(
            create_investigation_request(
                case_id=case_id, user_id=user_id, case_title="First", case_description="First case"
            )
        )
        ...
        inv3 = await investigation_data_service.create_investigation(
            create_investigation_request(
                case_id=case_id, user_id=user_id, case_title="Third", case_description="Third case"
            )
        )

        # Test
        result = await service.get_investigation_context(
            context=build_request_context(case_id=case_id, user_id=user_id),
            case_id=case_id,
            user_id=user_id,
        )
        ...
        assert result.id == inv3.id, (
            f"Expected investigation {inv3.id} ({inv3.case_title}), got {result.id} ({result.case_title})"
        )
    ...
    async def test_resolve_missing_investigation_raises_resource_not_found(
        ...
        with pytest.raises(ResourceNotFoundError) as exc_info:
            await service.get_investigation_context(
                context=build_request_context(
                    investigation_id=unique_investigation_id, user_id=unique_user_id
                ),
    ...
    async def test_resolve_case_id_no_investigations_raises_resource_not_found(
        ...
        with pytest.raises(ResourceNotFoundError) as exc_info:
            await service.get_investigation_context(
                context=build_request_context(case_id=unique_case_id, user_id=unique_user_id),
    ...
    async def test_resolve_without_user_id_logs_security_warning(
        ...
        with caplog.at_level(logging.WARNING):
            result = await service.get_investigation_context(
                context=build_request_context(case_id=created_investigation.case_id, user_id=None),
...
class TestOperatorEnrichment:
    ...
    async def test_enrich_multiple_bound_operators(
        ...
        g8e_context = build_g8e_http_context(bound_operators=bound_operators)
...
class TestContextGatheringErrorHandling:
    ...
    async def test_context_without_required_parameters(
        ...
        with pytest.raises(ResourceNotFoundError) as exc_info:
            await service.get_investigation_context(context=build_request_context())
