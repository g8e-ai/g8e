...
class TestReputationPipelineIntegration:
    ...
    async def test_successful_tribunal_command_triggers_resolution(self, mock_tool_executor):
        ...
        investigation = EnrichedInvestigationContext(
            id="rep-test-inv", case_id="rep-test-case", user_id="rep-test-user", sentinel_mode=False
        )
        ...
        mock_tool_executor.reputation_service.resolve_stakes.return_value = MagicMock(
            resolutions=[resolution]
        )

        # Execute loop with Tribunal mock
        with patch(
            "app.services.ai.agent_tool_loop.TribunalInvoker.run", new_callable=AsyncMock
        ) as mock_tribunal:
    ...
    async def test_failed_tribunal_command_triggers_tier2_slash(self, mock_tool_executor):
        ...
        investigation = EnrichedInvestigationContext(
            id="slash-inv", case_id="slash-case", user_id="slash-user", sentinel_mode=False
        )
        tool_call = ToolCall(
            id="call_456",
            name=OperatorToolName.RUN_COMMANDS,
            args={"request": "rm", "target_operators": ["all"]},
        )
        ...
        published = [e.event_type for e in event_svc._published_events]
        assert EventType.OPERATOR_REPUTATION_STATE_UPDATED in published
        assert EventType.OPERATOR_REPUTATION_SLASH_TIER_2 in published
    ...
    async def test_non_tribunal_tool_skips_resolution(self, mock_tool_executor):
        ...
        await orchestrate_tool_execution(
            ...
            tool_executor=mock_tool_executor,
            investigation=EnrichedInvestigationContext(
                id="inv", case_id="case", user_id="user", sentinel_mode=False
            ),
            g8e_context=G8eHttpContext(
                web_session_id="sess",
                user_id="user",
