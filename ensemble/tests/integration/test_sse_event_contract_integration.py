...
if PROTOCOL_SSE_EVENTS is None:
    pytest.skip(
...
class TestSSEEventContract:
    ...
    async def test_processing_started_matches_protocol_fixture(self):
        ...
        started_events = [
            e for e in published if e.event_type == EventType.AI_LLM_CHAT_ITERATION_STARTED
        ]
    ...
    async def test_text_chunk_received_matches_protocol_fixture(self):
        ...
        text_chunk_events = [
            e
            for e in published
            if e.event_type == EventType.AI_LLM_CHAT_ITERATION_TEXT_CHUNK_RECEIVED
        ]
    ...
    async def test_text_completed_matches_protocol_fixture(self):
        ...
        published = event_svc._published_events
        completed_events = [
            e for e in published if e.event_type == EventType.AI_LLM_CHAT_ITERATION_TEXT_COMPLETED
        ]
    ...
    async def test_chat_iteration_failed_matches_protocol_fixture(self):
        ...
        published = event_svc._published_events
        failed_events = [
            e for e in published if e.event_type == EventType.AI_LLM_CHAT_ITERATION_FAILED
        ]
    ...
    async def test_error_chunk_skips_completion_event(self):
        ...
        assert len(completion_events) == 0, (
            "Completion event should not be published when ERROR chunk is received"
        )
    ...
    async def test_search_web_events_match_protocol_fixtures(self):
        ...
        published = event_svc._published_events
        tool_call_started_events = [
            e for e in published if e.event_type == EventType.AI_LLM_TOOL_G8E_WEB_SEARCH_REQUESTED
        ]
    ...
    async def test_tribunal_consensus_failed_matches_protocol_fixture(self):
        ...
        vote_breakdown = VoteBreakdown(
            ...
            candidates_by_command=dict(
                expected_fixture["data"]["vote_breakdown"]["candidates_by_command"].items()
            ),
            ...
            dissenters_by_command=dict(
                expected_fixture["data"]["vote_breakdown"]["dissenters_by_command"].items()
            ),
    ...
    for event_type in required_event_types:
        ...
        if not event_type.startswith("platform_sse_"):
            assert "investigation_id" in data, (
                f"Event {event_type} missing 'investigation_id' in data"
            )
...
async def test_protocol_fixture_event_types_match_constants():
    ...
    fixture_to_constant_mapping = {
        ...
        "tribunal_voting_consensus_failed": EventType.AI_CONSENSUS_VOTING_CONSENSUS_FAILED,
        "tribunal_voting_dissent_recorded": EventType.AI_CONSENSUS_VOTING_DISSENT_RECORDED,
    }
