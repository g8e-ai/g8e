...
class TestKVCacheClientInit:
    ...
    def test_default_init(self):
        with (
            patch("app.clients.kv_cache_client.SettingsService"),
            patch("app.clients.kv_cache_client.GatewaySettings") as mock_listen,
        ):
            mock_listen.from_bootstrap.return_value = MagicMock(
                http_url="https://default-localhost:443"
            )
            client = KVCacheClient()
