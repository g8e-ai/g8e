...
class TestRequestTrace:
    ...
    def test_from_headers_generates_request_id_when_absent(self):
        trace = RequestTrace.from_headers({}, component_id=G8EE_COMPONENT)
    ...
    def test_from_headers_preserves_existing_request_id(self):
        trace = RequestTrace.from_headers(
            {EXECUTION_ID: "exec-existing-123"}, component_id=G8EE_COMPONENT
        )
...
class TestCircuitBreaker:
    ...
    async def test_non_countable_failure_does_not_open_circuit(self):
        cb = CircuitBreaker(
            CircuitBreakerConfig(failure_threshold=1, recovery_time=9999.0),
            endpoint="https://localhost:8443",
        )
        await cb.record_failure(countable=False)
        assert cb.state is CircuitBreakerState.CLOSED
        assert await cb.allow_request() is True
...
class TestG8eHTTPClientPrepareRequest:
    ...
    async def test_prepare_request_injects_request_id(self, client):
        _url, _headers, trace, request_ctx = await client._prepare_request(
            "GET", "/api/health", headers={}, context=None
        )
    ...
    async def test_prepare_request_auth_token_formatted_as_bearer(self, authed_client):
        _url, headers, _trace, _ctx = await authed_client._prepare_request(
            "GET", "/api/health", headers={}, context=None
        )
    ...
    async def test_prepare_request_joins_base_url_with_path(self, client):
        url, _headers, _trace, _ctx = await client._prepare_request(
            "GET", "/api/health", headers={}, context=None
        )
        assert url == "https://localhost:8443/api/health"
...
class TestG8eHTTPClientRequest:
    ...
    async def test_gateway_idempotent_post_retries_transient_503(self):
        resp_503 = _make_mock_response(503, b"service unavailable")
        original_request = MagicMock(return_value=resp_503)
        ...
        assert original_request.call_count == 4

    async def test_connection_error_does_not_trip_circuit_breaker(self):
        resp = MagicMock()
        resp.__aenter__ = AsyncMock(
            side_effect=aiohttp.ClientConnectorError(
                connection_key=MagicMock(), os_error=OSError("connection refused")
