...
DBClient is an HTTP shim over the Gateway. Tests mock the HTTP layer via
...
class InMemoryOperator:
    ...
    Parses paths prefixed with GatewayAPIPaths.DATA_DB ("/api/v1/data/").
    """
    ...
    def _split_path(path: str) -> list[str]:
        """Strip the DATA_DB prefix and return [collection, doc_id_or_query]."""
        rel = path.removeprefix(GatewayAPIPaths.DATA_DB).strip("/")
        return rel.split("/")
    ...
    async def handle_list(self, method: str, path: str, **kwargs) -> list:
        parts = self._split_path(path)
        if len(parts) == 2 and parts[1] == "_query":
            collection = parts[0]
...
class TestCRUD:
    ...
    async def test_query_collection_no_filters(self, db_client):
        ...
        result = await db_client.query_collection(
            "items", field_filters=[], order_by={}, limit=0, select_fields=[]
        )
        assert result.success is True
    ...
    async def test_query_collection_empty_returns_empty_list(self, db_client):
        result = await db_client.query_collection(
            "empty-col", field_filters=[], order_by={}, limit=0, select_fields=[]
        )
        assert result.success is True
        assert result.data == []
    ...
    async def test_query_collection_with_select_fields(self, db_client):
        ...
        result = await db_client.query_collection(
            "items", field_filters=[], order_by={}, limit=0, select_fields=["name"]
        )
        assert result.success is True
        doc = result.data[0]
        assert "name" in doc
...
class TestArrayUnionRemove:
    ...
    async def test_array_remove_removes_items(self, db_client):
        ...
        result = await db_client.update_document("docs", "d1", {"tags": ArrayRemove(["b"])})
        assert result.success is True
    ...
    async def test_array_remove_missing_item_is_noop(self, db_client):
        await db_client.create_document("docs", "d1", {"tags": ["a", "b"]})
        result = await db_client.update_document("docs", "d1", {"tags": ArrayRemove(["z"])})
        assert result.success is True
...
class TestUpdateWithArrayUnion:
    async def test_append_to_empty_list(self, db_client):
        await db_client.create_document("docs", "d1", {"history": []})
    ...
    async def test_with_additional_updates(self, db_client):
        await db_client.create_document("docs", "d1", {"history": [], "updated_at": None})
        await db_client.update_with_array_union(
            collection="docs",
            document_id="d1",
            array_field="history",
...
class TestBatchWrite:
    async def test_set_operations(self, db_client):
        result = await db_client.batch_write(
            [
                BatchWriteOperation(
                    op_type=BatchWriteOpType.SET, collection="docs", doc_id="d1", data={"x": 1}
                ),
                BatchWriteOperation(
                    op_type=BatchWriteOpType.SET, collection="docs", doc_id="d2", data={"x": 2}
                ),
            ]
        )
        assert result.success is True
    ...
    async def test_delete_operation(self, db_client):
        ...
        result = await db_client.batch_write(
            [
                BatchWriteOperation(
                    op_type=BatchWriteOpType.DELETE, collection="docs", doc_id="d1"
                ),
            ]
        )
        assert result.success is True
    ...
    async def test_mixed_operations_executed_in_order(self, db_client):
        result = await db_client.batch_write(
            [
                BatchWriteOperation(
                    op_type=BatchWriteOpType.SET, collection="docs", doc_id="d1", data={"v": 1}
                ),
                BatchWriteOperation(
                    op_type=BatchWriteOpType.UPDATE,
                    collection="docs",
                ...
                BatchWriteOperation(
                    op_type=BatchWriteOpType.SET, collection="docs", doc_id="d2", data={"v": 3}
                ),
                BatchWriteOperation(
                    op_type=BatchWriteOpType.DELETE, collection="docs", doc_id="d2"
                ),
            ]
        )
        assert result.success is True
...
class TestQueryCollectionOptions:
    async def test_order_by_desc(self, db_client):
        ...
        result = await db_client.query_collection(
            "items", field_filters=[], order_by={"score": "desc"}, limit=0, select_fields=[]
        )
        assert result.success is True
    ...
    async def test_order_by_asc(self, db_client):
        ...
        result = await db_client.query_collection(
            "items", field_filters=[], order_by={"score": "asc"}, limit=0, select_fields=[]
        )
        assert result.success is True
        scores = [d["score"] for d in result.data]
    ...
    async def test_limit_restricts_result_count(self, db_client):
        ...
        result = await db_client.query_collection(
            "items", field_filters=[], order_by={}, limit=3, select_fields=[]
        )
        assert result.success is True
    ...
    async def test_limit_larger_than_collection_returns_all(self, db_client):
        ...
        result = await db_client.query_collection(
            "items", field_filters=[], order_by={}, limit=100, select_fields=[]
        )
        assert result.success is True
        assert len(result.data) == 2
...
class TestUpdateDocumentArrayEdgeCases:
    ...
    async def test_array_union_when_existing_doc_returns_none_initializes_from_empty(
        self, db_client
    ):
        patched: dict = {}
    ...
    async def test_array_remove_on_non_list_field_produces_empty(self, db_client):
        ...
        result = await db_client.update_document("docs", "d1", {"tags": ArrayRemove(["x"])})
        assert result.success is True
...
class TestErrorPropagation:
    ...
    async def test_query_propagates_network_error(self, db_client_http_error):
        with pytest.raises(NetworkError):
            await db_client_http_error.query_collection(
                "col", field_filters=[], order_by={}, limit=0, select_fields=[]
            )
    ...
    async def test_batch_write_propagates_network_error(self, db_client_http_error):
        with pytest.raises(NetworkError):
            await db_client_http_error.batch_write(
                [
                    BatchWriteOperation(
                        op_type=BatchWriteOpType.SET, collection="col", doc_id="d1", data={"x": 1}
                    )
                ]
            )
