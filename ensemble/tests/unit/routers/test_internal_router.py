...
@pytest.mark.asyncio
async def test_internal_chat_new_case(request_context, g8e_context, task_tracker):
...
@pytest.mark.asyncio
async def test_internal_chat_missing_investigation(request_context, g8e_context, task_tracker):
    request_context = request_context.model_copy(update={"investigation_id": ""})
    request = ChatMessageRequest(
        context=request_context, message="test message", sentinel_mode=True
    )
...
async def test_generate_and_update_title_success():
    ...
    mock_case_service.update_case = AsyncMock(
        return_value=MagicMock(updated_at="2023-01-01T00:00:00Z")
    )
    mock_case_service.publish_case_update_sse = AsyncMock()
    mock_investigation_service = MagicMock()
    mock_investigation_service.update_investigation = AsyncMock()
    ...
    with patch(
        "app.routers.internal_router.generate_case_title", new_callable=AsyncMock
    ) as mock_generate:
...
@pytest.mark.asyncio
async def test_operator_approval_respond(request_context, g8e_context):
    request_context = request_context.model_copy(
        update={
            "bound_operators": [BoundOperator(operator_id="op-1", operator_session_id="opsess-1")],
        }
    )
    request = OperatorApprovalResponse(
        context=request_context, approval_id="app-123", approved=True
    )
...
@pytest.mark.asyncio
async def test_update_case_with_sse(request_context, g8e_context):
    case_id = "case-123"
...
@pytest.mark.asyncio
async def test_delete_case_success(request_context, g8e_context):
    case_id = "case-123"
...
async def test_delete_case_not_found_idempotent(request_context, g8e_context):
    ...
    mock_case_service.get_case = AsyncMock(
        side_effect=ResourceNotFoundError(
            message="not found", resource_type="case", resource_id=case_id
        )
    )
    mock_case_service.delete_case = AsyncMock()
...
@pytest.mark.asyncio
async def test_bind_operators_success(request_context, g8e_context):
...
@pytest.mark.asyncio
async def test_bind_operators_unauthorized(request_context, g8e_context):
    request = OperatorBindRequest(
        context=request_context,
        operator_ids=["op-123"],
    )
...
@pytest.mark.asyncio
async def test_unbind_operators_success(request_context, g8e_context):
...
@pytest.mark.asyncio
async def test_unbind_operators_unauthorized(request_context, g8e_context):
    request = OperatorUnbindRequest(
        context=request_context,
        operator_ids=["op-123"],
    )
...
@pytest.mark.asyncio
async def test_internal_chat_settings_service_di_regression(
