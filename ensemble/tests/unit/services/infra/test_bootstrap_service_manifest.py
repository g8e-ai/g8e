...
def test_load_auditor_hmac_key_reads_file_and_caches(
    volume: Path, bootstrap: BootstrapService
) -> None:
...
def test_load_auditor_hmac_key_returns_none_when_missing(
    volume: Path, bootstrap: BootstrapService
) -> None:
    assert bootstrap.load_auditor_hmac_key() is None


def test_is_available_true_when_only_auditor_hmac_key_present(
    volume: Path, bootstrap: BootstrapService
) -> None:
