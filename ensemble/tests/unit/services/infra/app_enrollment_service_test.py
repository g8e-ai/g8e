...
class TestEnrollPlatformEnrollment:
    ...
    async def test_completion_transcript_is_byte_identical_to_shared_parity_vector(
        ...
        """Verify the manually constructed transcript matches the shared
        ...
        constructs the same bytes manually. This test asserts against the
        shared golden vector in
        ``protocol/constants/platform_enrollment_completion_transcript_vectors.json``
        (ensemble vector), which the Go (operator) and JavaScript (dashboard)
        clients also assert against for their respective component kinds.
        """
        service = AppEnrollmentService()

        transcript = service._build_completion_transcript(
            request_id="parity-req-001",
            token_hash="aabbccdd",
            instance_id="parity-host",
            fingerprint="parity-app-fp",
        )

        # Expected hex from the shared parity vector (ensemble, component_kind=2).
        # Generated by proto.MarshalOptions{Deterministic: true} on the Go side.
        expected_hex = (
            "0a0131120e7061726974792d7265712d3030311a086161626263636464"
            "20022a0b7061726974792d686f7374320f0a0d7061726974792d617070"
            "2d6670"
        )
        assert transcript.hex() == expected_hex
