...
def _make_failed_envelope(
    error: str = "Connection refused", execution_id: str = "test-exec-id"
) -> G8eoResultEnvelope:
    return build_g8eo_result_envelope(
...
class TestOperatorResolution:
    ...
    async def test_no_operator_documents_returns_error(self):
        service, *_ = _make_service(
            resolve_error=BusinessLogicError(
                "No operators bound to this session", component="g8ee"
            ),
        )
    ...
    async def test_validation_error_from_resolve_returns_error(self):
        service, *_ = _make_service(
            resolve_error=ValidationError(
                "Could not resolve target_operator 'bad'", component="g8ee"
            ),
        )
        investigation = _make_investigation()
        result = await service.execute_port_check(
            _make_args(target_operators=["bad"]),
            investigation,
            _make_context(),
        )
        assert result.success is False
    ...
    async def test_resolve_called_with_correct_args(self, task_tracker):
        ...
        await service.execute_port_check(
            _make_args(target_operators=["op-1"]),
            investigation,
            _make_context(),
        )
    ...
    async def test_resolve_called_with_none_target_for_single_operator(self, task_tracker):
        ...
        await service.execute_port_check(
            _make_args(target_operators=["all"]),
            investigation,
            _make_context(),
        )
