...
class TestOperatorAuthService:
    ...
    @pytest.fixture
    def mock_cache(self):
        cache = AsyncMock(spec=CacheAsideService)
        # Configure kv property to return a mock with async methods
        kv_mock = AsyncMock()
        cache.kv = kv_mock
        return cache
    ...
    async def test_authenticate_via_api_key_happy(
        self,
        auth_service,
        mock_api_key_service,
        mock_operator_data_service,
        ...
        mock_api_key_service.validate_key.return_value = (
            True,
            MagicMock(user_id=user_id, operator_id=operator_id, organization_id=org_id),
            None,
        )
        ...
        mock_certificate_service.generate_operator_certificate.return_value = {
            "cert": "CERT",
            "key": "KEY",
        }
    ...
    async def test_authenticate_via_api_key_unknown_operator(
        self, auth_service, mock_api_key_service, mock_operator_data_service
    ):
        mock_api_key_service.validate_key.return_value = (
            True,
            MagicMock(user_id="u", operator_id="missing"),
            None,
        )
    ...
    async def test_authenticate_via_api_key_user_mismatch(
        self, auth_service, mock_api_key_service, mock_operator_data_service
    ):
        mock_api_key_service.validate_key.return_value = (
            True,
            MagicMock(user_id="user-a", operator_id="op-1"),
            None,
        )
    ...
    async def test_register_device_link_operator_happy(
        self,
        auth_service,
        mock_operator_data_service,
        mock_session_service,
    ...
    async def test_register_device_link_operator_on_demand_creation(
        self,
        auth_service,
        mock_operator_data_service,
        mock_session_service,
    ...
    async def test_register_device_link_operator_concurrent_slot_assignment(
        self,
        auth_service,
        mock_operator_data_service,
        mock_session_service,
    ...
    async def test_register_device_link_operator_no_api_key_on_slot(
        self, auth_service, mock_operator_data_service
    ):
        operator_doc = MagicMock(spec=OperatorDocument)
