...
class TestRegisterOperatorSession:
    ...
    async def test_tracks_active_session(self, command_service):
        ...
        assert (
            "op-abc",
            "sess-xyz",
        ) in command_service._pubsub_service._active_operator_sessions_set
    ...
    async def test_multiple_sessions_subscribed_independently(self, command_service):
        ...
        subscribe_calls = [
            str(c) for c in command_service._pubsub_service.pubsub_client.subscribe.call_args_list
        ]
...
class TestDeregisterOperatorSession:
    ...
    async def test_removes_from_active_sessions(self, command_service):
        ...
        assert (
            "op-abc",
            "sess-xyz",
        ) not in command_service._pubsub_service._active_operator_sessions_set
...
class TestDispatchResultsMessage:
    ...
    async def test_routes_to_result_handler_uap(self, command_service):
        ...
        with patch.object(
            command_service._pubsub_service, "_handle_pubsub_result_message", new=AsyncMock()
        ) as mock_handle:
