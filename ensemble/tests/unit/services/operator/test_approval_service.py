...
class TestRequestCommandApproval:
    ...
    async def test_request_command_approval_granted(
        self, mock_pending_approval_class, mock_generate_approval_id
    ):
        ...
        assert event_service.publish.called
...
class TestRequestFileEditApproval:
    ...
    @patch("app.services.operator.approval_service.generate_approval_id")
    @patch("app.services.operator.approval_service.PendingApproval")
    async def test_request_file_edit_approval_granted(
        self, mock_pending_approval_class, mock_generate_approval_id
    ):
...
class TestRequestIntentApproval:
    ...
    @patch("app.services.operator.approval_service.PendingApproval")
    async def test_request_intent_approval_granted(
        self, mock_pending_approval_class, mock_generate_intent_approval_id
    ):
...
class TestRequestAgentContinueApproval:
    ...
    @patch("app.services.operator.approval_service.generate_approval_id")
    @patch("app.services.operator.approval_service.PendingApproval")
    async def test_request_agent_continue_approval_granted(
        self, mock_pending_approval_class, mock_generate_approval_id
    ):
        ...
        published_event = event_service.publish.call_args.args[0]
    ...
    @patch("app.services.operator.approval_service.PendingApproval")
    async def test_request_agent_continue_approval_publish_failure(
        self, mock_pending_approval_class
    ):
        """Returns error result and does not await when publish raises."""
        event_service = MagicMock(spec=EventServiceProtocol)
        event_service.publish.side_effect = RuntimeError("broker down")
        operator_data_service = AsyncMock(spec=OperatorDataServiceProtocol)
        investigation_data_service = AsyncMock(spec=InvestigationDataServiceProtocol)
        service = OperatorApprovalService(
            event_service=event_service,
