...
from app.constants import OperatorStatus, OperatorType
...
class TestOperatorLifecycleService:
    ...
    def lifecycle_service(self, operator_data_service):
        ...
        mock_api_key_service.rotate_operator_key = AsyncMock(
            return_value={"success": True, "api_key": "new-key"}
        )
    ...
    async def test_claim_operator_slot_success(
        self, lifecycle_service, operator_data_service, mock_cache
    ):
        operator_id = "op-123"
    ...
    async def test_terminate_operator_success(
        self, lifecycle_service, operator_data_service, mock_cache
    ):
        ...
        result = await lifecycle_service.terminate_operator(
            operator_id=operator_id, actor=HistoryActor.USER, summary="Manual termination"
        )
    ...
    async def test_terminate_operator_not_found_raises_validation_error(
        self, lifecycle_service, mock_cache
    ):
        mock_cache.get_document_with_cache.return_value = None
    ...
    async def test_terminate_operator_cache_failure_raises_error(
        self, lifecycle_service, mock_cache
    ):
        operator_id = "op-fail"
        ...
        mock_cache.update_document.return_value = CacheOperationResult(
            success=False, error="cache down"
        )
    ...
    async def test_update_operator_status_active_does_not_set_heartbeat(
        self, lifecycle_service, mock_cache
    ):
        operator_id = "op-123"
        mock_cache.get_document_with_cache.return_value = {
    ...
    async def test_update_operator_status_not_found_returns_false(
        self, lifecycle_service, mock_cache
    ):
        mock_cache.get_document_with_cache.return_value = None
    ...
    async def test_claim_operator_slot_active_status_different_session_preempts(
        self, lifecycle_service, mock_cache
    ):
    ...
    async def test_claim_operator_slot_active_status_same_session_fails(
        self, lifecycle_service, mock_cache
    ):
    ...
    async def test_claim_operator_slot_stale_status_different_session_preempts(
        self, lifecycle_service, mock_cache
    ):
