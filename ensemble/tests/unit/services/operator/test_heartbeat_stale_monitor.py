...
def test_operator_status_to_event_type():
    assert (
        operator_status_to_event_type(OperatorStatus.STALE)
        == EventType.OPERATOR_STATUS_UPDATED_STALE
    )
    assert (
        operator_status_to_event_type(OperatorStatus.OFFLINE)
...
async def test_tick_transitions_stale_bound_to_stale():
    ...
    operator_data_service.update_operator_status.assert_called_once_with(
        operator_id="op-1", status=OperatorStatus.STALE
    )
...
async def test_tick_recovers_stale_to_bound():
    ...
    operator_data_service.update_operator_status.assert_called_once_with(
        operator_id="op-1", status=OperatorStatus.BOUND
    )
...
async def test_tick_coalesces_concurrent_calls():
    ...
    service = HeartbeatStaleMonitorService(
        operator_data_service=operator_data_service, event_service=event_service
    )
