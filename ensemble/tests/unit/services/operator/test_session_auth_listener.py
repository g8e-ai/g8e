...
class TestSessionAuthListener:
    ...
    async def test_message_handler_success(
        self, gateway, mock_pubsub_client, mock_session_service, mock_operator_data_service
    ):
        operator_session_id = "ops_123"
        operator_id = "op_456"
        user_id = "user_789"
    ...
    async def test_message_handler_invalid_session(
        self, gateway, mock_pubsub_client, mock_session_service
    ):
        operator_session_id = "ops_123"
        session_hash = hashlib.sha256(operator_session_id.encode()).hexdigest()
        auth_channel = f"auth.publish:session:{session_hash}"
    ...
    async def test_message_handler_internal_error(
        self, gateway, mock_pubsub_client, mock_session_service
    ):
        operator_session_id = "ops_123"
        session_hash = hashlib.sha256(operator_session_id.encode()).hexdigest()
    ...
    async def test_message_handler_wrong_channel(
        self, gateway, mock_pubsub_client, mock_session_service
    ):
        operator_session_id = "ops_123"
        await gateway.gateway(operator_session_id, "op1", "u1", "org1")
