...
class TestBuildAttachCommand:
    ...
    def test_different_intents_produce_different_commands(self, builder):
        assert builder.build_attach_command("ec2_discovery") != builder.build_attach_command(
            "s3_read"
        )
...
class TestBuildDetachCommand:
    ...
    def test_different_intents_produce_different_commands(self, builder):
        assert builder.build_detach_command("s3_read") != builder.build_detach_command(
            "rds_management"
        )
    ...
    def test_detach_differs_from_attach_for_same_intent(self, builder):
        assert builder.build_detach_command("ec2_discovery") != builder.build_attach_command(
            "ec2_discovery"
        )
