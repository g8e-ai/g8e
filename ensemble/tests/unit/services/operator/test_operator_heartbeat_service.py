...
class TestOnHeartbeatMessage:
    ...
    async def test_on_heartbeat_message_parses_dict_data_and_calls_process(self, service):
        ...
        with patch.object(
            service, "process_heartbeat_message", new=AsyncMock(return_value=True)
        ) as mock_proc:
            await service.on_heartbeat_message(
                PubSubChannel.heartbeat("op-222", "op-session-111"), data
            )
    ...
    async def test_on_heartbeat_message_tracks_unknown_session(self, service):
        ...
        with patch.object(service, "process_heartbeat_message", new=AsyncMock(return_value=True)):
            await service.on_heartbeat_message(
                PubSubChannel.heartbeat("op-new", "op-session-new"), data
            )
    ...
    async def test_on_heartbeat_message_swallows_exceptions_silently(self, service):
        ...
        data = _make_envelope(payload.model_dump(mode="json"))
        with patch.object(
            service, "process_heartbeat_message", new=AsyncMock(side_effect=RuntimeError("boom"))
        ):
            await service.on_heartbeat_message(
                PubSubChannel.heartbeat("op-222", "op-session-111"), data
            )
...
class TestHeartbeatSnapshotServiceOperatorValidation:
    ...
    async def test_cache_hit_returns_operator(self, service, mock_operator_data_service):
        operator = OperatorDocument(
            id="op-222", status=OperatorStatus.ACTIVE, user_id="user-1", bound_web_session_id="ws-1"
        )
    ...
    async def test_any_known_operator_status_accepted(self, service, mock_operator_data_service):
        ...
        for status in OperatorStatus:
            operator = OperatorDocument(
                id="op-222", status=status, user_id="user-1", bound_web_session_id="ws-1"
            )
            mock_operator_data_service.get_operator.return_value = operator
...
class TestHeartbeatSnapshotServiceProcessMessage:
    ...
    def service(self, mock_operator_data_service, mock_event_service):
        return _make_service(
            operator_data_service=mock_operator_data_service, event_service=mock_event_service
        )
    ...
    async def test_identity_mismatch_returns_false(
        ...
        result = await service.process_heartbeat_message(
            "op-222", "op-session-111", _make_payload(operator_id="op-EVIL")
        )
