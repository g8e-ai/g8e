...
async def test_triage_handles_low_confidence_classification(fake_provider, mock_settings):
    """Triage classifies low confidence without generating follow-up questions.

    Per architecture alignment: Triage is a classifier only; questions are
    generated by reasoning agents (Dash/Sage) per the Interrogation Protocol.
    """
    fake_provider.add_response("""{
...
async def test_triage_uses_provided_model_override(fake_provider, mock_settings):
    fake_provider.add_response(
        '{"complexity": "simple", "intent": "information", "intent_confidence": "high", "complexity_confidence": "high", "intent_summary": "test"}'
    )
