...
class TestValidateCommandSafety:
    ...
    def test_blacklist_enforcement(self):
        ...
        mock_blacklist.validate_command.return_value = CommandBlacklistResult(
            is_allowed=False, reason="Blocked"
        )
    ...
    def test_whitelist_enforcement(self):
        ...
        mock_whitelist.validate_command.return_value = CommandValidationResult(
            is_valid=False, command="echo", reason="Not whitelisted"
        )
    ...
    def test_whitelist_override_forwarded_to_validator(self):
        ...
        mock_whitelist.validate_command.assert_called_once_with(
            "uptime", ANY, allowed_commands_override=["uptime", "df"]
        )
...
class TestGenerateCommandSafety:
    ...
    async def test_rejects_unsafe_final_command(self):
        ...
        with (
            patch("app.services.ai.generator.get_llm_provider", return_value=mock_provider),
            patch("app.services.ai.generator.resolve_model", return_value="test-model"),
