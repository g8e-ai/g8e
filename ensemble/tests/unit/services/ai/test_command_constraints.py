...
def mock_whitelist_validator():
    ...
    validator.validate_command = MagicMock(
        return_value=CommandValidationResult(
            is_valid=True,
            command="ping",
        )
    )
    validator.get_available_commands_with_metadata = MagicMock(
        return_value=[
...
def mock_blacklist_validator():
    ...
    validator.get_forbidden_commands = MagicMock(
        return_value=[
            {"command": "rm", "reason": "Destructive command"},
            {"command": "dd", "reason": "Disk destruction"},
        ]
    )
    validator.get_forbidden_substrings = MagicMock(
        return_value=[
            {"substring": "format", "reason": "Disk formatting"},
        ]
    )
    validator.get_forbidden_patterns = MagicMock(
        return_value=[
            {"pattern": r"rm\s+-rf\s+/", "reason": "Recursive delete"},
        ]
    )
    validator.validate_command = MagicMock(
        return_value=CommandBlacklistResult(
            is_allowed=True,
        )
    )
    return validator
...
async def test_handle_get_command_constraints_both_disabled(
    ...
    tool_service = tool_service_builder(
        whitelist_validator=mock_whitelist_validator, blacklist_validator=mock_blacklist_validator
    )
...
async def test_get_tools_includes_get_command_constraints(
    ...
    tools_operator = tool_service.get_tools(agent_mode=AgentMode.G8E_BOUND, model_to_use=None)
