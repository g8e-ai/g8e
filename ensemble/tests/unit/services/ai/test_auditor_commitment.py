...
async def _seed_states(service: ReputationDataService, states: list[ReputationState]) -> None:
    ...
    for s in states:
        await service.upsert_state(s, context=ctx)
...
def _expected_signature(
    merkle_root_hex: str, prev_root: str, tribunal_command_id: str, key: str
) -> str:
    return hmac.new(
        key.encode("utf-8"),
...
class TestCommitReputation:
    ...
    async def test_db_write_failure_propagates_and_leaves_no_commitment(
        ...
        # Force the underlying DB client's update_document to raise.
