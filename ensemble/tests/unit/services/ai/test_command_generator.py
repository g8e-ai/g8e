...
def _make_mock_provider(generate_content_lite_side_effect=None, generate_content_lite_return=None):
    ...
    if generate_content_lite_side_effect is not None:
        mock_provider.generate_content_lite = AsyncMock(
            side_effect=generate_content_lite_side_effect
        )
    ...
    async def test_generation_pass_uses_role_user_in_content(self):
        ...
        result = await _run_generation_pass(
            ...
            operator_context=_make_mock_operator_context(
                os="linux", shell="bash", working_directory="/home/user"
            ),
            pass_index=0,
            emitter=emitter,
        ...
        with pytest.raises(TribunalDisabledError) as exc_info:
            await generate_command(
                make_tribunal_generation_request(
                    ...
                    guidelines="",
                    operator_context=_make_mock_operator_context(
                        os="linux", shell="bash", working_directory="/tmp", username="root", uid=0
                    ),
...
class TestGenerateCommandSystemError:
    ...
    async def test_provider_routing_uses_settings_provider(self):
        ...
        with patch(
            ...
            assert mock_factory.call_args_list[1] == (
                (settings.llm,),
                {"is_assistant": False, "is_lite": False},
            )
...
class TestTribunalModelNotConfiguredError:
    ...
    async def test_raises_on_no_model_configured(self):
        ...
        with patch(
            ...
            event = call_args[0][0]
            assert event.event_type == EventType.AI_CONSENSUS_SESSION_MODEL_NOT_CONFIGURED
...
class TestRunAuditStageWardenRiskAnalysis:
    ...
    @staticmethod
    def _make_analyzer(
        risk_level, error_user_message: str | None = None, error_suggested_fix: str | None = None
    ):
        analyzer = MagicMock()
...
class TestGenerateCommandHappyPath:
    ...
    async def test_consensus_path_auditor_disabled(self):
        ...
        assert EventType.AI_CONSENSUS_VOTING_CONSENSUS_REACHED in emitted_types
        assert EventType.AI_CONSENSUS_SESSION_COMPLETED in emitted_types
        assert EventType.AI_CONSENSUS_VOTING_AUDIT_STARTED not in emitted_types
    ...
    async def test_event_emission_order(self):
        ...
        started_idx = emitted_types.index(EventType.AI_CONSENSUS_SESSION_STARTED)
        pass_indices = [
            i
            for i, t in enumerate(emitted_types)
            if t == EventType.AI_CONSENSUS_VOTING_PASS_COMPLETED
        ]
        consensus_idx = emitted_types.index(EventType.AI_CONSENSUS_VOTING_CONSENSUS_REACHED)
        review_started_idx = emitted_types.index(EventType.AI_CONSENSUS_VOTING_AUDIT_STARTED)
        review_completed_idx = emitted_types.index(EventType.AI_CONSENSUS_VOTING_AUDIT_COMPLETED)
        completed_idx = emitted_types.index(EventType.AI_CONSENSUS_SESSION_COMPLETED)
    ...
    async def test_refined_command_differs_from_original(self):
        ...
        completed_calls = [
            call
            for call in mock_event_service.publish.call_args_list
            if call.args[0].event_type == EventType.AI_CONSENSUS_SESSION_COMPLETED
        ]
...
class TestGenerateCommandAuditorFailure:
    ...
    async def test_empty_auditor_response_raises_through_generate_command(self):
        ...
        assert EventType.AI_CONSENSUS_VOTING_CONSENSUS_REACHED in emitted_types
        assert EventType.AI_CONSENSUS_VOTING_AUDIT_STARTED in emitted_types
        assert EventType.AI_CONSENSUS_SESSION_AUDITOR_FAILED in emitted_types
        assert EventType.AI_CONSENSUS_VOTING_AUDIT_COMPLETED not in emitted_types
        assert EventType.AI_CONSENSUS_SESSION_COMPLETED not in emitted_types
    ...
    async def test_no_valid_revision_raises_through_generate_command(self):
        ...
        mock_provider = self._provider_with_auditor_behavior(
            "ls -la",
            auditor_return=same_command_response,
        )
        settings = self._settings()
    ...
    async def test_auditor_exception_raises_through_generate_command(self):
        ...
        mock_provider = self._provider_with_auditor_behavior(
            "ls -la",
            auditor_side_effect=RuntimeError("Auditor API timeout"),
        )
        settings = self._settings()
    ...
    async def test_single_pass_auditor_failure_raises(self):
        ...
        assert emitted_types.count(EventType.AI_CONSENSUS_VOTING_PASS_COMPLETED) == 2
...
class TestConsensusMemberCycling:
    ...
    def test_member_for_pass_cycles_correctly(self):
        ...
        assert member_for_pass(6) == ConsensusMember.CONCORD
        assert member_for_pass(7) == ConsensusMember.VARIANCE
        assert member_for_pass(8) == ConsensusMember.PRAGMA
        assert member_for_pass(9) == ConsensusMember.NEMESIS
...
class TestTribunalEmitter:
    ...
    async def test_round_2_peer_review_flow(self):
        ...
        async def round_based_side_effect(**kwargs):
            ...
            if call_count <= 3:
                ...
                if call_count == 1:
                    mock_response.text = "cmd1"
                elif call_count == 2:
                    mock_response.text = "cmd2"
                elif call_count == 3:
                    mock_response.text = "cmd3"
            else:
        ...
        mock_provider = _make_mock_provider(
            generate_content_lite_side_effect=round_based_side_effect
        )
        ...
        with patch("app.services.ai.generator.get_llm_provider", return_value=mock_provider):
            ...
            emitted_types = [
                args[0][0].event_type for args in mock_event_service.publish.call_args_list
            ]
            assert EventType.AI_CONSENSUS_VOTING_ROUND_2_STARTED in emitted_types
            assert EventType.AI_CONSENSUS_VOTING_ROUND_2_CONSENSUS_REACHED in emitted_types
    ...
    async def test_all_terminal_events_raise_on_publish_failure(self):
        ...
        terminal_events = [
            ...
            (
                EventType.AI_CONSENSUS_SESSION_DISABLED,
                TribunalSessionDisabledPayload(request="test"),
            ),
            (
                EventType.AI_CONSENSUS_SESSION_MODEL_NOT_CONFIGURED,
                TribunalSessionModelNotConfiguredPayload(
                    request="test", provider="ollama", error="model not configured"
                ),
            ),
            (
                EventType.AI_CONSENSUS_SESSION_PROVIDER_UNAVAILABLE,
                TribunalSessionProviderUnavailablePayload(
                    request="test", provider="ollama", error="provider unavailable"
                ),
            ),
            (
                EventType.AI_CONSENSUS_SESSION_SYSTEM_ERROR,
...
class TestDefaultConfigCoversAllMembers:
    ...
    def test_default_pass_count_covers_all_members(self):
        ...
        assert default_passes == 5, (
            f"Default llm_command_gen_passes should be 5, got {default_passes}"
        )
