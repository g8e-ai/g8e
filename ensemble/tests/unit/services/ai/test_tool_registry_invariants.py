...
def test_every_spec_has_non_empty_display_metadata():
    ...
    for spec in TOOL_SPECS:
        ...
        assert spec.display_icon, f"ToolSpec {spec.name.value} has empty display_icon"
        assert spec.display_category, f"ToolSpec {spec.name.value} has empty display_category"
...
def test_operator_gated_specs_only_exposed_in_bound_modes():
    ...
    for spec in TOOL_SPECS:
        if spec.scope is ToolScope.OPERATOR_GATED:
            assert AgentMode.G8E_NOT_BOUND not in spec.agent_modes, (
                f"OPERATOR_GATED tool {spec.name.value} must not be exposed in G8E_NOT_BOUND"
            )
...
def test_get_tools_partitions_by_agent_mode():
    ...
    not_bound = svc.get_tools(AgentMode.G8E_NOT_BOUND, model_to_use=None)
    bound = svc.get_tools(AgentMode.G8E_BOUND, model_to_use=None)
