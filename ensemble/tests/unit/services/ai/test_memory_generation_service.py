...
class TestConversationToContentsPayload:
    ...
    def _all_payload_text(contents: list[Content]) -> str:
        return " ".join(part.text for c in contents for part in c.parts if part.text)
    ...
    async def test_full_replacement_when_llm_returns_all_fields(self):
        ...
        async def mock_ai_update(memory, history, settings):
            ...
            memory.communication_preferences = (
                ai_response.communication_preferences or memory.communication_preferences
            )
            memory.technical_background = (
                ai_response.technical_background or memory.technical_background
            )
