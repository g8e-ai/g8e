...
class TestMatchPayload:
    ...
    def test_failure_includes_description(self):
        matchers = [
            PayloadMatcher(field="command", pattern=r"nonexistent", description="must have flag X")
        ]
...
class TestComputeBenchmarkPercentage:
    ...
    def test_all_fail(self):
        grades = [
            BenchmarkGrade(passed=False, tool_called=True, matchers_total=1, matchers_passed=0)
        ] * 10
    ...
    def test_single_fail(self):
        assert (
            compute_benchmark_percentage(
                [
                    BenchmarkGrade(
                        passed=False, tool_called=False, matchers_total=1, matchers_passed=0
                    )
                ]
            )
            == 0.0
    ...
    def test_port_conflict_resolution_compound(self):
        scenario = _scenario(
            matchers=[
                {
                    "field": "command",
                    "pattern": r"lsof\s+-[it].*:80|ss\s+-[tlnp].*:80|fuser\s+80/tcp",
                },
                {
                    "field": "command",
                    "pattern": r"kill\s+(-9\s+|-SIGKILL\s+|-KILL\s+)?|pkill|fuser\s+-k",
                },
                {
                    "field": "command",
                    "pattern": r"systemctl\s+restart\s+nginx|service\s+nginx\s+restart",
                },
            ]
        )
        tool_calls = [
    ...
    def test_grep_with_context_lines(self):
        scenario = _scenario(
            matchers=[
                {
                    "field": "command",
                    "pattern": r"grep\s+.*-[a-zA-Z]*[iI]?.*error|grep\s+.*error.*-[a-zA-Z]*[iI]",
                },
