...
def create_real_llm_response(text: str | None) -> GenerateContentResponse:
    ...
    return GenerateContentResponse(
        candidates=[Candidate(content=Content(role="model", parts=parts), finish_reason="STOP")]
    )
...
@pytest.mark.asyncio
async def test_analyze_command_risk_accepts_json_after_preamble(
...
@pytest.mark.asyncio
async def test_analyze_command_risk_fallback_on_empty_response(
    analyzer, fake_provider, mock_settings
):
    # Queue a response with no text parts
    response = GenerateContentResponse(
        candidates=[Candidate(content=Content(role="model", parts=[]), finish_reason="STOP")]
    )
...
@pytest.mark.asyncio
async def test_analyze_command_risk_with_investigation_context(
    analyzer, fake_provider, mock_settings
):
...
@pytest.mark.asyncio
async def test_analyze_error_enforces_escalation_at_retry_limit(
    analyzer, fake_provider, mock_settings
):
...
@pytest.mark.asyncio
async def test_analyze_file_operation_risk_system_file_override(
    analyzer, fake_provider, mock_settings
):
...
@pytest.mark.asyncio
async def test_analyze_file_operation_risk_fallback_on_exception(
    analyzer, fake_provider, mock_settings
):
    with patch(
        "app.services.ai.response_analyzer.get_llm_provider", side_effect=Exception("LLM fail")
...
@pytest.mark.asyncio
async def test_analyze_file_operation_risk_system_file_with_backup(
    analyzer, fake_provider, mock_settings
):
