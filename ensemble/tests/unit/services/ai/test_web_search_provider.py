...
def _make_provider() -> WebSearchProvider:
    with mock.patch(_PATCH_CREDENTIALS), mock.patch(_PATCH_CLIENT):
        return WebSearchProvider(
            project_id="test-project", engine_id="test-engine", api_key="test-key"
        )
...
class TestWebSearchProviderSearch:
    ...
    async def test_returns_results_on_success(self):
        ...
        with mock.patch.object(
            provider, "_execute_search_lite", new_callable=mock.AsyncMock
        ) as mock_exec:
    ...
    async def test_result_count_capped_at_num(self):
        sdk_results = [_make_sdk_result(f"R{i}", f"https://x.com/{i}", "s") for i in range(10)]
    ...
    async def test_constructs_correct_serving_config(self):
        ...
        with mock.patch(_PATCH_CREDENTIALS), mock.patch(_PATCH_CLIENT):
            p = WebSearchProvider(
                project_id="my-proj", engine_id="my-engine", api_key="test-key", location="global"
            )
            ...
            supports=[
                GroundingSupport(
                    segment=GroundingSegment(
                        start_index=0, end_index=32, text="Both sources agree on this point."
                    ),
    ...
    def test_populates_sources_on_metadata(self, provider):
        ...
        metadata = self._make_metadata(
            ...
            supports=[
                GroundingSupport(
                    segment=GroundingSegment(
                        start_index=0, end_index=24, text="Test response text here."
                    ),
                    grounding_chunk_indices=[0],
                    text="Test response text here.",
                )
...
class TestBuildG8eWebSearchGrounding:
    ...
    def test_returns_grounding_used_true_with_results(self, provider):
        ...
        result = SearchWebResult(
            ...
            results=[
                WebSearchResultItem(
                    title="reuters.com", link="https://reuters.com/news", snippet="s1"
                ),
