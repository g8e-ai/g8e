    ...
    async def test_emits_citations_when_grounding_present(self):
        ...
        async def mock_execute(**kwargs):
            ...
            yield StreamChunkFromModel(
                type=StreamChunkFromModelType.TOOL_RESULT, data=StreamChunkData()
            )
            yield StreamChunkFromModel(
                type=StreamChunkFromModelType.TOOL_RESULT, data=StreamChunkData(result=result)
            )
...
class TestInterrogationGate:
    ...
    async def test_interrogation_gate_suppresses_tool_execution(self):
        ...
        provider.generate_content_stream_primary = MagicMock(
            side_effect=interrogation_with_tool_stream
        )
