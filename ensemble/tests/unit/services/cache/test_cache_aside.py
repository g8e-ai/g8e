...
class TestCacheAsideService:
    ...
    async def test_create_document_invalidates_kv_cache(
    ...
    async def test_create_document_kv_failure_still_succeeds(
        self, service, mock_kv_cache_client, mock_db_client
    ):
        mock_db_client.get_document.return_value = DocumentResult(success=True, data=None)
    ...
    async def test_update_document_success_invalidates_cache(
        self, service, mock_kv_cache_client, mock_db_client
    ):
    ...
    async def test_update_document_invalidates_query_cache_for_collection(
        ...
        result = await service.update_document(
            DB_COLLECTION_USERS, "user-5", {"latest_heartbeat_snapshot": {"timestamp": "fresh"}}
        )
        assert result.cache_invalidated is True

    async def test_create_document_invalidates_query_cache_for_collection(
        self, service, mock_kv_cache_client, mock_db_client
    ):
        mock_db_client.get_document.return_value = DocumentResult(success=True, data=None)
    ...
    async def test_delete_document_invalidates_query_cache_for_collection(
        self, service, mock_kv_cache_client
    ):
    ...
    async def test_delete_document_success_removes_from_cache(
        self, service, mock_kv_cache_client, mock_db_client
    ):
    ...
    async def test_get_document_cache_miss_reads_db_and_populates_cache(
        self, service, mock_kv_cache_client, mock_db_client
    ):
    ...
    async def test_get_query_result_cache_hit(self, service, mock_kv_cache_client):
        query_params = {
            "filters": [{"field": "status", "op": "==", "value": OperatorStatus.ACTIVE}]
        }
    ...
    async def test_get_query_result_uses_ttl_query_cache_default(
        self, service, mock_kv_cache_client
    ):
        query_params = {"q": "test"}
        query_str = json.dumps(query_params, sort_keys=True)
    ...
    async def test_set_query_result_uses_ttl_query_cache_default(
        self, service, mock_kv_cache_client
    ):
        results = [{"id": "r1"}]
    ...
    async def test_batch_create_documents_success(
        ...
        operations = [
            ...
            BatchCreateDocumentOperation(
                collection=DB_COLLECTION_USERS, document_id="u2", data={"id": "u2"}
            ),
        ]
    ...
    async def test_get_document_skips_cache_when_read_disabled(
        self, service, mock_kv_cache_client, mock_db_client
    ):
