...
class TestAttachmentServiceRetrieval:
    ...
    async def test_get_attachments_by_metadata_skips_missing_store_key(
        self, service, mock_blob_service
    ):
        key1 = "att:inv1/a1"
        att1 = AttachmentData(filename="file1.txt", base64_data="data1")
    ...
    async def test_process_attachments_multiple_types(self, service):
        attachments = [
            ...
            AttachmentData(
                ...
                content_type="application/pdf",
                file_size=5,
                base64_data=base64.b64encode(b"pdf01").decode(),
            ),
            AttachmentData(
                filename="screen.png",
                content_type="image/png",
                file_size=5,
                base64_data=base64.b64encode(b"png01").decode(),
            ),
            AttachmentData(
                filename="data.zip",
                content_type="application/zip",
                file_size=5,
                base64_data=base64.b64encode(b"zip01").decode(),
            ),
        ]
