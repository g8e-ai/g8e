...
class TestG8eHttpContext:
    ...
    def test_web_session_id_required(self):
        ...
        ctx = G8eHttpContext(
            user_id="u", case_id="c", investigation_id="i", source_component=G8EE_COMPONENT
        )
    ...
    def test_all_session_fields_required_when_not_client_relay(self):
        ...
        with pytest.raises(
            ValidationError, match="web_session_id, cli_session_id or user_id are required"
        ):
            G8eHttpContext(
                web_session_id=None,
                cli_session_id=None,
    ...
    def test_validation_fails_for_client_without_relay_flag(self):
        ...
        with pytest.raises(
            ValidationError,
            match="Context must have either web_session_id or cli_session_id for CLIENT source",
        ):
            G8eHttpContext(
                web_session_id=None,
                cli_session_id=None,
    ...
    def test_investigation_id_optional(self):
        ...
        ctx = G8eHttpContext(
            web_session_id="s", user_id="u", case_id="c", source_component=ComponentName.CLIENT
        )
        assert ctx.investigation_id is None

    def test_source_component_defaults_to_g8ee(self):
        ctx = G8eHttpContext(web_session_id="s", user_id="u", case_id="c", investigation_id="i")
        assert ctx.source_component == G8EE_COMPONENT

    def test_source_component_is_enum(self):
        ctx = self._make(source_component=G8EE_COMPONENT)
        assert ctx.source_component == G8EE_COMPONENT
...
class TestG8eCompatibility:
    ...
    def test_request_context_preserves_operator_id_field(self):
        ctx = RequestContext(
            web_session_id="s",
            user_id="u",
            source_component=G8EE_COMPONENT,
        ...
        assert ctx.operator_session_id == "sess-1"

    def test_request_context_source_component_defaults_to_g8ee(self):
        ctx = RequestContext(web_session_id="s", user_id="u")
        assert ctx.source_component == G8EE_COMPONENT

    def test_request_context_overrides_g8e_validator(self):
        """g8ee's RequestContext should NOT enforce session identity (that's G8eHttpContext's job)."""
        ctx = RequestContext(
            source_component=ComponentName.CLIENT,
        )
        assert ctx.source_component == ComponentName.CLIENT
    ...
    def test_request_context_serialization_round_trip(self):
        ...
        assert restored.operator_session_id == "sess-1"
        assert restored.source_component == G8EE_COMPONENT

    def test_bound_operator_status_str_comparison_works(self):
        """BoundOperator.status is str in g8e; OperatorStatus comparisons must still work."""
        op = BoundOperator(operator_id="op-1", status=OperatorStatus.BOUND)
        assert op.status == OperatorStatus.BOUND
        assert op.status == "bound"
