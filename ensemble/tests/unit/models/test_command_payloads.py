...
class TestFsListRequestPayload:
    ...
    def test_wire_dump_only_canonical_fields(self):
        p = FsListRequestPayload(
            path=".", execution_id="x", max_depth=0, max_entries=100, target_operators=["op-1"]
        )
        wire = p.model_dump(mode="json")
...
class TestFsReadRequestPayload:
    def test_required_path(self):
        p = FsReadRequestPayload(
            path="/etc/hosts", execution_id="exec-test", target_operators=["op-1"]
        )
    ...
    def test_optional_fields_default_to_none(self):
        p = FsReadRequestPayload(
            path="/app/config.json", execution_id="exec-test", target_operators=["op-1"]
        )
        assert p.execution_id == "exec-test"
    ...
    def test_wire_dump_includes_set_fields(self):
        p = FsReadRequestPayload(
            path="/etc/passwd", execution_id="exec-2", max_size=4096, target_operators=["op-1"]
        )
        wire = p.model_dump(mode="json")
    ...
    def test_wire_dump_excludes_none_optional_fields(self):
        p = FsReadRequestPayload(
            path="/app/log.txt", execution_id="exec-test", target_operators=["op-1"]
        )
        wire = p.model_dump(mode="json")
    ...
    def test_wire_dump_only_canonical_fields(self):
        p = FsReadRequestPayload(
            path="/app/main.py", execution_id="e", max_size=102400, target_operators=["op-1"]
        )
        wire = p.model_dump(mode="json")
        assert set(wire.keys()) <= {
            "path",
    ...
    def test_wire_dump_no_non_canonical_fields(self):
        p = FsReadRequestPayload(
            path="/var/log/app.log", execution_id="exec-test", target_operators=["op-1"]
        )
        wire = p.model_dump(mode="json")
        assert "requested_at" not in wire
        assert "source" not in wire
...
class TestFetchFileHistoryRequestPayload:
    ...
    def test_target_operators_required(self):
        p = FetchFileHistoryRequestPayload(
            execution_id="exec-test", file_path="/etc/hosts", target_operators=["op-1"]
        )
        assert p.target_operators == ["op-1"]

    def test_target_operators_all(self):
        p = FetchFileHistoryRequestPayload(
            execution_id="exec-test", file_path="/etc/hosts", target_operators=["all"]
        )
    ...
    def test_wire_dump_includes_target_operators(self):
        p = FetchFileHistoryRequestPayload(
            execution_id="exec-1", file_path="/app/config.json", target_operators=["op-456"]
        )
        wire = p.model_dump(mode="json")
        assert wire["execution_id"] == "exec-1"
...
class TestFetchFileDiffRequestPayload:
    ...
    def test_target_operators_required(self):
        p = FetchFileDiffRequestPayload(
            execution_id="exec-test", diff_id="diff-123", target_operators=["op-1"]
        )
        assert p.target_operators == ["op-1"]

    def test_target_operators_multiple(self):
        p = FetchFileDiffRequestPayload(
            execution_id="exec-test", diff_id="diff-123", target_operators=["op-1", "op-2"]
        )
    ...
    def test_wire_dump_includes_target_operators(self):
        p = FetchFileDiffRequestPayload(
            execution_id="exec-1", diff_id="diff-1", target_operators=["op-456"]
        )
        wire = p.model_dump(mode="json")
        assert wire["execution_id"] == "exec-1"
