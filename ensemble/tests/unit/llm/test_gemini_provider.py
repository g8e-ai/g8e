...
class TestContentToGenai:
    ...
    def test_tool_response_part(self):
        ...
        content = Content(role=Role.TOOL, parts=[Part(tool_response=tr)])
        result = _content_to_genai(content)
        assert result["role"] == Role.USER
        assert result["parts"] == [
    ...
    def test_mixed_parts(self):
        ...
        content = Content(
            role=Role.MODEL,
            parts=[Part(text="thought", thought=True, thought_signature=sig), Part(text="reply")],
        )
        result = _content_to_genai(content)
...
class TestResponseParsing:
    ...
    def test_parse_full_response(self, mock_client):
        ...
        mock_candidate.content.parts = [
            MagicMock(text="reply", thought=False, function_call=None, thought_signature=None)
        ]
        mock_candidate.finish_reason.name = "STOP"
        mock_candidate.grounding_metadata = None
...
class TestGeminiProvider:
    ...
    def test_content_to_genai_thought_only(self):
        ...
        content = Content(
            role=Role.MODEL, parts=[Part(tool_call=ToolCall(name="t1", args={}), thought=True)]
        )
        result = _content_to_genai(content)
    ...
    def test_build_thinking_config_gemini3_none(self):
        ...
        assert GeminiProvider._build_thinking_config_gemini3(None, "model", genai_types) == (
            None,
            None,
        )
    ...
    def test_build_genai_config_assistant_sampling(self):
        ...
        settings = AssistantLLMSettings(
            max_output_tokens=100, top_p_nucleus_sampling=0.8, top_k_filtering=20
        )
    ...
    def test_build_genai_config_assistant_json(self):
        ...
        settings = AssistantLLMSettings(
            max_output_tokens=512, response_format=ResponseFormat(json_schema=schema)
        )
    ...
    async def test_stream_with_retry_loop(self, mock_client):
        ...
        mock_chunk = MagicMock()
        mock_cand = MagicMock()
        mock_cand.content.parts = [
            MagicMock(text="hi", thought=False, function_call=None, thought_signature=None)
        ]
        mock_cand.finish_reason = None
        mock_chunk.candidates = [mock_cand]
        mock_chunk.usage_metadata = None
