...
2. ``services/g8ee/app/prompts_data/modes/<mode>/tools.txt`` carries
...
_PROMPTS_DATA_DIR: Path = Path(__file__).parent.parent.parent / "app" / "prompts_data"
...
class TestPersonaPlaceholderHygiene:
    ...
    def test_allowlist_and_pure_voice_sets_are_disjoint(self) -> None:
        ...
        assert not overlap, (
            f"Personas cannot be both pure-voice and placeholder-allowlisted: {sorted(overlap)}"
        )
...
class TestModeToolsFilesAreRulesOnly:
    ...
    def test_mode_tools_files_carry_no_tool_description_tags(self) -> None:
        ...
        assert not offenders, (
            ...
            "(those live in prompts_data/tools/<name>.txt). Offenders: " + ", ".join(offenders)
        )
...
class TestEveryActiveToolHasADescriptionFile:
    ...
    def test_no_orphan_tool_description_files(self) -> None:
        ...
        orphans = [
            path.name for path in sorted(_TOOLS_DIR.glob("*.txt")) if path.stem not in enum_values
        ]
        assert not orphans, (
