...
def pytest_collection_modifyitems(config, items):
    ...
    has_llm_credentials = (
        has_test_llm_creds
        if env_llm_provider
        else (llm is not None and llm.primary_provider is not None)
    )
    ...
    for item in items:
        ...
        if "test_agent_accuracy" in item.name or "test_gemini_accuracy" in item.name:
            ...
            if scenario:
                if scenario.get("agent_mode") == "g8e.bound" or scenario.get("expected_tools"):
                    item.add_marker(pytest.mark.tools)

                # Assume all complex scenarios benefit from thinking
                if scenario.get("agent_mode") == "g8e.bound":
...
class TaskTracker:
    ...
    def patch_create_task(self, module_path):
        ...
        class _TaskPatchContext:
            ...
            def __enter__(self):
                self._patch = patch(
                    f"{module_path}.asyncio.create_task",
                    side_effect=self._tracker._fake_create_task,
                )
...
async def mock_gateway():
    """Start an in-memory mock g8e gateway for integration tests.

    Provides HTTPS endpoints (health, db, kv, blob) and a WebSocket pub/sub
    broker at /api/v1/pubsub/stream using the same protobuf wire protocol as
    the real g8e Gateway.
