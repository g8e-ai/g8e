...
The Python package requires Python 3.10 or later and depends on `pydantic>=2.0.0`, `protobuf>=4.0.0`, and `PyNaCl>=1.5.0`. Install with `pip install g8e`.
...
protocol/
  ...
  python/                    Python package (g8e)
    g8e/                     Import namespace (g8e)
      ...
      common/v1/             Generated common protobuf module and .pyi stub
      operator/v1/           Generated operator protobuf module and .pyi stub
      pubsub/v1/             Generated pub/sub protobuf module and .pyi stub
      ...
      receipts.py            Canonical receipt parsing and Ed25519 verification
      py.typed               PEP 561 type marker
      _data/                 Bundled JSON constants and doctrine registries for PyPI installs (generated by make python-build)
  ...
  vectors/                   Shared receipt and persistence-attestation canonicalization vectors
...
- **`g8e.common.v1`, `g8e.operator.v1`, and `g8e.pubsub.v1`**: Generated protobuf `_pb2.py` modules with matching `_pb2.pyi` type stubs. The operator package includes `ActionReceipt`, `DeterministicStageEvidence`, `ReceiptPersistenceAttestation`, and `CommitmentAttestation`.
- **`g8e.receipts`**: Strict protojson receipt parsing, canonical dictionary serialization, canonical receipt and persistence-attestation encoding, Ed25519 receipt signature verification, and final persistence-attestation verification.

### Examples

The `examples/` directory contains Go example programs:

- `governance_envelope/main.go`: Demonstrates constructing a `GovernanceEnvelope` with L1, L2, and L3 governance metadata, a `CommandRequested` payload, transaction hash computation, and protojson round-trip serialization
...
- `vectors/action_receipt_canonicalization.json`: Shared Go/Python vectors for base receipt canonicalization and signatures.
- `vectors/action_receipt_stage_evidence_canonicalization.json`: Shared vectors proving deterministic stage evidence changes the signed receipt bytes.
- `vectors/receipt_persistence_attestation_canonicalization.json`: Shared vectors for durable-persistence attestation canonicalization and signatures.
...
- `make openapi`: Stub for OpenAPI spec generation from protobuf (prints setup instructions; use `make proto` for protobuf generation)

Protobuf code generation uses buf. The generation config is in `buf.gen.yaml` at the repository root. Run `make proto` from the repository root to regenerate Go structs, gRPC stubs, and Markdown API documentation. Run `uv run --project protocol/python --extra dev python protocol/python/scripts/generate_protos.py` to regenerate the committed Python modules and `.pyi` stubs; use `--check` to verify synchronization without writing.
