...
Mutation action types: `ActionTypeA2aCall`, `ActionTypeCancel`, `ActionTypeExecuteBash`, `ActionTypeFileEdit`, `ActionTypeMcpCall`, `ActionTypeRestoreFile`, `ActionTypeShutdown`, `ActionTypePlatformEnrollmentCreate`, `ActionTypePlatformEnrollmentDecide`, `ActionTypePlatformEnrollmentIssue`, `ActionTypePlatformEnrollmentPersistPolicy`, `ActionTypePlatformEnrollmentCreateSession`.
...
Platform error variables defined as `error` values using `errors.New()`. The file contains over 900 error variables covering standard platform errors, keystore errors, ledger errors, CLI approval errors, notary errors, CLI authentication errors, HTTP client errors, process manager errors, filesystem errors, FsGrep errors, pubsub client errors, execution service errors, MCP service errors, MCP registry errors, MCP validation errors, MCP OOM detection errors, MCP SSH known hosts errors, MCP git ops errors, MCP TLS cert inspect errors, run shell command errors, network identity detection errors, system utils errors, audit service errors, audit store errors, execution vault errors, pubsub service errors, scrubbing service errors, gateway service errors, gateway approval errors, MCP native handler errors, SQLite validation errors, SQLite utility errors, SQLite compression errors, passkey bootstrap errors, enrollment token errors, passkey credential validation errors, Windows-specific errors, data command errors, test command errors, vault command/crypto errors, and platform enrollment errors (`ErrPlatformEnrollmentRequiresActivation`, `ErrPlatformEnrollmentInvalidComponent`, `ErrPlatformEnrollmentInstanceIDRequired`, `ErrPlatformEnrollmentInvalidInstanceID`, `ErrPlatformEnrollmentHostnameRequired`, `ErrPlatformEnrollmentInvalidHostname`, `ErrPlatformEnrollmentFingerprintRequired`, `ErrPlatformEnrollmentInvalidPayload`, `ErrPlatformEnrollmentInvalidCSR`, `ErrPlatformEnrollmentUnsupportedKey`, `ErrPlatformEnrollmentDuplicateKey`, `ErrPlatformEnrollmentRequestIDRequired`, `ErrPlatformEnrollmentTokenRequired`, `ErrPlatformEnrollmentInvalidToken`, `ErrPlatformEnrollmentRequestNotFound`, `ErrPlatformEnrollmentInvalidDecision`, `ErrPlatformEnrollmentReasonTooLong`, and additional expiry/lease/state-transition errors).
...
Owner-approved platform workload enrollment constants. The gateway starts with zero users and issues no platform certificates until the first owner enrolls and approves pending enrollment requests. Each platform workload (operator, dashboard, ensemble) submits a platform enrollment request with a P-256 CSR and system fingerprint; the owner approves the request by exact request ID via authenticated mTLS; the component signs a canonical completion transcript and receives its enrolled credentials. See [auth.md](../../docs/architecture/auth.md) §1.5 for the full protocol.

- Governance action types (typed `PlatformEnrollmentGovernanceAction`): `PlatformEnrollmentActionCreate` (`PLATFORM_ENROLLMENT_CREATE`), `PlatformEnrollmentActionDecide` (`PLATFORM_ENROLLMENT_DECIDE`), `PlatformEnrollmentActionIssue` (`PLATFORM_ENROLLMENT_ISSUE`), `PlatformEnrollmentActionPersistPolicy` (`PLATFORM_ENROLLMENT_PERSIST_POLICY`), `PlatformEnrollmentActionCreateSession` (`PLATFORM_ENROLLMENT_CREATE_SESSION`)
- Governance intents (typed `PlatformEnrollmentGovernanceIntent`): `PlatformEnrollmentIntentRequest` (`platform_enrollment.request`), `PlatformEnrollmentIntentApprove` (`platform_enrollment.approve`), `PlatformEnrollmentIntentDeny` (`platform_enrollment.deny`), `PlatformEnrollmentIntentIssue` (`platform_enrollment.issue`)
- Timeouts and limits: `PlatformEnrollmentRequestTTL` (30m), `PlatformEnrollmentTokenBytes` (32), `PlatformEnrollmentProtocolVersion` (`"1"`), `PlatformEnrollmentMaxHostnameBytes` (253), `PlatformEnrollmentMaxInstanceIDBytes` (128), `PlatformEnrollmentMaxReasonBytes` (512), `PlatformEnrollmentMaxRequestBytes` (64 KiB), `PlatformEnrollmentIssuanceLeaseTTL` (30s, crash-recovery bound for the issuing state), `PlatformEnrollmentMaxLiveRequestsPerComponent` (3, bounds concurrent non-terminal requests per component kind), `PlatformEnrollmentCleanupInterval` (5m, reconciliation of expired leases and terminal request retention), `PlatformEnrollmentCleanupRetention` (24h, terminal request retention before cleanup removal; issued artifacts on completed requests are never removed), `PlatformEnrollmentRetryAfterSeconds` (5, Retry-After header for clients polling a request in the issuing state with a live lease)
...
JSON files: `agents.json`, `api_paths.json`, `auth.json`, `channels.json`, `collections.json`, `document_ids.json`, `env_vars.json`, `events.json`, `exit_codes.json`, `field_paths.json`, `headers.json`, `intents.json`, `kv_keys.json`, `network.json`, `output.json`, `platform.json`, `platform_enrollment.json` (owner-approved platform enrollment protocol constants: governance actions, intents, timeouts, limits, and state definitions), `platform_enrollment_completion_transcript_vectors.json` (shared golden parity vector file defining fixed inputs and expected hex bytes for the dashboard, ensemble, and operator completion transcript constructions, used by Go, JavaScript, and Python tests to prove byte-identical transcript serialization), `ports.json`, `prompts.json`, `pubsub.json`, `senders.json`, `status.json`, `timestamp.json`.
