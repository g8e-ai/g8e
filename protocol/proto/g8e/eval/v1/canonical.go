...
func MarshalCanonical(message proto.Message) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("evaluation protocol: marshal canonical protojson: %w", err)
	}
	var compact bytes.Buffer
	if err := json.Compact(&compact, encoded); err != nil {
		return nil, fmt.Errorf("evaluation protocol: compact canonical protojson: %w", err)
	}
...
func UnmarshalCanonical(encoded []byte, message proto.Message) error {
	if err := canonicalUnmarshalOptions.Unmarshal(encoded, message); err != nil {
		return fmt.Errorf("evaluation protocol: unmarshal canonical protojson: %w", err)
	}
	...
	if !bytes.Equal(encoded, canonical) {
		...
		return fmt.Errorf("evaluation protocol: input is not canonical protojson at byte %d", index)
	}
	return nil
}
