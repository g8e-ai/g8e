...
from app.models.tool_args import RecursiveGrepArgs
...
async def handle(
    ...
    args = convert_args_to_payload(tool_args, FsGrepRequestPayload, execution_id, investigation)
    logger.info(
        "[RECURSIVE_GREP] Path: %s pattern: %s includes: %s max_matches: %s",
        args.path, args.pattern, args.includes, args.max_matches,
    )
    result = await svc.operator_command_service.execute_fs_grep(
        args=args, investigation=investigation, g8e_context=g8e_context,
    )
    logger.info("[RECURSIVE_GREP] matches=%d truncated=%s",
