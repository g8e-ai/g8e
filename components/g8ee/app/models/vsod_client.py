...
"""
VSOD HTTP client request and response models.
"""
...
class SSEPushResponse(VSOBaseModel):
    """Response from POST /api/internal/sse/push.

    Mirrors VSOD internal_sse_routes.js: { success, delivered }
    """
...
class GrantIntentResponse(VSOBaseModel):
    """Response from POST /api/internal/operators/:operatorId/grant-intent.

    Mirrors VSOD internal_operator_routes.js: { success, message, operator_id, granted_intents, expires_at }
    """
...
class RevokeIntentResponse(VSOBaseModel):
    """Response from POST /api/internal/operators/:operatorId/revoke-intent.

    Mirrors VSOD internal_operator_routes.js: { success, message, operator_id, granted_intents }
    """
...
class AISearchWebPayload(VSOBaseModel):
...
class OperatorNetworkPortCheckPayload(VSOBaseModel):
...
class ChatTurnCompletePayload(VSOBaseModel):
...
class ChatResponseChunkPayload(VSOBaseModel):
...
class ChatCitationsReadyPayload(VSOBaseModel):
...
class ChatResponseCompletePayload(VSOBaseModel):
...
class ChatErrorPayload(VSOBaseModel):
    """Payload for EventType.LLM_CHAT_ITERATION_FAILED."""

    error: str = Field(description="Error message")


class AiProcessingStoppedPayload(VSOBaseModel):
...
class IntentDeniedPayload(VSOBaseModel):
...
class IntentGrantedPayload(VSOBaseModel):
...
class IntentRevokedPayload(VSOBaseModel):
...
class IntentOperationResult(VSOBaseModel):
    """Result of a grant-intent or revoke-intent operation against VSOD."""
