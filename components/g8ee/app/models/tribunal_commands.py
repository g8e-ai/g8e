...
from app.constants import CommandGenerationOutcome, TribunalMember, TieBreakReason, VerifierReason
...
class TribunalCommandVerifier(G8eIdentifiableModel.model.__base__):
    verifier_passed: bool | None = Field(default=None, description="True if the verifier approved the vote winner (or an equivalent dissenter cluster)")
    verifier_revision: str | None = Field(default=None, description="Revised command produced by the verifier when verifier_passed=false")
    verifier_reason: VerifierReason | None = Field(default=None, description="The verifier's stated reason")
    swap_to_cluster: str | None = Field(default=None, description="Opaque cluster id the Verifier swapped to (set only on reason=swapped_to_dissenter)")
...
class TribunalCommand(G8eIdentifiableModel):
    ...
    voting breakdown, verifier decisions, and final outcome. Links to an
    ...
    verifier: TribunalCommandVerifier | None = Field(default=None, description="Verifier pass results when enabled")
