...
"""
Typed VSOD publish events.

All events published to VSOD are represented as one of two typed envelopes:
...
- BackgroundEvent: system-initiated, no browser session. VSOD routes by
...
accordingly. publish_event_to_vsod() no longer exists.
"""
...
class SessionEvent(VSOBaseModel):
    ...
    event_type: EventType = Field(description="VSOD event type")
    payload: VSOBaseModel = Field(description="Typed event-specific payload")
...
class BackgroundEvent(VSOBaseModel):
    """System-initiated event with no connected browser session.

    VSOD routes by investigation_id. Use this for background/async events
