...
from app.models.base import VSOBaseModel, Field
...
from tests.fakes.fake_vsod_client import FakeVSODClient as create_mock_vsod_http_client


class _SimplePayload(VSOBaseModel):
...
class _AnalysisResult(VSOBaseModel):
    status: str | None = Field(default=None)
    results: list[str] | None = Field(default=None)
    count: int | None = Field(default=None)


class _ComplexPayload(VSOBaseModel):
...
def mock_vsod_http_client():
    return create_mock_vsod_http_client()


@pytest.fixture
def service(mock_vsod_http_client):
    return EventService(internal_http_client=mock_vsod_http_client)


class TestEventServiceInit:

    def test_initialization_with_client(self, mock_vsod_http_client):
        service = EventService(internal_http_client=mock_vsod_http_client)

        assert service.vsod_client is not None
        assert service.vsod_client == mock_vsod_http_client


@pytest.mark.asyncio(loop_scope="session")
class TestPublishEventToVSOD:

    async def test_publish_with_web_session_id(self, service, mock_vsod_http_client):
        event = SessionEvent(
        ...
        mock_vsod_http_client.assert_push_sse_event_called_with(event)

    async def test_publish_background_event(self, service, mock_vsod_http_client):
    ...
    async def test_session_event_typed_event_passed_to_client(self, service, mock_vsod_http_client):
        event = SessionEvent(
    ...
    async def test_publish_background_event_no_web_session(self, service, mock_vsod_http_client):
        event = BackgroundEvent(
    ...
    async def test_publish_with_all_optional_fields(self, service, mock_vsod_http_client):
        event = SessionEvent(
    ...
    async def test_web_session_id_on_session_event(self, service, mock_vsod_http_client):
        event = SessionEvent(
            event_type=EventType.LLM_CHAT_ITERATION_TEXT_CHUNK_RECEIVED,
    ...
    async def test_correlation_ids_on_session_event(self, service, mock_vsod_http_client):
        event = SessionEvent(
            event_type=EventType.LLM_CHAT_ITERATION_TEXT_CHUNK_RECEIVED,
    ...
    async def test_publish_failure_raises_network_error(self, service, mock_vsod_http_client):
        mock_vsod_http_client.set_push_sse_event_return_value(False)
    ...
    async def test_publish_with_complex_payload(self, service, mock_vsod_http_client):
    ...
    async def test_investigation_id_on_session_event(self, service, mock_vsod_http_client):
        event = SessionEvent(
    ...
    async def test_network_error_message_contains_event_type(self, service, mock_vsod_http_client):
        mock_vsod_http_client.set_push_sse_event_return_value(False)
    ...
    async def test_background_event_with_optional_case_id_none(self, service, mock_vsod_http_client):
        event = BackgroundEvent(
            event_type=EventType.OPERATOR_STATUS_UPDATED_ACTIVE,
            payload=_SimplePayload(status="active"),
    ...
    async def test_background_event_with_explicit_case_id(self, service, mock_vsod_http_client):
        event = BackgroundEvent(
            event_type=EventType.OPERATOR_STATUS_UPDATED_ACTIVE,
            payload=_SimplePayload(status="active"),
    ...
    async def test_background_event_task_id_on_event(self, service, mock_vsod_http_client):
        event = BackgroundEvent(
            event_type=EventType.OPERATOR_STATUS_UPDATED_ACTIVE,
            payload=_SimplePayload(status="active"),
    ...
    async def test_session_event_task_id_none_when_omitted(self, service, mock_vsod_http_client):
        event = SessionEvent(
            event_type=EventType.LLM_CHAT_MESSAGE_SENT,
...
class TestEventServiceProtocol:
    ...
    def test_event_service_satisfies_protocol(self, mock_vsod_http_client):
        service = EventService(internal_http_client=mock_vsod_http_client)
