...
class TestPubSubChannelMatchesSharedJSON:
    ...
    def test_segment_count(self, channels):
        assert channels["pubsub"]["segment_count"] == CHANNEL_SEGMENT_COUNT
...
class TestOperatorChannelContractRegression:
    """Contract regression tests for OperatorChannel with realistic session IDs.

    Ensures channel identity is preserved between g8ee and g8eo using the actual
    session ID format generated by g8ed: operator_session_<ts>_<uuid>.
    This prevents phantom "double prefix" bugs by locking down the contract.
    """

    def test_results_channel_with_realistic_session_id(self):
        operator_id = "op-abc123"
        session_id = "operator_session_1764000000000_abc-123-def-456-ghi-789"
        expected = f"results:{operator_id}:{session_id}"
        actual = OperatorChannel.results(operator_id, session_id)
        assert actual == expected

    def test_cmd_channel_with_realistic_session_id(self):
        operator_id = "op-xyz789"
        session_id = "operator_session_1764000000000_987-fed-654-321-cba"
        expected = f"cmd:{operator_id}:{session_id}"
        actual = OperatorChannel.cmd(operator_id, session_id)
        assert actual == expected

    def test_heartbeat_channel_with_realistic_session_id(self):
        operator_id = "op-def456"
        session_id = "operator_session_1764000000000_111-222-333-444-555"
        expected = f"heartbeat:{operator_id}:{session_id}"
        actual = OperatorChannel.heartbeat(operator_id, session_id)
        assert actual == expected

    def test_parse_round_trip_with_realistic_session_id(self):
        ...
        prefix, parsed_op_id, parsed_session_id = OperatorChannel.parse(channel)
        assert prefix == "results"
        assert parsed_op_id == operator_id
        assert parsed_session_id == session_id

    def test_parse_handles_colon_in_session_id(self):
        operator_id = "op-colon-test"
        session_id = "operator_session_1764000000000_with:colon:inside"
        channel = OperatorChannel.cmd(operator_id, session_id)
        prefix, parsed_op_id, parsed_session_id = OperatorChannel.parse(channel)
        assert prefix == "cmd"
        assert parsed_op_id == operator_id
        assert parsed_session_id == session_id
