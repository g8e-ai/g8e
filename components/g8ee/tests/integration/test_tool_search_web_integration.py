...
from tests.fakes.agent_helpers import make_agent_run_args, make_client_event_service
...
async def _collect_sse_events(chunks, inputs=None, state=None):
    ...
    await deliver_via_sse(stream=_gen(), inputs=inputs, state=state, client_event_service=event_svc)
