...
if [ -f "${SSL_DIR}/internal_auth_token" ]; then
    ...
    echo "[g8e-pod] Loaded G8E_INTERNAL_AUTH_TOKEN from volume"
fi
...
if [ -f "${SSL_DIR}/session_encryption_key" ]; then
    ...
    echo "[g8e-pod] Loaded G8E_SESSION_ENCRYPTION_KEY from volume"
fi
...
if [ -f "${SSL_DIR}/ca.crt" ]; then
    ...
    echo "[g8e-pod] Configured CA certificate paths from ${SSL_DIR}/ca.crt"
fi
...
[program:operator]
command=/app/components/g8e-pod/scripts/fetch-key-and-run.sh
autostart=true
autorestart=true
startsecs=2
startretries=3
...
EOF
    echo "[g8e-pod] Supervisor config written (endpoint: ${endpoint})"
}

_write_supervisor_conf

echo "[g8e-pod] Starting supervisord"
