...
export const USER_SETTINGS = Object.freeze([
    ...
    Object.freeze({
        ...
        description: 'Comma-separated list of blacklisted commands. Blocked if Command Blacklisting is enabled.',
        type: 'text',
        secret: false,
        placeholder: 'rm,format,mkfs',
        default: 'nc,netcat,socat,telnet,ssh,scp,sftp,ftp,rsync,traceroute,mtr,tcpdump,nmap,iptables,ufw,route,arp,ifconfig,ip,xargs,find,base64,tar,zip,unzip,gzip,gunzip,bzip2,tee,split,python,python3,perl,ruby,php,node,gcc,make,go,vi,vim,nano,emacs,less,more,man,tmux,screen,mosh,dd,rm,cp,mv,mkdir,rmdir,touch,patch,chmod,chown,mount,umount,fdisk,mkfs,blkid,lsblk,sudo,su,pkexec,doas,crontab,systemctl,service,journalctl,env,printenv,export,source,alias,history,bash,sh,zsh,csh,tcsh,awk,sed,strace,ltrace,watch,perf,kill,pkill,killall,bg,fg,jobs,nohup,docker,docker-compose,kubectl,kubens,kubectx,helm,terraform,terragrunt,ansible,ansible-playbook,aws,gcloud,az,vagrant,minikube,podman,skopeo,buildah,git,groups,passwd,w,who,reboot,shutdown,poweroff,halt,init,telinit,sysctl,insmod,rmmod,modprobe,depmod,useradd,usermod,userdel,groupadd,groupmod,groupdel,apt,apt-get,apt-cache,yum,dnf,rpm,pacman,pip,pip3,npm,yarn,cargo,gem,composer,mysql,psql,redis-cli,mongosh,sqlite3,openssl,gpg,parted,resize2fs,xfs_growfs,losetup,runc,ctr,hping3,tcpkill,dump,restore',
        validate: v => {
            if (typeof v !== 'string') return false;
            if (v === '') return true;
            const parts = v.split(',').map(p => p.trim()).filter(Boolean);
...
export const PLATFORM_SETTINGS = Object.freeze([
    ...
    Object.freeze({ key: 'g8e_internal_http_url',   default: process.env.G8E_INTERNAL_HTTP_URL || 'https://g8es:9000' }),
    Object.freeze({ key: 'g8e_internal_pubsub_url', default: process.env.G8E_INTERNAL_PUBSUB_URL || 'wss://g8es:9001' }),
    ...
    Object.freeze({ key: 'ssl_dir',                     default: process.env.G8E_SSL_DIR || '/g8es'       }),
    Object.freeze({ key: 'tls_cert_path',               default: ''                 }),
    Object.freeze({ key: 'tls_key_path',                default: ''                 }),
    Object.freeze({ key: 'g8e_pubsub_ca_cert',      default: process.env.G8E_SSL_DIR ? `${process.env.G8E_SSL_DIR}/ca.crt` : '/g8es/ssl/ca.crt' }),
