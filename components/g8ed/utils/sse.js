 ...
 * Enforces the G8eBaseModel boundary — callers cannot pass plain objects.
 *
 * @param {import('http').ServerResponse} res
 * @param {G8eBaseModel} eventData
 */
export function writeSSEFrame(res, eventData) {
    if (!(eventData instanceof G8eBaseModel)) {
        throw new Error(`writeSSEFrame requires a G8eBaseModel instance, got ${typeof eventData}`);
    }
