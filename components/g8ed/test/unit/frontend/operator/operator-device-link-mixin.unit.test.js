...
beforeEach(async () => {
    ...
    vi.doMock('@g8ed/public/js/utils/html.js', () => ({
        escapeHtml: (text) => {
            if (!text) return '';
            const div = document.createElement('div');
            div.textContent = text;
    ...
    ({ OperatorDeviceLinkMixin } = await import('@g8ed/public/js/components/operator-device-link-mixin.js'));
    ({ operatorPanelService } = await import('@g8ed/public/js/utils/operator-panel-service.js'));
    ({ devLogger } = await import('@g8ed/public/js/utils/dev-logger.js'));
