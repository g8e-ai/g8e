...
describe('AuthManager [UNIT]', () => {
    ...
    describe('_fetchSession()', () => {
        it('GETs ApiPaths.auth.webSession() via window.serviceClient', async () => {
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.webSession(), makeOkResponse(makeValidSessionResponse()));
        ...
        it('returns response and parsed data', async () => {
            ...
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.webSession(), makeOkResponse(body));
    ...
    describe('validateSession() — invalid session response', () => {
        it('session remains null', async () => {
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.webSession(), makeOkResponse(makeInvalidSessionResponse()));
    ...
    describe('validateSession() — network error', () => {
        it('session remains null', async () => {
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.webSession(), Promise.reject(new Error('Network error')));
    ...
    describe('logout()', () => {
        it('calls ApiPaths.auth.logout(), clears session, and navigates home', async () => {
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.webSession(), makeOkResponse(makeValidSessionResponse()));
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.logout(), makeOkResponse({}));
            ...
            expect(logoutCall.service).toBe(ComponentName.G8ED);
            expect(auth.session).toBeNull();
        ...
        it('clears session even when logout request fails', async () => {
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.webSession(), makeOkResponse(makeValidSessionResponse()));
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.logout(), Promise.reject(new Error('Network error')));
    ...
    describe('_subscribeToSSEFailed() / SSE-driven session invalidation', () => {
        it('calls handleSessionExpired() when SSE FAILED fires and user is authenticated', async () => {
            const { EventBus } = await import('@g8ed/public/js/utils/eventbus.js');
    ...
    describe('_renderPasskeyRegisterForm() — passkey cancel and retry', () => {
        ...
        beforeEach(() => {
            serviceClient.setResponse(ComponentName.G8ED, ApiPaths.auth.webSession(), {
