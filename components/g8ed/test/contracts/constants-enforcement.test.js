 ...
 * 
 * Scans g8ed source files for raw string literals that should use constants
...
/**
 * Directories to scan for violations (relative to G8ED_ROOT)
 */
...
function buildEnforcedValues() {
    ...
    for (const file of CONSTANTS_FILES) {
        const filePath = path.join(G8ED_ROOT, 'constants', file);
...
function getFilesToScan() {
    ...
    for (const dir of SCAN_DIRS) {
        const dirPath = path.join(G8ED_ROOT, dir);
    ...
    it('should not use raw string literals where constants exist', () => {
        ...
        for (const filePath of files) {
            ...
            for (const v of violations) {
                allViolations.push({
                    file: path.relative(G8ED_ROOT, filePath),
                    ...v,
                });
            }
        }
