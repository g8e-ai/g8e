...
async function getEnforcedPaths() {
    ...
    // Enforce g8ed internal paths
    for (const [key, pathValue] of Object.entries(InternalApiPaths.g8ed)) {
        enforced.set(pathValue, { category: 'g8ed', key });
    }
    ...
    it('should not use raw strings for internal API paths', () => {
        ...
        for (const file of files) {
            ...
            for (const v of violations) {
                allViolations.push({
                    file: path.relative(G8ED_ROOT, file),
                    ...v
                });
            }
        }
