        ...
        if (response.success && response.operator_id && response.api_key) {
            ...
            if (this.apiKeyService) {
                ...
                if (!issueResult || !issueResult.success) {
                    logger.error('[OPERATOR-SLOT] Failed to issue API key for slot', { id: response.operator_id, error: issueResult?.error || 'Unknown error' });
                    return OperatorSlotCreationResponse.forFailure('Failed to issue API key');
                }
            }
