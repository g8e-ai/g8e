...
/**
 * G8esPubSubClient — g8es Pub/Sub WebSocket client.
 * 
 * Handles pub/sub messaging over WebSocket to g8es ($G8E_INTERNAL_PUBSUB_URL/ws/pubsub).
...
class G8esPubSubClient {
    ...
    constructor({ pubsubUrl, caCertPath, internalAuthToken = null } = {}) {
        if (!pubsubUrl) {
            throw new Error('G8esPubSubClient: pubsubUrl is required');
        }
    ...
    _connectWS() {
        ...
        this._ws.on('open', () => {
            logger.info('[G8ES-CLIENT] Pub/sub WebSocket connected');
        });
    ...
    async terminate() {
        ...
        logger.info('[G8ES-PUBSUB] Client terminated');
    }
