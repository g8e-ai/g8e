...
export function createSSERouter({
    ...
    const getResolvedOperatorService = () => {
        try {
            ...
            logger.error('[VSOD-SSE] Failed to get OperatorService', { error: e.message });
            throw e;
        }
    };
    ...
    router.get(SSEPaths.EVENTS, sseRateLimiter, requireAuth, async (req, res, next) => {
        ...
        try {
            ...
            logger.info(`[VSOD-SSE] New SSE connection established`, {
            ...
            logger.error('[VSOD-SSE] Connection setup failed', { error: err.message, webSessionId: redactWebSessionId(connectionId) });
            res.end();
            return;
        }
        ...
        sseService.pushInitialState(req.userId, connectionId, organizationId).catch(err => {
            logger.error('[VSOD-SSE] pushInitialState failed', { error: err.message });
        });

        // Sync operator session state
        resolvedOperatorService.syncSessionOnConnect(req.userId, connectionId).catch(err => {
            logger.error('[VSOD-SSE] syncSessionOnConnect failed', { error: err.message });
        });
        ...
        const cleanupConnection = async () => {
            ...
            logger.info(`[VSOD-SSE] Connection cleaned up`, {
                webSessionId: redactWebSessionId(connectionId),
        ...
        keepaliveInterval = setInterval(async () => {
            if (sseService.hasLocalConnection(connectionId)) {
                try {
                    ...
                    logger.error(`[VSOD-SSE] Keepalive failed for ${connectionId}:`, error);
                    cleanupConnection();
                }
            } else {
                clearInterval(keepaliveInterval);
        ...
        req.on('close', () => {
            const connectionDuration = Date.now() - connectionStartTime;
            logger.info(`[VSOD-SSE] SSE connection closed`, {
                webSessionId: redactWebSessionId(connectionId),
                connectionDuration: Math.floor(connectionDuration / 1000) + 's'
            });
            cleanupConnection();
        ...
        req.on('error', (error) => {
            ...
            logger.error(`[VSOD-SSE] SSE connection error`, {
                webSessionId: redactWebSessionId(connectionId),
                error: error.message,
