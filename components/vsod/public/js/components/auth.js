 ...
 *
 * Architecture:
 * - Single source of truth: `session` (WebSessionModel from VSODB KV via backend)
...
export class AuthManager {
    ...
    async _fetchSession() {
        const response = await window.serviceClient.get(ServiceName.VSOD, ApiPaths.auth.webSession());
    ...
    async startPasskeyRegistration(userId) {
        try {
            const challengeRes = await window.serviceClient.post(ServiceName.VSOD, ApiPaths.auth.passkey.registerChallenge(), { user_id: userId });
            ...
            const verifyRes = await window.serviceClient.post(ServiceName.VSOD, ApiPaths.auth.passkey.registerVerify(), {
                user_id: userId,
    ...
    async passkeyLogin(email) {
        try {
            const challengeRes = await window.serviceClient.post(ServiceName.VSOD, ApiPaths.auth.passkey.authChallenge(), { email });
            const challengeData = await challengeRes.json();
            if (!challengeRes.ok || !challengeData.success) {
            ...
            const verifyRes = await window.serviceClient.post(ServiceName.VSOD, ApiPaths.auth.passkey.authVerify(), {
                email,
    ...
    async logout() {
        try {
            await window.serviceClient.post(ServiceName.VSOD, ApiPaths.auth.logout());
        } catch (error) {
    ...
    _renderPasskeyRegisterForm(card) {
        ...
        submitBtn.addEventListener('click', async () => {
            ...
            const response = await window.serviceClient.post(ServiceName.VSOD, ApiPaths.auth.register(), { name, email });
