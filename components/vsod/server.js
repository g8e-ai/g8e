...
/**
 * VSOD Node.js Server - Main Entry Point
 * 
 * VSO Dashboard (VSOD) - localhost platform with local authentication,
...
import { createVSODApp } from './app_factory.js';
...
import { VSODB_PUBSUB_PATH } from './constants/http_client.js';
...
class VSODServer {
    ...
    async initialize() {
        try {
            ...
            logger.error('Failed to initialize VSOD server:', error);
            process.exit(1);
        }
    }
    ...
    async start() {
        ...
        // Simple TLS check - look for certs in standard VSODB SSL location
        const sslDir = bootstrapService.getSslDir();
        let tlsOptions = null;
        ...
        if (sslDir) {
            ...
            if (fs.existsSync(certPath) && fs.existsSync(keyPath)) {
                ...
                logger.info('[VSOD] TLS certificates loaded', { certPath, keyPath });
            }
        }
        ...
        if (tlsOptions) {
            ...
            this.server.listen(httpsPort, () => {
                logger.info(`VSOD HTTPS server running on port ${httpsPort}`);
            });
            ...
            this.server = this.app.listen(internalPort, () => {
                logger.info(`VSOD server running on port ${internalPort} (HTTP - no TLS certs found)`);
            });
        }
        ...
        // Proxy WebSocket upgrade requests for /ws/pubsub to VSODB.
        // VSOD is the single external entry point — operators dial VSOD:443 for
        // both HTTP auth and WebSocket pub/sub, and VSOD tunnels the latter to VSODB.
        this.server.on('upgrade', (request, socket, head) => {
            const pathname = new URL(request.url, 'https://localhost').pathname;
            if (pathname === VSODB_PUBSUB_PATH) {
                this._proxyWebSocket(request, socket, head);
            } else {
                logger.warn('[VSOD] WebSocket upgrade rejected — unsupported path', { pathname });
                socket.destroy();
            }
        });
    }
    ...
    _createHttpServer(httpPort, httpsPort) {
        ...
        const sslDir = bootstrapService.getSslDir();
        if (!sslDir) {
            logger.warn('[VSOD] SSL directory not available, HTTP server disabled');
            return;
        }
        ...
        this.httpRedirectServer = http.createServer(async (req, res) => {
            try {
                ...
                logger.error('[VSOD] HTTP server error', { error: err.message });
        ...
        this.httpRedirectServer.listen(httpPort, () => {
            logger.info(`VSOD HTTP server running on port ${httpPort} (CA trust page)`);
        });
    }
    ...
    _proxyWebSocket(request, socket, head) {
        // Use default VSODB WebSocket URL - no config dependency
        const vsodbUrl = new URL('wss://vsodb:9001');
        const port = parseInt(vsodbUrl.port, 10) || 9001;
        const host = vsodbUrl.hostname;
        const isSecure = vsodbUrl.protocol === 'wss:';
        ...
        const upstream = connect(() => {
            ...
            // Inject internal auth token into headers for the upstream VSODB connection
        ...
        upstream.on('error', (err) => {
            logger.error('[VSOD] WebSocket proxy to VSODB failed', { error: err.message });
            socket.destroy();
        });
    ...
    async shutdown() {
        logger.info('Shutting down VSOD server...');

        if (this.httpRedirectServer) {
            this.httpRedirectServer.close(() => {
                logger.info('VSOD HTTP redirect server closed');
            });
        }

        if (this.internalServer) {
            this.internalServer.close(() => {
                logger.info('VSOD internal HTTP server closed');
            });
        }

        if (this.server) {
            this.server.close(() => {
                logger.info('VSOD server closed');
                process.exit(0);
            });
        }
    }
...
const server = new VSODServer();
server.start().catch((error) => {
    logger.error('Failed to start VSOD server:', error);
    process.exit(1);
});

export default VSODServer;
