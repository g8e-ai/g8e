...
// ---------------------------------------------------------------------------
// AuthAdminAuditEntry  (auth admin access audit record stored in VSODB)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// DownloadTokenData  (one-time download token payload stored in VSODB KV)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// DownloadAuditEntry  (operator binary download token audit record stored in VSODB)
// ---------------------------------------------------------------------------
...
export class LoginAttemptEntry extends VSOBaseModel {
    static fields = {
        timestamp:          { type: F.date,   default: () => now() },
        ip:                 { type: F.string, default: null },
        user_agent:         { type: F.string, default: null },
...
// ---------------------------------------------------------------------------
// AccountLockData  (persisted to KV + VSODB document store on account lockout)
// ---------------------------------------------------------------------------
...
export class FailedAttemptsData extends VSOIdentifiableModel {
    static fields = {
...
export class IpTrackEntry extends VSOBaseModel {
    static fields = {
...
// ---------------------------------------------------------------------------
// LoginAuditEntry  (login audit log record stored in VSODB)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// AuthAuditEntry  (operator auth audit log record stored in VSODB)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// ApiKeyDocument  (API key record stored in VSODB document store + VSODB KV)
// ---------------------------------------------------------------------------
...
export class SessionDocument extends VSOIdentifiableModel {
    static fields = {
        id:                   { type: F.string,  required: true },
    ...
    static parse(raw = {}) {
        ...
        return VSOIdentifiableModel.parse.call(SessionDocument, raw);
    }
}
...
export class WebSessionDocument extends SessionDocument {
    ...
    static _parse(raw = {}) {
        return VSOIdentifiableModel.parse.call(WebSessionDocument, raw);
    }
...
export class OperatorSessionDocument extends SessionDocument {
    ...
    static _parse(raw = {}) {
        return VSOIdentifiableModel.parse.call(OperatorSessionDocument, raw);
    }
...
export class DeviceLinkClaim extends VSOBaseModel {
    static fields = {
...
// ---------------------------------------------------------------------------
// DeviceLinkData  (device link record stored in VSODB KV)
// ---------------------------------------------------------------------------
