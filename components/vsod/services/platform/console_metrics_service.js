...
class ConsoleMetricsService {
    ...
    async getSystemHealth() {
        const health = {
            vsodb: { status: SystemHealth.UNKNOWN, latencyMs: null },
        ...
        try {
            ...
            health.vsodb = { status: SystemHealth.UNHEALTHY, error: error.message };
        }
        ...
        health.overall = (health.vsodb.status === SystemHealth.HEALTHY && health.db.status === SystemHealth.HEALTHY)
    ...
    async getComponentHealth() {
        ...
        try {
            ...
            components.vsodb_kv = { name: 'VSODB KV', status: SystemHealth.HEALTHY, latencyMs: Date.now() - kvStart };
        } catch (error) {
            components.vsodb_kv = { name: 'VSODB KV', status: SystemHealth.UNHEALTHY, latencyMs: null, error: error.message };
        }

        try {
            const dbStart = Date.now();
            await this._cache_aside.getDocument(Collections.PLATFORM_SETTINGS, 'platform_settings');
            components.vsodb_db = { name: 'VSODB DB', status: SystemHealth.HEALTHY, latencyMs: Date.now() - dbStart };
        } catch (error) {
            components.vsodb_db = { name: 'VSODB DB', status: SystemHealth.UNHEALTHY, latencyMs: null, error: error.message };
        }
