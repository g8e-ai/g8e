...
/**
 * User Service - VSOD Authoritative User Management
 *
 * VSOD owns all user/auth data. VSODB document store is the source of truth.
 * VSODB KV is a write-through read cache.
 *
...
class UserService {
    ...
    /**
     * Write-through: VSODB document store first (source of truth), then KV cache.
     ...
     * KV-first, falls back to VSODB document store with cache warm on miss.
     * @param {string} userId
    ...
    /**
     * Delete user by ID. Removes from VSODB document store and evicts KV cache.
     * @param {string} userId
     * @returns {Promise<boolean>}
     */
