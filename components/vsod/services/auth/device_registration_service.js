...
import { VSOHttpContext, BoundOperatorContext } from '../../models/request_models.js';
...
export class DeviceRegistrationService {
     ...
     *   vsoContext:    { web_session_id: string|null, user_id: string, organization_id: string|null },
     * }} params
     * @returns {Promise<{ success: boolean, operator_session_id?: string, operator_id?: string, system_info?: object, error?: string }>}
     */
    async registerDevice({ operator_id, deviceInfo, operator_type = OperatorType.SYSTEM, vsoContext }) {
        const { user_id, web_session_id } = vsoContext;
        ...
        await this._userService.updateUserOperator(user_id, operator_id, OperatorStatus.ACTIVE);

        const relayContext = VSOHttpContext.parse({
            web_session_id:  web_session_id || operator_session_id,
            user_id,
            organization_id: vsoContext.organization_id || null,
            bound_operators: [
        ...
        return { success: true, operator_session_id, operator_id, system_info };
    }
}
