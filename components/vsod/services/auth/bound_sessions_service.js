 ...
 *
 * Binding state is stored bidirectionally in VSODB KV (fast lookup path):
 *   sessionBindOperators(operatorSessionId) → webSessionId  (STRING)
 *   sessionWebBind(webSessionId)            → {operatorSessionId, ...}  (SET)
 *
 * A BoundSessionsDocument is persisted to VSODB document store (bound_sessions
...
export class BoundSessionsService {
     ...
     *
     * Creates or updates the BoundSessionsDocument in VSODB document store.
     *
     ...
     *
     * Updates the BoundSessionsDocument in VSODB document store.
     *
     * @param {string} operatorSessionId
     * @param {string} webSessionId
     * @param {string} operatorId - The durable operator ID
