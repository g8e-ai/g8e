...
/**
 * VSODBHttpClient — Purpose-built HTTP client for VSODB.
 * 
 * Shared base for VSODBDocumentClient and KVCacheClient.
 * Provides timeout, error logging, and auth header propagation
 * for all HTTP calls to VSODB (Operator --listen mode).
 * 
 * Architecture (from docs/architecture/storage-data-flows.md):
 *   VSOD -> VSODB uses HTTP ($G8E_INTERNAL_HTTP_URL) for KV, document store.
...
class VSODBHttpError extends Error {
    constructor(message, status) {
        super(message);
        this.name = 'VSODBHttpError';
        this.status = status;
    }
}

class VSODBHttpClient {
    /**
     * @param {object} config
     ...
     */
    constructor({ listenUrl, component = 'VSODB-HTTP', internalAuthToken = null, caCertPath = null } = {}) {
        if (!listenUrl) {
            throw new Error('VSODBHttpClient: listenUrl is required');
        }
    ...
    /**
     * Make an HTTP request to VSODB with timeout and structured error handling.
     *
    ...
    async request(method, path, options = {}) {
        ...
        const timeoutId = setTimeout(() => timeoutController.abort(), VSODB_HTTP_TIMEOUT_MS);
        ...
        try {
            ...
            try {
                ...
                throw new VSODBHttpError(`VSODB returned non-JSON response: ${text}`, res.status);
            }
            ...
            if (!res.ok) {
                ...
                throw new VSODBHttpError(errMsg || `HTTP ${res.status}`, res.status);
            }
            return data;
        } catch (error) {
            clearTimeout(timeoutId);

            if (error instanceof VSODBHttpError) {
                throw error;
            }

            if (error.name === 'AbortError') {
                logger.error(`[${this.component}] ${method} ${path} timeout after ${VSODB_HTTP_TIMEOUT_MS}ms`);
                throw new Error(`VSODB request timeout: ${method} ${path} after ${VSODB_HTTP_TIMEOUT_MS}ms`);
            }
    ...
    async put(path, body, options = {}) {
        if (typeof body !== 'string') {
            throw new Error(`VSODBHttpClient.put: body must be a pre-serialized JSON string, got ${typeof body}`);
        }
    ...
    async patch(path, body, options = {}) {
        if (typeof body !== 'string') {
            throw new Error(`VSODBHttpClient.patch: body must be a pre-serialized JSON string, got ${typeof body}`);
        }
    ...
    async post(path, body, options = {}) {
        if (typeof body !== 'string') {
            throw new Error(`VSODBHttpClient.post: body must be a pre-serialized JSON string, got ${typeof body}`);
        }
...
export { VSODBHttpClient, VSODBHttpError };
