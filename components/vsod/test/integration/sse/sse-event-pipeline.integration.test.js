 ...
 *
 * Proves the full wire path for every event type that VSE publishes:
 *
 *   VSEPassthroughEvent.forWire()
 ...
 *
 * This test catches:
 *   - Event type string mismatches between VSE constants and frontend constants
 ...
 *   VSEPassthroughEvent (server model) — forWire() produces the exact wire object
 ...
 *
 * Only mocked:
 *   @vsod/utils/logger.js — silence output
 *   SSEService dependencies (OperatorDataService, vsodConfig, etc.) — not exercised here
 */
...
/**
 * Simulate the full pipeline for a single VSEPassthroughEvent:
...
describe('SSE pipeline — LLM_CHAT_ITERATION_TEXT_CHUNK_RECEIVED [INTEGRATION - jsdom]', () => {
    it('emits on eventBus with content and session fields intact', async () => {
        ...
        const eventBus = await pipelineRun(new VSEPassthroughEvent({ _payload: payload }));
...
// ---------------------------------------------------------------------------
// Wire shape contract — VSEPassthroughEvent.forWire() shape assertion
// Proves the server-side model produces the exact shape the frontend expects
// ---------------------------------------------------------------------------

describe('SSE pipeline — VSEPassthroughEvent wire shape contract [INTEGRATION - jsdom]', () => {
    it('forWire() returns the inner _payload directly (no outer wrapper)', () => {
        ...
        const event = new VSEPassthroughEvent({ _payload: innerPayload });
    ...
    it('the SSE frame written to the stream is exactly "data: <json>\\n\\n"', async () => {
        ...
        await sseService.publishEvent(WEB_SESSION_ID, new VSEPassthroughEvent({ _payload: innerPayload }));
