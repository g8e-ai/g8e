...
import { SETTINGS_DOC_ID } from '@vsod/constants/service_config.js';
...
// Mock InternalHttpClient to verify the sync call to VSE
const mockVseRequest = vi.fn();
vi.mock('@vsod/services/clients/internal_http_client.js', () => ({
    getInternalHttpClient: vi.fn(() => ({
        request: mockVseRequest.mockResolvedValue({ success: true })
    }))
}));

describe('Settings -> VSE Sync Integration [CROSS-SERVICE]', () => {
    let cacheAside;
    let settingsService;
    ...
    it('syncs user settings to VSE when saved for a user', async () => {
        ...
        // Verify Sync to VSE
        expect(mockVseRequest).toHaveBeenCalledWith(
            'vse',
            apiPaths.vse.settingsUser(),
            expect.objectContaining({
                method: 'PATCH',
                body: updates
            })
    ...
    it('syncs to platform settings and NOT VSE when userId is null (setup context)', async () => {
        const updates = {
            g8e_internal_http_url: 'https://vsodb:9000',
            https_port: '443'
        };
        ...
        // Verify NO Sync to VSE (VSE loads platform settings directly from DB on boot/refresh)
        expect(mockVseRequest).not.toHaveBeenCalled();
    });
});
