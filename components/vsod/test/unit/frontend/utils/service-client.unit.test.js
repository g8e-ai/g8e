...
} from '@vsod/public/js/constants/service-client-constants.js';
...
beforeEach(async () => {
    ...
    ({ ServiceClient, RateLimitError } = await import('@vsod/public/js/utils/service-client.js'));
});
...
describe('window.serviceClient initialization [UNIT - jsdom]', () => {
    ...
    it('does not overwrite an existing window.serviceClient on re-evaluation', async () => {
        ...
        await import('@vsod/public/js/utils/service-client.js');
...
describe('ServiceClient.getServiceEndpoints [UNIT - jsdom]', () => {
    ...
    it('returns window.location.origin for VSOD', () => {
        const endpoints = client.getServiceEndpoints(ServiceName.VSOD);
        expect(endpoints).toEqual(['https://localhost']);
    });

    it('reflects window.location.origin change for VSOD', () => {
...
describe('ServiceClient.getAuthHeaders [UNIT - jsdom]', () => {
    ...
    it('sets Authorization, Cookie, and session ID header when accessor provides a session ID', () => {
        ...
        const headers = client.getAuthHeaders(ServiceName.VSOD);
...
describe('ServiceClient.sendRequest — URL and fetch [UNIT - jsdom]', () => {
    ...
    it('constructs URL from VSOD origin + path', async () => {
        await client.sendRequest(ServiceName.VSOD, '/api/test');
    ...
    it('passes credentials: include to fetch', async () => {
        await client.sendRequest(ServiceName.VSOD, '/api/test', { method: HttpMethod.GET });
    ...
    it('returns the fetch response on success', async () => {
        ...
        const result = await client.sendRequest(ServiceName.VSOD, '/api/test', { method: HttpMethod.GET });
...
describe('ServiceClient.sendRequest — timeout selection [UNIT - jsdom]', () => {
    ...
    async function assertTimeoutMs(path, method, expectedTimeoutMs) {
        ...
        const req = client.sendRequest(ServiceName.VSOD, path, { method });
    ...
    it('abort causes sendRequest to throw', async () => {
        vi.stubGlobal('fetch', makeHangingFetch());
        const req = client.sendRequest(ServiceName.VSOD, '/api/user/me', { method: HttpMethod.GET });
        await Promise.resolve();
...
describe('ServiceClient.sendRequest — error handling [UNIT - jsdom]', () => {
    ...
    it('throws RateLimitError on 429', async () => {
        ...
        await expect(
            client.sendRequest(ServiceName.VSOD, '/api/endpoint', { method: HttpMethod.GET })
    ...
    it('sets retryAfter from RATE_LIMIT_RESET_HEADER when present', async () => {
        ...
        let thrown;
        try {
            await client.sendRequest(ServiceName.VSOD, '/api/endpoint', { method: HttpMethod.GET });
        } catch (err) {
            thrown = err;
        }
    ...
    it('calls response.headers.get with RATE_LIMIT_RESET_HEADER constant', async () => {
        ...
        await client.sendRequest(ServiceName.VSOD, '/api/endpoint', { method: HttpMethod.GET }).catch(() => {});
...
describe('ServiceClient.get [UNIT - jsdom]', () => {
    ...
    it('issues a GET request', async () => {
        await client.get(ServiceName.VSOD, '/api/user/me');
        const opts = fetch.mock.calls[0][1];
        expect(opts.method).toBe(HttpMethod.GET);
    });

    it('passes extra options through to sendRequest', async () => {
        await client.get(ServiceName.VSOD, '/api/user/me', { headers: { 'X-Test': 'yes' } });
        const opts = fetch.mock.calls[0][1];
...
describe('ServiceClient.post [UNIT - jsdom]', () => {
    ...
    it('issues a POST request', async () => {
        await client.post(ServiceName.VSOD, '/api/chat/stream', { message: 'hello' });
        const opts = fetch.mock.calls[0][1];
...
describe('ServiceClient.put [UNIT - jsdom]', () => {
    ...
    it('issues a PUT request', async () => {
        await client.put(ServiceName.VSOD, '/api/settings', { theme: 'dark' });
        const opts = fetch.mock.calls[0][1];
    ...
    it('serializes data as JSON body', async () => {
        ...
        await client.put(ServiceName.VSOD, '/api/settings', data);
        const opts = fetch.mock.calls[0][1];
        expect(opts.body).toBe(JSON.stringify(data));
    });
    ...
    it('omits body when data is null', async () => {
        await client.put(ServiceName.VSOD, '/api/settings', null);
        const opts = fetch.mock.calls[0][1];
        expect(opts.body).toBeUndefined();
    });
});
...
describe('ServiceClient.delete [UNIT - jsdom]', () => {
    ...
    it('issues a DELETE request', async () => {
        await client.delete(ServiceName.VSOD, '/api/chat/cases/case_123');
        const opts = fetch.mock.calls[0][1];
...
describe('ServiceClient.upload [UNIT - jsdom]', () => {
    ...
    it('issues a POST request', async () => {
        ...
        await client.upload(ServiceName.VSOD, '/api/upload', formData);
        const opts = fetch.mock.calls[0][1];
        expect(opts.method).toBe(HttpMethod.POST);
    });
