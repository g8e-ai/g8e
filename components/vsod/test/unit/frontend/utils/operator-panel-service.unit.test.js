...
describe('OperatorPanelService [UNIT - jsdom]', () => {
    ...
    describe('dependency injection', () => {
        it('uses the injected client instead of window.serviceClient', async () => {
            ...
            injected.setResponse('vsod', '/api/operators/bind', makeOkResponse());
        ...
        it('falls back to window.serviceClient when no client is injected', async () => {
            ...
            windowClient.setResponse('vsod', '/api/operators/bind', makeOkResponse());
    ...
    describe('operator lifecycle', () => {
        ...
        describe('bindOperator', () => {
            it('POSTs to /api/operators/bind with operator_id', async () => {
                client.setResponse('vsod', '/api/operators/bind', makeOkResponse({ success: true, operator: {} }));

                const resp = await operatorPanelService.bindOperator('op-123');

                expect(resp.ok).toBe(true);
                const [req] = client.getRequestLog();
                expect(req).toMatchObject({ method: 'POST', service: 'vsod', path: '/api/operators/bind', body: { operator_id: 'op-123' } });
            });

            it('returns the raw Response so callers can inspect ok/status', async () => {
                client.setResponse('vsod', '/api/operators/bind', makeErrorResponse(403, { error: 'Slot limit reached' }));
        ...
        describe('unbindOperator', () => {
            it('POSTs to /api/operators/unbind with empty body by default', async () => {
                client.setResponse('vsod', '/api/operators/unbind', makeOkResponse());
        ...
        describe('bindAllOperators', () => {
            it('POSTs to /api/operators/bind-all with operator_ids array', async () => {
                client.setResponse('vsod', '/api/operators/bind-all', makeOkResponse());
        ...
        describe('unbindAllOperators', () => {
            it('POSTs to /api/operators/unbind-all with operator_ids array', async () => {
                client.setResponse('vsod', '/api/operators/unbind-all', makeOkResponse());
        ...
        describe('stopOperator', () => {
            it('POSTs to /api/operators/:id/stop', async () => {
                client.setResponse('vsod', '/api/operators/op-789/stop', makeOkResponse());
    ...
    describe('operator details & API keys', () => {
        ...
        describe('getOperatorDetails', () => {
            it('GETs /api/operators/:id/details', async () => {
                client.setResponse('vsod', '/api/operators/op-abc/details', makeOkResponse({ data: { operator_id: 'op-abc' } }));

                const resp = await operatorPanelService.getOperatorDetails('op-abc');

                expect(resp.ok).toBe(true);
                const [req] = client.getRequestLog();
                expect(req).toMatchObject({ method: 'GET', service: 'vsod', path: '/api/operators/op-abc/details' });
            });
        ...
        describe('getOperatorApiKey', () => {
            it('GETs /api/operators/:id/api-key', async () => {
                client.setResponse('vsod', '/api/operators/op-abc/api-key', makeOkResponse({ api_key: 'key-xyz' }));
        ...
        describe('refreshOperatorApiKey', () => {
            it('POSTs to /api/operators/:id/refresh-api-key', async () => {
                client.setResponse('vsod', '/api/operators/op-abc/refresh-api-key', makeOkResponse({ new_api_key: 'new-key', slot_number: 1 }));
    ...
    describe('device links', () => {
        ...
        describe('generateDeviceLink', () => {
            it('POSTs to /api/auth/link/generate with operator_id', async () => {
                client.setResponse('vsod', '/api/auth/link/generate', makeOkResponse({ token: 'tok-123', expires_at: '2026-01-01T00:00:00Z' }));
        ...
        describe('createDeviceLink', () => {
            it('POSTs to /api/device-links with max_uses, expires_in_hours, and name', async () => {
                client.setResponse('vsod', '/api/device-links', makeOkResponse({ success: true }));
            ...
            it('omits name from body when name is not provided', async () => {
                client.setResponse('vsod', '/api/device-links', makeOkResponse());
        ...
        describe('listDeviceLinks', () => {
            it('GETs /api/device-links', async () => {
                client.setResponse('vsod', '/api/device-links', makeOkResponse({ links: [] }));

                await operatorPanelService.listDeviceLinks();

                const [req] = client.getRequestLog();
                expect(req).toMatchObject({ method: 'GET', service: 'vsod', path: '/api/device-links' });
            });
        });

        describe('revokeDeviceLink', () => {
            it('DELETEs /api/device-links/:tokenId without query string', async () => {
                client.setResponse('vsod', '/api/device-links/token-abc', makeOkResponse());

                await operatorPanelService.revokeDeviceLink('token-abc');

                const [req] = client.getRequestLog();
                expect(req).toMatchObject({ method: 'DELETE', service: 'vsod', path: '/api/device-links/token-abc' });
            });
        ...
        describe('deleteDeviceLink', () => {
            it('DELETEs /api/device-links/:tokenId?action=delete', async () => {
                client.setResponse('vsod', '/api/device-links/token-abc?action=delete', makeOkResponse());

                await operatorPanelService.deleteDeviceLink('token-abc');

                const [req] = client.getRequestLog();
                expect(req).toMatchObject({ method: 'DELETE', service: 'vsod', path: '/api/device-links/token-abc?action=delete' });
            });
        });
    });
    ...
    describe('device authorization', () => {
        ...
        describe('authorizeDevice', () => {
            it('POSTs to /api/auth/link/:token/authorize', async () => {
                client.setResponse('vsod', '/api/auth/link/tok-xyz/authorize', makeOkResponse());

                await operatorPanelService.authorizeDevice('tok-xyz');

                const [req] = client.getRequestLog();
                expect(req).toMatchObject({ method: 'POST', service: 'vsod', path: '/api/auth/link/tok-xyz/authorize' });
            });

            it('returns error response for unknown token', async () => {
                client.setResponse('vsod', '/api/auth/link/bad-tok/authorize', makeErrorResponse(404, { error: 'Token not found' }));
        ...
        describe('rejectDevice', () => {
            it('POSTs to /api/auth/link/:token/reject', async () => {
                client.setResponse('vsod', '/api/auth/link/tok-xyz/reject', makeOkResponse());

                await operatorPanelService.rejectDevice('tok-xyz');

                const [req] = client.getRequestLog();
                expect(req).toMatchObject({ method: 'POST', service: 'vsod', path: '/api/auth/link/tok-xyz/reject' });
            });
        });
    });
});
