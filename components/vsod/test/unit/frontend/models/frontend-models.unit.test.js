...
import { FrontendBaseModel, FrontendIdentifiableModel, F } from '@vsod/public/js/models/base.js';
import {
...
} from '@vsod/public/js/models/investigation-models.js';
import { InvestigationStatus } from '@vsod/public/js/constants/investigation-constants.js';
...
describe('InvestigationModel [UNIT]', () => {
    ...
    it('history_trail entries are InvestigationHistoryEntry instances', () => {
        const m = InvestigationModel.parse({
            ...minimal,
            history_trail: [{ event_type: EventType.CASE_CREATED, actor: 'vsod', summary: 'Init' }],
        });
    ...
    it('addHistoryEntry() pushes a parsed entry and calls updateTimestamp', () => {
        ...
        m.addHistoryEntry('status_changed', 'vsod', 'Changed to closed');
    ...
    it('updateStatus() updates status and records history entry', () => {
        const m = InvestigationModel.parse(minimal);
        m.updateStatus('Closed', 'vsod', 'Closing');
