...
beforeEach(async () => {
    ...
    vi.doMock('@vsod/public/js/components/cases-manager.js', () => ({
        CasesManager: MockCasesManager,
    }));

    const mod = await import('@vsod/public/js/components/chat-auth.js');
    ChatAuthMixin = mod.ChatAuthMixin;

    const casesMod = await import('@vsod/public/js/components/cases-manager.js');
