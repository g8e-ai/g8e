...
import { SessionAuthListener } from '@vsod/services/auth/session_auth_listener.js';
import { PubSubChannel } from '@vsod/constants/channels.js';
import { ApiKeyError, SESSION_AUTH_LISTEN_TTL_MS } from '@vsod/constants/auth.js';
import { DEFAULT_OPERATOR_CONFIG } from '@vsod/constants/operator_defaults.js';
...
describe('SessionAuthListener', () => {
    ...
    const sessionHash = crypto.createHash('sha256').update(mockVsoContext.operator_session_id).digest('hex');
    ...
    describe('listen', () => {
        it('should set up subscription and timeout cleanup', async () => {
            listener.listen(mockVsoContext);
