...
describe('BootstrapService [UNIT - filesystem isolated]', () => {
    ...
    beforeEach(async () => {
        vi.clearAllMocks();
        const module = await import('@vsod/services/platform/bootstrap_service.js');
    ...
    describe('constructor', () => {
        it('should use default volume path /vsodb when not provided', () => {
            const service = new BootstrapService();
            expect(service.volumePath).toBe('/vsodb');
        });

        it('should use custom volume path when provided', () => {
            const customPath = '/custom/vsodb';
    ...
    describe('loadCaCertPath', () => {
        it('should find and cache CA cert at /vsodb/ca.crt', () => {
        ...
        it('should find and cache CA cert at /vsodb/ca/ca.crt (legacy path)', () => {
            const caDir = join(tmpVolumePath, 'ca');
            mkdirSync(caDir, { recursive: true });
            writeFileSync(join(caDir, 'ca.crt'), 'ca-cert-content');
        ...
        it('should prefer /vsodb/ca.crt over /vsodb/ca/ca.crt', () => {
    ...
    describe('getSslDir', () => {
        ...
        it('should return default /vsodb when no custom path set', () => {
            const service = new BootstrapService();

            expect(service.getSslDir()).toBe('/vsodb');
        });
    });
