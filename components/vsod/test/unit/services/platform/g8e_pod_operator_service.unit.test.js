...
import { G8ENodeOperatorService } from '@vsod/services/platform/g8e_pod_operator_service.js';
...
describe('G8ENodeOperatorService [UNIT]', () => {
    ...
    describe('launchG8ENodeOperator', () => {
        ...
        it('throws a specific error when savePlatformSettings fails', async () => {
            settingsService.savePlatformSettings.mockRejectedValue(new Error('VSODB write timeout'));

            await expect(service.launchG8ENodeOperator('sk-test-key'))
                .rejects.toThrow('Failed to persist operator API key to platform_settings: VSODB write timeout');
    ...
    describe('g8e-pod script validation', () => {
        ...
        function extractVsodbUrls(filePath) {
            ...
            const urls = [];
            let match;
            while ((match = vsodbUrlPattern.exec(content)) !== null) {
                urls.push({ url: match[0], port: match[1] || null, line: content.substring(0, match.index).split('\n').length });
            }
            return urls;
        }

        it('fetch-key-and-run.sh uses port 9000 for all VSODB URLs', () => {
            const urls = extractVsodbUrls(path.join(scriptsDir, 'fetch-key-and-run.sh'));
        ...
        it('fetch-key-and-run.sh downloads binary from blob store when not present locally', () => {
            const content = fs.readFileSync(path.join(scriptsDir, 'fetch-key-and-run.sh'), 'utf-8');
            expect(content).toMatch(/BLOB_URL="https:\/\/vsodb:9000\/blob\/operator-binary"/);
