...
import { 
    ...
    DatabaseError, 
    InternalServerError 
} from '@vsod/services/error_service.js';
import { ErrorCode, ErrorCategory, ErrorSeverity } from '@vsod/constants/errors.js';

describe('ErrorService', () => {
    describe('VSOError', () => {
        it('should initialize with default values', () => {
            const message = 'Test error message';
            const error = new VSOError(message);

            expect(error.message).toBe(message);
            expect(error.name).toBe('VSOError');
            expect(error.code).toBe(ErrorCode.GENERIC_ERROR);
            expect(error.category).toBe(ErrorCategory.INTERNAL);
            expect(error.severity).toBe(ErrorSeverity.MEDIUM);
            expect(error.source).toBe('vsod');
            expect(error.component).toBe('vsod');
        ...
        it('should initialize with provided options', () => {
            ...
            const error = new VSOError('Message', options);
        ...
        it('should handle cause option correctly', () => {
            ...
            const error = new VSOError('New error', { cause });
        ...
        it('should convert to standardized wire format via toVSOErrorResponse', () => {
            const options = {
            ...
            const error = new VSOError('Test message', options);
            const response = error.toVSOErrorResponse();
        ...
        it('should return correct HTTP status codes', () => {
            ...
            testCases.forEach(({ category, expected }) => {
                const error = new VSOError('msg', { category });
