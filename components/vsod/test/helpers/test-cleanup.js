 ...
 * 
 * Provides automatic cleanup for VSODB KV and VSODB document store resources
...
export class TestCleanupHelper {
    ...
    generateOperatorId(userId) {
        ...
        // Track all related VSODB KV keys
        this.trackVSODBKey(KVKey.doc(this.operatorsCollection, opId));
        this.trackVSODBKey(KVKey.operatorFirstDeployed(opId));
        this.trackVSODBKey(KVKey.operatorTrackedStatus(opId));
        this.trackVSODBKey(KVKey.userOperators(userId));
        
        // Track session keys with unified format
        this.trackVSODBKey(KVKey.operatorSessionKey(operatorSessionId));
        this.trackVSODBKey(KVKey.webSessionKey(webSessionId));
        this.trackVSODBKey(KVKey.userWebSessions(userId));
    ...
    /**
     * Track a VSODB KV key for cleanup
     */
    trackVSODBKey(key) {
        this.kvKeys.add(key);
    }
     ...
     * 
     * This will clean up:
     * - User document in VSODB document store
     * - All operators for this user (VSODB document store + VSODB KV)
     * - All API keys for this user (VSODB document store + VSODB KV)
     * - User's download API key (VSODB KV)
     * - WebSession audit logs for this user
     * - Operator usage documents for this user
     * - User cache and session tracking keys (VSODB KV)
    ...
    trackUser(userId, email = null) {
        // Track user sessions VSODB KV key
        this.trackVSODBKey(KVKey.userWebSessions(userId));
        
        // Track user operators VSODB KV key
        this.trackVSODBKey(KVKey.userOperators(userId));
        
        // Track user doc VSODB KV key
        this.trackVSODBKey(KVKey.doc(this.usersCollection, userId));
    ...
    trackWebSession(sessionId, userId = null) {
        this.trackVSODBKey(KVKey.webSessionKey(sessionId));
        if (userId) {
            this.trackVSODBKey(KVKey.userWebSessions(userId));
        }
    ...
    trackOperatorSession(sessionId, userId = null) {
        this.trackVSODBKey(KVKey.operatorSessionKey(sessionId));
        if (userId) {
            this.trackVSODBKey(KVKey.userOperators(userId));
        }
    ...
    trackLoginAudit(identifier) {
        ...
        this.trackVSODBKey(KVKey.loginFailed(identifier));
        this.trackVSODBKey(KVKey.loginLock(identifier));
    }
    ...
    /**
     * Track multiple VSODB KV keys matching a pattern
     */
    ...
    /**
     * Check if VSODB KVCacheClient connection is available for operations
     */
    _isKVAvailable() {
    ...
    /**
     * Safely delete VSODB KV key(s), ignoring connection closed errors
     */
    ...
    /**
     * Scan VSODB KV keys matching a pattern
     */
