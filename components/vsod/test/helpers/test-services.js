 ...
 * 3. Use real infrastructure (VSODB) for all tests.
 */

import { logger } from '../../utils/logger.js';

const GLOBAL_KEY = '__VSOD_TEST_SERVICES__';
...
export async function initializeTestServices() {
    ...
    state.initPromise = (async () => {
        try {
            ...
            // This sets up real VSODB clients, cache-aside, settings, and all services.
            ...
            const services = {
                ...
                operatorAuthService:    initModule.getOperatorAuthService(),
                ...
                sessionAuthListener:    initModule.getSessionAuthListener(),
                healthCheckService:     initModule.getHealthCheckService(),
                blobClient:             initModule.getVSODBBlobClient(),
                initModule,
            };
...
/**
 * Get the shared VSODBPubSubClient.
 */
export async function getTestVSODBPubSubClient() {
    const state = getGlobalState();
