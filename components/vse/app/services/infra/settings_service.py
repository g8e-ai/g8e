...
class SettingsServiceProtocol(Protocol):
    ...
    async def get_platform_settings(self) -> VSEPlatformSettings:
        """Retrieve platform-level settings from VSODB with cache-aside."""
        ...

    async def get_user_settings(self, user_id: str) -> VSEUserSettings:
        """Retrieve settings for a specific user, overlaid on platform settings."""
        ...

    def get_local_settings(self) -> VSEPlatformSettings:
...
class SettingsService:
    """Service for managing VSE settings with bootstrap loading and cache-aside logic."""
    ...
    def get_local_settings(self) -> VSEPlatformSettings:
        ...
        """
        settings = VSEPlatformSettings(port=443) # Default port
    ...
    def overlay_platform_data(self, settings: VSEPlatformSettings, data: PlatformSettingsData) -> VSEPlatformSettings:
        """Overlay PlatformSettingsData onto a VSEPlatformSettings instance."""
    ...
    async def get_platform_settings(self) -> VSEPlatformSettings:
        ...
        if not doc_dict:
            raise ConfigurationError(
                "VSE cannot start: platform_settings document missing in VSODB",
    ...
    async def get_user_settings(self, user_id: str) -> VSEUserSettings:
        ...
        if not user_doc_dict:
            ...
            # User request said explicit VSEUserSettings, so we must return one.
