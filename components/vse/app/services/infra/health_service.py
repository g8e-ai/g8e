...
class HealthService:
    """Service for checking the health of VSE and its dependencies."""

    @staticmethod
    async def check_dependencies(request_context: Request) -> HealthCheckResult:
        """
        Check the health of all registered VSE dependencies.
        ...
        await _check("settings", get_vse_platform_settings(request_context))
        dependencies["llm_provider"] = DependencyStatus(status=HealthStatus.HEALTHY)
        await _check("cache_aside_service", get_vse_cache_aside_service(request_context))
        await _check("investigation_data_service", get_vse_investigation_data_service(request_context))
        await _check("investigation_service", get_vse_investigation_service(request_context))
        await _check("memory_service", get_vse_memory_service(request_context))
        await _check("chat_pipeline", get_vse_chat_pipeline(request_context))
        await _check("attachment_service", get_vse_attachment_service(request_context))
        ...
        logger.info("VSE dependency health check completed: %s", overall_status)
