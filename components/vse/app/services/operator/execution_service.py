...
class OperatorExecutionService(ExecutionServiceProtocol):
    ...
    def resolve_target_operator(
        ...
        if not operator_documents:
            raise BusinessLogicError("No operators bound to this session", component="vse")
    ...
    def resolve_multiple_operators(
        ...
        if not target_operators:
            raise ValidationError("target_operators list is empty", component="vse")
    ...
    async def dispatch_command(
        ...
        if not operator_id or not operator_session_id:
            raise ValidationError("operator_id and operator_session_id are required", component="vse")
    ...
    async def send_command_to_operator(
        ...
        if not vso_context.bound_operators:
            raise ValidationError("No bound operators", component="vse")
        bound = vso_context.bound_operators[0]
        if not bound.operator_session_id:
            raise ValidationError("Operator not bound", component="vse")
