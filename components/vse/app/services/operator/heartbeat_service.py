...
class OperatorHeartbeatService:
    ...
    def set_pubsub_client(self, client: PubSubClient) -> None:
        if not client:
            raise ConfigurationError("client is required for heartbeat pub/sub", component="vse")
    ...
    async def start(self) -> None:
        ...
        if not self._pubsub_client:
            raise ConfigurationError("pubsub_client must be set before calling start()", component="vse")
    ...
    async def register_operator_session(self, operator_id: str, operator_session_id: str) -> None:
        ...
        if key in self._active_sessions:
            return
        if not self._pubsub_client:
            raise ConfigurationError("pubsub_client is not set", component="vse")
