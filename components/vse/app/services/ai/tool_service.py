...
class AIToolService:
    ...
    async def execute_tool_call(
        ...
        vso_context: VSOHttpContext,
        request_settings: VSEUserSettings,
    ) -> ToolResult:
        ...
        try:
            ...
            raise ExternalServiceError(f"Tool execution failed for {tool_name}: {e}", service_name=tool_name, component=ComponentName.VSE) from e
    ...
    async def execute_command(
        ...
        vso_context: VSOHttpContext,
        investigation: EnrichedInvestigationContext,
        request_settings: VSEUserSettings | None = None,
    ) -> CommandExecutionResult:
    ...
    async def _execute_intent_permission_request(
        ...
        *,
        ...
        if vso_context is None:
            raise ValidationError("vso_context is required for execute_intent_permission_request", component=ComponentName.VSE)
    ...
    async def _execute_intent_revocation(
        ...
        *,
        ...
        if vso_context is None:
            raise ValidationError("vso_context is required for execute_intent_revocation", component=ComponentName.VSE)
