...
class OperatorCommandValidator:
    ...
    async def validate_command_execution(
        ...
        if session_token and self.session_service:
            ...
            raise AuthenticationError("WebSession token required for command execution", component="vse")
    ...
    async def _validate_session(
        ...
        if operator.session_expires_at:
            if now() > operator.session_expires_at:
                ...
                raise AuthenticationError("WebSession expired - Operator must re-bootstrap", component="vse")
        ...
        if operator.session_token and operator.session_token != session_token:
            ...
            raise AuthenticationError("Invalid session token", component="vse")
        ...
        if self.session_service:
            try:
                ...
                raise AuthenticationError(f"WebSession validation failed: {e!s}", component="vse")
    ...
    async def validate_operator_ownership(
        ...
        if operator.user_id != user_id:
            ...
            raise AuthorizationError("Not authorized to control this operator", component="vse")
