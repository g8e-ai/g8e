...
"""VSE FastAPI Application - Main Entry Point.

Virtual Service Engineer (VSE) - AI engine for g8e platform. Agentic AI system with
...
Bootstrap responsibilities (this file):
    ...
    2. Raw VSODB client connections (5 core clients: DB, KV, PubSub, Blob, HTTP)
    ...
    5. Platform settings load from VSODB
...
from .constants import (
    ...
    CORS_ALLOWED_ORIGIN_VSE,
    CORS_ALLOWED_ORIGIN_VSOD_HTTP,
    CORS_ALLOWED_ORIGIN_VSOD_HTTPS,
    HTTP_ACCEPT_HEADER,
    ...
    VSE_APP_CONTACT_EMAIL,
    VSE_APP_CONTACT_NAME,
    VSE_APP_CONTACT_URL,
    VSE_APP_DESCRIPTION,
    VSE_APP_LICENSE_NAME,
    VSE_APP_LICENSE_URL,
    VSE_APP_TITLE,
    ComponentName,
    VSOHeaders,
...
from .utils.service_init import initialize_vso_service
...
async def _connect_clients(settings):
    """Create and connect the 5 core VSODB transport clients.
    ...
    kv_cache_client = KVCacheClient(
        component_name=ComponentName.VSE, ca_cert_path=ca, internal_auth_token=token,
    )
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        settings = await initialize_vso_service(
            "vse", settings=initial_settings,
        ...
        # -- Phase 1: Core VSODB clients (db, kv, pubsub, blob) --
        (
            app.state.db_client,
        ...
        logger.info("VSODB transport clients connected (db, kv, pubsub, blob)")
        ...
        logger.info("VSE startup completed successfully")

        yield

    except Exception as exc:
        logger.critical("VSE startup failed: %s", exc)
        raise

    finally:
        logger.info("=== VSE SHUTDOWN INITIATED ===")
        ...
        await _close_client(
            getattr(app.state, "internal_http_client", None), "VSOD HTTP client",
        )
        ...
        if db_service is not None:
            try:
                ...
                logger.info("VSODB document service closed")
            except Exception as exc:
                logger.error("Error closing VSODB document service: %s", exc)

        logger.info("VSE shutdown complete")
...
def _build_app() -> FastAPI:
    ...
    application = FastAPI(
        title=VSE_APP_TITLE,
        description=VSE_APP_DESCRIPTION,
        version=get_version(),
        lifespan=lifespan,
        openapi_tags=[
        ...
        contact={
            "name": VSE_APP_CONTACT_NAME,
            "url": VSE_APP_CONTACT_URL,
            "email": VSE_APP_CONTACT_EMAIL,
        },
        license_info={
            "name": VSE_APP_LICENSE_NAME,
            "url": VSE_APP_LICENSE_URL,
        },
    )
    ...
    application.add_middleware(
        ...
        allow_headers=[
            ...
            HTTP_REQUESTED_WITH_HEADER,
            VSOHeaders.SERVICE, VSOHeaders.CLIENT, VSOHeaders.WEB_SESSION_ID,
            VSOHeaders.USER_ID, VSOHeaders.ORGANIZATION_ID, VSOHeaders.CASE_ID,
            VSOHeaders.INVESTIGATION_ID, VSOHeaders.TASK_ID,
            VSOHeaders.BOUND_OPERATORS, VSOHeaders.OPERATOR_STATUS,
            VSOHeaders.EXECUTION_ID, VSOHeaders.SOURCE_COMPONENT,
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("components.vse.app.main:app", host="0.0.0.0", port=443, reload=True)
