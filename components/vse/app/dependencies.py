...
def _verify_internal_auth_token(request: Request, settings: VSEPlatformSettings) -> bool:
...
async def get_vse_settings_service(request: Request) -> SettingsServiceProtocol:
...
async def get_vse_platform_settings(request: Request) -> VSEPlatformSettings:
    if not hasattr(request.app.state, "settings"):
        logger.error("Settings not found in app state - VSE initialization may have failed")
        raise ConfigurationError("Settings not available")

    return request.app.state.settings


async def get_vse_pubsub_client(request: Request) -> PubSubClient:
    client = getattr(request.app.state, "pubsub_client", None)
    if not client:
        logger.error("PubSubClient not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("PubSubClient not available")

    return client


async def get_vse_kv_cache_client(request: Request) -> KVCacheClient:
    client = getattr(request.app.state, "kv_cache_client", None)
    if not client:
        logger.error("KVCacheClient not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("KVCacheClient not available")

    return client


async def get_vse_blob_client(request: Request) -> BlobClient:
    client = getattr(request.app.state, "blob_client", None)
    if not client:
        logger.error("BlobClient not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("BlobClient not available")

    return client


async def get_vse_cache_aside_service(request: Request) -> CacheAsideService:
    service = getattr(request.app.state, "cache_aside_service", None)
    if not service:
        logger.error("Cache service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Cache service not available")

    return service


async def get_vse_case_data_service(request: Request) -> CaseDataService:
    service = getattr(request.app.state, "case_data_service", None)
    if not service:
        logger.error("Case Data Service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Case Data Service not available")
    return service


async def get_vse_investigation_data_service(request: Request) -> InvestigationDataService:
    service = getattr(request.app.state, "investigation_data_service", None)
    if not service:
        logger.error("Investigation Data Service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Investigation Data Service not available")

    return service


async def get_vse_investigation_service(request: Request) -> InvestigationService:
...
async def get_vse_execution_registry(request: Request) -> ExecutionRegistryProtocol:
...
async def get_vse_memory_service(request: Request) -> MemoryDataService:
    service = getattr(request.app.state, "memory_service", None)
    if not service:
        logger.error("Memory Data service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Memory service not available")
    return service


async def get_vse_memory_generation_service(request: Request) -> MemoryGenerationService:
    service = getattr(request.app.state, "memory_generation_service", None)
    if not service:
        logger.error("Memory generation service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Memory generation service not available")
    return service


async def get_vse_chat_pipeline(request: Request) -> ChatPipelineService:
    service = getattr(request.app.state, "chat_pipeline", None)
    if not service:
        logger.error("Chat Pipeline not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Chat Pipeline not available", component=ComponentName.VSE)

    return service


async def get_vse_investigation_domain_service(request: Request) -> InvestigationService:
    return await get_vse_investigation_service(request)


async def get_vse_grounding_service(request: Request) -> GroundingService:
    service = getattr(request.app.state, "grounding_service", None)
    if not service:
        logger.error("Grounding service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Grounding service not available", component=ComponentName.VSE)

    return service


async def get_vseweb_search_provider(request: Request) -> WebSearchProvider | None:
    return getattr(request.app.state, "web_search_provider", None)


async def get_vse_chat_task_manager(request: Request) -> ChatTaskManager:
    service = getattr(request.app.state, "chat_task_manager", None)
    if not service:
        logger.error("Chat Task Manager not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Chat Task Manager not available", component=ComponentName.VSE)

    return service


async def get_vse_operator_cache(request: Request) -> CacheAsideService:
    service = getattr(request.app.state, "operator_cache_aside_service", None)
    if not service:
        logger.error("Operator cache service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Operator cache service not available")

    return service


async def get_vse_approval_service(request: Request) -> OperatorApprovalService:
    service = getattr(request.app.state, "approval_service", None)
    if not service:
        logger.error("Operator Approval Service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Operator Approval Service not available")
    return service


async def get_vse_operator_command_service(request: Request) -> OperatorCommandService:
    service = getattr(request.app.state, "operator_command_service", None)
    if not service:
        logger.error("Operator Command Service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Operator Command Service not available")

    return service


async def get_vse_heartbeat_service(request: Request) -> OperatorHeartbeatService:
    service = getattr(request.app.state, "heartbeat_service", None)
    if not service:
        logger.error("Operator Heartbeat Service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Operator Heartbeat Service not available")
    return service


async def get_vse_blob_service(request: Request) -> BlobService:
    service = getattr(request.app.state, "blob_service", None)
    if not service:
        logger.error("Blob Service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Blob Service not available")
    return service


async def get_vse_mcp_gateway_service(request: Request) -> MCPGatewayService:
    service = getattr(request.app.state, "mcp_gateway_service", None)
    if not service:
        logger.error("MCP Gateway Service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("MCP Gateway Service not available")
    return service


async def get_vse_attachment_service(request: Request) -> AttachmentService:
    service = getattr(request.app.state, "attachment_service", None)
    if not service:
        logger.error("Attachment Service not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("Attachment Service not available")

    return service


async def get_vse_vsod_http_client(request: Request) -> InternalHttpClient:
    client = getattr(request.app.state, "internal_http_client", None)
    if not client:
        logger.error("VSOD HTTP client not found in app state - VSE initialization may have failed")
        raise ServiceUnavailableError("VSOD HTTP client not available")

    return client


async def get_vse_event_service(request: Request) -> EventService:
    service = getattr(request.app.state, "vsod_event_service", None)
    if not service:
        logger.error("VSOD event service not found in app state - VSE initialization may have failed")
...
async def get_vse_user_settings(
    request: Request,
    settings_service: SettingsServiceProtocol = Depends(get_vse_settings_service),
) -> VSEUserSettings:
    """Load per-request VSEUserSettings following Platform Settings < User Settings.
    ...
    if not user_id:
        # We need to return VSEUserSettings, so we'll get it via the service 
...
async def get_vse_current_active_user(request: Request) -> AuthenticatedUser:
...
async def require_proxy_auth(
    request: Request,
    settings: VSEPlatformSettings = Depends(get_vse_platform_settings)
) -> AuthenticatedUser:
...
__all__ = [
    ...
    "get_vse_grounding_service",
    "get_vseweb_search_provider",
    "get_vse_mcp_gateway_service",
]
