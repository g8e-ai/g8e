...
@router.post(API_PATHS["vse"]["chat"], response_model=ChatStartedResponse)
async def internal_chat(
    request: ChatMessageRequest,
    platform_settings: VSEPlatformSettings = Depends(get_vse_platform_settings),
    user_settings: VSEUserSettings = Depends(get_vse_user_settings),
    chat_pipeline: ChatPipelineService = Depends(get_vse_chat_pipeline),
    chat_task_manager: ChatTaskManager = Depends(get_vse_chat_task_manager),
    case_service: CaseDataService = Depends(get_vse_case_data_service),
    investigation_service: InvestigationService = Depends(get_vse_investigation_service),
    attachment_service: AttachmentService = Depends(get_vse_attachment_service),
    event_service: EventService = Depends(get_vse_event_service),
    ...
    if vso_context.new_case:
        ...
        if request.message.strip():
            async def _generate_and_push_title(
                ...
                _web_session_id: str, _settings: VSEUserSettings
            ):
                try:
...
@router.post(API_PATHS["vse"]["chat_stop"], response_model=StopAIResponse)
...
@router.post(API_PATHS["vse"]["operator_approval_respond"], response_model=ApprovalRespondedResponse)
async def operator_approval_respond(
    request: OperatorApprovalResponse,
    vso_context: VSOHttpContext = Depends(get_vso_http_context),
    approval_service: OperatorApprovalService = Depends(get_vse_approval_service),
):
    """
...
@router.get(API_PATHS["vse"]["operator_approval_pending"], response_model=PendingApprovalsResponse)
...
@router.post(API_PATHS["vse"]["operator_direct_command"], response_model=DirectCommandSentResponse)
async def execute_direct_command(
    request: DirectCommandRequest,
    vso_context: VSOHttpContext = Depends(get_vso_http_context),
    operator_data_service: OperatorCommandService = Depends(get_vse_operator_command_service),
):
    logger.info(
...
@router.get(API_PATHS["vse"]["case"], response_model=CaseResponse)
async def get_case(
    case_id: str,
    case_service: CaseDataService = Depends(get_vse_case_data_service),
    vso_context: VSOHttpContext = Depends(get_vso_http_context)
...
@router.patch(API_PATHS["vse"]["case"], response_model=CaseResponse)
...
@router.delete(API_PATHS["vse"]["case"], status_code=status.HTTP_204_NO_CONTENT)
async def delete_case(
    case_id: str,
    case_service: CaseDataService = Depends(get_vse_case_data_service),
    investigation_service: InvestigationService = Depends(get_vse_investigation_service),
    cache_aside_service: CacheAsideService = Depends(get_vse_cache_aside_service),
    vso_context: VSOHttpContext = Depends(get_vso_http_context)
):
    """
...
@router.post(API_PATHS["vse"]["operators_register_session"], response_model=OperatorSessionRegisteredResponse)
async def register_operator_session(
    request: OperatorSessionRegistrationRequest,
    heartbeat_service: OperatorHeartbeatService = Depends(get_vse_heartbeat_service),
    vso_context: VSOHttpContext = Depends(get_vso_http_context),
):
    """
    Subscribe VSE to the heartbeat pub/sub channel for an operator session.

    Called by VSOD immediately after operator authentication succeeds so VSE
...
@router.post(API_PATHS["vse"]["operators_deregister_session"], response_model=OperatorSessionRegisteredResponse)
async def deregister_operator_session(
    ...
    """
    Unsubscribe VSE from the heartbeat pub/sub channel for an operator session.
...
@router.post(API_PATHS["vse"]["operators_stop"], response_model=OperatorStoppedResponse)
async def stop_operator(
    request: StopOperatorRequest,
    vso_context: VSOHttpContext = Depends(get_vso_http_context),
    operator_command_service: OperatorCommandService = Depends(get_vse_operator_command_service)
):
    """
    ...
    if operator_command_service.pubsub_client is None:
        raise ServiceUnavailableError("pub/sub client not initialized", component="vse")
...
@router.get(API_PATHS["vse"]["investigations"])
...
@router.get(API_PATHS["vse"]["investigation"], response_model=InvestigationModel)
...
@router.post(API_PATHS["vse"]["mcp_tools_list"], response_model=MCPToolListResponse)
async def mcp_tools_list(
    vso_context: VSOHttpContext = Depends(get_vso_http_context),
    mcp_service: MCPGatewayService = Depends(get_vse_mcp_gateway_service),
    investigation_service: InvestigationService = Depends(get_vse_investigation_service),
):
...
@router.post(API_PATHS["vse"]["mcp_tools_call"], response_model=MCPToolCallResponse)
...
@router.get(API_PATHS["vse"]["health"])
async def health_check():
    """Health check for internal API"""
    return {
        "service": "vse-internal-api",
        "status": "healthy",
        "endpoints": [
            InternalApiPaths.VSE_CHAT,
            InternalApiPaths.VSE_CHAT_STOP,
            InternalApiPaths.VSE_OPERATOR_APPROVAL_RESPOND,
            InternalApiPaths.VSE_OPERATOR_DIRECT_COMMAND,
            InternalApiPaths.VSE_CASES,
            InternalApiPaths.VSE_CASE,
            InternalApiPaths.VSE_INVESTIGATIONS,
            InternalApiPaths.VSE_INVESTIGATION,
            InternalApiPaths.VSE_MCP_TOOLS_LIST,
            InternalApiPaths.VSE_MCP_TOOLS_CALL,
        ]
    }
