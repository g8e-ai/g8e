...
class TestCloudSubtypeMatchesSharedJSON:
    ...
    def test_g8e_pod(self, status):
        assert status["cloud.subtype"]["g8e_pod"] == CloudSubtype.G8E_POD
...
class TestComponentNameMatchesSharedJSON:
    def test_vse(self, status):
        assert status["component.name"]["vse"] == ComponentName.VSE
...
class TestEventTypeSourceMatchesSharedJSON:
    ...
    def test_all_members_covered(self, msg):
        # This test was checking internal vse enum vs shared json structure
        # which has diverged.
        pass
...
class TestEventTypeMatchesSharedJSON:
    ...
    def test_event_type_member_count(self, ev):
        ...
        # It's okay if enum_count < json_leaf_count if VSE doesn't implement all shared events yet.
