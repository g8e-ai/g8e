...
class TestRequestTrace:
    ...
    def test_from_headers_generates_request_id_when_absent(self):
        trace = RequestTrace.from_headers({}, component_id=ComponentName.VSE)
    ...
    def test_from_headers_preserves_existing_request_id(self):
        trace = RequestTrace.from_headers(
            {VSOHeaders.EXECUTION_ID: "exec-existing-123"}, component_id=ComponentName.VSE
        )
    ...
    def test_as_headers_contains_request_id(self):
        trace = RequestTrace.from_headers(
            {VSOHeaders.EXECUTION_ID: "req-abc"}, component_id=ComponentName.VSE
        )
    ...
    def test_raises_validation_error_when_base_url_absent(self):
        with pytest.raises(ValidationError):
            get_service_client(
                ...
                source_service=ComponentName.VSE,
                base_url="",
                timeout=DEFAULT_TIMEOUT,
                auth_token="",
            )
