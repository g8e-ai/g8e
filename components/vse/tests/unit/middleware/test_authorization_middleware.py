...
class TestAuthorizationMiddleware:
    ...
    async def test_user_id_mismatch_rejected(self, middleware, mock_request, mock_call_next):
        ...
        mock_request.state.vso_context = build_vso_http_context(
    ...
    async def test_extract_vso_context_present(self, middleware, mock_request):
        vso_context = build_vso_http_context(
            web_session_id="session-123",
            user_id="user-456",
            organization_id="org-789",
        )
        mock_request.state.vso_context = vso_context
        assert middleware._extract_vso_context(mock_request) == vso_context

    async def test_extract_vso_context_missing(self, middleware, mock_request):
        delattr(mock_request.state, "vso_context")
        assert middleware._extract_vso_context(mock_request) is None

    async def test_request_without_vso_context_allowed(self, middleware, mock_request, mock_call_next):
        mock_request.url.path = "/investigations"
        mock_request.query_params = {}
        delattr(mock_request.state, "vso_context")
    ...
    async def test_vso_context_present_but_non_scoped_path_skips_checks(self, middleware, mock_request, mock_call_next):
