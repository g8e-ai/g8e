...
Verifies that VSA Go structs in models/ exactly match the canonical field names
defined in shared/models/wire/*.json.

VSA duplicates shared JSON field names as struct json tags (no code generation).
...
func init() {
	dir, err := filepath.Abs(filepath.Join(vsaRoot, "../../shared/models/wire"))
	if err != nil {
...
func TestVSOMessageMatchesEnvelopeSchema(t *testing.T) {
	...
	// All required fields from envelope.json that VSA populates
