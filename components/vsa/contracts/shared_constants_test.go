...
Verifies that VSA Go constants in constants/events.go and constants/status.go
exactly match the canonical values in shared/constants/*.json.

VSA duplicates shared JSON values as compile-time Go constants (no //go:embed,
...
func init() {
	// shared/constants/ is two levels up from vsaRoot: vsa -> components -> repo root
	dir, err := filepath.Abs(filepath.Join(vsaRoot, "../../shared/constants"))
	if err != nil {
...
func TestSharedStatusMatchesGoConstants(t *testing.T) {
	...
	t.Run("component.name", func(t *testing.T) {
		assert.Equal(t, st.ComponentName.VSE, constants.Status.ComponentName.VSE)
		assert.Equal(t, st.ComponentName.VSA, constants.Status.ComponentName.VSA)
