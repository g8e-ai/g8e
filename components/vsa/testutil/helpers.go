...
func NewTestConfig(t *testing.T) *config.Config {
	...
	return &config.Config{
		...
		ServiceName:        constants.Status.ComponentName.VSA,
		Version:            "test",
		APIKey:             "test-api-key",
...
// GetTestVSODBDirectURL returns the VSOD WebSocket gateway base URL for VSA pub/sub tests.
// VSA connects to pub/sub via VSOD (the single external entry point) at port 443; VSOD
