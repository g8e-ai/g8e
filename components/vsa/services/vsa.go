...
import (
	...
	"github.com/g8e-ai/g8e/components/vsa/services/execution"
	"github.com/g8e-ai/g8e/components/vsa/services/pubsub"
	"github.com/g8e-ai/g8e/components/vsa/services/sentinel"
	"github.com/g8e-ai/g8e/components/vsa/services/storage"
...
// NewVSAService creates a new Operator service in Outbound Mode.
...
func NewVSAService(cfg *config.Config, logger *slog.Logger) (*VSAService, error) {
	service := &VSAService{
		config:    cfg,
		logger:    logger,
...
func (vs *VSAService) SetPubSubClient(client pubsub.PubSubClient) {
	vs.mu.Lock()
	defer vs.mu.Unlock()
	vs.pubSubClient = client
}

func (vs *VSAService) Start(ctx context.Context) error {
	vs.mu.Lock()
	defer vs.mu.Unlock()
	...
	if vs.pubSubClient == nil {
		vs.pubSubClient, err = pubsub.NewVSODBPubSubClient(vs.config.PubSubURL, vs.config.TLSServerName, vs.logger)
		if err != nil {
			return fmt.Errorf("failed to create VSODB pub/sub client: %w", err)
		}
	}
	...
	go func() {
		...
		case reason := <-vs.pubSubCommands.ShutdownChan:
			vs.logger.Info("VSA Service received external shutdown request", "reason", reason)
...
// Stop gracefully shuts down all VSA sub-services.
func (vs *VSAService) Stop(ctx context.Context) error {
	vs.mu.Lock()
	defer vs.mu.Unlock()
