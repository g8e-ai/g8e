...
func TestVSAService_Start_SuccessFlow(t *testing.T) {
	// 1. Setup mock VSOD server for bootstrap
	...
	// 2. Configure VSA to point to the mock server
	...
	service, err := NewVSAService(cfg, testutil.NewVerboseTestLogger(t))
	...
	mockPubSub := pubsub.NewMockVSODBPubSubClient()
	...
	case err := <-errCh:
		require.NoError(t, err)
	case <-ctx.Done():
		t.Fatal("Timed out waiting for VSAService to start")
	}
