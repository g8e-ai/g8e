...
// Loopback tests verify the full VSA pub/sub round-trip using an in-process
...
//
// This covers the dual role VSA plays:
//   - Normal mode: subscriber on cmd:{op}:{sess}, publisher on results/heartbeat channels
//   - Listen mode: VSA IS the broker (PubSubBroker) — other components connect to it
//
...
// This is exactly what VSA does in listen mode — it connects to the broker
