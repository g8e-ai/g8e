...
// MockVSODBPubSubClient is a test double for VSODBPubSubClient.
// It records published messages and allows tests to inject incoming messages.
type MockVSODBPubSubClient struct {
	mu          sync.Mutex
...
// NewMockVSODBPubSubClient creates a new mock client
func NewMockVSODBPubSubClient() *MockVSODBPubSubClient {
	return &MockVSODBPubSubClient{
		subscribers: make(map[string][]chan []byte),
	}
}

// Subscribe returns a channel that receives messages injected via InjectMessage
func (m *MockVSODBPubSubClient) Subscribe(_ context.Context, channel string) (<-chan []byte, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockVSODBPubSubClient) Publish(_ context.Context, channel string, data []byte) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockVSODBPubSubClient) Close() {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.closed = true
...
func (m *MockVSODBPubSubClient) InjectMessage(channel string, data []byte) {
	m.mu.Lock()
	defer m.mu.Unlock()
	for _, ch := range m.subscribers[channel] {
		select {
...
func (m *MockVSODBPubSubClient) Published() []MockPublishedMsg {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockVSODBPubSubClient) PublishedCount() int {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockVSODBPubSubClient) LastPublished() *MockPublishedMsg {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockVSODBPubSubClient) Reset() {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.published = nil
}
