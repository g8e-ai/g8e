...
func (cs *CertStore) EnsureCerts(extraIPs []net.IP) error {
	...
	if needCA {
		...
		// Mirror ca.crt to ssl root for VSE/VSOD consumers
...
func (cs *CertStore) generateServerCert(keyPath, certPath string, extraIPs []net.IP) error {
	...
	dnsNames := []string{"g8e.local", "localhost", "vsodb", constants.Status.ComponentName.VSE, constants.Status.ComponentName.VSOD}
