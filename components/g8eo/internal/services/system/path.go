...
func ResolveProjectRoot() string {
	...
	// Try to find the root by looking for components/g8eo or .git
	current := cwd
	for {
	...
	// Fallback to relative path from components/g8eo if we are likely there
	if strings.Contains(cwd, filepath.Join("components", "g8eo")) {
		// We are inside g8eo, likely <root>/components/g8eo
