...
// TestVSODBPubSubClient_CheckTLSConnectivity verifies that checkTLSConnectivity
// correctly distinguishes between transport-layer failures and proxy rejections.
func TestVSODBPubSubClient_CheckTLSConnectivity(t *testing.T) {
	t.Run("proxy rejection is treated as connectivity success", func(t *testing.T) {
		// Simulate the VSOD proxy rejecting a sessionless connection with HTTP 401.
		...
		client, err := NewVSODBPubSubClient(wsURL, "", logger)
	...
	t.Run("connection refused returns error", func(t *testing.T) {
		...
		logger := testutil.NewTestLogger()
		client, err := NewVSODBPubSubClient("ws://127.0.0.1:19999", "", logger)
		require.NoError(t, err)
...
// TestVSODBPubSubClient_Close verifies that Close marks the client closed and
// prevents subsequent Publish calls.
func TestVSODBPubSubClient_Close(t *testing.T) {
	t.Run("publish after close returns error", func(t *testing.T) {
		logger := testutil.NewTestLogger()
		client, err := NewVSODBPubSubClient("ws://"+constants.DefaultEndpoint+":443", "", logger)
		require.NoError(t, err)
...
// TestVSODBPubSubClient_ConnectPubWs verifies that connectPubWs (formerly
...
func TestVSODBPubSubClient_ConnectPubWs_ReusesConnection(t *testing.T) {
	upgrader := websocket.Upgrader{
		CheckOrigin: func(r *http.Request) bool { return true },
	}
...
// TestVSODBPubSubClient_WaitForSubscribedACK_ContextCancellation verifies that
// waitForSubscribedACK returns promptly when the context is cancelled, without
// waiting for the full ackTimeout.
func TestVSODBPubSubClient_WaitForSubscribedACK_ContextCancellation(t *testing.T) {
