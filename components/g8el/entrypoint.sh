...
CONTEXT_SIZE=${G8EL_CONTEXT_SIZE:-49152}
...
exec /app/llama-server -m "/models/${MODEL_NAME}" -c "${CONTEXT_SIZE}" --host "${HOST}" --port "${PORT}" -t "${THREADS}" --mlock --flash-attn on --cache-reuse 256 --keep -1 --parallel 6 