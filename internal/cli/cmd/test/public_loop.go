...
func readPublicLoopCandidate(path string) (publicLoopCandidate, error) {
	...
	if candidate.ContentHash != computed || !version.IsHex64(candidate.SourceTreeHash) || !version.IsHex64(candidate.ExecutionSourceManifestHash) || !version.IsHex64(candidate.BinarySHA256) {
	...
	for _, image := range candidate.Images {
		if len(image.ImageID) != len("sha256:")+64 || image.ImageID[:len("sha256:")] != "sha256:" || !version.IsHex64(image.ImageID[len("sha256:"):]) {
