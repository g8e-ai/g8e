...
func TestComplianceReleaseEvidenceProjection_UsesVerifiedPublicBundleScope(t *testing.T) {
	...
	require.NoError(t, err)
	reader := &releaseProjectionBundleReader{bodies: map[string][]byte{
		constants.ComplianceBundleScopeFilename: scopeBody,
		constants.ComplianceBundleMarkdownPath:  []byte("# canonical public report\n"),
