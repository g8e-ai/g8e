...
func TestChatEvalEnsureOperatorBinding_RefreshesStaleBinding(t *testing.T) {
	...
	deps := chatEvalDeps{refreshClientFactory: stubRefreshClientFactory(stub)}
...
func TestChatEvalEnsureOperatorBinding_RefreshFailure(t *testing.T) {
	deps := chatEvalDeps{
		refreshClientFactory: stubRefreshClientFactory(&stubRefreshClient{err: context.Canceled}),
	}
