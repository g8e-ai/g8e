...
func TestPromptApproveComponent_NoPendingRequestSkips(t *testing.T) {
	client := &stubWalkthroughAPIClient{
		getResponses: map[string][]byte{constants.APIPaths.AuthPlatformEnrollmentPending: mustMarshalPendingResp(t, nil)},
	}
...
func TestPromptApproveComponent_UserDeclinesPostsNothing(t *testing.T) {
	...
	client := &stubWalkthroughAPIClient{
		getResponses: map[string][]byte{constants.APIPaths.AuthPlatformEnrollmentPending: mustMarshalPendingResp(t, []models.PlatformEnrollmentPendingRequest{ensembleReq})},
	}
...
func TestPromptApproveComponent_UserApprovesPostsDecision(t *testing.T) {
	...
	client := &stubWalkthroughAPIClient{
		getResponses:  map[string][]byte{constants.APIPaths.AuthPlatformEnrollmentPending: mustMarshalPendingResp(t, []models.PlatformEnrollmentPendingRequest{ensembleReq})},
		postResponses: map[string][]byte{constants.APIPaths.AuthPlatformEnrollmentDecision: mustMarshalDecisionResp(t, decisionResp)},
	}
...
func TestRunDockerStartWalkthrough_OperatorNotRunningWrapsEnrollmentFailed(t *testing.T) {
	...
	deps := dockerStartDeps{
		...
		enrollerFactory:    authcmd.PanickingEnrollerFactory(),
		waitGatewayHealthy: func(*cobra.Command) error { return nil },
		cfg:                cfg,
		fileSvc:            fileSvc,
	}
