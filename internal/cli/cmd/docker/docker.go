...
already-enrolled CLI.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !skipBuild {
				buildArgs, err := dockerBuildArgs(shared.VersionInfoFromCmd(cmd), noCache)
				if err != nil {
			...
			if err := reportDockerPublicSpectatorReady(cmd); err != nil {
				...
				if err != nil {
					...
					eval.WriteCampaignMirrorRestoreInitSummary(cmd.OutOrStdout(), result)
				}
			}
...
func dockerStartCmd() *cobra.Command {
	return dockerStartCmdWithConfig(shared.LoadConfig, shared.NewFileSvc, authcmd.DefaultAPIClientFactory, auth.CheckOperatorRunning, authcmd.NewDefaultEnrollmentCoordinator)
}
...
func fetchPendingPlatformEnrollments(client authcmd.APIClient) ([]models.PlatformEnrollmentPendingRequest, error) {
	pendingBody, err := client.Get(constants.APIPaths.AuthPlatformEnrollmentPending)
	if err != nil {
...
func runDockerInitApprovals(cmd *cobra.Command, client authcmd.APIClient) error {
	const (
		maxRounds    = 90
	...
	for round := 0; round < maxRounds; round++ {
		...
		authcmd.PrintPlatformEnrollmentRequestDetails(cmd, next)
		decisionReq := models.PlatformEnrollmentDecisionRequest{
...
func dockerBuildArgs(vi serve.VersionInfo, noCache bool) ([]string, error) {
	sourceTreeHash, err := version.EffectiveSourceTreeHash(vi)
	if err != nil {
		return nil, err
	}
	imageVersion := strings.TrimSpace(vi.Version)
	if imageVersion == "" {
		imageVersion = string(constants.SystemHealthUnknown)
	}
	buildTime := strings.TrimSpace(vi.BuildTime)
	if _, err := time.Parse(time.RFC3339, buildTime); err != nil {
		buildTime = time.Now().UTC().Format(time.RFC3339)
	}
	buildID := version.EffectiveBuildID(vi)
	if buildID == "" {
	...
	sourceRevision := version.EffectiveSourceRevision(vi, buildinfo.ReadVCSStamp())
	if version.IsUnstampedMetadata(sourceRevision) {
