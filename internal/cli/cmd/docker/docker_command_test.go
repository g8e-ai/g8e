...
func TestDockerCommand_RegisteredOnRoot(t *testing.T) {
	// Attachment to the root command is asserted by package cmd TestCommandRegistration.
...
func TestImageProvenance_RequiresAllLabels(t *testing.T) {
	_, err := imageProvenance(dockerImage{ID: "sha256:image", Config: dockerImageConfig{Labels: map[string]string{constants.G8eImageVersionLabel: "2.1.12"}}})
