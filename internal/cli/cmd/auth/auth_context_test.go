...
func TestAuthContextCmdWithConfig_ExportsCanonicalCLIIdentity(t *testing.T) {
	...
	cmd := authContextCmdWithConfig(cmdtest.ConfigLoaderFor(cfg), PanickingClientFactory(), cmdtest.FileSvcFactoryFor(fileSvc))
...
func TestAuthContextCmdWithConfig_ResolvesBindingFromCLISessionEndpoint(t *testing.T) {
	...
	client := &cmdtest.MockAPIClient{GetResp: response}
	cmd := authContextCmdWithConfig(cmdtest.ConfigLoaderFor(cfg), MockClientFactory(client), cmdtest.FileSvcFactoryFor(fileSvc))
	var output bytes.Buffer
	cmd.SetOut(&output)
