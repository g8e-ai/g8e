...
func TestEnrollCmdWithConfig_ConfigLoaderError(t *testing.T) {
	...
	cmd := enrollUserCmdWithConfig(failLoader, shared.NewFileSvc, auth.CheckOperatorRunning, NewDefaultEnrollmentCoordinator)
...
func TestEnrollCmdWithConfig_OperatorNotRunningReturnsError(t *testing.T) {
	...
	cmd := enrollUserCmdWithConfig(loader, shared.NewFileSvc, auth.CheckOperatorRunning, NewDefaultEnrollmentCoordinator)
	var buf bytes.Buffer
	cmd.SetOut(&buf)
	cmd.SetErr(&buf)
...
func TestEnrollCmdWithConfig_NoTPMFlagOnNonWindows(t *testing.T) {
	cmd := enrollUserCmdWithConfig(shared.LoadConfig, shared.NewFileSvc, auth.CheckOperatorRunning, NewDefaultEnrollmentCoordinator)
...
func TestEnrollCmdWithConfig_FlagsRegistered(t *testing.T) {
	...
	cmd := enrollUserCmdWithConfig(func(string) (*config.Config, error) { return cfg, nil }, cmdtest.FileSvcFactoryFor(fileSvc), auth.CheckOperatorRunning, NewDefaultEnrollmentCoordinator)
...
func TestEnrollCmdWithConfig_GatewayDownReturnsError(t *testing.T) {
	...
	cmd := enrollUserCmdWithConfig(func(string) (*config.Config, error) {
		return cfg, nil
	}, cmdtest.FileSvcFactoryFor(fileSvc), auth.CheckOperatorRunning, NewDefaultEnrollmentCoordinator)
	var buf bytes.Buffer
	cmd.SetOut(&buf)
	cmd.SetErr(&buf)
...
func TestLogoutCmd_OSRootCARetained(t *testing.T) {
	...
	for _, p := range []string{cfg.CredentialsFile(), cfg.CLICertFile(), cfg.CLIKeyFile()} {
		exists, err := fileSvc.FileExists(context.Background(), cmdtest.MustRel(t, fileSvc, p))
		require.NoError(t, err)
	...
	exists, err := fileSvc.FileExists(context.Background(), cmdtest.MustRel(t, fileSvc, cfg.ResolvedTrustBundlePath()))
	require.NoError(t, err)
