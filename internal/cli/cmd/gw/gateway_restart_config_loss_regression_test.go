...
func TestGatewayRestartCmd_FailsClosedWhenNoLaunchProfile_AfterFix(t *testing.T) {
	...
	cmd := gatewayRestartCmdWithConfig(cmdtest.ConfigLoaderFor(cfg), cmdtest.FileSvcFactoryFor(fileSvc))
	var buf bytes.Buffer
	cmd.SetOut(&buf)
	cmd.SetErr(&buf)

	err := cmd.RunE(cmd, nil)
	require.Error(t, err, cmdtest.RegressionMarkerAfterFix)
	assert.ErrorIs(t, err, constants.ErrLaunchProfileMissing, cmdtest.RegressionMarkerAfterFix)
	...
	assert.Equal(t, "consensus", string(postureData), cmdtest.RegressionMarkerAfterFix)

	_ = cmdtest.RegressionMarkerIssue
}
...
func TestGatewayRestartCmd_RestoresCompleteConfigFromProfile_AfterFix(t *testing.T) {
	...
	err := cmd.RunE(cmd, nil)
	require.Error(t, err, cmdtest.RegressionMarkerAfterFix)
	assert.ErrorIs(t, err, constants.ErrProcessStartFailed, cmdtest.RegressionMarkerAfterFix)
	...
	require.NoError(t, json.Unmarshal(profileData, &profile), cmdtest.RegressionMarkerAfterFix)
	assert.Equal(t, serve.LaunchProfileVersion, profile.Version, cmdtest.RegressionMarkerAfterFix)
	assert.Equal(t, "your-app.lovable.app", profile.Config.PasskeyRpID, cmdtest.RegressionMarkerAfterFix)
	assert.Equal(t, []string{"https://your-app.lovable.app"}, profile.Config.AllowedOrigins, cmdtest.RegressionMarkerAfterFix)
	assert.Equal(t, "https://your-app.lovable.app", profile.Config.PublicBaseURL, cmdtest.RegressionMarkerAfterFix)
	assert.Equal(t, 8080, profile.Config.HTTPPort, cmdtest.RegressionMarkerAfterFix)
	assert.Equal(t, 8443, profile.Config.HTTPSPort, cmdtest.RegressionMarkerAfterFix)
	assert.Equal(t, "trib-001", profile.Config.ConsensusID, cmdtest.RegressionMarkerAfterFix)
...
func TestGatewayRestartCmd_PreservesCompleteConfigFields_AfterFix(t *testing.T) {
	...
	assert.NotEmpty(t, restartCfg.AllowedOrigins, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.PasskeyRpID, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.PasskeyRpName, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.PasskeyRpOrigins, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.PublicBaseURL, cmdtest.RegressionMarkerAfterFix)
	assert.NotZero(t, restartCfg.HTTPPort, cmdtest.RegressionMarkerAfterFix)
	assert.NotZero(t, restartCfg.HTTPSPort, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.CertIdentityMode, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.ConsensusID, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.ConsensusURL, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.MCPDownstreamURL, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.A2ADownstreamURL, cmdtest.RegressionMarkerAfterFix)
	assert.NotZero(t, restartCfg.RateLimitRPS, cmdtest.RegressionMarkerAfterFix)
	assert.NotZero(t, restartCfg.RateLimitBurst, cmdtest.RegressionMarkerAfterFix)
	assert.NotEmpty(t, restartCfg.DoctrineDir, cmdtest.RegressionMarkerAfterFix)
