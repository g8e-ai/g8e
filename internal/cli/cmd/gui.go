...
CORS preflight.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cmd.Printf("   GET %s/api/v1/sse/stream\n", httpsURL)
			cmd.Println("   (requires authenticated session cookie; web_session_id derived from cookie)")
			cmd.Println()
...
func printEnrollConfig(cmd *cobra.Command, origin, rpID, rpName, publicBaseURL string) {
	...
	cmd.Println("}")
	cmd.Println()
	cmd.Println("// SSE stream (requires authenticated session cookie)")
	cmd.Println("function connectSSE() {")
	cmd.Printf("  const es = new EventSource(`${API_BASE_URL}/api/v1/sse/stream`, { withCredentials: true });\n")
	cmd.Println("  es.onmessage = (ev) => console.log('SSE event:', JSON.parse(ev.data));")
	cmd.Println("  es.onerror = () => { es.close(); setTimeout(() => connectSSE(), 3000); };")
	...
	cmd.Println("//   GET  /api/v1/sse/stream                       - SSE live events (session from cookie)")
