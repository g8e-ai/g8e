...
func evalCmdWithConfig(deps nativeEvalDeps) *cobra.Command {
	...
	cmd.AddCommand(nativeEvalRunCmd(deps), nativeEvalVerifyCmd(deps), nativeEvalShowCmd(deps), inferenceEvalCmd(deps), chatEvalCmd(deps), inventoryEvalCmd(deps), campaignEvalCmd(deps))
	return cmd
}
