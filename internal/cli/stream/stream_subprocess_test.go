...
func TestRunStream_G8eBinaryMissing_Subprocess(t *testing.T) {
	...
	dir := testutil.TempDir(t)
	cmd := exec.Command(os.Args[0], "-test.run=TestRunStream_G8eBinaryMissing_Subprocess")
	cmd.Env = append(os.Environ(),
...
func TestRunStream_ValidG8eBinary_UnreachableHost_Subprocess(t *testing.T) {
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, os.Args[0], "-test.run=TestRunStream_ValidG8eBinary_UnreachableHost_Subprocess", "-test.timeout=12s")
	cmd.Env = append(os.Environ(),
