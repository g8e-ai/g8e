...
var (
	...
	ErrAuditStoreReceiptLookupConflict       = errors.New("receipt lookup accepts one correlation field")
	ErrAuditStoreReceiptCorrelationDuplicate = errors.New("multiple action receipts match the investigation")
	...
	ErrGatewayOperatorSessionDuplicate         = errors.New("duplicate operator session records")
	ErrGatewayOperatorSessionUserMismatch      = errors.New("operator session user mismatch")
	ErrGatewayCLISessionBindingMismatch        = errors.New("CLI session binding mismatch")
...
// Eval facade errors. The Go `./g8e eval` facade is orchestration, not
// protocol authorization. These sentinels cover the distinct launch,
// environment, config, authority, lease, and process failure modes. They
// are mapped from stable Python engine error codes in one adapter and are
// never used to classify provider/model/task terminal outcomes as launcher
// defects.
var (
	ErrEvalProjectNotFound              = errors.New("eval: project not found")
	ErrEvalEngineUnavailable            = errors.New("eval: engine unavailable; run './g8e eval setup'")
	ErrEvalEngineNotSetUp               = errors.New("eval: engine not set up; run './g8e eval setup'")
	ErrEvalUVUnavailable                = errors.New("eval: uv executable unavailable")
	ErrEvalUVSyncFailed                 = errors.New("eval: locked environment synchronization failed")
	ErrEvalEngineSelfCheckFailed        = errors.New("eval: internal engine self-check failed")
	ErrEvalEngineProtocolMismatch       = errors.New("eval: internal engine protocol/version mismatch")
	ErrEvalConfigInvalid                = errors.New("eval: config invalid")
	ErrEvalAuthorityInvalid             = errors.New("eval: authority invalid or drifted")
	ErrEvalLeaseMissing                 = errors.New("eval: lease missing")
	ErrEvalLeaseInactive                = errors.New("eval: lease inactive")
	ErrEvalLeaseExpired                 = errors.New("eval: lease expired")
	ErrEvalLeaseMismatched              = errors.New("eval: lease mismatched")
	ErrEvalLeaseConsumed                = errors.New("eval: lease already consumed")
	ErrEvalCandidateDrift               = errors.New("eval: candidate identity drift")
	ErrEvalInventoryDrift               = errors.New("eval: model inventory drift")
	ErrEvalReportRootReused             = errors.New("eval: report root reused")
	ErrEvalEvidenceKeyInvalid           = errors.New("eval: evidence key invalid")
	ErrEvalPlatformIdentityUnavailable  = errors.New("eval: platform identity unavailable")
	ErrEvalPlatformUnhealthy            = errors.New("eval: platform component unhealthy")
	ErrEvalProviderUnreachable          = errors.New("eval: provider endpoint unreachable")
	ErrEvalBudgetPreflightFailed        = errors.New("eval: budget preflight failed")
	ErrEvalChildStartFailed             = errors.New("eval: child process start failed")
	ErrEvalChildExitNonZero             = errors.New("eval: child process exited nonzero")
	ErrEvalChildInterrupted             = errors.New("eval: child process interrupted")
	ErrEvalStatusReconciliationFailed   = errors.New("eval: status reconciliation failed")
