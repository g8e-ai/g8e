...
func TestRegistryScenarioOrder(t *testing.T) {
	...
	validLastPrefixes := []string{"consensus", "envelope", "notary", "tribunal", "delegation", "dow", "dhs", "gov", "finance", "secure", "migration", "swarm"}
	found := false
...
func TestRegistryCount(t *testing.T) {
	scenarios := Registry()
	expectedCount := len(mcpScenarios()) + len(a2aScenarios()) + len(governanceScenarios()) + len(dowScenarios()) + len(dhsScenarios()) + len(govFinanceScenarios()) + len(secureDataScenarios()) + len(swarmScenarios())
