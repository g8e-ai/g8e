...
func TestDeviceLinkData(t *testing.T) {
	t.Run("creates valid device link", func(t *testing.T) {
		now := time.Now().UTC()
		...
		link := &DeviceLinkData{
			...
			MaxUses:        1,
			Uses:           0,
			Status:         constants.DeviceLinkStatusPending,
			CreatedAt:      now,
			ExpiresAt:      expiresAt,
		}
		...
		assert.Equal(t, 1, link.MaxUses)
		assert.Equal(t, 0, link.Uses)
		assert.Equal(t, constants.DeviceLinkStatusPending, link.Status)
	})

	t.Run("creates device link with device info", func(t *testing.T) {
		now := time.Now().UTC()
		deviceInfo := &DeviceLinkInfo{
		...
		link := &DeviceLinkData{
			...
			MaxUses:    1,
			Uses:       1,
			Status:     constants.DeviceLinkStatusUsed,
			CreatedAt:  now,
			ExpiresAt:  now.Add(24 * time.Hour),
			DeviceInfo: deviceInfo,
			Claims: []DeviceLinkClaim{
				{
					SystemFingerprint: "fp-123",
					Hostname:          "test-host",
					OperatorID:        "operator-123",
		...
		assert.NotNil(t, link.DeviceInfo)
		assert.Equal(t, "fp-123", link.DeviceInfo.SystemFingerprint)
		assert.Len(t, link.Claims, 1)
	})
}

func TestCreateDeviceLinkRequest(t *testing.T) {
	t.Run("creates valid request", func(t *testing.T) {
		req := &CreateDeviceLinkRequest{
			UserID:         "user-123",
			OrganizationID: "org-123",
			OperatorID:     "operator-123",
			WebSessionID:   "session-123",
		...
		assert.Equal(t, 1, req.MaxUses)
		assert.Equal(t, 3600, req.TTLSeconds)
	})
}

func TestDeviceLinkResponse(t *testing.T) {
	t.Run("creates valid response", func(t *testing.T) {
		...
		resp := &DeviceLinkResponse{
			...
			Token:           "token-123",
			UserID:          "user-123",
			OperatorCommand: "./g8e operator enroll --token token-123",
			Name:            "Test Device",
			MaxUses:         1,
			ExpiresAt:       expiresAt,
		}
		...
		assert.Contains(t, resp.OperatorCommand, "token-123")
	})
}

func TestDeviceLinkListItem(t *testing.T) {
	t.Run("creates valid list item", func(t *testing.T) {
		now := time.Now().UTC()
		expiresAt := now.Add(24 * time.Hour)

		item := &DeviceLinkListItem{
			Token:     "token-123",
			Name:      "Test Device",
			MaxUses:   1,
			Uses:      0,
		...
		assert.Equal(t, "token-123", item.Token)
		assert.Equal(t, 0, item.Uses)
	})
}

func TestDeviceLinkListResponse(t *testing.T) {
	t.Run("creates valid list response", func(t *testing.T) {
		resp := &DeviceLinkListResponse{
			Success: true,
			Links: []DeviceLinkListItem{
				{Token: "token-1", MaxUses: 1, Uses: 0},
				{Token: "token-2", MaxUses: 1, Uses: 1},
			},
		}
...
func TestDeviceLinkInfo(t *testing.T) {
	t.Run("creates valid device info", func(t *testing.T) {
		info := &DeviceLinkInfo{
			SystemFingerprint: "fp-123",
			Hostname:          "test-host",
			OS:                "linux",
		...
		assert.Equal(t, "fp-123", info.SystemFingerprint)
		assert.Equal(t, "test-host", info.Hostname)
	})
}

func TestDeviceLinkClaim(t *testing.T) {
	t.Run("creates valid claim", func(t *testing.T) {
		now := time.Now().UTC()
		claim := DeviceLinkClaim{
			SystemFingerprint: "fp-123",
			Hostname:          "test-host",
			OperatorID:        "operator-123",
			ClaimedAt:         now,
		...
		assert.Equal(t, "operator-123", claim.OperatorID)
	})
}
