...
func ValidateDemoScenarioResult(result *compliancev1.DemoScenarioResult, definition *compliancev1.DemoScenarioDefinition, scopeID string) error {
	...
	for _, values := range [][]string{result.InvestigationIds, result.AttemptIds, result.ExecutionIds, result.TransactionIds, result.ReceiptRefs, result.StateObservationRefs, result.MetricRefs, result.KsiRefs, result.ProtocolChainRefs} {
		if err := validateUniqueStrings(values); err != nil {
