...
func TestBuildExecutorFailureAssignmentResult_ClassifiesProviderFailures(t *testing.T) {
	...
	result, err := BuildExecutorFailureAssignmentResult(req, errors.New("evaluation: execute assignment: wait for trace: deadline exceeded"), time.Unix(1_700_000_000, 0).UTC(), func(prefix string) string { return prefix + "-1" })
