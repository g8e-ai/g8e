...
class ServiceClient {
    ...
    getAuthHeaders(serviceName = null) {
        // Gateway-direct requests authenticate via the HttpOnly
        // g8e_web_session_cookie, sent automatically by the browser because of
        // credentials: 'include'. No auth headers are injected — the gateway
        // does not accept x-session-id, Authorization, Cookie, or X-API-Key
        // headers for browser auth.
        if (serviceName === ServiceName.GATEWAY) {
            return {};
        }
        ...
        if (webSessionId) {
            ...
            devLogger.warn(`[ ServiceClient ] No session ID found for ${serviceName || ServiceName.g8ed}`);
        }
