 ...
 * chat message payload. g8ed backend stores them securely in g8edB KV.
 * 
 * Flow:
 ...
 * 5. g8ed stores in g8edB KV, forwards metadata to VSE
 * 6. VSE retrieves from g8edB KV for AI processing
 */
...
export class AttachmentsManager {
     ...
     * Includes base64 data so g8ed can store in g8edB KV.
     */
...
export class CompactAttachmentsUI {
    ...
    renderCompactAttachment(file) {
        ...
        const thumbnailHtml = isImage && file.previewUrl
            ? `<img src="${file.previewUrl}" class="compact-attachment-thumbnail" alt="${this._escapeHtml(file.name)}" />` 
        ...
        return `
            <div class="compact-attachment-item ${isImage ? 'has-thumbnail' : ''}" data-file-id="${file.id}" title="${this._escapeHtml(file.name)} (${this.manager.formatFileSize(file.size)})">
                <div class="compact-attachment-preview">
                    ${thumbnailHtml}
                </div>
                <span class="compact-attachment-name">${this._escapeHtml(displayName)}</span>
                <button class="compact-remove-btn" data-file-id="${file.id}" title="Remove ${this._escapeHtml(file.name)}">×</button>
            </div>
        `;
    }
