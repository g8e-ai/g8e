...
class MarkdownRenderer {
    ...
    async renderMermaidDiagrams(container) {
        ...
        for (const element of diagrams) {
            ...
            try {
                ...
                element.innerHTML = `<div class="mermaid-error">Failed to render diagram: ${this.escapeHtml(err.message)}</div>`;
            }
        }
    }
    ...
    parseStreamingContent(content) {
        ...
        if (incompleteOpening) {
            ...
            const beforeFence = content.substring(0, lastFenceIndex);
            const fenceAndAfter = content.substring(lastFenceIndex);
            const fencesBeforeCount = (beforeFence.match(/^```/gm) ?? []).length;
            
            if (fencesBeforeCount % 2 === 0) {
                const parsed = this.parseMarkdown(beforeFence, false);
                const escaped = this.escapeHtml(fenceAndAfter);
                return parsed + escaped + '<span class="streaming-cursor">▊</span>';
            }
        }
