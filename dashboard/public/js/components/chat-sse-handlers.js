...
export const ChatSSEHandlersMixin = {
    ...
    handleAITextChunk(data) {
        ...
        const existingContent = this.streamingContent.get(data.web_session_id);
        if (existingContent === undefined && this.anchoredTerminal) {
            this.anchoredTerminal.resetAutoScroll();
        }
        const newContent = (existingContent ?? '') + textContent;
        this.streamingContent.set(data.web_session_id, newContent);

        if (!this.anchoredTerminal) return;

        // Use debounced rendering to prevent DOM thrashing
        this._debouncedRenderChunk(data.web_session_id, newContent);
    },
    ...
    handleTurnComplete(data) {
        ...
        this._finalizeInterTurnBubble(webSessionId);
    },

    _finalizeInterTurnBubble(webSessionId) {
        if (!webSessionId) return;

        this._debouncedRenderChunk.cancel();

        if (this.anchoredTerminal && this.streamingContent.has(webSessionId)) {
            this._sealStreamingBubble(webSessionId);
        }
        this.streamingContent.delete(webSessionId);
    },

    _sealStreamingBubble(webSessionId) {
        const rawContent = this.streamingContent.get(webSessionId);
        if (!rawContent) return;
        if (!this.markdownRenderer) {
            throw new Error('[CHAT] markdownRenderer is not initialized');
        }
        const decoded = decodeHtmlEntities(rawContent);
        const finalHtml = this.markdownRenderer.parseMarkdown(decoded, false);
        this.anchoredTerminal.finalizeAIResponseChunk(webSessionId, finalHtml);
    },
    ...
    handleCitationsReady(data) {
        if (!this.shouldProcessEvent(data)) return;
        if (!data || !data.web_session_id) {
            return;
        }

        const webSessionId = data.web_session_id;
        const groundingMetadata = data.grounding_metadata;

        if (!this.pendingCitations) {
            this.pendingCitations = new Map();
        }
        this.pendingCitations.set(webSessionId, groundingMetadata);
    },
    ...
    handleResponseComplete(data) {
        ...
        if (this.anchoredTerminal) {
            ...
            const hasStreamedContent = !!this.streamingContent.get(webSessionId);
            if (hasStreamedContent) {
                this._sealStreamingBubble(webSessionId);
                if (this.pendingCitations?.has(webSessionId)) {
                    this.anchoredTerminal.applyCitationsAfterFinalize(webSessionId, this.pendingCitations.get(webSessionId));
                    this.pendingCitations.delete(webSessionId);
                }
                ...
                this.anchoredTerminal.appendAIResponse(html, null, groundingMetadata);
            }
        }
    ...
    handleChatStopped(data = {}) {
        if (!this.shouldProcessEvent(data)) return;
        const webSessionId = data.web_session_id;
        if (webSessionId && this.streamingContent.has(webSessionId)) {
            this._finalizeInterTurnBubble(webSessionId);
        }
        this.streamingActive = false;
        this.approvalPending = false;
