...
export const OperatorDeviceAuthMixin = {
    ...
    handleDevicePendingAuthorization(data) {
        ...
        if (hostnameEl) {
            ...
            hostnameEl.textContent = this._escapeHtml(hostname);
        ...
        approveBtn.dataset.token = token;
        approveBtn.dataset.operatorId = operator_id;
        ...
        denyBtn.dataset.token = token;
        denyBtn.dataset.operatorId = operator_id;
    ...
    async authorizeDevice(token, operatorId) {
        try {
            ...
            if (!response.ok) {
                ...
                alert(`Failed to authorize device: ${data.error || 'Unknown error'}`);
                return;
            }
            ...
            alert(`Failed to authorize device: ${error.message}`);
        }
    },
