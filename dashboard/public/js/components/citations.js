...
export class CitationsHandler {
                ...
                if (trigger) {
                    const citationData = Array.from(trigger.querySelectorAll('.citation-data-item')).map(el =>
                        new CitationItem({
                            ...
                            fullTitle:   el.dataset.fullTitle || null,
                            citationNum: Number(el.dataset.citationNum),
                        })
                    );
    ...
    _showHoverCard(wrapper, citationData) {
        ...
        const citationItems = citationData.map(cite => {
            ...
            const safeUri = this._escapeHtml(cite.uri);
            const safeTitle = this._escapeHtml(displayTitle);
            const safeDomain = this._escapeHtml(cite.domain);
            const safeCitationNum = this._escapeHtml(String(cite.citationNum));
    ...
    _buildCitationHoverCard(citationData) {
        const itemElements = citationData.map(item => {
            const uri         = this._escapeHtml(item.uri);
            const displayName = this._escapeHtml(item.displayName);
            const domain      = this._escapeHtml(item.domain);
            const fullTitle   = item.fullTitle ? this._escapeHtml(item.fullTitle) : '';
            const citationNum = this._escapeHtml(String(item.citationNum));
