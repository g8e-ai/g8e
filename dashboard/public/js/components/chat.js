...
export class ChatComponent {
    constructor(eventBus) {
        ...
        // Initialize debounced rendering for text chunks
        this._debouncedRenderChunk = debounce((webSessionId, content) => {
            if (this.anchoredTerminal) {
                const html = this.markdownRenderer.parseStreamingContent(content);
                this.anchoredTerminal.appendAIResponseChunk(webSessionId, html);
            }
    ...
    _handleLLMChatIterationFailed(data) {
        ...
        this.streamingActive = false;
        this.hideAIStopButton();
        this.streamingContent.delete(data.web_session_id);
    }
