...
export const OperatorDeviceLinkMixin = {
    ...
    async _createDeviceLink(overlay) {
        ...
        const maxUses = parseInt(maxRegInput?.value, 10) || 100;
        const expiresInHours = parseInt(expirySelect?.value, 10) || 48;
    ...
    _createDeviceLinkElement(token, overlay) {
        ...
        el.dataset.tokenId = token.token;
        ...
        el.innerHTML = `
            <div class="device-link-item-header">
                <div class="device-link-item-name">
                    ...
                    <span>${this._escapeHtml(token.name || '(unnamed)')}</span>
                </div>
    ...
    async _revokeDeviceLink(tokenId, overlay) {
        const confirmed = confirm(OperatorDialogs.REVOKE_DEVICE_LINK_CONFIRM);
        if (!confirmed) return;
