...
export const OperatorListMixin = {
        ...
        paginatedOperators.forEach(operator => {
            ...
            if (bindBtn) {
                bindBtn.addEventListener('click', async (e) => {
                    ...
                    const operatorId = bindBtn.dataset.operatorId;
                    const isBoundState = bindBtn.dataset.isBound === 'true';
                    const isStaleState = bindBtn.dataset.isStale === 'true';
            ...
            if (apiKeyBtn) {
                apiKeyBtn.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    await this.copyOperatorApiKey(apiKeyBtn.dataset.operatorId, apiKeyBtn);
                });
            }
            ...
            if (refreshKeyBtn) {
                refreshKeyBtn.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    await this.refreshOperatorApiKey(refreshKeyBtn.dataset.operatorId);
                });
            }
            ...
            if (stopBtn) {
                stopBtn.addEventListener('click', async (e) => {
                    ...
                    await this.stopOperator(stopBtn.dataset.operatorId);
                });
            }
    ...
    _selectMetricsOperator(operatorId) {
        ...
        if (this.operatorList) {
            this.operatorList.querySelectorAll('.operator-list-item').forEach(el => {
                el.classList.toggle('metrics-selected', el.dataset.operatorId === operatorId);
            });
        }
    ...
    async refreshOperatorApiKey(operatorId) {
        try {
            const confirmed = confirm(OperatorDialogs.RESET_SLOT_CONFIRM);
            ...
            alert(`Failed to refresh API key: ${error.message}`);
        }
    },
    ...
    async generateDeviceLink(operatorId, buttonElement) {
        try {
            ...
            devLogger.log('[OPERATOR] Device link generated (pre-authorized):', {
                token_prefix: result.token?.substring(0, 12) + '...',
    ...
    async stopOperator(operatorId) {
        try {
            ...
            const confirmed = confirm(OperatorDialogs.STOP_OPERATOR_CONFIRM);
            ...
            alert(`${operatorName} on ${hostname} has been shutdown.`);

        } catch (error) {
            devLogger.error('[OPERATOR] Failed to stop operator:', error);
            alert(`Failed to stop operator: ${error.message}`);
        }
    },
