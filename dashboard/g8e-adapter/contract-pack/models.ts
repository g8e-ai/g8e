...
export type PublicFeedOutboxStatus = 'pending' | 'sent' | 'acknowledged' | 'failed';
...
export type PublicFeedIngestRejectionReason = 'signature_invalid' | 'sequence_out_of_order' | 'hash_chain_mismatch' | 'duplicate_sequence' | 'oversized_batch' | 'revoked_key' | 'unknown_key';
export const PUBLIC_FEED_INGEST_REJECTION_REASONS: readonly PublicFeedIngestRejectionReason[] = [
  'signature_invalid', 'sequence_out_of_order', 'hash_chain_mismatch', 'duplicate_sequence', 'oversized_batch', 'revoked_key', 'unknown_key',
] as const;
...
export interface PublicIngestResponse {
  accepted: boolean;
  high_water_sequence?: number;
  feed_chain_hash?: string;
  rejection_reason?: PublicFeedIngestRejectionReason;
}

export interface PublicKeyRevocationRecord {
...
export function isPublicIngestResponse(value: unknown): value is PublicIngestResponse {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false;
  const v = value as Record<string, unknown>;
  if (!(typeof v.accepted === 'boolean')) return false; /* accepted */
  if (v.high_water_sequence !== undefined && !(typeof v.high_water_sequence === 'number' && Number.isInteger(v.high_water_sequence))) return false; /* high_water_sequence */
  if (v.feed_chain_hash !== undefined && !(typeof v.feed_chain_hash === 'string')) return false; /* feed_chain_hash */
  if (v.rejection_reason !== undefined && !(typeof v.rejection_reason === 'string' && (PUBLIC_FEED_INGEST_REJECTION_REASONS as readonly string[]).includes(v.rejection_reason))) return false; /* rejection_reason */
  return true;
}

export function isPublicKeyRevocationRecord(value: unknown): value is PublicKeyRevocationRecord {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false;
  const v = value as Record<string, unknown>;
  if (!(typeof v.revoked_key_id === 'string')) return false; /* revoked_key_id */
  if (!(typeof v.revoked_at === 'string')) return false; /* revoked_at */
  if (!(typeof v.new_key_id === 'string')) return false; /* new_key_id */
  if (!(typeof v.revocation_signature === 'string')) return false; /* revocation_signature */
  return true;
}
