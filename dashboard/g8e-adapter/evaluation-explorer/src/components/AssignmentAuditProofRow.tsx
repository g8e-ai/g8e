...
export function AssignmentAuditProofRow({ bindings }: { bindings: PublicEvidenceBinding[] | undefined }) {
  ...
  if (!sliceBinding || !keyBinding) return null;

  const dbUrl = origin ? `${origin}/proofs/${sliceBinding.sha256}` : undefined;
  const keyUrl = origin ? `${origin}/proofs/${keyBinding.sha256}` : undefined;
    ...
    <section className="assignment-audit-proof" aria-label="Audit slice">
      <div className="assignment-audit-proof-row">
        ...
        <span className="assignment-audit-proof-actions">
          {dbUrl ? (
            <a href={dbUrl} download={`assignment-${sliceBinding.sha256.slice(0, 8)}.db`}>
              Download SQLite
            </a>
          ) : (
          ...
          {keyUrl ? (
            <a href={keyUrl} download={`assignment-${keyBinding.sha256.slice(0, 8)}.vault.key`}>
              Download vault key
            </a>
          ) : (
