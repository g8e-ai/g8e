...
describe('ChatComponent message rendering [FRONTEND - jsdom]', () => {
    ...
    describe('handleAITextChunk — no prefix on first chunk', () => {
        ...
        it('stores only the chunk text on the first chunk', () => {
            chat.handleAITextChunk(makeChunk('Good evening!'));

            expect(chat.streamingContent.get(WEB_SESSION_ID)).toBe('Good evening!');
        });

        it('accumulated content matches expected text', () => {
            chat.handleAITextChunk(makeChunk('Good evening!'));

            const stored = chat.streamingContent.get(WEB_SESSION_ID);
            expect(stored).toBe('Good evening!');
        });

        it('accumulates subsequent chunks without duplication', () => {
            chat.handleAITextChunk(makeChunk('Hello'));
            chat.handleAITextChunk(makeChunk(' world'));

            expect(chat.streamingContent.get(WEB_SESSION_ID)).toBe('Hello world');
        });

        it('passes streaming markdown HTML to the terminal', () => {
            chat.handleAITextChunk(makeChunk('First chunk'));

            // Advance timers to trigger debounced function
            vi.advanceTimersByTime(150);

            expect(terminalSpy.aiResponseChunks.length).toBe(1);
            expect(terminalSpy.aiResponseChunks[0].text).toContain('First chunk');
            expect(terminalSpy.aiResponseChunks[0].text).toContain('streaming-cursor');
        });
        ...
        it('tracks independent sessions without cross-contamination', () => {
            ...
            expect(chat.streamingContent.get('session-A')).toBe('A');
            expect(chat.streamingContent.get('session-B')).toBe('B');
        });
