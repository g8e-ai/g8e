...
function installGlobals() {
    ...
    seedTemplates(mockTemplateLoader, ['activity-indicator', 'tribunal']);
}
...
vi.mock('@g8ed/public/js/utils/template-loader.js', () => {
    return {
        templateLoader: {
            get cache() {
                return mockTemplateLoader?.cache ?? new Map();
            },
            async load(name) {
                return mockTemplateLoader?.load(name);
            },
            async preload() {},
            replace(template, vars) {
                return mockTemplateLoader?.replace(template, vars) ?? template;
            },
        }
    };
});
...
describe('TerminalOutputMixin — DOM rendering [FRONTEND - jsdom]', () => {
    ...
    describe('appendAIResponseChunk', () => {
        it('sets innerHTML of the content element to the provided HTML', () => {
            terminal.appendAIResponseChunk(WEB_SESSION_ID, '<p>Good morning</p>');
        ...
        it('replaces content on subsequent calls (full re-render, not accumulation)', () => {
            terminal.appendAIResponseChunk(WEB_SESSION_ID, '<p>first</p>');
            terminal.appendAIResponseChunk(WEB_SESSION_ID, '<p>first second</p>');
        ...
        it('renders HTML markup (not escaped)', () => {
            terminal.appendAIResponseChunk(WEB_SESSION_ID, '<strong>bold</strong>');
        ...
        it('calls scrollToBottom after each chunk', () => {
            ...
            terminal.appendAIResponseChunk(WEB_SESSION_ID, '<p>chunk</p>');
        ...
        it('removes the waiting indicator when creating the AI response entry', () => {
            ...
            terminal.appendAIResponseChunk(WEB_SESSION_ID, '<p>text</p>');

            expect(document.getElementById('waiting-indicator')).toBeNull();
        });
    });

    describe('appendAIResponse', () => {
        it('appends an AI response element to the output container', () => {
            terminal.appendAIResponse('<p>Full response</p>');
        ...
        it('sets innerHTML of the content to the provided HTML', () => {
            terminal.appendAIResponse('<p>Hello there</p>');
        ...
        it('does NOT have the "streaming" class', () => {
            terminal.appendAIResponse('<p>done</p>');
        ...
        it('uses the provided timestamp', () => {
            terminal.appendAIResponse('<p>ok</p>', '03:45:00 PM');
        ...
        it('uses formatTimestamp when timestamp is null', () => {
            ...
            terminal.appendAIResponse('<p>ok</p>', null);
        ...
        it('returns the created DOM element', () => {
            const el = terminal.appendAIResponse('<p>ok</p>');
        ...
        it('appends multiple responses in order', () => {
            terminal.appendAIResponse('<p>first</p>');
            terminal.appendAIResponse('<p>second</p>');
        ...
        it('applies inline citations when grounding_used is true', () => {
            ...
            terminal.appendAIResponse('<p>text</p>', null, metadata);
        ...
        it('does not call citation handler when grounding_used is false', () => {
            ...
            terminal.appendAIResponse('<p>text</p>', null, { grounding_used: false, sources: [] });
        ...
        it('does not call citation handler when groundingMetadata is null', () => {
            ...
            terminal.appendAIResponse('<p>text</p>', null, null);
    ...
    describe('applyCitations', () => {
        ...
        it('falls back to searching by prefixed ID after finalizeAIResponseChunk renames the element', () => {
            terminal.finalizeAIResponseChunk(WEB_SESSION_ID, '<p>sealed</p>');
    ...
    describe('showTribunal', () => {
        ...
        it('renders the initial command literally via textContent', () => {
            terminal.showTribunal({ id: WIDGET_ID, model: 'test-model', numPasses: 3, command: TEST_COMMAND });
    ...
    describe('completeTribunal', () => {
        ...
        beforeEach(() => {
            terminal.showTribunal({ id: WIDGET_ID, model: 'test-model', numPasses: 3, command: 'ls -la' });
        });
    ...
    describe('failTribunal', () => {
        ...
        beforeEach(() => {
            terminal.showTribunal({ id: WIDGET_ID, model: 'test-model', numPasses: 3, command: 'rm -rf /' });
        });
    ...
    describe('Approval Flow: Preparing to Requested', () => {
        beforeEach(() => {
            // Seed templates needed for approval flow
            seedTemplates(mockTemplateLoader, ['preparing-indicator', 'approval-card', 'approval-status']);
        });

        it('hides "Preparing" indicator when approval.requested arrives after preparing', () => {
            ...
            terminal.handleApprovalRequest({
                execution_id: executionId,
        ...
        it('does not hide preparing indicator for different execution_id', () => {
        ...
        it('handles approval.requested without preceding preparing event', () => {
            const executionId = 'exec-123';
