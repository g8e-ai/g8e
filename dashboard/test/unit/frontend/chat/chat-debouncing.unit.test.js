    ...
    describe('performance with fake timers', () => {
        it('should complete complex streaming scenario quickly with fake timers', async () => {
            ...
            const events = [
                ...
                { type: EventType.AI_LLM_TOOL_G8E_WEB_SEARCH_REQUESTED, query: 'test', delay: 30 },
                { type: EventType.AI_LLM_TOOL_G8E_WEB_SEARCH_COMPLETED, delay: 50 },
            ...
            events.forEach(event => {
                setTimeout(() => {
                    ...
                    } else if (event.type === EventType.AI_LLM_TOOL_G8E_WEB_SEARCH_REQUESTED) {
                        eventBus.emit(event.type, {
                            web_session_id: WEB_SESSION_ID,
                            investigation_id: INVESTIGATION_ID,
                            query: event.query
