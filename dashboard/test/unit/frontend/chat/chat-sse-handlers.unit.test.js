...
describe('ChatComponent — handleAITextChunk edge cases [FRONTEND - jsdom]', () => {
    ...
    it('ignores chunks with no content', () => {
        ...
        expect(terminalSpy.appendAIResponseChunk).not.toHaveBeenCalled();
        expect(chat.streamingContent.has(WEB_SESSION_ID)).toBe(false);
    });

    it('reads text from data.content (wire payload field)', () => {
        chat.handleAITextChunk({
            web_session_id: WEB_SESSION_ID,
            investigation_id: INVESTIGATION_ID,
        ...
        expect(chat.streamingContent.get(WEB_SESSION_ID)).toBe('content field text');
    });
    ...
    it('passes streaming markdown HTML to appendAIResponseChunk', () => {
        chat.handleAITextChunk({
            web_session_id: WEB_SESSION_ID,
            investigation_id: INVESTIGATION_ID,
            content: '**bold** text',
    ...
    it('event bus wiring: LLM_CHAT_ITERATION_TEXT_CHUNK_RECEIVED triggers handleAITextChunk', () => {
        ...
        expect(terminalSpy.appendAIResponseChunk).toHaveBeenCalledOnce();
        const [, html] = terminalSpy.appendAIResponseChunk.mock.calls[0];
        expect(html).toContain('streamed token');
        expect(html).toContain('streaming-cursor');
    });
});
...
describe('ChatComponent — handleChatStopped [FRONTEND - jsdom]', () => {
    ...
    it('finalizes any in-progress streaming bubble before resetting', () => {
        chat.streamingContent.set(WEB_SESSION_ID, 'partial response');
        chat.markdownRenderer = { parseMarkdown: vi.fn(() => '<p>partial response</p>') };
...
describe('ChatComponent — handleCitationsReady [FRONTEND - jsdom]', () => {
    let ChatComponent;
    let eventBus;
    let chat;
    let terminalSpy;
    ...
    it('stores grounding_metadata in pendingCitations keyed by web_session_id', () => {
        const metadata = { grounding_used: true, sources: [{ url: 'https://a.com', title: 'A' }] };

        chat.handleCitationsReady({
            web_session_id: WEB_SESSION_ID,
            investigation_id: INVESTIGATION_ID,
            grounding_metadata: metadata,
        });

        expect(chat.pendingCitations?.get(WEB_SESSION_ID)).toBe(metadata);
    });

    it('does not call applyCitations at this point — only stores for finalization', () => {
        chat.handleCitationsReady({
            web_session_id: WEB_SESSION_ID,
            investigation_id: INVESTIGATION_ID,
            grounding_metadata: { grounding_used: true, sources: [] },
        });

        expect(terminalSpy.applyCitations).not.toHaveBeenCalled();
    });
    ...
    it('rejects event when investigation_id does not match', () => {
        ...
        expect(chat.pendingCitations?.has(WEB_SESSION_ID)).toBeFalsy();
    });

    it('stores separate citations for separate sessions', () => {
        const metaA = { grounding_used: true, sources: [{ url: 'a.com', title: 'A' }] };
        const metaB = { grounding_used: true, sources: [{ url: 'b.com', title: 'B' }] };

        chat.handleCitationsReady({ web_session_id: 'session-A', investigation_id: INVESTIGATION_ID, grounding_metadata: metaA });
        chat.handleCitationsReady({ web_session_id: 'session-B', investigation_id: INVESTIGATION_ID, grounding_metadata: metaB });

        expect(chat.pendingCitations.get('session-A')).toBe(metaA);
        expect(chat.pendingCitations.get('session-B')).toBe(metaB);
    });

    it('does not throw when data is null', () => {
        expect(() => chat.handleCitationsReady(null)).not.toThrow();
    });

    it('does not throw when web_session_id is absent', () => {
        expect(() => chat.handleCitationsReady({
            investigation_id: INVESTIGATION_ID,
            grounding_metadata: { grounding_used: true, sources: [] },
        })).not.toThrow();
    });
