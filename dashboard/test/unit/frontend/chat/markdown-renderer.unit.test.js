...
import { MarkdownRenderer } from '@g8ed/public/js/utils/markdown.js';
...
describe('MarkdownRenderer [FRONTEND - jsdom]', () => {
    ...
    describe('parseStreamingContent — Bug 5 regression', () => {
        let renderer;
        ...
        it('returns empty string for empty input', () => {
            expect(renderer.parseStreamingContent('')).toBe('');
        });

        it('returns empty string for null input', () => {
            expect(renderer.parseStreamingContent(null)).toBe('');
        });

        it('does not throw when content starts with newline then fence opening', () => {
            expect(() => renderer.parseStreamingContent('\n```python\n')).not.toThrow();
        });

        it('does not throw when content starts directly with a fence opening (no preceding newline)', () => {
            expect(() => renderer.parseStreamingContent('```js\n')).not.toThrow();
        });

        it('returns a string for incomplete fence at end of content', () => {
            const result = renderer.parseStreamingContent('Some text\n```python\n');
            expect(typeof result).toBe('string');
        });

        it('appends streaming cursor for normal prose content', () => {
            const result = renderer.parseStreamingContent('hello world');
            expect(result).toContain('streaming-cursor');
        });

        it('splits at incomplete fence when even number of complete fences precede it', () => {
            const content = '```python\nprint("hi")\n```\nsome text\n```python\n';
            const result = renderer.parseStreamingContent(content);
            expect(result).toContain('streaming-cursor');
            expect(result).toContain('some text');
        });

        it('does not split at incomplete fence when odd number of complete fences precede it (inside open block)', () => {
            const content = '```python\nprint("hi")\n```python\n';
            const result = renderer.parseStreamingContent(content);
            expect(typeof result).toBe('string');
        });

        it('appends streaming cursor when content is complete markdown with no open fence', () => {
            const result = renderer.parseStreamingContent('**bold** text with no fence');
            expect(result).toContain('streaming-cursor');
        });
    });
