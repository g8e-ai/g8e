...
describe('UserDocument — parse() [UNIT]', () => {
    ...
    it('parse() sets drop_key absent — field not on model', () => {
        const m = UserDocument.parse({ id: 'u1', email: 'a@b.com', drop_key: 'key' });
        expect(m).not.toHaveProperty('drop_key');
    });
});
...
describe('UserDocument — getAvatar() [UNIT]', () => {
    it('returns default avatar path when profile_picture is absent', () => {
        expect(makeUser().getAvatar()).toBe('/media/default-avatar.png');
    });
