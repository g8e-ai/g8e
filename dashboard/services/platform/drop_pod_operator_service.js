...
/**
 * Drop Pod Operator Service
 *
 * Manages the lifecycle of the dropops-operator process running inside the
 * drop-pod container on behalf of a user. Called fire-and-forget after login
 * so that an operator on the drop-pod is active and ready to be bound by the
 * time the user's browser finishes loading.
 *
 * Explicit flow (three separate responsibilities):
 *
 *   1. getDropPodOperatorForUser(user_id)
 *      → Queries operator slots. Returns the first operator that is already
 *        ACTIVE on the drop-pod, or the first AVAILABLE slot to use. Returns
 *        null when no usable slot exists.
 *
 *   2. launchDropPodOperator(apiKey)
 *      → Persists the operator API key to the platform_settings document in
 *        g8edB, then starts the supervised operator process via XML-RPC.
 *        The operator command fetches the key from g8edB at startup.
 *        Returns immediately — the binary authenticates and SSE delivers
 *        OPERATOR_STATUS_UPDATED to the browser asynchronously.
 *
 * Top-level entry point:
 *
 *   activateDropPodOperatorForUser(user_id)
...
import {
    DROP_POD_CONTAINER_NAME,
    DROP_POD_OPERATOR_LAUNCH_TIMEOUT_MS,
} from '../../constants/service_config.js';

class DropPodOperatorService {
    /**
    ...
    constructor({ settingsService, operatorService } = {}) {
        if (!settingsService) throw new Error('DropPodOperatorService requires settingsService');
    ...
    async _resolveSettings() {
        ...
        return {
            supervisorUrl: `http://drop-pod:${port}/RPC2`,
            authHeader: `Basic ${Buffer.from(`vso-internal:${token}`).toString('base64')}`,
        };
    }

    /**
     * Returns the specific drop-pod operator slot for this user.
     *
     * Queries for the operator with is_drop_pod=true for the given user.
     *
     * @param {string} user_id
     * @returns {Promise<{ operator: Object, alreadyActive: boolean }|null>}
     */
    async getDropPodOperatorForUser(user_id) {
        if (!this._operatorService) throw new Error('operatorService is required for getDropPodOperatorForUser');
        
        // Find the specific slot designated as the drop-pod for this user
        const operators = await this._operatorService.queryOperators([
            { field: 'user_id', operator: '==', value: user_id },
            { field: 'is_drop_pod', operator: '==', value: true }
        ]);

        const operatorData = operators && operators.length > 0 ? operators[0] : null;

        if (!operatorData) {
            logger.info('[DROP-POD-OPERATOR] No drop-pod operator slot found for user', { user_id });
            return null;
        }
        ...
        logger.info('[DROP-POD-OPERATOR] Drop-pod operator slot resolved', {
            user_id,
    ...
    /**
     * Launches the operator inside the drop-pod container.
     *
     * Persists the operator API key to the platform_settings document in g8edB,
     * then starts the supervised operator process via XML-RPC. The operator
     * command in drop-pod fetches the key from g8edB at startup.
     *
     * @param {string} apiKey  - operator API key from the operator document
     * @returns {Promise<void>}
     */
    async launchDropPodOperator(apiKey) {
        logger.info('[DROP-POD-OPERATOR] Starting operator in drop-pod via XML-RPC', {
            container: DROP_POD_CONTAINER_NAME,
        });

        try {
            await this._settingsService.savePlatformSettings({ drop_pod_operator_api_key: apiKey });
        } catch (err) {
        ...
        logger.info('[DROP-POD-OPERATOR] Operator service signaled in drop-pod', {
            container: DROP_POD_CONTAINER_NAME,
        });
    }
  ...
  <params>
    ${params.map(p => `<param><value>${typeof p === 'boolean' ? `<boolean>${p ? '1' : '0'}</boolean>` : `<string>${p}</string>`}</value></param>`).join('\n')}
        ...
        const response = await fetch(supervisorUrl, {
            ...
            body,
            signal: AbortSignal.timeout(DROP_POD_OPERATOR_LAUNCH_TIMEOUT_MS)
        });
        ...
        if (xml.includes('<fault>')) {
            ...
            switch (faultCode) {
                case 10: // BAD_NAME
                    throw new Error('Operator process not found in drop-pod configuration.');
                case 60: // ALREADY_STARTED
                    throw new Error('ALREADY_STARTED');
                case 70: // NOT_RUNNING
                    return xml; // Ignore if trying to stop
                case 90: // SPAWN_ERROR
                    throw new Error('Failed to spawn operator process. Check drop-pod logs.');
                default:
    ...
    /**
     * Kills any running operator process in the drop-pod container for this
     ...
     */
    async relaunchDropPodOperatorForUser(user_id) {
        if (!this._operatorService) throw new Error('operatorService is required for relaunchDropPodOperatorForUser');

        const slotResult = await this.getDropPodOperatorForUser(user_id);
        if (!slotResult) {
            logger.warn('[DROP-POD-OPERATOR] Relaunch requested but no drop-pod operator slot found for user', { user_id });
            return { success: false, error: 'No drop-pod operator slot found for user' };
        }

        const { operator } = slotResult;
        const operator_id = operator.operator_id;

        logger.info('[DROP-POD-OPERATOR] Stopping supervised operator service in drop-pod', {
            user_id,
            operator_id,
        });
        ...
        await this.launchDropPodOperator(apiKey);

        logger.info('[DROP-POD-OPERATOR] Drop-pod operator relaunched successfully', {
            user_id,
            operator_id,
        });
    ...
    /**
     * Orchestrates the full drop-pod operator activation for a user who has
     * just logged in.
     *
     * Steps:
     ...
     */
    async activateDropPodOperatorForUser(user_id, organization_id, web_session_id) {
        try {
            const slotResult = await this.getDropPodOperatorForUser(user_id);

            if (!slotResult) {
                logger.info('[DROP-POD-OPERATOR] No operator slot available for user — skipping drop-pod launch', { user_id });
                return;
            }

            if (slotResult.alreadyActive) {
                logger.info('[DROP-POD-OPERATOR] Drop-pod operator already active for user — skipping launch', {
                    user_id,
            ...
            if (!apiKey) {
                logger.warn('[DROP-POD-OPERATOR] Operator slot has no API key — drop-pod operator will not be launched', {
                    user_id,
                    operator_id: operator.operator_id,
                });
                return;
            ...
            logger.warn('[DROP-POD-OPERATOR] Drop-pod operator activation failed (non-fatal)', {
                user_id,
                error: err.message
            });
        }
