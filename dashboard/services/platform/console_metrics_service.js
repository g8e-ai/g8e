...
import { VSOBaseModel, F, now, addSeconds } from '../../models/base.js';
...
class MetricsSnapshot extends VSOBaseModel {
    static fields = {
...
class DateQueryValue extends VSOBaseModel {
    static fields = {
...
class ConsoleMetricsService {
     ...
     * @param {Object} options.cacheAside - CacheAsideService instance
     * @param {Object} [options.internalHttpClient] - InternalHttpClient instance
     */
    constructor({ cacheAside, internalHttpClient = null }) {
    ...
    async getUserStats() {
        return this._getCachedMetric('user_stats', async () => {
            // Get all users
            const users = await this.cacheAside.queryDocuments(Collections.USERS, []);
            const ts = now();
    ...
    async getOperatorStats() {
        return this._getCachedMetric('operator_stats', async () => {
            const operators = await this.cacheAside.queryDocuments(Collections.OPERATORS, []);
    ...
    async getSessionStats() {
        return this._getCachedMetric('session_stats', async () => {
            try {
                const webSessions = await this._scanKeys(this.cacheAside, KVScanPattern.scanWebSessions());
                const operatorSessions = await this._scanKeys(this.cacheAside, KVScanPattern.scanOperatorSessions());

                const boundOperatorKeys = await this._scanKeys(this.cacheAside, KVScanPattern.allSessionWebBinds());
                let totalBoundOperators = 0;
                for (const key of boundOperatorKeys) {
                    const count = await this.cacheAside.kvScard(key);
    ...
    async getLoginAuditStats() {
        return this._getCachedMetric('login_audit_stats', async () => {
            ...
            const events = await this.cacheAside.queryDocuments(Collections.LOGIN_AUDIT, [
    ...
    async getAIUsageStats() {
        return this._getCachedMetric('ai_usage_stats', async () => {
            ...
            const investigations = await this.cacheAside.queryDocuments(Collections.INVESTIGATIONS, [
    ...
    async scanKV(pattern, cursor, count) {
        const [nextCursor, keys] = await this.cacheAside.kvScan(cursor, 'MATCH', pattern, 'COUNT', count);
        return { cursor: nextCursor, keys, count: keys.length };
    }

    async getKVKey(key) {
        const [value, ttl] = await Promise.all([this.cacheAside.kvGet(key), this.cacheAside.kvTtl(key)]);
        return { key, value, ttl, exists: value !== null };
    }

    async queryCollection(collection, limit) {
        const documents = await this.cacheAside.queryDocuments(collection, [], limit);
