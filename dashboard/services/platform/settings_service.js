...
import { g8edB_INTERNAL_HTTP_URL } from '../../constants/http_client.js';
...
class SettingsService {
     ...
     */
    constructor({ cacheAside, internalHttpClient, bootstrapService, collectionName }) {
        if (!cacheAside) {
            throw new Error('SettingsService requires a cacheAside instance');
        }
    ...
    async initialize() {
        ...
        logger.info('[SETTINGS-SERVICE] Initializing platform settings', {
            g8edbUrl: g8edB_INTERNAL_HTTP_URL ,
            hasInternalAuthToken: !!internalAuthToken,
            caCertPath,
            g8edbVolumePath: this.bootstrap.volumePath
        });
        ...
        try {
            ...
            logger.warn('[SETTINGS-SERVICE] Failed to load platform settings from DB (using defaults)', {
                error: err.message
            });
        }
    ...
    async getPlatformSettings() {
        const doc = await this.cacheAside.getDocument(this.collectionName, SETTINGS_DOC_ID);
    ...
    async getUserSettings(userId) {
        ...
        const doc = await this.cacheAside.getDocument(this.collectionName, userDocId);
        return (doc && doc.settings) ? doc.settings : {};
    }
    ...
    async savePlatformSettings(updates) {
        const existingDoc = await this.cacheAside.getDocument(this.collectionName, SETTINGS_DOC_ID);
        ...
        let result;
        if (existingDoc) {
            result = await this.cacheAside.updateDocument(this.collectionName, SETTINGS_DOC_ID, {
                settings: newSettings,
                updated_at: now(),
            });
        } else {
            result = await this.cacheAside.createDocument(this.collectionName, SETTINGS_DOC_ID, {
                settings: newSettings,
                created_at: now(),
                updated_at: now(),
            });
    ...
    async updateUserSettings(userId, updates) {
        ...
        const userDocId = `${USER_SETTINGS_DOC_PREFIX}${userId}`;
        const existingDoc = await this.cacheAside.getDocument(this.collectionName, userDocId);
        const existingSettings = (existingDoc && existingDoc.settings) ? existingDoc.settings : {};
        const newSettings = { ...existingSettings, ...validation.valid };

        let result;
        if (existingDoc) {
            result = await this.cacheAside.updateDocument(this.collectionName, userDocId, {
                settings: newSettings,
                updated_at: now(),
            });
        } else {
            result = await this.cacheAside.createDocument(this.collectionName, userDocId, {
                settings: newSettings,
                created_at: now(),
                updated_at: now(),
            });
