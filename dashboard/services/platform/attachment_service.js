...
export class AttachmentService {
    ...
    async storeAttachments(investigationId, userId, attachments) {
        if (!this.cacheAside || !this.blobStorage) {
        ...
        for (const attachment of attachments) {
            ...
            try {
                ...
                await this.cacheAside.kvSetJson(kvKey, record.forKV(), CacheTTL.ATTACHMENT);
                await this.cacheAside.kvRpush(indexKey, attachmentId);
                await this.cacheAside.kvExpire(indexKey, CacheTTL.ATTACHMENT);
    ...
    async getAttachment(attachmentKey) {
        ...
        try {
            const data = await this.cacheAside.kvGetJson(attachmentKey);
            if (!data) {
    ...
    async getAttachmentsForInvestigation(investigationId) {
        ...
        const indexKey = KVKey.attachmentIndex(investigationId);
        const attachmentIds = await this.cacheAside.kvLrange(indexKey, 0, -1);
    ...
    async deleteAttachmentsForInvestigation(investigationId) {
        ...
        await Promise.all([
            this.cacheAside.kvDel(...keysToDelete),
