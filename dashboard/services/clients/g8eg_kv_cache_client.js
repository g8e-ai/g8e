...
class KVCacheClient {
    ...
    async get(key) {
        try {
            const data = await this._http.get(ApiPaths.kv.key(key));
    ...
    async set(key, value, ...args) {
        ...
        await this._http.put(ApiPaths.kv.key(key), JSON.stringify({ value, ttl }));
        return 'OK';
    }
    ...
    async del(...keys) {
        ...
        for (const key of keys.flat()) {
            try {
                await this._http.delete(ApiPaths.kv.key(key));
                count++;
            } catch {}
        }
        return count;
    ...
    async keys(pattern) {
        const p = pattern || '*';
        try {
            const data = await this._http.post(ApiPaths.kv.keys(), JSON.stringify({ pattern: p }));
            return data.keys;
        } catch (error) {
    ...
    async ping() {
        await this._http.post(ApiPaths.kv.keys(), JSON.stringify({ pattern: '__ping__', count: 1 }));
        return 'PONG';
    }
    ...
    async expire(key, seconds) {
        try {
            await this._http.put(ApiPaths.kv.expire(key), JSON.stringify({ ttl: seconds }));
            return 1;
        } catch {
            return 0;
        }
    ...
    async ttl(key) {
        try {
            const data = await this._http.get(ApiPaths.kv.ttl(key));
            return data.ttl;
        } catch {
            return -2;
        }
    ...
    async scan(cursor, ...args) {
        ...
        try {
            const data = await this._http.post(ApiPaths.kv.scan(), JSON.stringify({
