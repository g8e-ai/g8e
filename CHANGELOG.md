...
|---------|------|-------------|-------|
| 1.6.4 | 2026-07-25 | Demo consolidation (removes secure-data, dow, swarm demos), L5 Actuator decomposition with fail-closed rehydration, error sentinel migration to internal/constants, ScrubbingService constructor fail-closed on token load, MCP gateway AuditEventRecorder interface, InputSchema.ToMap removal, DispatchToDownstream params envelope fix, demo L3 notary for headless environments, demo-verify CI job. | [v1.6.4](docs/release_notes/v1.6.x/v1.6.4.md) |
