...
case "$COMMAND" in
    show)
        ...
        _exec_in_pod python3 /app/scripts/data/manage-operator.py settings show --section llm $USER_ID_ARG
        exit 0
        ;;
    get)
        [[ -n "$ARG_USER_ID" ]] && USER_ID_ARG="--user-id=$ARG_USER_ID" || USER_ID_ARG=""
        _exec_in_pod python3 /app/scripts/data/manage-operator.py settings get "${EXT_ARGS[@]}" $USER_ID_ARG
        exit 0
        ;;
    set)
        ...
        [[ -n "$ARG_USER_ID" ]] && USER_ID_ARG="--user-id=$ARG_USER_ID" || USER_ID_ARG=""
        _exec_in_pod python3 /app/scripts/data/manage-operator.py settings set "${SET_ARGS[@]}" $USER_ID_ARG
...
if [[ $_write_rc -eq 0 ]]; then
    ...
    _header "Effective LLM Settings"
    [[ -n "$ARG_USER_ID" ]] && USER_ID_ARG="--user-id=$ARG_USER_ID" || USER_ID_ARG=""
    _exec_in_pod python3 /app/scripts/data/manage-operator.py settings show --section llm $USER_ID_ARG 2>/dev/null || true
fi
