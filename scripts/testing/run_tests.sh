    ...
    case "$1" in
        --help|-h)
            if [[ "$DETECTED_COMPONENT" == "g8eo" ]]; then
                ...
                echo "  --assistant-provider <provider>     Assistant LLM provider"
                echo "  --lite-provider <provider>          Lite LLM provider"
                ...
                echo "  -e, --llm-endpoint-url <url>        Primary endpoint URL"
                echo "  --assistant-endpoint <url>          Assistant endpoint URL"
                echo "  --lite-endpoint <url>               Lite endpoint URL"
                echo "  -k, --llm-api-key <key>             Primary API key"
                echo "  --assistant-api-key <key>           Assistant API key"
                echo "  --lite-api-key <key>                Lite API key"
                echo ""
...
_show_llm_config() {
    if [[ -n "${G8E_TEST_LLM_PRIMARY_PROVIDER:-}" ]]; then
        ...
        [[ -n "${G8E_TEST_LLM_ASSISTANT_API_KEY:-}" ]]      && echo -e "  Assistant API Key:  (set)"
        [[ -n "${G8E_TEST_LLM_LITE_API_KEY:-}" ]]           && echo -e "  Lite API Key:       (set)"
        echo ""
    else
        echo ""
