...
for name in exact_credentials:
    content = re.sub(rf'(RegexScrubber\(\s*"{name}",\s*r?[\'"][^,]+[\'"],\s*[\'"][^"]+[\'"](?:,\s*re\.[A-Z_]+)?)\s*\)', rf'\1, priority=ScrubberPriority.EXACT_CREDENTIAL)', content)

for name in contextual_credentials:
    content = re.sub(rf'(RegexScrubber\(\s*"{name}",\s*r?[\'"][^,]+[\'"],\s*[\'"][^"]+[\'"](?:,\s*re\.[A-Z_]+)?)\s*\)', rf'\1, priority=ScrubberPriority.CONTEXTUAL_CREDENTIAL)', content)

for name in url_or_connections:
    content = re.sub(rf'(RegexScrubber\(\s*"{name}",\s*r?[\'"][^,]+[\'"],\s*[\'"][^"]+[\'"](?:,\s*re\.[A-Z_]+)?)\s*\)', rf'\1, priority=ScrubberPriority.URL_OR_CONNECTION)', content)

for name in generic_piis:
    content = re.sub(rf'(RegexScrubber\(\s*"{name}",\s*r?[\'"][^,]+[\'"],\s*[\'"][^"]+[\'"](?:,\s*re\.[A-Z_]+)?)\s*\)', rf'\1, priority=ScrubberPriority.GENERIC_PII)', content)

# Sort them before returning
old_return = "return scrubbers"
new_return = "scrubbers.sort(key=lambda s: s.priority)\n        return scrubbers"
if "scrubbers.sort" not in content:
