...
func TestExportIntegration(t *testing.T) {
	...
	headersSh := filepath.Join(scriptDir, "headers.sh")
	shLines := []string{
		"#!/usr/bin/env bash",
		"# Code generated by services/g8eo/cmd/exporter. DO NOT EDIT.",
		"",
	...
	for _, entry := range snapshot.Headers {
		...
		shLines = append(shLines, fmt.Sprintf("export G8E_HEADER_%s=%q", varName, entry.Value))
	}
	shLines = append(shLines, "")
	shContent := strings.Join(shLines, "\n")
	if err := os.WriteFile(headersSh, []byte(shContent), 0755); err != nil {
		t.Fatalf("failed to write headers.sh: %v", err)
	}

	// Verify shell script exists and has content
	shFileContent, err := os.ReadFile(headersSh)
	if err != nil {
		t.Fatalf("failed to read headers.sh: %v", err)
	}
	if !strings.Contains(string(shFileContent), "# Code generated by services/g8eo/cmd/exporter") {
		t.Error("headers.sh missing generated header")
	}
	if !strings.Contains(string(shFileContent), "export G8E_HEADER_") {
		t.Error("headers.sh missing export statements")
	}
}
