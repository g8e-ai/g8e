...
func main() {
	...
	// Emit Go constants file for ApiPaths
	emitGoApiPaths := func() {
		path := filepath.Join(*root, "services", "g8eo", "internal", "constants", "api_paths_generated.go")
		lines := []string{
			...
			"package constants",
			"",
			"// ApiPathsGenerated defines the canonical G8E API paths as a mirrored constant structure.",
			"var ApiPathsGenerated = struct {",
			"	InternalPrefix string            `json:\"internal_prefix\"`",
			"	OperatorPrefix string            `json:\"operator_prefix\"`",
			"	G8ee           map[string]string `json:\"g8ee\"`",
			"	G8eeFull       map[string]string `json:\"g8ee_full\"`",
		...
		// Since ApiPaths is an interface{} in the snapshot, we need to cast it
		...
		for _, k := range g8eeKeys {
			lines = append(lines, fmt.Sprintf("\t\t%q: %q,", k, constants.ApiPaths.G8ee[k]))
		}
		lines = append(lines, "\t},")
		...
		lines = append(lines, "\tClient: map[string]string{")
		...
		for _, k := range clientKeys {
			lines = append(lines, fmt.Sprintf("\t\t%q: %q,", k, constants.ApiPaths.Client[k]))
		}
		lines = append(lines, "\t},")

		lines = append(lines, "\tClientFull: map[string]string{")
		for _, k := range clientKeys {
			prefix := constants.ApiPaths.OperatorPrefix
