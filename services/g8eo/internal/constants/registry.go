	...
	return Snapshot{
		...
		DocumentIds: DocumentIdsSnapshot{
			DocumentIds: map[string]Entry{
				"platform_settings":    {Value: string(DocIDPlatformSettings), GoConst: "DocIDPlatformSettings", PythonConst: "PLATFORM_SETTINGS"},
		...
		Paths: PathsSnapshot{
			...
			Ports: map[string]int{
				"operator_http":      Ports.OperatorHttp,
				"operator_bootstrap": Ports.OperatorBootstrap,
				"operator_public":    Ports.OperatorPublic,
				"g8ee_http":          Ports.G8eeHttp,
				"openclaw_gateway":   Ports.OpenclawGateway,
			},
			Host: "localhost",
		},
		Ports: map[string]int{
			"OperatorHttp":      Ports.OperatorHttp,
			"OperatorBootstrap": Ports.OperatorBootstrap,
			"OperatorPublic":    Ports.OperatorPublic,
			"G8eeHttp":          Ports.G8eeHttp,
			"OpenclawGateway":   Ports.OpenclawGateway,
		},
		EnvVars: map[string]string{
			"OperatorHTTPPort":          string(EnvVar.OperatorHTTPPort),
			"OperatorBootstrapPort":     string(EnvVar.OperatorBootstrapPort),
			"OperatorPublicPort":        string(EnvVar.OperatorPublicPort),
			"OperatorWSSPort":           string(EnvVar.OperatorWSSPort),
			"G8EEHTTPPort":              string(EnvVar.G8EEHTTPPort),
