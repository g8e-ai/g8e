...
class TestIamHelperMethods:
    ...
    def test_build_iam_detach_command_contains_intent_name(self):
        cmd = _make_service()._build_iam_detach_command("ec2_discovery")  # noqa: SLF001
        assert "ec2_discovery" in cmd

    def test_build_iam_detach_command_contains_detach_role_policy(self):
        cmd = _make_service()._build_iam_detach_command("s3_read")  # noqa: SLF001
    ...
    def test_build_iam_detach_command_is_unique_per_intent(self):
        svc = _make_service()
        exec_ec2 = svc._build_iam_detach_command("ec2_discovery")  # noqa: SLF001
        exec_s3 = svc._build_iam_detach_command("s3_read")  # noqa: SLF001
    ...
    def test_build_iam_attach_command_contains_intent_name(self):
        cmd = _make_service()._build_iam_attach_command("ec2_discovery")  # noqa: SLF001
        assert "ec2_discovery" in cmd
    ...
    def test_build_iam_attach_command_is_unique_per_intent(self):
        svc = _make_service()
        exec_ec2 = svc._build_iam_attach_command("ec2_discovery")  # noqa: SLF001
        exec_s3 = svc._build_iam_attach_command("s3_read")  # noqa: SLF001
        assert exec_ec2 != exec_s3
        assert "ec2_discovery" in exec_ec2
        assert "s3_read" in exec_s3

    def test_build_iam_verify_command_contains_intent_action(self):
        cmd = _make_service()._build_iam_verify_command("ec2_discovery", "ec2:DescribeInstances")  # noqa: SLF001
    ...
    def test_get_verification_action_returns_action_for_known_intent(self):
        action = _make_service()._get_verification_action_for_intent("ec2_discovery")  # noqa: SLF001
        assert action is not None
        assert "ec2:" in action

    def test_get_verification_action_returns_none_for_unknown_intent(self):
        action = _make_service()._get_verification_action_for_intent("nonexistent_intent_xyz")  # noqa: SLF001
    ...
    def test_detach_and_attach_commands_differ_for_same_intent(self):
        ...
        svc = _make_service()
        attach = svc._build_iam_attach_command("ec2_discovery")  # noqa: SLF001
        detach = svc._build_iam_detach_command("ec2_discovery")  # noqa: SLF001
...
class TestIntentDependencyResolution:
    ...
    def test_no_dependencies_returns_same_intent(self):
        result = _make_service()._resolve_intent_dependencies(["ec2_discovery"])  # noqa: SLF001
    ...
    def test_dependent_intent_pulls_in_prerequisite(self):
        ...
        result = _make_service()._resolve_intent_dependencies(["ec2_management"])  # noqa: SLF001
        assert "ec2_management" in result
        assert "ec2_discovery" in result

    def test_already_included_dependency_not_duplicated(self):
        result = _make_service()._resolve_intent_dependencies(["ec2_management", "ec2_discovery"])  # noqa: SLF001
        assert result.count("ec2_discovery") == 1

    def test_multiple_independent_intents_all_included(self):
        result = _make_service()._resolve_intent_dependencies(["ec2_discovery", "s3_read"])  # noqa: SLF001
        assert "ec2_discovery" in result
        assert "s3_read" in result

    def test_result_is_sorted(self):
        result = _make_service()._resolve_intent_dependencies(["s3_read", "ec2_discovery"])  # noqa: SLF001
        assert result == sorted(result)

    def test_empty_input_returns_empty(self):
        result = _make_service()._resolve_intent_dependencies([])  # noqa: SLF001
        assert result == []

    def test_transitive_dependencies_resolved(self):
        """If B depends on A and C depends on B, requesting C must include A and B."""
        svc = _make_service()
        result = svc._resolve_intent_dependencies(["ec2_management"])  # noqa: SLF001
        assert len(result) >= 2
