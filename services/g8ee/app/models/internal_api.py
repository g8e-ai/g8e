...
class InternalOperatorAuthCall(G8eBaseModel):
    ...
    operator_session_id: str = Field(..., description="g8eo Gateway operator session UUID - used as the g8ee session document ID so the CLI Bearer token resolves directly")
...
class OperatorDeviceLinkRegisterRequest(G8eBaseModel):
    ...
    operator_session_id: str | None = Field(default=None, description="g8eo Gateway operator session ID - used as the g8ee session document ID so the CLI Bearer token resolves directly")
