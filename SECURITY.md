...
- **CA cert containment** -- the platform CA never leaves the `g8es-data` volume; `./g8e security certs trust` streams it directly from the `g8es` container via `docker exec` with no intermediate file written to the host; remote workstation instructions use SSH streaming (`ssh <host> "docker exec g8es cat /g8es/ssl/ca.crt"`) for the same reason
